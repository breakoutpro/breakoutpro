  ...
  return (
    <div style={{background:BG,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
      <div style={{background:BG,padding:"12px 14px",borderBottom:"1px solid "+BD,display:"flex",alignItems:"center",gap:10,position:"sticky",top:0,zIndex:10}}>
        ...
        <div style={{flex:1}}><div style={{fontSize:16,fontWeight:900,color:T1}}>Practice Mode</div><div style={{fontSize:9,color:T2}}>Guess the pattern</div></div>
        <div style={{textAlign:"right"}}><div style={{fontSize:13,fontWeight:900,color:UP}}>{score}</div><div style={{fontSize:8,color:T2}}>Score</div></div>
      </div>

      <div style={{padding:14}}>
        <div style={{fontSize:11.5,color:T1,fontWeight:700,marginBottom:10,textAlign:"center"}}>Which pattern is forming?</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginTop:16}}>
          {q.opts.map(function(p){
            ...
            return (
              <button key={p.id} onClick={function(){pick(p);}} style={{background:bg,border:"1px solid "+bd,borderRadius:10,padding:"13px 8px",color:tc,fontSize:11,fontWeight:700,cursor:picked?"default":"pointer",fontFamily:"inherit"}}>{p.name}</button>
            );
          })}
        </div>
        ...
        {picked?(
          <div style={{marginTop:14}}>
            <div style={{background:picked.id==q.correct.id?"rgba(34,197,94,0.08)":"rgba(239,68,68,0.08)",border:"1px solid "+(picked.id==q.correct.id?"rgba(34,197,94,0.25)":"rgba(239,68,68,0.25)"),borderRadius:12,padding:13,marginBottom:12}}>
              <div style={{fontSize:13,fontWeight:800,color:picked.id==q.correct.id?UP:DOWN,marginBottom:4}}>{picked.id==q.correct.id?"Correct!":"Not quite"}</div>
              <div style={{fontSize:11,color:T1,lineHeight:1.5}}>This is a <b>{q.correct.name}</b> ({q.correct.bias}).</div>
            </div>
            <button onClick={next} style={{width:"100%",background:CYAN,border:"none",borderRadius:10,padding:"13px",color:"#04060D",fontSize:13,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>Next Pattern &#8594;</button>
          </div>
        ):null}
      </div>
    </div>
...
export function PatternQuiz(props){
  var theme = useTheme(); // reuses the existing ThemeProvider - no new theme system
  // Theme-sourced overrides - shadow the module-level hardcoded fallbacks above.
  var CARD = theme.c.card, GOLD = theme.c.gold, T2 = theme.c.text2;
  ...
  return (
    <div style={{background:BG,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
      <div style={{background:BG,padding:"12px 14px",borderBottom:"1px solid "+BD,display:"flex",alignItems:"center",gap:10,position:"sticky",top:0,zIndex:10}}>
        <button onClick={props.onBack} style={{background:"rgba(255,255,255,0.06)",border:"none",borderRadius:8,width:32,height:32,color:T1,fontSize:16,cursor:"pointer"}}>&#8592;</button>
        <div style={{flex:1}}><div style={{fontSize:16,fontWeight:900,color:T1}}>{name} Quiz</div><div style={{fontSize:9,color:T2}}>{done?"Complete":"Question "+(idx+1)+" of "+QS.length}</div></div>
      </div>

      <div style={{padding:14}}>
        {done?(
          <div style={{textAlign:"center",padding:"30px 10px"}}>
            <div style={{fontSize:34,fontWeight:900,color:score>=3?UP:GOLD}}>{score}/{QS.length}</div>
            <div style={{fontSize:13,color:T1,fontWeight:700,marginTop:6}}>{score>=3?"Well done!":"Keep practicing"}</div>
            <button onClick={props.onBack} style={{marginTop:18,background:CYAN,border:"none",borderRadius:10,padding:"12px 26px",color:"#04060D",fontSize:13,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>Back to Pattern</button>
          </div>
        ):(
          <div>
            <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:15,marginBottom:14}}>
              <div style={{fontSize:13,fontWeight:700,color:T1,lineHeight:1.5}}>{cur.q}</div>
            </div>
            {cur.opts.map(function(o,i){
              ...
              var bd=state=="correct"?UP:state=="wrong"?DOWN:BD;
              var tc=state=="correct"?UP:state=="wrong"?DOWN:T1;
              return <button key={i} onClick={function(){pick(i);}} style={{display:"block",width:"100%",textAlign:"left",background:bg,border:"1px solid "+bd,borderRadius:10,padding:"13px",marginBottom:8,color:tc,fontSize:12,fontWeight:600,cursor:picked!=null?"default":"pointer",fontFamily:"inherit"}}>{o}</button>;
            })}
            {picked!=null?<button onClick={next} style={{width:"100%",background:CYAN,border:"none",borderRadius:10,padding:"13px",marginTop:6,color:"#04060D",fontSize:13,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>{idx>=QS.length-1?"See Result":"Next"} &#8594;</button>:null}
          </div>
        )}
      </div>
    </div>
