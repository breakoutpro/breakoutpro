...
function inputStyle(){
  return {width:"100%",background:CARD,border:"1px solid "+BD,borderRadius:9,padding:"8px 12px",color:T1,fontSize:12,fontFamily:"inherit",boxSizing:"border-box",marginBottom:12};
}
function labelStyle(){
...
var SECTIONS = [
  { id:"greeks", label:"Option Greeks Learning", ic:"&#128218;" },
  { id:"simulator", label:"Greeks Simulator", ic:"&#128200;" },
  { id:"maxpain", label:"Max Pain Learning", ic:"&#127919;" },
  { id:"pcr", label:"PCR Learning & Calculator", ic:"&#128202;" },
  { id:"ivcrush", label:"IV Crush Simulator", ic:"&#128165;" },
  { id:"walls", label:"Call Wall / Put Wall", ic:"&#128257;" },
  { id:"oibuilder", label:"Open Interest Builder", ic:"&#128736;" },
  { id:"strategies", label:"Expiry Strategy Cards", ic:"&#128196;" },
  { id:"quiz", label:"Mini Quiz (10 Questions)", ic:"&#10067;" }
];
...
export default function OptionsIntelPro(props){
  ...
  var [section, setSection] = useState(props.initialSection || null);
  ...
  if(!section){
    ...
    return (
      <div style={{background:theme.c.bg,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
        ...
        <div style={{padding:responsive.isMobile?12:16}}>
          {SECTIONS.map(function(s){
            return (
              <div key={s.id} onClick={function(){setSection(s.id);}} style={{background:CARD,border:"1px solid "+BD,borderRadius:14,padding:responsive.isMobile?"12px 14px":16,marginBottom:10,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}>
                <div style={{display:"flex",alignItems:"center",gap:10,minWidth:0}}>
                  <span style={{fontSize:16,flexShrink:0}} dangerouslySetInnerHTML={{__html:s.ic}}/>
                  <span style={{fontSize:12,fontWeight:700,color:T1,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{s.label}</span>
                </div>
                {s.id=="quiz" && quizDone ? <span style={{fontSize:11,fontWeight:700,color:BLUE,flexShrink:0}}>Completed</span> : <span style={{color:T3,fontSize:16,flexShrink:0}}>&#8250;</span>}
              </div>
            );
          })}
