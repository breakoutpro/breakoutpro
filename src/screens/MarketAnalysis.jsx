...
function AnalysisCard(props) {
  ...
  var tColor=res.bullSig>=3?G2:res.bearSig>=3?R:GOLD;
  ...
  return (
    <div style={{background:CB,border:"1px solid "+BD,borderRadius:12,marginBottom:8,overflow:"hidden"}}>
      <div style={{padding:"11px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer"}} onClick={function(){setDetail(detail?null:analyze(item));}}>
        <div style={{flex:1}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <div style={{fontSize:12,fontWeight:700,color:T1}}>{item.sym}</div>
            <span style={{fontSize:7,color:T2,background:"rgba(255,255,255,0.05)",borderRadius:4,padding:"1px 5px"}}>{item.sect}</span>
          </div>
        ...
        <div style={{textAlign:"right"}}>
          <div style={{fontSize:10,fontWeight:700,color:tColor}}>{trend}</div>
          <div style={{fontSize:8,color:T2}}>{res.bullSig}B / {res.bearSig}Be</div>
        </div>
      ...
      {detail?(
        <div style={{padding:"0 14px 12px",borderTop:"1px solid "+BD}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginTop:10}}>
            {[["RSI",detail.rsi,detail.rsi<40?G2:detail.rsi>60?R:GOLD],["EMA9",detail.ema9>=1000?(detail.ema9/1000).toFixed(1)+"K":detail.ema9,item.ltp>detail.ema9?G2:R],["VWAP",detail.vwap>=1000?(detail.vwap/1000).toFixed(1)+"K":detail.vwap,item.ltp>detail.vwap?G2:R]].map(function(r){
              return (
                <div key={r[0]} style={{background:"rgba(255,255,255,0.03)",borderRadius:8,padding:"7px",textAlign:"center"}}>
          ...
          <div style={{display:"flex",gap:6,marginTop:8}}>
            <div style={{flex:1,background:"rgba(0,200,83,0.08)",border:"1px solid rgba(0,200,83,0.2)",borderRadius:8,padding:"6px 8px",textAlign:"center"}}>
              <div style={{fontSize:8,color:T2}}>Bullish</div>
              <div style={{fontSize:14,fontWeight:900,color:G2}}>{detail.bullSig}/5</div>
            </div>
            <div style={{flex:1,background:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.2)",borderRadius:8,padding:"6px 8px",textAlign:"center"}}>
              <div style={{fontSize:8,color:T2}}>Bearish</div>
              <div style={{fontSize:14,fontWeight:900,color:R}}>{detail.bearSig}/5</div>
            </div>
            <div style={{flex:1,background:"rgba(245,158,11,0.08)",border:"1px solid rgba(245,158,11,0.2)",borderRadius:8,padding:"6px 8px",textAlign:"center"}}>
              <div style={{fontSize:8,color:T2}}>Volatility</div>
              <div style={{fontSize:12,fontWeight:700,color:GOLD}}>{vol}</div>
            </div>
          </div>
        </div>
      ):null}
    ...
    <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:80}}>
      <div style={{background:CB,padding:"12px 14px",borderBottom:"1px solid "+BD}}>
        ...
        <div style={{display:"flex",gap:6}}>
          {[["indices","Indices (6)"],["stocks","Stocks (20)"]].map(function(t){
            ...
            return <button key={t[0]} onClick={function(){setActiveTab(t[0]);}} style={{background:act?G:"rgba(255,255,255,0.06)",border:"1px solid "+(act?G:BD),borderRadius:20,padding:"6px 16px",color:act?"#fff":T2,fontSize:10,fontWeight:act?700:500,cursor:"pointer",fontFamily:"inherit"}}>{t[1]}</button>;
          })}
        </div>
      </div>
      <div style={{padding:"12px 14px"}}>
        <div style={{background:"rgba(30,144,255,0.06)",border:"1px solid rgba(30,144,255,0.15)",borderRadius:10,padding:"8px 12px",marginBottom:12}}>
          <div style={{fontSize:9,color:"#1E90FF"}}>Tap any row to see RSI, EMA, VWAP analysis. Educational only.</div>
        </div>
        ...
        <div style={{background:"rgba(249,115,22,0.08)",border:"1px solid rgba(249,115,22,0.2)",borderRadius:10,padding:10,marginTop:4}}>
          <div style={{fontSize:8,color:"#F97316",lineHeight:1.7}}>This application is for educational purposes only. It does not provide investment advice or buy/sell recommendations. Users should conduct their own research before making any investment decisions.</div>
        </div>
      </div>
    </div>
  );
