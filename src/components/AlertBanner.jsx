  ...
  return (
    <div style={{
      ...
      <div style={{width:36,height:36,borderRadius:10,background:color+"22",border:"1px solid "+color+"44",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
        <span style={{fontSize:12,fontWeight:900,color:color}}>{current.sym ? current.sym.slice(0,3) : "AL"}</span>
      </div>
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
          <span style={{fontSize:12,fontWeight:800,color:color,background:color+"22",borderRadius:5,padding:"4px 8px"}}>{current.type}</span>
          <span style={{fontSize:12,fontWeight:700,color:theme.c.text1}}>{current.sym}</span>
        </div>
        <div style={{fontSize:12,color:theme.c.text2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{current.msg}</div>
      </div>
      <div style={{textAlign:"right",flexShrink:0}}>
        <div style={{fontFamily:"monospace",fontSize:12,fontWeight:700,color:color}}>Rs{current.ltp >= 1000 ? (current.ltp/1000).toFixed(1)+"K" : current.ltp ? current.ltp.toFixed(2) : ""}</div>
        <div style={{fontSize:12,color:theme.c.text3}}>{current.time}</div>
      </div>
