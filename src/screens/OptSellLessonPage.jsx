  ...
  return (
    <div style={{background:BG,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
      ...
      <div style={{background:BG,padding:"12px 14px",borderBottom:"1px solid "+BD,display:"flex",alignItems:"center",gap:10,position:"sticky",top:0,zIndex:10}}>
        ...
        <button onClick={speak} style={{background:"rgba(59,130,246,0.12)",border:"1px solid rgba(59,130,246,0.3)",borderRadius:8,width:32,height:32,color:BLUE,fontSize:13,cursor:"pointer"}} dangerouslySetInnerHTML={{__html:"&#128266;"}}/>
      </div>
      ...
      <div style={{padding:14}}>
        ...
        <Sec t="LEARNING OBJECTIVES">
          <Box>
            {d.objectives.map(function(o,i){
              return <div key={i} style={{fontSize:11,color:T1,lineHeight:1.5,padding:"4px 0"}}><span style={{color:BLUE}}>&#9656;</span>  {o}</div>;
            })}
          </Box>
        </Sec>

        {/* explanation */}
        <div style={{background:theme.c.card,border:"1px solid rgba(59,130,246,0.25)",borderRadius:13,padding:14,marginBottom:15}}>
            ...
            return (
              <div key={i} style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:13,marginBottom:8}}>
                <div style={{display:"flex",gap:9,alignItems:"flex-start"}}>
                  <span style={{width:20,height:20,borderRadius:"50%",background:"rgba(59,130,246,0.15)",color:BLUE,fontSize:10,fontWeight:800,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{i+1}</span>
        ...
        <Sec t="AI EXPLANATION">
          <div style={{background:CARD,border:"1px solid rgba(59,130,246,0.2)",borderRadius:12,padding:13}}>
            <div style={{fontSize:11.5,color:T1,lineHeight:1.65}}><span style={{color:BLUE,fontWeight:700}}>AI: </span>{d.ai}</div>
          </div>
        </Sec>
        ...
        <Sec t={"QUIZ ("+d.quiz.length+" QUESTIONS)"}>
          ...
          {!showResult?(
            <button onClick={submitQuiz} disabled={Object.keys(answers).length<d.quiz.length} style={{width:"100%",background:Object.keys(answers).length<d.quiz.length?"rgba(255,255,255,0.06)":BLUE,border:"none",borderRadius:10,padding:"13px",color:Object.keys(answers).length<d.quiz.length?T3:"#04060D",fontSize:12.5,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>Submit Quiz</button>
          ):(
        ...
        {d.related && d.related.length?(
          <Sec t="RELATED LESSONS">
            <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
              {d.related.map(function(r,i){ return <span key={i} style={{fontSize:10,color:BLUE,background:"rgba(59,130,246,0.1)",border:"1px solid rgba(59,130,246,0.25)",borderRadius:14,padding:"6px 12px"}}>{r}</span>; })}
            </div>
          </Sec>
        ):null}
        ...
        <div style={{display:"flex",gap:9,marginBottom:14}}>
          ...
          <button onClick={function(){if(nextId&&props.onNavigate)props.onNavigate(nextId);}} disabled={!nextId} style={{flex:1,background:nextId?"rgba(59,130,246,0.15)":"rgba(255,255,255,0.03)",border:"1px solid "+(nextId?"rgba(59,130,246,0.3)":BD),borderRadius:10,padding:"12px",color:nextId?BLUE:T3,fontSize:11,fontWeight:700,cursor:nextId?"pointer":"default",fontFamily:"inherit"}}>Next &#8594;</button>
        </div>

        <div style={{background:"rgba(249,115,22,0.06)",border:"1px solid rgba(249,115,22,0.15)",borderRadius:10,padding:11}}>
          <div style={{fontSize:9,color:theme.c.warn}}>Educational Purpose Only. Not Investment Advice.</div>
        </div>
      </div>
    </div>
  );
