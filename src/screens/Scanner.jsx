...
export default function ScannerScreen(props) {
  ...
  var G=theme.c.up, G2=theme.c.up, R=theme.c.down, GOLD=theme.c.gold, BLUE=theme.c.blue;
    ...
    <div style={{background:DB,minHeight:"100%",paddingBottom:80,fontFamily:"Inter,Arial,sans-serif"}}>
      ...
      <div style={{display:"flex",gap:6,overflowX:"auto",padding:"12px 14px",background:CB,borderBottom:"1px solid "+BD}}>
        {SCANS.map(function(sc){
          ...
          return (
            <button key={sc.id} style={{background:act?BLUE:"rgba(255,255,255,0.06)",border:"1px solid "+(act?BLUE:BD),borderRadius:20,padding:"7px 16px",color:act?"#fff":T2,fontSize:12,fontWeight:act?700:500,cursor:"pointer",fontFamily:"inherit",flexShrink:0}} onClick={function(){runScan(sc.id);}}>
              {sc.label}
            </button>
          );
        })}
      ...
      {/* Scan description */}
      <div style={{background:"rgba(37,99,235,0.05)",borderBottom:"1px solid rgba(37,99,235,0.1)",padding:"8px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
