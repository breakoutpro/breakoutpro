...
const SECTOR_DATA = [
  {name:"IT",chg:1.82,stocks:["TCS","INFY","WIPRO"]},
  {name:"Bank",chg:1.24,stocks:["HDFCBANK","ICICIBANK","SBIN"]},
  {name:"Auto",chg:0.94,stocks:["TATAMOTORS","MARUTI","M&M"]},
  {name:"Pharma",chg:-0.32,stocks:["SUNPHARMA","DRREDDY"]},
  {name:"Energy",chg:1.35,stocks:["RELIANCE","ONGC"]},
  {name:"Metal",chg:3.24,stocks:["TATASTEEL","HINDALCO"]},
  {name:"FMCG",chg:0.18,stocks:["HINDUNILVR","ITC"]},
  {name:"Realty",chg:2.10,stocks:["DLF","GODREJPROP"]},
  {name:"Infra",chg:0.76,stocks:["POWERGRID","NTPC"]},
  {name:"PSU",chg:1.44,stocks:["SBIN","BHEL","COAL"]},
  {name:"Media",chg:-0.55,stocks:["ZEEL","SUNTV"]},
  {name:"Chemicals",chg:0.62,stocks:["PIDILITIND","ATUL"]},
];



// Gemini API Key — Vercel: set VITE_GEMINI_KEY in environment variables
var __GEMINI_KEY__ = (typeof window !== "undefined" && window.GEMINI_KEY) ? window.GEMINI_KEY : "";
...
const CSS="@keyframes bpPulse{0%,100%{transform:scale(1);opacity:1}50%{transform:scale(1.1);opacity:0.7}}@keyframes slideIn{from{transform:translateX(-100%)}to{transform:translateX(0)}}@keyframes slideUp{from{transform:translateY(100%);opacity:0}to{transform:translateY(0);opacity:1}}@keyframes fadeIn{from{opacity:0}to{opacity:1}}@keyframes ticker{0%{transform:translateX(100%)}100%{transform:translateX(-200%)}}@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}.tk-wrap{overflow:hidden;flex:1}.ticker{display:inline-block;white-space:nowrap;animation:ticker 38s linear infinite}::-webkit-scrollbar{width:3px;height:3px}::-webkit-scrollbar-track{background:#0B0B0B}::-webkit-scrollbar-thumb{background:#222;border-radius:3px}";
...
function LogoSVG(props){
  ...
  var light=props.light||false; // true = white bg, false = dark bg
  var textColor=light?"#111827":"#fff";
  var accentColor="#00C853";
  var tagColor=light?"#F97316":"#ff4400";
  return(
    <div style={{textAlign:"center",userSelect:"none",padding:Math.round(6*size)+"px 0"}}>
      <div style={{fontFamily:"Inter,Arial,sans-serif",fontSize:Math.round(32*size),fontWeight:900,color:textColor,letterSpacing:-1.5,lineHeight:1}}>
        Breakout<span style={{color:accentColor}}> Pro</span>
      </div>
      <div style={{fontSize:Math.round(7*size),fontWeight:800,letterSpacing:2,color:tagColor,marginTop:3}}>CATCH EVERY BREAKOUT</div>
    </div>
  );
}
...
function FInp(props){
  return(
    <div style={{marginBottom:12}}>
      {props.label&&<div style={{fontSize:11,color:"#374151",fontWeight:600,marginBottom:5}}>{props.label}</div>}
      <div style={{display:"flex",alignItems:"center",gap:8,background:"#fff",border:"1.5px solid #E5E7EB",borderRadius:10,padding:"11px 13px"}}>
        {props.icon&&<span style={{fontSize:14,flexShrink:0}}>{props.icon}</span>}
        <input
          type={props.type||"text"} value={props.value}
          onChange={function(e){props.onChange(e.target.value);}}
          placeholder={props.placeholder} maxLength={props.maxLen}
          style={{flex:1,background:"none",border:"none",outline:"none",color:"#111827",fontSize:13,fontFamily:"inherit"}}
          onFocus={function(e){e.target.parentNode.style.borderColor="#00C853";}}
          onBlur={function(e){e.target.parentNode.style.borderColor="#E5E7EB";}}
        />
      </div>
    </div>
  );
...
const EDU_BREAKOUT=["Resistance breakout = price broke above a key resistance level with high volume. Study: What is resistance? Price level where supply historically exceeds demand.","Consolidation breakout = price broke out of a tight range after compressing. Study: Tight consolidations often precede large moves in technical analysis.","Support breakout = previous support turned resistance, price broke above. Study: Support/Resistance role reversal is a classic TA concept.","Gap up breakout = price opened significantly above previous close. Study: Gap ups indicate strong pre-market demand and bullish sentiment.","Trendline breakout = price broke above a downward trendline. Study: Trendlines connect swing highs/lows to visualise trend direction.","Range breakout = price broke out of a horizontal trading range. Study: Range-bound stocks often make significant moves when ranges are broken."];
const EDU_BREAKDOWN=["Support breakdown = price fell below a key support level with volume. Study: When support breaks, previous support often becomes resistance.","Trendline breakdown = price broke below an upward trendline. Study: Trendline breaks signal potential trend change in technical analysis.","Range breakdown = price fell below horizontal trading range support. Study: Range breakdown can lead to further decline to next support.","Bearish structure = market making lower highs and lower lows. Study: LH-LL structure is classic downtrend in Dow Theory.","Distribution breakdown = heavy selling at price top. Study: Smart money distribution is studied in Wyckoff analysis."];
const BREAKOUT_PATTERNS=["Resistance Breakout","Support Breakout","Consolidation Breakout","Range Breakout","Trendline Breakout","52W High Breakout","Gap Up Breakout"];
const BREAKDOWN_PATTERNS=["Support Breakdown","Trendline Breakdown","Range Breakdown","Bearish Structure","Distribution Breakdown","Gap Down Breakdown"];
...
export default function StocksBuddy(){
  ...
  const [bankNifty,  setBankNifty]  = useState({ltp:48234.60,chg:412.30,pct:0.86,up:true});
  const [midcap,     setMidcap]     = useState({ltp:43876.20,chg:320.10,pct:0.74,up:true});
  const [smallcap,   setSmallcap]   = useState({ltp:14523.80,chg:-82.40,pct:-0.56,up:false});
  const [niftyNext50,setNiftyNext50]= useState({ltp:67234.50,chg:198.60,pct:0.30,up:true});
  ...
  async function fetchNseIndices(){
    try{
      ...
      var gn=parseIndex(all,"GIFT NIFTY")||parseIndex(all,"NIFTY FUTURES");
      var vx=parseIndex(all,"INDIA VIX");
      ...
      if(gn){setGiftNifty({ltp:gn.ltp,chg:gn.chg,pct:gn.pct,up:gn.up});}
      if(vx){setVix(vx.ltp);}
  ...
  async function fetchNseGainersLosers(){
    try{
      var res=await nseGet("gainers");
      var res2=await nseGet("losers");
      var parse=function(item){return{
        sym:item.symbol||"",
        name:item.companyName||item.symbol||"",
        ltp:parseFloat(item.ltp||item.lastPrice||0),
        chgPct:parseFloat(item.netPrice||item.pChange||0),
        chg:parseFloat(item.netPrice||0),
        up:parseFloat(item.netPrice||0)>=0,
        vol:item.tradedQuantity||"",
        sect:item.industry||"",
        open:parseFloat(item.openPrice||0),
        high:parseFloat(item.dayHigh||0),
        low:parseFloat(item.dayLow||0),
        wk52h:parseFloat(item["52wHigh"]||0),
        wk52l:parseFloat(item["52wLow"]||0),
        sup:parseFloat(item.dayLow||0),
        res:parseFloat(item.dayHigh||0),
        cap:item.marketCap||"",
        pe:item.pe||"-",
        trend:"bull",
        spark:Array(14).fill(parseFloat(item.ltp||0)).map(function(_,i){return parseFloat(item.ltp||0)+sg()*rnd(0,parseFloat(item.ltp||0)*0.005)*i;}),
        news:[],
        qtr:{dt:"Q4FY25",rev:"--",pft:"--",yoy:"--",qoq:"--"},
      };};
      if(res.data&&res.data.length>0){
        setStocks(function(prev){
          var liveMap={};
          res.data.slice(0,10).forEach(function(item){liveMap[item.symbol]=parse(item);});
          res2.data&&res2.data.slice(0,10).forEach(function(item){liveMap[item.symbol]=parse(item);});
          // Merge live data with existing stocks
          return prev.map(function(s){
            if(liveMap[s.sym]){
              return Object.assign({},s,{
                ltp:liveMap[s.sym].ltp,
                chgPct:liveMap[s.sym].chgPct,
                chg:liveMap[s.sym].chg,
                up:liveMap[s.sym].up,
              });
            }
            return s;
  ...
  async function fetchNseMarketStatus(){
    try{
      var data=await nseGet("marketstatus");
      if(data&&data.marketState&&data.marketState[0]){
        var st=data.marketState[0].marketStatus;
        var isOpen=st==="Open";
        setMktStatus(isOpen?
          {session:"stocks",label:"Stocks Open",sub:"NSE/BSE Live | 9:15 AM - 3:30 PM",color:"#00C853"}:
          {session:"closed",label:"Market Closed",sub:"Next opens 9:15 AM",color:"#9CA3AF"}
        );
      }
    }catch(e){}
  }
  ...
  useEffect(function(){
    ...
    fetchNseIndices();
    fetchNseFiiDii();
    fetchNseMarketStatus();
    fetchNseGainersLosers();
    // Market hours: refresh every 15s
    var mkt=getMarketStatus();
    if(mkt.session==="stocks"){
      var t1=setInterval(function(){fetchNseIndices();},15000);
      var t2=setInterval(function(){fetchNseGainersLosers();},30000);
      var t3=setInterval(function(){fetchNseMarketStatus();},60000);
  ...
  useEffect(function(){
    var el=document.getElementById('bp-css');
    if(!el){
  ...
  async function sendGemini(userMsg){
    ...
    var GEMINI_KEY="";
    try{if(typeof window!=="undefined"&&window.VITE_GEMINI_KEY){GEMINI_KEY=window.VITE_GEMINI_KEY;}}catch(e){}
    if(!GEMINI_KEY&&typeof window!=="undefined"&&window.GEMINI_KEY){GEMINI_KEY=window.GEMINI_KEY;}
    if(!GEMINI_KEY){GEMINI_KEY=__GEMINI_KEY__||"";}
  ...
  var G="linear-gradient(135deg,#00C853,#00A040)";

  function renderSplash(){
    return(
<div style={{position:"absolute",top:0,left:0,right:0,bottom:0,background:"#000",display:"flex",alignItems:"center",justifyContent:"center",padding:20,zIndex:10}}>
          <div style={{textAlign:"center"}}>
        <LogoSVG size={0.9}/>
        {splashS==2 ? <div style={{width:"100%",maxWidth:240,margin:"20px auto 0"}}><div style={{height:5,background:"#1a2a1a",borderRadius:5,overflow:"hidden"}}><div style={{height:"100%",width:splashP+"%",background:G,transition:"width 0.025s linear",borderRadius:5}}></div></div><div style={{color:"#555",fontSize:11,textAlign:"center",marginTop:9,letterSpacing:3}}>Loading...</div></div> : null}
        {splashS==3 ? <div style={{position:"relative",width:100,height:100,margin:"20px auto 0"}}><svg width="100" height="100" style={{transform:"rotate(-90deg)"}}><circle cx="50" cy="50" r="40" fill="none" stroke="#1a2a1a" strokeWidth="7"/><circle cx="50" cy="50" r="40" fill="none" stroke="#22C55E" strokeWidth="7" strokeDasharray="251.33 0" strokeLinecap="round"/></svg><div style={{position:"absolute",top:0,left:0,right:0,bottom:0,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"monospace",fontSize:19,fontWeight:900,color:"#22C55E"}}>100%</div></div> : null}
          </div>
        </div>
    );
  }
  ...
  function renderAuth(){
    return(
<div style={{position:"absolute",top:0,left:0,right:0,bottom:0,background:"#F8F9FA",color:"#111827",fontFamily:"Inter,Poppins,Segoe UI,sans-serif",overflowY:"auto",zIndex:10}}>

          {/* Header */}
      <div style={{background:"#fff",padding:"20px 20px 16px",textAlign:"center",borderBottom:"1px solid #F0F0F0",boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,marginBottom:4}}>
          <div style={{width:40,height:40,borderRadius:12,background:"linear-gradient(135deg,#00C853,#00A846)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 3px 10px rgba(0,200,83,0.3)"}}>
            <span style={{fontFamily:"Arial",fontSize:14,fontWeight:900,color:"#fff"}}>BP</span>
          </div>
          <div style={{textAlign:"left"}}>
            <div style={{fontSize:20,fontWeight:900,color:"#111827",letterSpacing:-0.5}}>Breakout <span style={{color:"#00C853"}}>Pro</span></div>
            <div style={{fontSize:8,color:"#F97316",fontWeight:700,letterSpacing:1.5}}>CATCH EVERY BREAKOUT</div>
          </div>
        </div>
        <div style={{fontSize:9,color:"#9CA3AF",marginTop:4}}>India's #1 Stock Market Education Platform</div>
      </div>

      {/* OTP Screen */}
      {otpSt=="otp"&&<div style={{padding:"24px 20px"}}>
        <div style={{textAlign:"center",marginBottom:24}}>
          <div style={{width:64,height:64,borderRadius:"50%",background:"#DCFCE7",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 12px",fontSize:28}}>📱</div>
          <div style={{fontSize:18,fontWeight:800,color:"#111827",marginBottom:4}}>Enter OTP</div>
          <div style={{fontSize:12,color:"#6B7280"}}>Sent to +91 {form.phone}</div>
        </div>
        {otpErr&&<div style={{background:"#FEF2F2",border:"1px solid #FECACA",borderRadius:10,padding:"10px 12px",marginBottom:12,fontSize:11,color:"#DC2626",textAlign:"center"}}>{otpErr}</div>}
        <FInp placeholder="Enter 6-digit OTP" value={entOTP} onChange={setEntOTP} type="number" maxLen={6}/>
        <div style={{textAlign:"right",marginBottom:12}}>{timer>0?<span style={{fontSize:10,color:"#9CA3AF"}}>Resend in {timer}s</span>:<button onClick={sendOTP} style={{background:"none",border:"none",color:"#00C853",fontSize:10,cursor:"pointer",fontFamily:"inherit",fontWeight:600}}>Resend OTP</button>}</div>
        <button onClick={verifyOTP} style={{width:"100%",background:"#00C853",border:"none",borderRadius:12,padding:"14px",fontSize:13,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:"inherit",boxShadow:"0 3px 10px rgba(0,200,83,0.3)",marginBottom:8}}>Verify OTP ✓</button>
        <button onClick={function(){setOtpSt("form");}} style={{width:"100%",background:"#F9FAFB",border:"1px solid #E5E7EB",borderRadius:12,padding:"12px",color:"#6B7280",cursor:"pointer",fontSize:12,fontFamily:"inherit"}}>← Change Number</button>
      </div>}

      {/* Forgot Password */}
      {otpSt=="forgot"&&<div style={{padding:"24px 20px"}}>
        <div style={{textAlign:"center",marginBottom:24}}>
          <div style={{width:64,height:64,borderRadius:"50%",background:"#FEF3C7",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 12px",fontSize:28}}>🔑</div>
          <div style={{fontSize:18,fontWeight:800,color:"#111827",marginBottom:4}}>Forgot Password?</div>
          <div style={{fontSize:12,color:"#6B7280"}}>Enter your registered phone number</div>
        </div>
        {otpErr&&<div style={{background:"#FEF2F2",border:"1px solid #FECACA",borderRadius:10,padding:"10px 12px",marginBottom:12,fontSize:11,color:"#DC2626"}}>{otpErr}</div>}
        <FInp label="Phone Number" placeholder="10-digit phone" value={forgotPh} onChange={setForgotPh} type="tel" maxLen={10}/>
        <button onClick={handleForgot} style={{width:"100%",background:"#00C853",border:"none",borderRadius:12,padding:"14px",fontSize:13,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:"inherit",marginBottom:8}}>Send Reset OTP</button>
        <button onClick={function(){setOtpSt("form");setOtpErr("");}} style={{width:"100%",background:"#F9FAFB",border:"1px solid #E5E7EB",borderRadius:12,padding:"12px",color:"#6B7280",cursor:"pointer",fontSize:12,fontFamily:"inherit"}}>← Back to Login</button>
      </div>}

      {/* Forgot OTP */}
      {otpSt=="forgotOtp"&&<div style={{padding:"24px 20px"}}>
        <div style={{textAlign:"center",marginBottom:24}}>
          <div style={{width:64,height:64,borderRadius:"50%",background:"#DCFCE7",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 12px",fontSize:28}}>📱</div>
          <div style={{fontSize:18,fontWeight:800,color:"#111827"}}>Enter Reset OTP</div>
        </div>
        {otpErr&&<div style={{background:"#FEF2F2",border:"1px solid #FECACA",borderRadius:10,padding:"10px",marginBottom:12,fontSize:11,color:"#DC2626"}}>{otpErr}</div>}
        <FInp placeholder="Enter 6-digit OTP" value={entOTP} onChange={setEntOTP} type="number" maxLen={6}/>
        <button onClick={verifyForgotOTP} style={{width:"100%",background:"#00C853",border:"none",borderRadius:12,padding:"14px",fontSize:13,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:"inherit"}}>Verify OTP</button>
      </div>}

      {/* Reset Password */}
      {otpSt=="resetPass"&&<div style={{padding:"24px 20px"}}>
        <div style={{textAlign:"center",marginBottom:24}}>
          <div style={{width:64,height:64,borderRadius:"50%",background:"#EDE9FE",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 12px",fontSize:28}}>🔒</div>
          <div style={{fontSize:18,fontWeight:800,color:"#111827"}}>Set New Password</div>
        </div>
        {otpErr&&<div style={{background:"#FEF2F2",border:"1px solid #FECACA",borderRadius:10,padding:"10px",marginBottom:12,fontSize:11,color:"#DC2626"}}>{otpErr}</div>}
        <FInp label="New Password" placeholder="Min 6 characters" value={newPass} onChange={setNewPass} type="password"/>
        <button onClick={resetPassword} style={{width:"100%",background:"#00C853",border:"none",borderRadius:12,padding:"14px",fontSize:13,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:"inherit"}}>Set Password</button>
      </div>}

      {/* Reset Done */}
      {otpSt=="resetDone"&&<div style={{padding:"40px 20px",textAlign:"center"}}>
        <div style={{width:72,height:72,borderRadius:"50%",background:"#DCFCE7",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px",fontSize:32}}>✅</div>
        <div style={{fontSize:18,fontWeight:800,color:"#111827",marginBottom:6}}>Password Reset!</div>
        <div style={{fontSize:12,color:"#6B7280",marginBottom:24}}>Login with your new password.</div>
        <button onClick={function(){setOtpSt("form");setEntOTP("");setRealOTP("");setNewPass("");setForgotPh("");}} style={{width:"100%",background:"#00C853",border:"none",borderRadius:12,padding:"14px",fontSize:13,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:"inherit"}}>Go to Login</button>
      </div>}

      {/* Gmail verify */}
      {otpSt=="gmail"&&<div style={{padding:"32px 20px",textAlign:"center"}}>
        <div style={{width:72,height:72,borderRadius:"50%",background:"#DCFCE7",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px",fontSize:32}}>✅</div>
        <div style={{fontSize:18,fontWeight:800,color:"#111827",marginBottom:4}}>Phone Verified!</div>
        <div style={{fontSize:12,color:"#6B7280",marginBottom:24}}>Verify Gmail for account recovery?</div>
        <button onClick={function(){alert("Gmail verification sent! (Demo)");finishAuth(true);}} style={{width:"100%",background:"#00C853",border:"none",borderRadius:12,padding:"14px",fontSize:13,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:"inherit",marginBottom:10}}>Verify Gmail</button>
        <button onClick={function(){finishAuth(false);}} style={{width:"100%",background:"#F9FAFB",border:"1px solid #E5E7EB",borderRadius:12,padding:"12px",color:"#6B7280",cursor:"pointer",fontSize:12,fontFamily:"inherit"}}>Skip for now</button>
      </div>}

      {/* Main Login/Register Form */}
      {otpSt=="form"&&<div style={{padding:"20px 20px 32px"}}>

        {/* Tab toggle */}
        <div style={{display:"flex",background:"#F3F4F6",borderRadius:12,padding:4,marginBottom:20,border:"1px solid #E5E7EB"}}>
          {[["login","Login"],["register","Register"]].map(function(kl){return(
            <button key={kl[0]} onClick={function(){setMode(kl[0]);setAuthErr("");}} style={{flex:1,padding:"10px",borderRadius:9,border:"none",background:mode===kl[0]?"#fff":"transparent",color:mode===kl[0]?"#111827":"#6B7280",fontWeight:mode===kl[0]?700:500,fontSize:12,cursor:"pointer",fontFamily:"inherit",boxShadow:mode===kl[0]?"0 1px 4px rgba(0,0,0,0.08)":"none",transition:"all 0.2s"}}>{kl[1]}</button>
          );})}
        </div>

        {authErr&&<div style={{background:"#FEF2F2",border:"1px solid #FECACA",borderRadius:10,padding:"10px 12px",marginBottom:14,fontSize:11,color:"#DC2626",display:"flex",alignItems:"center",gap:6}}>⚠️ {authErr}</div>}
        ...
        <FInp label="Phone Number *" placeholder="10-digit mobile number" value={form.phone} onChange={function(v){setForm(function(p){return Object.assign({},p,{phone:v});});}} type="tel" maxLen={10}/>
        <FInp label="Password *" placeholder="Min 6 characters" value={form.pass} onChange={function(v){setForm(function(p){return Object.assign({},p,{pass:v});});}} type="password"/>
        ...
        {mode=="register"&&<div style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:16,padding:"10px",background:"#F0FDF4",borderRadius:10,border:"1px solid #BBF7D0"}}>
          <div onClick={function(){setTermsOk(function(p){return !p;});}} style={{width:20,height:20,borderRadius:5,border:"2px solid "+(termsOk?"#00C853":"#D1D5DB"),background:termsOk?"#00C853":"#fff",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0,marginTop:1}}>
            {termsOk&&<span style={{color:"#fff",fontSize:11,fontWeight:900}}>✓</span>}
          </div>
          <div style={{fontSize:10,color:"#374151",lineHeight:1.6}}>I agree to the <button onClick={function(){setShowT(true);}} style={{background:"none",border:"none",color:"#00C853",fontSize:10,cursor:"pointer",fontFamily:"inherit",fontWeight:600,textDecoration:"underline"}}>Terms</button> — Educational platform only</div>
        </div>}

        <button onClick={handleSubmit} style={{width:"100%",background:"#00C853",border:"none",borderRadius:12,padding:"14px",fontSize:13,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:"inherit",boxShadow:"0 3px 12px rgba(0,200,83,0.3)",marginBottom:10}}>
          {mode=="login"?"Login →":"Create Account →"}
        </button>

        {mode=="login"&&<button onClick={function(){setOtpSt("forgot");setOtpErr("");}} style={{width:"100%",background:"none",border:"none",color:"#00C853",cursor:"pointer",fontSize:11,fontFamily:"inherit",fontWeight:600}}>Forgot Password?</button>}

        {/* Language */}
        <div style={{marginTop:16,padding:"12px",background:"#fff",borderRadius:10,border:"1px solid #E5E7EB"}}>
          <div style={{fontSize:9,color:"#9CA3AF",fontWeight:600,marginBottom:6,textAlign:"center",letterSpacing:0.5}}>LANGUAGE / भाषा / భాష</div>
          <div style={{display:"flex",gap:6}}>
            {[["en","🇬🇧 English"],["hi","🇮🇳 हिंदी"],["te","🇮🇳 తెలుగు"]].map(function(kl){return(
              <button key={kl[0]} onClick={function(){setLang(kl[0]);}} style={{flex:1,background:lang===kl[0]?"#DCFCE7":"#F9FAFB",border:"1px solid "+(lang===kl[0]?"#00C853":"#E5E7EB"),borderRadius:8,padding:"7px 4px",fontSize:9,fontWeight:lang===kl[0]?700:500,color:lang===kl[0]?"#166534":"#6B7280",cursor:"pointer",fontFamily:"inherit"}}>{kl[1]}</button>
            );})}
          </div>
        </div>

        {/* Disclaimer */}
        <div style={{marginTop:14,background:"#FFF7ED",border:"1px solid #FED7AA",borderRadius:10,padding:"10px 12px",fontSize:8,color:"#92400E",lineHeight:1.7,textAlign:"center"}}>
          ⚠️ {DISCLAIMER}
        </div>
      </div>}
  ...
  function renderApp(){
    ...
    <div style={{position:"relative",flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          ...
          {/* ── TOPBAR ── */}
          <div style={{display:"flex",alignItems:"center",padding:"10px 14px 8px",background:tab=="home"?"#fff":"#0B0B0B",borderBottom:"1px solid "+(tab=="home"?"#E5E7EB":"#1A1A1A"),zIndex:50,flexShrink:0}}>
            {/* Logo */}
            <div style={{flex:1,display:"flex",alignItems:"center",gap:10}}>
              <div style={{width:36,height:36,borderRadius:10,background:"linear-gradient(135deg,#00C853,#00A040)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 2px 8px rgba(0,200,83,0.3)",flexShrink:0}}>
                <span style={{fontFamily:"Arial",fontSize:13,fontWeight:900,color:"#fff",letterSpacing:-0.5}}>BP</span>
              </div>
              <div>
                <div style={{fontFamily:"Inter,Arial,sans-serif",fontSize:17,fontWeight:900,color:tab=="home"?"#111827":"#fff",letterSpacing:-0.8,lineHeight:1}}>
                  Breakout<span style={{color:"#00C853"}}> Pro</span>
                </div>
                <div style={{fontSize:6,color:"#F97316",fontWeight:800,letterSpacing:1.5,marginTop:1}}>CATCH EVERY BREAKOUT</div>
              </div>
            </div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              {/* Market status pill */}
              <div style={{
                background:mktStatus.session=="stocks"?(tab=="home"?"#DCFCE7":"#0a1a0a"):(tab=="home"?"#F3F4F6":"#1A1A1A"),
                border:"1px solid "+(mktStatus.session=="stocks"?(tab=="home"?"#BBF7D0":"#1a3a1a"):(tab=="home"?"#E5E7EB":"#222")),
                borderRadius:20,padding:"4px 10px",display:"flex",alignItems:"center",gap:4
              }}>
                <div style={{width:5,height:5,borderRadius:"50%",background:mktStatus.session=="stocks"?"#00C853":"#9CA3AF",flexShrink:0}}></div>
                <span style={{fontSize:8,fontWeight:700,color:mktStatus.session=="stocks"?"#00C853":(tab=="home"?"#6B7280":"#777"),letterSpacing:0.2}}>{mktStatus.label}</span>
              </div>
              {isPrem&&<span style={{background:tab=="home"?"#FEF3C7":"#1C1400",color:"#D97706",border:"1px solid "+(tab=="home"?"#FDE68A":"#292000"),borderRadius:20,padding:"3px 8px",fontSize:8,fontWeight:700}}>PRO</span>}
              <button onClick={function(){setSidebar(true);}} style={{background:tab=="home"?"#F9FAFB":"#1A1A1A",border:"1px solid "+(tab=="home"?"#E5E7EB":"#222"),borderRadius:9,width:32,height:32,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:15,color:tab=="home"?"#374151":"#aaa",flexShrink:0}}>☰</button>
            </div>
          </div>
          ...
          <div style={{overflowY:"auto",flex:1,paddingBottom:72}}>
            ...
            {tab=="home"&&(
              <div style={{background:"#F8F9FA",minHeight:"100%",paddingBottom:80}}>
                ...
                {/* ── GREETING ROW ── */}
                <div style={{background:"#fff",padding:"12px 16px 10px",borderBottom:"1px solid #F0F0F0",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    <div style={{width:40,height:40,borderRadius:"50%",background:"linear-gradient(135deg,#00C853,#00A040)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:900,color:"#fff",flexShrink:0,boxShadow:"0 2px 8px rgba(0,200,83,0.3)"}}>
                      {(user&&user.name?user.name[0]:"T").toUpperCase()}
                    </div>
                    <div>
                      <div style={{fontSize:9,color:"#9CA3AF",fontWeight:500}}>{new Date().getHours()<12?t("goodMorn"):new Date().getHours()<17?t("goodAfter"):t("goodEve")}</div>
                      <div style={{fontSize:16,color:"#111827",fontWeight:800,letterSpacing:-0.3}}>{user&&user.name?user.name.split(" ")[0]:"Trader"} <span style={{fontSize:14}}>👋</span></div>
                    </div>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <button onClick={function(){setTab("news");}} style={{background:"#F9FAFB",border:"1px solid #E5E7EB",borderRadius:10,width:36,height:36,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",position:"relative"}}>
                      ...
                      {unread>0&&<span style={{position:"absolute",top:-3,right:-3,background:"#EF4444",color:"#fff",borderRadius:"50%",width:15,height:15,fontSize:7,fontWeight:800,display:"flex",alignItems:"center",justifyContent:"center"}}>{unread}</span>}
                    </button>
  ...
  var appStyle={position:"relative",width:"100%",maxWidth:430,margin:"0 auto",height:"100vh",background:"#0B0B0B",color:"#fff",fontFamily:"inherit",overflow:"hidden",display:"flex",flexDirection:"column"};
  var isSplash=phase=="splash";
  var isAuth=phase=="auth";

  return(
    <div style={appStyle}>
      {isSplash ? renderSplash() : isAuth ? renderAuth() : renderApp()}
    </div>
  );
