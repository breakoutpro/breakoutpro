...
export default function AIBriefing() {
  ...
  return (
    <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:80}}>
      ...
      {/* Header */}
      <div style={{background:"linear-gradient(135deg,#0F1629,#0A1024)",padding:"16px 14px",borderBottom:"1px solid "+BD}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
          <div>
            <div style={{fontSize:20,fontWeight:900,color:T1}}>AI Market <span style={{color:BLUE}}>Briefing</span></div>
      ...
      <div style={{padding:"14px 14px 0"}}>
        ...
        {!briefing && !loading && !error ? (
          <div style={{textAlign:"center",padding:"40px 20px"}}>
            ...
            <button onClick={getBriefing} style={{background:BLUE,border:"none",borderRadius:14,padding:"14px 30px",color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Get Today Briefing</button>
          </div>
        ) : null}

        <div style={{background:"rgba(249,115,22,0.08)",border:"1px solid rgba(249,115,22,0.2)",borderRadius:10,padding:10,marginTop:8}}>
          <div style={{fontSize:8,color:theme.c.warn,lineHeight:1.7}}>AI generated content. For educational purposes only. Not SEBI registered investment advice. Always do your own research.</div>
        </div>
      </div>
    </div>
  );
