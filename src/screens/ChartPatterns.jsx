    ...
    <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:80}}>
      <div style={{background:CB,padding:"12px 14px",borderBottom:"1px solid "+BD}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <div>
            <div style={{fontSize:16,fontWeight:900,color:T1}}>Chart <span style={{color:G}}>Patterns</span></div>
          ...
          <div style={{textAlign:"right"}}>
            ...
            <div style={{fontSize:14,fontWeight:900,color:progress==100?GOLD:G}}>{progress}%</div>
          </div>
        </div>
        <div style={{height:3,background:"rgba(255,255,255,0.06)",borderRadius:2,marginBottom:8,overflow:"hidden"}}>
          <div style={{height:"100%",width:progress+"%",background:progress==100?GOLD:G,borderRadius:2}}></div>
        </div>
        ...
        <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:2}}>
          {FILTERS.map(function(f){var act=filter==f;return <button key={f} onClick={function(){setFilter(f);}} style={{background:act?G:"rgba(255,255,255,0.05)",border:"1px solid "+(act?G:BD),borderRadius:20,padding:"4px 10px",color:act?"#fff":T2,fontSize:8,fontWeight:act?700:400,cursor:"pointer",fontFamily:"inherit",flexShrink:0}}>{f}</button>;})}
        </div>
      </div>
