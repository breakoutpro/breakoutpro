    ...
    <div style={{background:BG,minHeight:"100vh",fontFamily:"'Inter',Arial,sans-serif",color:T1,overflowX:"hidden",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,display:"flex",minHeight:0,width:"100%"}}>
        ...
        <div style={{flex:1,minWidth:0,padding:"12px 16px",overflowY:"auto"}}>
          ...
          {/* AI MARKET MOOD - card 1 of 3. Reuses the real Gauge component
              from MarketMoodParts.jsx (same one the full-screen page uses)
              rather than building a second gauge implementation. Content-
              driven height throughout - no fixed height anywhere on this
              card, so it grows to fit the full information hierarchy. */}
          <div onClick={function(){setTab("marketmood");}} style={{background:CARD2,border:"1px solid "+BD,borderRadius:10,padding:tierCfg.cardPadding+6,boxSizing:"border-box",cursor:"pointer",marginBottom:tierCfg.gridGap,minHeight:250}}>
            ...
            {(function(){
              ...
              return (
                <div>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:tierCfg.gridGap*2}}>
                    <div>
                      <div style={{fontSize:tierCfg.label,color:T3,fontWeight:700,marginBottom:6}}>WHY?</div>
                      {ai && (ai.now || ai.whatChanged || (ai.keyDrivers && ai.keyDrivers.length)) ? (
                        <div>
                          {ai.now ? <div style={{fontSize:tierCfg.secondaryText,color:T1,lineHeight:1.6,marginBottom:6}}>{ai.now}</div> : null}
                          {ai.whatChanged ? <div style={{fontSize:tierCfg.secondaryText,color:T2,lineHeight:1.6,marginBottom:6}}><b style={{color:T1}}>What changed: </b>{ai.whatChanged}</div> : null}
                          {ai.keyDrivers && ai.keyDrivers.length ? (
                            <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                              {ai.keyDrivers.map(function(d,i){
                                return <span key={i} style={{fontSize:tierCfg.label,color:T2,background:BG,border:"1px solid "+BD,borderRadius:6,padding:"3px 8px"}}>{d}</span>;
                              })}
                            </div>
                          ) : null}
                        </div>
                    ...
                    <div>
                      <div style={{fontSize:tierCfg.label,color:T3,fontWeight:700,marginBottom:6}}>WHAT TO WATCH?</div>
                      {ai && ai.watchNext ? <div style={{fontSize:tierCfg.secondaryText,color:T1,lineHeight:1.6}}>{ai.watchNext}</div> : <div style={{fontSize:tierCfg.secondaryText,color:T3}}>Data unavailable right now.</div>}
                      {ai && ai.riskNote ? <div style={{fontSize:tierCfg.label,color:WARN,lineHeight:1.6,marginTop:6}}>{ai.riskNote}</div> : null}
                    </div>
                  </div>
                </div>
              );
