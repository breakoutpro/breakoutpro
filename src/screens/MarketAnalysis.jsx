      ...
      <div style={{padding:"11px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer"}} onClick={function(){setDetail(detail?null:analyze(item));}}>
        <div style={{flex:1}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <span style={{fontSize:12,color:T2,background:"rgba(255,255,255,0.05)",borderRadius:4,padding:"1px 5px"}}>{item.sect}</span>
          </div>
            ...
            {[["RSI",detail.rsi,detail.rsi<40?G2:detail.rsi>60?R:GOLD],["EMA9",detail.ema9>=1000?(detail.ema9/1000).toFixed(1)+"K":detail.ema9,item.ltp>detail.ema9?G2:R],["VWAP",detail.vwap>=1000?(detail.vwap/1000).toFixed(1)+"K":detail.vwap,item.ltp>detail.vwap?G2:R]].map(function(r){
              return (
                <div key={r[0]} style={{background:"rgba(255,255,255,0.03)",borderRadius:8,padding:"7px",textAlign:"center"}}>
                  <div style={{fontSize:12,color:T2,marginBottom:2}}>{r[0]}</div>
  ...
  return (
    <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:80}}>
      <div style={{background:CB,padding:"12px 14px",borderBottom:"1px solid "+BD}}>
        ...
        <div style={{fontSize:12,color:"#F97316",fontWeight:700,letterSpacing:1,marginBottom:6}}>EDUCATIONAL - NOT INVESTMENT ADVICE</div>
        ...
        <div style={{display:"flex",gap:6}}>
          {[["indices","Indices (6)"],["stocks","Stocks (20)"]].map(function(t){
            ...
            return <button key={t[0]} onClick={function(){setActiveTab(t[0]);}} style={{background:act?G:"rgba(255,255,255,0.06)",border:"1px solid "+(act?G:BD),borderRadius:20,padding:"6px 16px",color:act?"#fff":T2,fontSize:12,fontWeight:act?700:500,cursor:"pointer",fontFamily:"inherit"}}>{t[1]}</button>;
          })}
        </div>
      </div>
      <div style={{padding:"12px 14px"}}>
        ...
        <div style={{background:"rgba(249,115,22,0.08)",border:"1px solid rgba(249,115,22,0.2)",borderRadius:10,padding:10,marginTop:4}}>
          <div style={{fontSize:12,color:"#F97316",lineHeight:1.7}}>This application is for educational purposes only. It does not provide investment advice or buy/sell recommendations. Users should conduct their own research before making any investment decisions.</div>
        </div>
      </div>
    </div>
  );
