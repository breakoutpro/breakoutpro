...
export default function EquityHome(props){
  ...
  return (
    <div style={{background:BG,minHeight:"100vh",fontFamily:"'Inter',Arial,sans-serif",paddingBottom:84,color:T1}}>
      ...
      {/* JUST IN - Moneycontrol style red ticker, 10s auto rotate, click to news */}
      <div onClick={function(){setTab("news");}} style={{background:"#0A0D12",borderBottom:"1px solid "+BD,display:"flex",alignItems:"stretch",overflow:"hidden",cursor:"pointer"}}>
        <div style={{background:"#EF4444",padding:"8px 11px",display:"flex",alignItems:"center",flexShrink:0}}>
          <span style={{width:5,height:5,borderRadius:"50%",background:"#fff",marginRight:6,animation:"pulse-dot 1.4s infinite"}}/>
          <span style={{fontSize:10,fontWeight:900,color:"#fff",letterSpacing:0.8}}>JUST IN</span>
        </div>
        <div style={{flex:1,display:"flex",alignItems:"center",padding:"0 12px",minWidth:0}}>
          <span key={tickerIdx} style={{fontSize:11,color:"#E8EAED",fontWeight:600,overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis",animation:"ticker-fade 0.4s ease"}}>{TICKER[tickerIdx]}</span>
        </div>
        <span style={{fontSize:13,color:T3,display:"flex",alignItems:"center",paddingRight:11,flexShrink:0}}>&#8250;</span>
      </div>
