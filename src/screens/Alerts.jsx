...
export default function SmartAlerts(props){
  ...
  useEffect(function(){
    var t=setInterval(function(){
      ...
      var basePrice=100+Math.floor(Math.random()*900)+Math.random();
      var trigPrice=basePrice.toFixed(2);
      var chgPct=((Math.random()*3-0.5)*(up?1:-1)).toFixed(2);
      var curPrice=(basePrice*(1+parseFloat(chgPct)/100)).toFixed(2);
      var volConf=Math.random()>0.4;
      addAlert({id:Date.now(),sym:sym,type:type,category:cat,conf:conf,up:up,trig:trigPrice,cur:curPrice,chg:chgPct,vol:volConf,ts:Date.now(),unread:true,time:new Date().toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"})});
    },8000);
          ...
          <div>
            {alerts.length==0?(
              <div style={{textAlign:"center",padding:"44px 26px"}}>
                <div style={{width:60,height:60,borderRadius:"50%",background:"rgba(34,197,94,0.08)",border:"1px solid rgba(34,197,94,0.2)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 14px"}}>
                  <span style={{fontSize:26}} dangerouslySetInnerHTML={{__html:"&#128276;"}}/>
                </div>
                <div style={{fontSize:13,fontWeight:800,color:T1,marginBottom:6}}>No alerts yet</div>
                <div style={{fontSize:10.5,color:T3,lineHeight:1.6,maxWidth:250,margin:"0 auto"}}>Your watchlist is being monitored. Alerts will appear automatically when a condition is triggered.</div>
              </div>
            ):(
