...
var CACHE = { quotes:null, quotesAt:0, global:null, globalAt:0, sectors:null, sectorsAt:0, hist:{}, histAt:{}, crumb:null, cookie:null, crumbAt:0 };
var QUOTE_TTL = 10 * 1000;
var MODERATE_TTL = 60 * 1000;
var HIST_TTL = 10 * 60 * 1000;
var UPSTREAM_TIMEOUT = 5000;
var CRUMB_TTL = 60 * 60 * 1000; // crumb/cookie stay valid well beyond an hour; refetched hourly to be safe

// Yahoo's v7 quote endpoint rejects bare unauthenticated requests with a
// 401 in current conditions - a session cookie plus a matching "crumb"
// token is required. This fetches both once and caches them; every quote
// request below reuses the cached pair instead of re-authenticating.
function getCrumb(){
  var now = Date.now();
  if(CACHE.crumb && CACHE.cookie && (now - CACHE.crumbAt) < CRUMB_TTL){
    return Promise.resolve({ crumb:CACHE.crumb, cookie:CACHE.cookie });
  }
  return fetch("https://fc.yahoo.com", { headers:{ "User-Agent":"Mozilla/5.0" } })
    .then(function(r){
      var setCookie = r.headers.get("set-cookie") || "";
      var cookie = setCookie.split(";")[0] || "";
      return fetch("https://query1.finance.yahoo.com/v1/test/getcrumb", {
        headers:{ "User-Agent":"Mozilla/5.0", "Cookie":cookie }
      }).then(function(cr){ return cr.text(); }).then(function(crumbText){
        if(crumbText && crumbText.indexOf("<")==-1){
          CACHE.crumb = crumbText.trim(); CACHE.cookie = cookie; CACHE.crumbAt = now;
          return { crumb:CACHE.crumb, cookie:CACHE.cookie };
        }
        return { crumb:null, cookie:null };
      });
    })
    .catch(function(){ return { crumb:null, cookie:null }; });
}
...
function fetchWithTimeout(url, cookie){
...
function fetchQuoteBatch(symbols){
  ...
  return getCrumb().then(function(auth){
    ...
    return fetchWithTimeout(url, auth.cookie).then(function(j){
...
var IDX = { NIFTY:"^NSEI", SENSEX:"^BSESN", BANKNIFTY:"^NSEBANK", FINNIFTY:"NIFTY_FIN_SERVICE.NS", VIX:"^INDIAVIX" };
var IDX_ENC = { NIFTY:"%5ENSEI", SENSEX:"%5EBSESN", BANKNIFTY:"%5ENSEBANK", FINNIFTY:"NIFTY_FIN_SERVICE.NS", VIX:"%5EINDIAVIX" }; // for chart endpoint
...
export default async function handler(req, res){
  ...
  try{
    if(!CACHE.quotes || (now - CACHE.quotesAt) >= QUOTE_TTL){
      CACHE.quotes = await fetchQuoteBatch([IDX.NIFTY, IDX.SENSEX, IDX.BANKNIFTY, IDX.FINNIFTY, IDX.VIX]);
      CACHE.quotesAt = now;
    }
  ...
  // --- Per-index daily history for all 4 core indices (extends the same
  // pattern already used for VIX below - reuses dailyHistory(), same cache
  // shape, same TTL). Provides real historical points for Market Snapshot
  // sparklines instead of only VIX having one. ---
  var indexHistory = {};
  var histSymbols = { NIFTY:IDX_ENC.NIFTY, BANKNIFTY:IDX_ENC.BANKNIFTY, SENSEX:IDX_ENC.SENSEX };
  for(var hk in histSymbols){
    if(!histSymbols.hasOwnProperty(hk)) continue;
    try{
      if(!CACHE.hist[hk] || (now - (CACHE.histAt[hk]||0)) > HIST_TTL){
        CACHE.hist[hk] = await dailyHistory(histSymbols[hk]);
        CACHE.histAt[hk] = now;
      }
      var hc = CACHE.hist[hk];
      if(hc && hc.length>=2){
        var recent = hc.slice(-10);
        var hcloses = recent.map(function(x){ return Math.round(x.c*100)/100; });
        var hcandles = recent.map(function(x){
          return {
            h: x.h!=null ? Math.round(x.h*100)/100 : null,
            l: x.l!=null ? Math.round(x.l*100)/100 : null,
            c: Math.round(x.c*100)/100
          };
        });
        indexHistory[hk] = { status:"DELAYED", points:hcloses, candles:hcandles, sessions:hc.length };
      } else {
        indexHistory[hk] = null;
      }
    }catch(e){ indexHistory[hk] = null; }
  }
  ...
  try{
    ...
    if(vc && vc.length>=5){
      var vrecent = vc.slice(-10);
      var closes = vrecent.map(function(x){ return Math.round(x.c*100)/100; });
      var vcandles = vrecent.map(function(x){
        return {
          h: x.h!=null ? Math.round(x.h*100)/100 : null,
          l: x.l!=null ? Math.round(x.l*100)/100 : null,
          c: Math.round(x.c*100)/100
      ...
      vixHistory = { status:"DELAYED", points:closes, candles:vcandles, sessions:vc.length, note:"Daily closes, last completed sessions" };
    }
  ...
  if(vixHistory && vixHistory.points) indexHistory.VIX = { status:vixHistory.status, points:vixHistory.points, candles:vixHistory.candles, sessions:vixHistory.sessions };
