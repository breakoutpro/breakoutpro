...
const LANGS = {en:"English",te:"Telugu",hi:"Hindi",ta:"Tamil",kn:"Kannada",ml:"Malayalam",gu:"Gujarati",mr:"Marathi",pa:"Punjabi",or:"Odia"};
...
const NEWS=[
  {id:1,cat:"Market",title:"NIFTY hits 22,800 amid strong FII buying",body:"FIIs bought Rs.8,245 Cr. NIFTY crosses key resistance.",time:"09:32",notif:true},
  {id:2,cat:"RBI",title:"RBI keeps repo rate at 6.5% - 6th hold",body:"MPC voted 4-2. Governor hints at rate cut in April 2025.",time:"10:15",notif:true},
  {id:3,cat:"Results",title:"TCS Q3 FY25: PAT Rs.12,380 Cr up 8.4% YoY",body:"Revenue Rs.61,237 Cr beats estimates. Deal wins 8.1B.",time:"07:30",notif:true},
  {id:4,cat:"Results",title:"HDFC Bank Q3: NII up 10%, Profit Rs.17,258 Cr",body:"Strong results. NIM at 3.5%. Gross NPA stable at 1.24%.",time:"16:05",notif:true},
  {id:5,cat:"Breakout",title:"TATASTEEL breaks above Rs.155 resistance",body:"Volume 3.2x above average. RSI 68. Study for education.",time:"09:42",notif:true},
  {id:6,cat:"Economy",title:"India GDP upgraded to 7.2% for FY25",body:"Strong domestic consumption. Manufacturing growing.",time:"14:20",notif:false},
  {id:7,cat:"Commodity",title:"Gold hits Rs.72,000 on global uncertainty",body:"MCX Gold surges 0.5%. Silver also up 0.7%.",time:"11:00",notif:false},
  {id:8,cat:"Auto",title:"India EV sales up 65% in December 2024",body:"Tata leads with 68%. Mahindra BE6e gets 1.2L bookings.",time:"12:30",notif:false},
  {id:9,cat:"Results",title:"SBI Q2: Profit Rs.18,331 Cr - Record High",body:"All-time record quarterly profit. Net NPA at 0.53%.",time:"16:30",notif:true},
  {id:10,cat:"SEBI",title:"SEBI new F&O margin rules from Feb 2025",body:"New peak margin norms. SEBI tightens FPI disclosure.",time:"15:45",notif:true},
  {id:11,cat:"Global",title:"US Fed signals 3 rate cuts in 2025",body:"FOMC minutes show easing consensus. Dollar weakens.",time:"23:00",notif:true},
];
...
export default function StocksBuddy(){
  ...
  var allCats=["All"].concat([...new Set(NEWS.map(function(n){return n.cat;}))]);
    ...
    <div style={Object.assign({},PS,{overflowY:"auto"})}>
      ...
      {otpSt==="form"&&<div style={{padding:"14px 18px 32px"}}>
        ...
        <div style={{marginTop:14,textAlign:"center"}}>
          <button onClick={function(){setShowLangModal(true);}} style={{background:"#0d0d0d",border:"1px solid #1a2a1a",borderRadius:10,padding:"9px 14px",color:"#39FF14",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",display:"inline-flex",alignItems:"center",gap:7}}>
            <span style={{fontSize:14}}>&#127760;</span>
            <span>Language: {LANGS[lang]||"English"}</span>
            <span style={{fontSize:9,color:"#666"}}>(Change)</span>
          </button>
        </div>
      ...
      {showLangModal&&(
        <div onClick={function(){setShowLangModal(false);}} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:"20px"}}>
          <div onClick={function(e){e.stopPropagation();}} style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:14,padding:"16px",maxWidth:340,width:"100%",maxHeight:"80vh",overflowY:"auto"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              <div>
                <div style={{fontSize:13,fontWeight:800,color:"#39FF14"}}>Select Language</div>
                <div style={{fontSize:8,color:"#445",marginTop:2}}>Choose your preferred language</div>
              </div>
              <button onClick={function(){setShowLangModal(false);}} style={{background:"#1a1a1a",border:"none",borderRadius:8,width:28,height:28,color:"#fff",fontSize:14,cursor:"pointer"}}>&#10005;</button>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:6}}>
              {Object.entries(LANGS).map(function(kl){return(
                <button key={kl[0]} onClick={function(){setLang(kl[0]);setShowLangModal(false);}} style={{background:lang===kl[0]?"#1a3a1a":"#0d0d0d",border:"1px solid "+(lang===kl[0]?"#39FF14":"#1a1a1a"),borderRadius:9,padding:"11px 9px",color:lang===kl[0]?"#39FF14":"#aaa",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",textAlign:"left"}}>
                  <div style={{fontSize:9,color:"#666",marginBottom:1}}>{kl[0].toUpperCase()}</div>
                  <div>{kl[1]}</div>
                </button>
              );})}
            </div>
            <div style={{marginTop:12,padding:"8px",background:"#0a1a0a",border:"1px solid #39FF1422",borderRadius:8,fontSize:8,color:"#aaa",textAlign:"center"}}>
              Languages: English, Telugu, Hindi, Tamil, Kannada, Malayalam, Gujarati, Marathi, Punjabi, Odia
            </div>
          </div>
        </div>
      )}
    ...
    <div style={PS}>
      ...
      <div style={{display:"flex",alignItems:"center",padding:"8px 9px 6px",borderBottom:"1px solid #0c0c0c",background:"rgba(0,0,0,0.98)",zIndex:50,flexShrink:0}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:5}}>
          ...
          <span style={{fontFamily:"monospace",fontSize:8,color:"#222"}}>{clk}</span>
          <button onClick={function(){setShowLangModal(true);}} style={{background:"#0d0d0d",border:"1px solid #1a2a1a",borderRadius:14,padding:"5px 9px",fontSize:9,fontWeight:700,color:"#39FF14",cursor:"pointer",fontFamily:"inherit",marginRight:5,display:"inline-flex",alignItems:"center",gap:3}}>
          ...
          <button onClick={function(){setShowNot(true);}} style={{background:"#0c0f00",border:"1px solid #1a1a1a",borderRadius:8,padding:"4px 6px",fontSize:14,cursor:"pointer",position:"relative"}}>
            &#128276;{unread>0&&<span style={{position:"absolute",top:-4,right:-4,background:"#ff3333",color:"#fff",fontSize:7,fontWeight:800,width:13,height:13,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",border:"1.5px solid #000"}}>{unread}</span>}
          </button>
      ...
      <div style={{overflowY:"auto",flex:1,paddingBottom:72}}>
        ...
        {tab==="home"&&(
          <div>
            <div style={{margin:"8px 9px",background:"#0d0d0d",border:"1px solid #1a1a1a",borderRadius:14,padding:"10px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <div>
                  <div style={{fontSize:11,fontWeight:800,color:"#39FF14"}}>{chartSym==="NSE%3ANIFTY"?"NIFTY 50":chartSym==="NSE%3ABANKNIFTY"?"BANK NIFTY":"SENSEX"} Live Chart</div>
                  <div style={{fontSize:8,color:"#445"}}>Real-time | TradingView</div>
                </div>
              ...
              <div style={{display:"flex",gap:4,marginBottom:6,flexWrap:"wrap"}}>
                {[["NSE%3ANIFTY","NIFTY"],["NSE%3ABANKNIFTY","BANKNIFTY"],["BSE%3ASENSEX","SENSEX"]].map(function(s){return(
                  <button key={s[0]} onClick={function(){setChartSym(s[0]);}} style={{flex:1,minWidth:70,background:chartSym===s[0]?"#39FF14":"#1a1a1a",color:chartSym===s[0]?"#000":"#aaa",border:"none",borderRadius:7,padding:"5px 8px",fontSize:9,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>{s[1]}</button>
                );})}
              </div>

              <div style={{display:"flex",gap:3,marginBottom:8,flexWrap:"wrap"}}>
                {[["1","1m"],["3","3m"],["5","5m"],["15","15m"],["60","1h"],["D","1D"]].map(function(t){return(
                  <button key={t[0]} onClick={function(){setChartInt(t[0]);}} style={{flex:1,minWidth:42,background:chartInt===t[0]?"#1a3a1a":"#0a0a0a",color:chartInt===t[0]?"#39FF14":"#666",border:"1px solid "+(chartInt===t[0]?"#39FF1466":"#1a1a1a"),borderRadius:6,padding:"4px 6px",fontSize:9,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>{t[1]}</button>
                );})}
              </div>

              <div style={{position:"relative",width:"100%",height:"340px",borderRadius:10,overflow:"hidden"}}>
                <iframe 
                  key={chartSym+chartInt}
                  src={"https://s.tradingview.com/widgetembed/?frameElementId=tv-chart&symbol="+chartSym+"&interval="+chartInt+"&hidesidetoolbar=0&symboledit=1&saveimage=1&toolbarbg=000000&studies=%5B%5D&theme=dark&style=1&timezone=Asia%2FKolkata&studies_overrides=%7B%7D&overrides=%7B%7D&enabled_features=%5B%5D&disabled_features=%5B%5D&locale=en"}
              ...
              <div style={{marginTop:7,padding:"6px 8px",background:"#0a1a0a",border:"1px solid #39FF1422",borderRadius:7,fontSize:8,color:"#aaa",lineHeight:1.5}}>
                <b style={{color:"#39FF14"}}>For Traders:</b> 1m=Scalping, 5m=Intraday, 15m=Swing, 1h=Positional, 1D=Investing
              </div>

              <div style={{marginTop:7,fontSize:8,color:"#445",textAlign:"center"}}>Educational only. Data via TradingView. Not investment advice.</div>
            </div>
          ...
          <div>
            {selSt?<StockDetail st={selSt} isPrem={isPrem} onBack={function(){setSelSt(null);}} onUp={function(){setShowSub(true);}}/>:(
              <div>
                ...
                {filtSt.map(function(st){return <SRow key={st.sym} st={st} onP={function(){setSelSt(st);}} full={true}/>;  })}
              </div>
            )}
          </div>
        )}
      ...
      <div style={{position:"absolute",bottom:0,left:0,right:0,background:"rgba(0,0,0,0.98)",borderTop:"1px solid #0d0d0d",display:"flex",justifyContent:"space-around",padding:"5px 3px 10px",zIndex:100}}>
        {[["home","&#127968;","Home"],["markets","&#128200;","Markets"],["ipo","&#127919;","IPO"],["fii","&#128176;","FII/DII"],["oi","&#128202;","OI"],["news","&#128240;","News"],["learn","&#128218;","Learn"]].concat(isAdmin?[["admin","&#128081;","Admin"]]:[]).map(function(item){
