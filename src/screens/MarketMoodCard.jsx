...
var UP="#22C55E",DOWN="#EF4444",GOLD="#D4AF37",BLUE="#3B82F6",CYAN="#60A5FA",WARN="#F97316";
...
export default function MarketMoodCard(props){
  ...
  return (
    <div style={{padding:"14px 14px 0",maxWidth:responsive.shell.centerMax,margin:"0 auto",width:"100%",boxSizing:"border-box"}}>
      <div onClick={function(){setOpen(true);}} style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:15,cursor:"pointer"}}>

        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",rowGap:8,marginBottom:11}}>
          <div style={{display:"flex",alignItems:"center",gap:7,minWidth:0}}>
            <span style={{fontSize:14,flexShrink:0}} dangerouslySetInnerHTML={{__html:"&#129504;"}}/>
            <span style={{fontSize:13,fontWeight:800,color:T1}}>AI Market Mood</span>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:5,background:CARD2,borderRadius:20,padding:"3px 10px",flexShrink:0}}>
            <span style={{width:6,height:6,borderRadius:"50%",background:badge.dot,display:"inline-block"}}></span>
            <span style={{fontSize:9,color:badge.col,fontWeight:700}}>{badge.text}</span>
          </div>
        </div>
        ...
        {loadingOrError ? (
          <div style={{padding:"14px 0"}}>
            ...
            <div style={{fontSize:9,color:T3,marginTop:4}}>No fabricated values are shown. Live data resumes automatically.</div>
          </div>
        ) : (
        <div>
          <div style={{display:"flex",alignItems:"center",gap:13,marginBottom:11}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:17,fontWeight:900,color:col}}>{label}</div>
              <div style={{fontSize:10,color:T2,marginTop:2}}>Stage: {stage}</div>
              <div style={{fontSize:9.5,color:T3,marginTop:1}}>{openingBias(scoreVal)}</div>
            </div>
          </div>

          <div style={{background:CARD2,borderRadius:10,padding:"9px 11px",marginBottom:9}}>
            <div style={{fontSize:8,color:T3,fontWeight:700,marginBottom:2}}>NOW</div>
            <div style={{fontSize:10.5,color:T1,fontWeight:600}}>{ai && ai.now ? ai.now : "Deterministic reading from live indices. AI commentary loading."}</div>
          </div>

          {ai && ai.keyDrivers && ai.keyDrivers.length ? (
            <div style={{marginBottom:9}}>
              <div style={{fontSize:8,color:T3,fontWeight:700,marginBottom:4}}>KEY DRIVERS</div>
              <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                {ai.keyDrivers.map(function(d,i){
                  return <span key={i} style={{fontSize:9,color:T2,background:CARD2,border:"1px solid "+BD,borderRadius:6,padding:"3px 7px"}}>{d}</span>;
                })}
              </div>
            </div>
          ) : null}

          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",rowGap:4,marginBottom:9}}>
            <span style={{fontSize:9,color:T3}}>Confidence: <span style={{color:mood.confidence=="High"?UP:mood.confidence=="Low"?DOWN:GOLD,fontWeight:700}}>{mood.confidence}</span></span>
            <span style={{fontSize:8.5,color:T3}}>{istTime(mm.lastUpdated)}</span>
          </div>

          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",rowGap:6,borderTop:"1px solid "+BD,paddingTop:9}}>
            <span style={{fontSize:8.5,color:T3,flex:"1 1 auto",minWidth:0}}>Educational Market Observation Only. Not Investment Advice.</span>
            <span style={{fontSize:11,color:CYAN,fontWeight:700,flexShrink:0}}>Details &#8594;</span>
          </div>
        </div>
        )}
