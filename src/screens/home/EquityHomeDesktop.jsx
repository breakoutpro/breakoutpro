...
export default function EquityHomeDesktop(props){
  ...
  var instrumentDataAvailable = data.workspaceHasLiveLtp || data.workspaceIsStock;
  ...
  function PanelHeader(p){
    ...
    return (
      <div onClick={function(){toggleCollapse(p.id);}} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 14px",cursor:"pointer",userSelect:"none"}}>
  ...
  function RailSection(p){
    ...
    return (
      <div style={{borderBottom:"1px solid "+BD2}}>
        ...
        {!isCollapsed ? <div style={{padding:"0 14px 14px"}}>{p.children}</div> : null}
      </div>
    );
  }
  ...
  var railSections = [
    {id:"aiMood", pinKey:"aiMood", title:"AI Market Mood"},
  ...
  function renderRailBody(id){
    ...
    if(id==="watchlist"){
      if(!data.wl.hasStoredWatchlist || data.wl.list.length===0) return <div style={{fontSize:11,color:T2}}>No watchlist symbols added yet</div>;
      return data.wl.list.slice(0,6).map(function(sym){
        var s = DEMO_STOCKS.filter(function(x){return x.sym===sym;})[0];
        return (
          <div key={sym} onClick={function(){data.selectStock(sym);}} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",cursor:"pointer"}}>
            <span style={{fontSize:12,fontWeight:700,color:T1}}>{sym}</span>
            {s ? <span style={{fontSize:12,fontWeight:700,color:s.chgPct>=0?UP:DOWN}}>{s.chgPct>=0?"+":""}{s.chgPct}%</span> : <ProvenanceBadge type="unavailable"/>}
          </div>
        );
      });
    }
  ...
  return (
    <div style={{background:BG,minHeight:"100vh",fontFamily:"'Inter',Arial,sans-serif",color:T1,overflowX:"hidden",display:"flex",flexDirection:"column"}}>
      ...
      {/* TOP BAR */}
      <div style={{display:"flex",alignItems:"center",padding:"10px 20px",borderBottom:"1px solid "+BD,gap:16}}>
        <div style={{fontSize:21,fontWeight:900,letterSpacing:-0.5,flexShrink:0}}>
          ...
          {searchOpen && searchResults.length>0 ? (
            <div style={{position:"absolute",top:"110%",left:0,right:0,background:CARD,border:"1px solid "+BD,borderRadius:10,overflow:"hidden",zIndex:20,boxShadow:"0 8px 24px rgba(0,0,0,0.4)"}}>
              {searchResults.map(function(s){
                return (
                  <div key={s.sym} onMouseDown={function(){data.selectStock(s.sym);setSearchQuery("");}} style={{padding:"8px 14px",cursor:"pointer",display:"flex",justifyContent:"space-between"}}>
      ...
      <div style={{display:"flex",borderBottom:"1px solid "+BD,background:CARD2}}>
        {data.spineRows.map(function(r,i){
          ...
          return (
            <div key={r.key} onClick={function(){data.selectIndex(r.key);}} style={{padding:"10px 20px",cursor:"pointer",borderRight:i<data.spineRows.length-1?"1px solid "+BD2:"none",background:active?"rgba(59,130,246,0.10)":"transparent",borderBottom:active?"2px solid "+BLUE:"2px solid transparent",display:"flex",alignItems:"center",gap:8}}>
        ...
        {data.workspaceIsStock ? (
          <div style={{padding:"10px 20px",display:"flex",alignItems:"center",gap:8,background:"rgba(59,130,246,0.10)",borderBottom:"2px solid "+BLUE}}>
      ...
      {/* WORKSPACE TABS */}
      <div style={{display:"flex",gap:22,padding:"0 20px",borderBottom:"1px solid "+BD}}>
        {["Breakout Intelligence","Overview","Scanner","Heatmap","FII/DII","News"].map(function(t,i){
          return <div key={t} style={{padding:"9px 0",fontSize:12,fontWeight:700,color:i===0?BLUE:T2,borderBottom:i===0?"2px solid "+BLUE:"2px solid transparent",cursor:"pointer"}}>{t}</div>;
        })}
      </div>

      {/* MAIN AREA - center workspace + resizable right rail */}
      <div style={{flex:1,display:"flex",minHeight:0,maxWidth:1900,margin:"0 auto",width:"100%"}}>

        {/* CENTER WORKSPACE - Breakout Intelligence */}
        <div style={{flex:1,minWidth:0,padding:"18px 22px",overflowY:"auto"}}>

          {/* Instrument header */}
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,paddingBottom:14,borderBottom:"1px solid "+BD2}}>
            <div>
              <div style={{fontSize:23,fontWeight:900,color:T1}}>{symLabel} <span style={{fontSize:11,fontWeight:700,color:T2,background:CARD2,border:"1px solid "+BD2,borderRadius:6,padding:"2px 6px",marginLeft:8}}>NSE</span></div>
            </div>
            <MarketMetric
              label="Live Price"
              ...
              color={(!data.workspaceHasLiveLtp && !data.workspaceIsStock) ? T3 : (isUp?UP:DOWN)} size={26} mono={true}
            />
          </div>

          {!instrumentDataAvailable ? (
            <div style={{padding:"40px 0",textAlign:"center"}}>
              <ProvenanceBadge type="unavailable" size="md"/>
              <div style={{fontSize:14,color:T2,marginTop:12}}>Live data for {symLabel} is not yet available. Breakout Intelligence requires a real, confirmed price feed and will not display estimated analysis.</div>
            </div>
          ) : (
          <>
          ...
          <div style={{marginBottom:18}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:4}}>
              <span style={{fontSize:23,fontWeight:900,color:verdictColor}}>{verdictText}</span>
          ...
          <div style={{marginBottom:20}}>
            <div style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid "+BD2}}>
          ...
          <div style={{marginBottom:20}}>
            <div style={{fontSize:13,fontWeight:800,color:T1,marginBottom:10}}>Support &amp; Resistance</div>
            {wz.resistance2 ? <div style={{display:"flex",justifyContent:"space-between",padding:"5px 10px"}}><span style={{fontSize:11,color:DOWN,fontWeight:700}}>R2</span><span style={{fontSize:12,fontWeight:800,fontFamily:"monospace"}}>{wz.resistance2.price}</span></div> : null}
            {wz.resistance ? <div style={{display:"flex",justifyContent:"space-between",padding:"5px 10px"}}><span style={{fontSize:11,color:DOWN,fontWeight:700}}>R1</span><span style={{fontSize:13,fontWeight:800,fontFamily:"monospace"}}>{wz.resistance.price}</span></div> : null}
            ...
            {wz.support ? <div style={{display:"flex",justifyContent:"space-between",padding:"5px 10px"}}><span style={{fontSize:11,color:UP,fontWeight:700}}>S1</span><span style={{fontSize:13,fontWeight:800,fontFamily:"monospace"}}>{wz.support.price}</span></div> : null}
            {wz.support2 ? <div style={{display:"flex",justifyContent:"space-between",padding:"5px 10px"}}><span style={{fontSize:11,color:UP,fontWeight:700}}>S2</span><span style={{fontSize:12,fontWeight:800,fontFamily:"monospace"}}>{wz.support2.price}</span></div> : null}
          </div>
          ...
          <div>
            ...
            {JUSTIN.slice(0,3).map(function(n){
              return <div key={n.id} style={{fontSize:12,color:T2,padding:"6px 0",borderBottom:"1px solid "+BD2}}>{n.headline}</div>;
            })}
          </div>
          </>
          )}
        ...
        {/* RIGHT RAIL - resizable, collapsible, pinnable */}
        <div style={{width:railWidth,flexShrink:0,borderLeft:"1px solid "+BD,background:CARD,overflowY:"auto"}}>
