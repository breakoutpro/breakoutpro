...
var UP_OBS=["Higher High","Breakout","Above VWAP","Above EMA","Bullish Engulfing","Cup & Handle","Strong Volume","Higher Low","Resistance Break","Positive OI Build-up"];
var DOWN_OBS=["Lower High","Breakdown","Below VWAP","Below EMA","Bearish Engulfing","Weak Volume","Lower Low","Support Break","Negative OI Build-up","Weak Relative Strength"];
    ...
    <div style={{background:BG,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:90}}>
      ...
      <div style={{padding:14}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8,marginBottom:18}}>
          ...
          <Sum k="Updated" v="Now" tone="blue" small={true}/>
        </div>
        ...
        {watch.length==0?(
          ...
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:11,marginTop:22}}>
...
function ObsCard(props){
  ...
  var o3=props.obs[(props.x.idx+6)%props.obs.length];
  return (
    <div onClick={props.onOpen} style={{background:CARD,border:"1px solid "+BD,borderLeft:"3px solid "+col,borderRadius:13,padding:13,marginBottom:10,cursor:"pointer"}}>
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:10}}>
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:"flex",alignItems:"center",gap:7,flexWrap:"wrap"}}>
            <span style={{fontSize:13.5,fontWeight:900,color:T1}}>{a.sym}</span>
            <span style={{fontSize:6.5,fontWeight:800,color:col,background:tone=="up"?"rgba(34,197,94,0.1)":"rgba(239,68,68,0.1)",border:"1px solid "+(tone=="up"?"rgba(34,197,94,0.3)":"rgba(239,68,68,0.3)"),padding:"2px 7px",borderRadius:4}}>{tone=="up"?"UPTREND":"DOWNTREND"}</span>
          </div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:10,paddingBottom:10,borderBottom:"1px solid "+BD2}}>
        ...
        <div style={{textAlign:"right"}}><div style={{fontSize:7.5,color:T3}}>Confidence</div><div style={{fontSize:12,fontWeight:800,color:col}}>{c.confidence}%</div></div>
      </div>

      {/* observation chips */}
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10}}>
        {[o1,o2,o3].map(function(o,i){
          return <span key={i} style={{fontSize:8.5,fontWeight:700,color:col,background:tone=="up"?"rgba(34,197,94,0.06)":"rgba(239,68,68,0.06)",border:"1px solid "+BD2,padding:"4px 8px",borderRadius:7}}>{o}</span>;
        })}
      </div>

      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <button onClick={function(e){e.stopPropagation();props.onRemove();}} style={{background:"none",border:"none",color:T3,fontSize:9,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Remove</button>
