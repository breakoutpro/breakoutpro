...
export function buildMT(theme) {
  return {
    ...
    SENT_GREEN: "#22C55E", SENT_RED: "#DC2626", SENT_YELLOW: "#EAB308"
  };
}
...
export function Gauge(props){
  ...
  var cx = 90, cy = 92, r = 70;
  var needleLen = 55;
  var tipX = cx + needleLen*Math.cos(ang), tipY = cy - needleLen*Math.sin(ang);
  var perp = ang + Math.PI/2, baseHalf = 4;
  var p1x = cx + baseHalf*Math.cos(perp), p1y = cy - baseHalf*Math.sin(perp);
  var p2x = cx - baseHalf*Math.cos(perp), p2y = cy + baseHalf*Math.sin(perp);
  ...
  return (
    <svg width="120" height="65" viewBox="0 0 180 100" style={{flexShrink:0}}>
      ...
      <path d={"M "+pts[2][0]+" "+pts[2][1]+" A "+r+" "+r+" 0 0 1 "+pts[3][0]+" "+pts[3][1]} fill="none" stroke="#22C55E" strokeWidth="12" strokeLinecap="round"/>
      <polygon points={p1x.toFixed(1)+","+p1y.toFixed(1)+" "+tipX.toFixed(1)+","+tipY.toFixed(1)+" "+p2x.toFixed(1)+","+p2y.toFixed(1)} fill="#FFFFFF"/>
