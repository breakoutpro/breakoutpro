...
const T={
  en:{
    home:t("home"),markets:t("markets"),oi:"OI & Charts",news:"News",learn:t("learn"),
    scanner:t("scanner"),tools:"Tools",mf:"MF/SIP",ipo:"IPO",admin:"Admin",
    ...
    noWatchlist:t("noWatchlist"),addStocks:t("addStocks"),
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",alignItems:"center",padding:"8px 9px",borderBottom:"1px solid #0d0d0d",background:"#050505",position:"sticky",top:0,zIndex:10}}>
        <button onClick={props.onBack} style={{background:"none",border:"none",color:"#39FF14",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit",padding:"3px 10px 3px 0"}}>{t("back")}</button>
        <div style={{flex:1}}>
      ...
      <div style={{padding:"8px 9px 0"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:7}}>
          <div style={{background:"#0a1a0a",border:"1px solid #39FF1433",borderRadius:9,padding:"10px"}}>
            <div style={{fontSize:7,color:"#39FF14",fontWeight:700,marginBottom:2}}>{t("support").toUpperCase()}</div>
          ...
          <div style={{background:"#1a0a0a",border:"1px solid #ff444433",borderRadius:9,padding:"10px"}}>
            <div style={{fontSize:7,color:"#ff4444",fontWeight:700,marginBottom:2}}>{t("resistance").toUpperCase()}</div>
...
export default function StocksBuddy(){
  ...
  // Jab bhi koi detail view open ho, history me ek entry push karo
  // Phone back swipe/button press pe popstate fire hota hai → close detail
  useEffect(function(){
    if(phase!=="app")return;
    function handlePop(){
      // Priority: selSt → sidebar → showSub → showNot → showDisc
      if(selSt){setSelSt(null);return;}
      if(sidebar){setSidebar(false);return;}
      if(showSub){setShowSub(false);return;}
      if(showNot){setShowNot(false);return;}
      if(showDisc){setShowDisc(false);return;}
      if(showTrm){setShowTrm(false);return;}
    }
    window.addEventListener("popstate",handlePop);
    return function(){window.removeEventListener("popstate",handlePop);};
  },[phase,selSt,sidebar,showSub,showNot,showDisc,showTrm]);

  // selSt open ainapudu → history stack lo push cheyyi
  useEffect(function(){
    if(phase!=="app")return;
    if(selSt){
      // Detail view open — push fake entry so back swipe works
      window.history.pushState({detail:selSt.sym},"","");
    }
  },[selSt,phase]);

  // Sidebar open ainapudu → push
  useEffect(function(){
    if(phase!=="app")return;
    if(sidebar){window.history.pushState({sidebar:true},"","");}
  },[sidebar,phase]);

  // Modal open ainapudu → push
  useEffect(function(){
    if(phase!=="app")return;
    if(showSub||showNot||showDisc||showTrm){
      window.history.pushState({modal:true},"","");
    }
  },[showSub,showNot,showDisc,showTrm,phase]);
            ...
            {selSt?(
              ...
              <div>
                ...
                <div style={{margin:"6px 9px 4px",display:"flex",alignItems:"center",gap:7,background:"#0d0d0d",border:"1px solid #1a1a1a",borderRadius:10,padding:"9px 11px"}}>
                  ...
                  <input value={srch} onChange={function(e){setSrch(e.target.value);}} placeholder={t("search")} style={{flex:1,background:"none",border:"none",outline:"none",color:"#e8eaf0",fontSize:12,fontFamily:"inherit"}}/>
