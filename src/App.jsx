...
export default function StocksBuddy(){
  ...
  // Approach: always keep ONE "app" entry in history stack.
  // Back press → close modals/detail only. Never let browser go back to login.
  useEffect(function(){
    if(phase!=="app")return;
    // Push initial app entry so we always have something to catch
  ...
  useEffect(function(){
    ...
    function handlePop(e){
      // Always push back immediately — prevent browser from going to login
      window.history.pushState({bp:"app"},"","");
      // Now close whatever is open, priority order:
      ...
      // Nothing open — do nothing (stay on current tab)
    }
