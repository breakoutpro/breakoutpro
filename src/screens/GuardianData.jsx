...
// Multi-timeframe candle observation strip. Returns per-TF status with direction.
// dir: 1 bullish, -1 bearish, 0 neutral. label optional pattern/structure text.
export function getMultiTF(asset){
  var c=getAssetCard(asset);
  var seed=0; var s=asset.sym; for(var i=0;i<s.length;i++){ seed+=s.charCodeAt(i); }
  var tfs=["1m","2m","3m","5m","15m","30m","1H","Daily"];
  var upLabels=["","","Doji","Bullish Engulfing","Above VWAP","Higher High","Above EMA","Uptrend"];
  var downLabels=["","M Pattern","Lower High","Bearish Engulfing","Below VWAP","Support Break","Downtrend","Lower Low"];
  var times=["5:46","5:45","5:43","5:40","5:30","5:15","",""];
  var out=[];
  for(var j=0;j<tfs.length;j++){
    // mostly follow the asset bias, with a couple of neutral/contra for realism
    var dir=c.up?1:-1;
    if((seed+j)%7==0) dir=0; // occasional neutral
    var label=c.up?upLabels[j]:downLabels[j];
    out.push({tf:tfs[j], dir:dir, label:label, time:times[j]});
  }
  return out;
}
...
// Alert mode per asset: off / normal / voice / priority. Reuses existing alert list.
export function loadAlertMode(sym){
  try{ return localStorage.getItem("bp_guardian_mode_"+sym)||"normal"; }catch(e){ return "normal"; }
}
export function saveAlertMode(sym,mode){
  try{ localStorage.setItem("bp_guardian_mode_"+sym,mode); }catch(e){}
}
// High-priority educational observations (used by priority mode).
export var PRIORITY_OBS = ["breakout","breakdown","trendchange","supportbreak","resistancebreak","strongbull","strongbear"];
export function isPriorityObs(id){
  var s=(id||"").toLowerCase();
  for(var i=0;i<PRIORITY_OBS.length;i++){ if(s.indexOf(PRIORITY_OBS[i])>=0) return true; }
  return false;
}
// Short educational voice phrase (under 3s). Never advisory.
export function voicePhrase(sym,pattern,tf){
  var p=pattern||"pattern";
  if(tf) return sym+". "+p+" on "+tf+" chart.";
  return sym+". "+p+" observed.";
}
