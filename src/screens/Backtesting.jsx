  ...
  return (
    <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:80}}>
      ...
      <div style={{padding:"12px 14px"}}>
          ...
          <div>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
              {TIMEFRAMES.map(function(t){
                var act=timeframe==t;
                return <button key={t} onClick={function(){setTimeframe(t);setResult(null);}} style={{background:act?"rgba(139,92,246,0.2)":"rgba(255,255,255,0.04)",border:"1px solid "+(act?BLUE:BD),borderRadius:6,padding:"3px 8px",color:act?"#A78BFA":T2,fontSize:8,fontWeight:act?700:400,cursor:"pointer",fontFamily:"inherit"}}>{t}</button>;
              })}
            </div>
          </div>
        </div>
        ...
        {/* Run Button */}
        <button onClick={runTest} disabled={running} style={{width:"100%",background:running?"rgba(59,130,246,0.3)":""+BLUE+"",border:"none",borderRadius:14,padding:"14px",color:"#fff",fontSize:13,fontWeight:700,cursor:running?"not-allowed":"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:14}}>
          {running?(
