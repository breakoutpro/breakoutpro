  ...
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:MT.BG,zIndex:350,overflowY:"auto"}}>
      ...
      <div style={{padding:wideLayout?"10px 24px 20px":"16px 24px 32px",maxWidth:1440,margin:"0 auto",width:"100%",boxSizing:"border-box"}}>
        ...
        <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:14,padding:padV,marginBottom:gapV,display:wideLayout?"flex":"block",alignItems:"center",gap:20,flexWrap:"wrap"}}>
          <SemicircleGauge mood={mood}/>
          <div style={{flexShrink:0}}>
            <div style={{display:"flex",gap:6,marginBottom:10}}>
              {[["BEARISH",MT.RED],["SIDEWAYS",MT.WARN],["BULLISH",MT.GREEN]].map(function(seg){
                var active = mood.label.toUpperCase().indexOf(seg[0])>=0 || (seg[0]==="SIDEWAYS" && mood.label.indexOf("Bull")<0 && mood.label.indexOf("Bear")<0);
                return (
                  <div key={seg[0]} style={{fontSize:10,fontWeight:800,padding:"4px 10px",borderRadius:6,color:active?"#fff":seg[1],background:active?seg[1]:seg[1]+"14",border:"1px solid "+seg[1]+(active?"":"55")}}>{seg[0]}</div>
                );
              })}
            </div>
            <div style={{fontSize:22,fontWeight:900,color:moodColor,marginBottom:6}}>Market Mood: {mood.label.toUpperCase()}</div>
            ...
            <div style={{fontSize:12,color:MT.T2,marginBottom:6}}>AI Bias: <b style={{color:moodColor}}>{mood.label}</b></div>
            <div style={{fontSize:10,color:MT.T3,background:MT.CARD2,border:"1px solid "+MT.BD,borderRadius:6,padding:"3px 8px",display:"inline-block"}}>AI Confidence Score: {mood.score}/100</div>
            {mm.lastUpdated ? <div style={{fontSize:11,color:MT.T3,marginTop:6}}>Last Updated: {new Date(mm.lastUpdated).toLocaleString("en-IN",{day:"numeric",month:"short",year:"numeric",hour:"numeric",minute:"2-digit"})}</div> : null}
          </div>
        ...
        {/* MARKET MOOD BREAKDOWN - percentages derived deterministically from
            the real mood.score via a zone-aware weighting (dominant zone
            gets more weight the deeper the score sits within its own zone,
            remainder split by proximity to the other two). Not a second
            data source - purely a visualization of the existing score. */}
        ...
        <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:14,padding:padV,marginBottom:gapV}}>
          {(function(){
            ...
            var rows = [["BEARISH",MT.RED,pct.bearish],["SIDEWAYS",MT.WARN,pct.sideways],["BULLISH",MT.GREEN,pct.bullish]];
        ...
        {/* WHY / WATCH / CHANGED / DRIVERS - single 4-column row on wide screens (was two separate 2-column rows), stacked on mobile. Same real ai.* fields as before. */}
            ...
            {usingDemo ? (
              <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:14,padding:padV}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,textAlign:"center"}}>
                  <div><div style={{fontSize:10,color:MT.T3}}>Advances</div><div style={{fontSize:13,fontWeight:800,color:MT.GREEN}}>{DEMO_BREADTH.advances} ({DEMO_BREADTH.advPct}%)</div></div>
                  <div><div style={{fontSize:10,color:MT.T3}}>Declines</div><div style={{fontSize:13,fontWeight:800,color:MT.RED}}>{DEMO_BREADTH.declines} ({DEMO_BREADTH.decPct}%)</div></div>
        ...
        {/* 3-DAY EVOLUTION + RISK LEVEL - paired 2-column. Support/Resistance,
            Scenarios and Important Events remain removed per an earlier
            round's explicit instruction to reduce this page's information
            architecture; Sector Rotation, Market Breadth, Global Markets and
            the Session table are restored above per this round's explicit
            "do not delete" instruction. Their underlying data functions were
            never deleted from the project either way. */}
