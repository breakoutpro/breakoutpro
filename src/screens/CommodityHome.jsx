      ...
      <div style={{background:"linear-gradient(180deg,#1A1208,"+BG+")",padding:"12px 16px",borderBottom:"1px solid "+BD}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <button onClick={function(){setTab("home");}} style={{background:"rgba(255,255,255,0.06)",border:"none",borderRadius:8,width:28,height:28,color:T1,fontSize:13,cursor:"pointer",flexShrink:0}}>&#8592;</button>
            <div>
          ...
          <div style={{textAlign:"right"}}>
            <div style={{fontFamily:"monospace",fontSize:11,fontWeight:600,color:T2}}>{timeStr}</div>
        ...
        <div style={{background:"rgba(245,158,11,0.08)",border:"1px solid rgba(245,158,11,0.25)",borderRadius:12,padding:"7px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            ...
            <span style={{fontSize:9,fontWeight:700,color:GOLD}}>MCX Commodity Live</span>
          </div>
          <span style={{fontSize:8,color:T3}}>3:30 PM - 11:30 PM</span>
        </div>
      </div>
      ...
      <div style={{padding:"10px 16px 0"}}>
        ...
        <div style={{background:"linear-gradient(135deg,#130F00,#101B2E)",border:"1px solid rgba(245,158,11,0.25)",borderRadius:18,padding:16,marginBottom:12,boxShadow:"0 4px 20px rgba(245,158,11,0.06)"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
            ...
            <span style={{fontSize:11,fontWeight:800,color:T1}}>AI Commodity Summary</span>
          </div>
          {[["GOLD","Bullish above Rs 71,500",UP],["CRUDE","Weak below Rs 6,850",DOWN],["NATURALGAS","Sideways movement",GOLD]].map(function(r){
            return (
              <div key={r[0]} style={{display:"flex",alignItems:"center",gap:8,marginBottom:7}}>
                ...
                <span style={{fontSize:10,fontWeight:700,color:T1}}>{r[0]}</span>
                <span style={{fontSize:10,color:T2}}>{r[1]}</span>
              </div>
            );
          })}
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
          {comms.map(function(c){
            return(
              <div key={c.sym} style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:"12px 14px",position:"relative",overflow:"hidden"}}>
                ...
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                  <div><div style={{fontSize:9,fontWeight:700,color:c.color}}>{c.sym}</div><div style={{fontSize:7,color:T3}}>{c.unit}</div></div>
                  <span style={{fontSize:9,fontWeight:700,color:c.up?UP:DOWN}}>{c.up?"+":""}{c.pct.toFixed(2)}%</span>
                </div>
          ...
          {ALERTS.map(function(a,i){
            return (
              <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 0",borderBottom:i<ALERTS.length-1?"1px solid "+BD:"none"}}>
                ...
                <div style={{flex:1}}>
                  <div style={{fontSize:10,fontWeight:700,color:T1}}>{a.sym} <span style={{color:a.color}}>{a.type}</span></div>
                  <div style={{fontSize:8,color:T2}}>{a.msg}</div>
                </div>
              </div>
            );
          })}
          ...
          {NEWS.map(function(n,i){
            return (
              <div key={i} style={{display:"flex",gap:8,marginBottom:i<NEWS.length-1?10:0,alignItems:"flex-start"}}>
                <span style={{background:(n.impact=="high"?DOWN:GOLD)+"18",color:n.impact=="high"?DOWN:GOLD,borderRadius:6,padding:"2px 7px",fontSize:7,fontWeight:700,flexShrink:0}}>{n.tag}</span>
                <span style={{fontSize:10,color:T1,lineHeight:1.5}}>{n.title}</span>
              </div>
            );
          })}
        </div>
        ...
        {/* 4. Global Factors */}
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:"12px 14px",marginBottom:12}}>
          <div style={{fontSize:9,fontWeight:700,color:T3,letterSpacing:0.8,marginBottom:10}}>GLOBAL FACTORS</div>
            ...
            {GLOBAL.map(function(g){
              return (
                <div key={g.label} style={{background:"rgba(255,255,255,0.03)",border:"1px solid "+BD,borderRadius:10,padding:"8px 10px"}}>
                  ...
                  <div style={{fontSize:11,fontWeight:700,color:T1}}>{g.val}</div>
                  {g.chg?<div style={{fontSize:8,fontWeight:600,color:g.up?UP:DOWN}}>{g.chg}</div>:null}
                </div>
              );
            })}
          </div>
        ...
        {/* 5. Tonight Events */}
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:"12px 14px",marginBottom:12}}>
          <div style={{fontSize:9,fontWeight:700,color:T3,letterSpacing:0.8,marginBottom:10}}>TONIGHT EVENTS</div>
            ...
            return (
              <div key={i} style={{display:"flex",alignItems:"center",gap:10,marginBottom:i<EVENTS.length-1?8:0}}>
                <div style={{background:e.color+"15",border:"1px solid "+e.color+"33",borderRadius:8,padding:"4px 8px",minWidth:58,textAlign:"center"}}>
                  <span style={{fontSize:8,fontWeight:700,color:e.color}}>{e.time}</span>
                </div>
                <span style={{fontSize:10,color:T1}}>{e.label}</span>
              </div>
            );
          })}
        </div>
        ...
        <div style={{background:"linear-gradient(135deg,#0E0820,#101B2E)",border:"1px solid rgba(124,58,237,0.25)",borderRadius:16,padding:"12px 14px",marginBottom:12}}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10}}>
            ...
            <span style={{fontSize:9,fontWeight:700,color:PURPLE2,letterSpacing:0.5}}>AI TRADE SETUP</span>
          </div>
            ...
            return (
              <div key={s.sym} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"rgba(255,255,255,0.03)",borderRadius:10,padding:"9px 12px",marginBottom:8}}>
                <div>
                  ...
                  <div style={{fontSize:9,color:s.color,fontWeight:600}}>{s.action} {s.action=="Buy"?"above":"below"} Rs{s.level.toLocaleString("en-IN")}</div>
                </div>
                <div style={{background:s.color+"18",border:"1px solid "+s.color+"33",borderRadius:8,padding:"4px 10px"}}>
                  <span style={{fontSize:9,fontWeight:800,color:s.color}}>{s.conf}%</span>
                </div>
              </div>
            );
          })}
          <div style={{fontSize:8,color:T3,marginTop:2}}>Educational analysis only. Not investment advice.</div>
        </div>

        {/* 7. OI & Sentiment */}
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:"12px 14px",marginBottom:14}}>
          <div style={{fontSize:9,fontWeight:700,color:T3,letterSpacing:0.8,marginBottom:10}}>COMMODITY OI and SENTIMENT</div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {OI_DATA.map(function(o){
              return <div key={o.sym} style={{background:o.color+"12",border:"1px solid "+o.color+"33",borderRadius:8,padding:"5px 10px"}}><span style={{fontSize:9,fontWeight:700,color:o.color}}>{o.sym}: {o.signal}</span></div>;
            })}
          </div>
        </div>
        ...
        <div style={{background:"rgba(249,115,22,0.06)",border:"1px solid rgba(249,115,22,0.15)",borderRadius:10,padding:10,marginBottom:10}}>
          <div style={{fontSize:8,color:"#F97316",lineHeight:1.7}}>This is AI-generated market analysis for educational purposes only. It is not investment advice. Please do your own research before trading.</div>
        </div>
