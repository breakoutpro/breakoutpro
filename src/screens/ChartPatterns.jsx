    ...
    <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:80}}>
      <div style={{background:CB,padding:"12px 16px",borderBottom:"1px solid "+BD}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <div>
            <div style={{fontSize:18,fontWeight:900,color:T1}}>Chart <span style={{color:BLUE}}>Patterns</span></div>
            <div style={{fontSize:12,color:T2}}>{PATTERNS.length} patterns  Beginner to Pro</div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:12,color:T2}}>Progress</div>
            <div style={{fontSize:14,fontWeight:900,color:progress==100?BLUE:BLUE}}>{progress}%</div>
          </div>
        </div>
        <div style={{height:3,background:"rgba(255,255,255,0.06)",borderRadius:2,marginBottom:8,overflow:"hidden"}}>
          <div style={{height:"100%",width:progress+"%",background:progress==100?BLUE:BLUE,borderRadius:2}}></div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:8,background:"rgba(255,255,255,0.05)",border:"1px solid "+BD,borderRadius:10,padding:"8px 12px",marginBottom:8}}>
        ...
        <div style={{display:"flex",gap:4,overflowX:"auto",paddingBottom:4}}>
          {FILTERS.map(function(f){var act=filter==f;return <button key={f} onClick={function(){setFilter(f);}} style={{background:act?BLUE:"rgba(255,255,255,0.05)",border:"1px solid "+(act?BLUE:BD),borderRadius:20,padding:"4px 12px",color:act?"#fff":T2,fontSize:12,fontWeight:act?700:400,cursor:"pointer",fontFamily:"inherit",flexShrink:0}}>{f}</button>;})}
        </div>
      </div>
      <div style={{padding:"12px 16px 0"}}>
        {progress==100?<div style={{background:theme.c.card,border:"1px solid rgba(245,158,11,0.3)",borderRadius:16,padding:16,marginBottom:12,textAlign:"center"}}><div style={{fontSize:18,fontWeight:900,color:BLUE}}>Chart Master Badge!</div><div style={{fontSize:12,color:T2}}>All {PATTERNS.length} patterns studied</div></div>:null}
        {recentPats.length>0&&!search&&filter=="All"?(
          <div style={{marginBottom:12}}>
            <div style={{fontSize:12,fontWeight:700,color:T2,marginBottom:8}}>RECENTLY VIEWED</div>
            <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:4}}>
              {recentPats.map(function(p){var tc=p.type=="Bullish"?G2:p.type=="Bearish"?R:BLUE;return <div key={p.id} onClick={function(){open(p);}} style={{background:CB,border:"1px solid "+BD,borderRadius:16,padding:"8px 12px",flexShrink:0,cursor:"pointer",minWidth:90,textAlign:"center"}}><div style={{fontSize:12,fontWeight:700,color:tc}}>{p.type}</div><div style={{fontSize:12,fontWeight:700,color:T1}}>{p.name.slice(0,12)}</div></div>;})}
            </div>
          </div>
        ):null}
        <div style={{fontSize:12,fontWeight:700,color:T2,marginBottom:8}}>{filtered.length} PATTERNS</div>
        ...
        {filtered.length==0?<div style={{textAlign:"center",padding:"40px 0",color:T2}}><div style={{fontSize:14,fontWeight:700,marginBottom:4}}>No patterns found</div><div style={{fontSize:12}}>Try different search or filter</div></div>:null}
      </div>
    </div>
  );
}
