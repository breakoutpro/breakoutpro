    ...
    <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:80}}>
      ...
      <div style={{padding:"14px"}}>
        ...
        {profile.kycStatus!="verified"&&(
          <div style={{background:"rgba(245,158,11,0.08)",border:"1px solid rgba(245,158,11,0.25)",borderRadius:14,padding:"12px 14px",marginBottom:14}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
              <span style={{fontSize:14}}>&#9888;&#65039;</span>
              <span style={{fontSize:11,fontWeight:700,color:GOLD}}>Complete your KYC</span>
            </div>
            <div style={{fontSize:10,color:T2,lineHeight:1.5,marginBottom:10}}>Add PAN and complete verification to unlock broker account linking and premium features.</div>
            <button onClick={function(){update("kycStatus","pending");setEditMode(true);}} style={{background:GOLD,border:"none",borderRadius:10,padding:"8px 16px",color:"#000",fontSize:10,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Complete KYC</button>
          </div>
        )}

        {/* Personal Info */}
        <div style={{background:CB,border:"1px solid "+BD,borderRadius:16,padding:"14px",marginBottom:14}}>
          <div style={{fontSize:11,fontWeight:700,color:T1,marginBottom:12}}>Personal Information</div>
        ...
        <div style={{background:"rgba(59,130,246,0.06)",border:"1px solid rgba(59,130,246,0.2)",borderRadius:10,padding:"8px 12px"}}>
          <div style={{fontSize:8,color:BLUE2}}>Your data is stored locally on this device. BreakoutPro does not collect or share personal data with third parties.</div>
        </div>
      </div>
    </div>
  );
