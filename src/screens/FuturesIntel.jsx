    ...
    return (
      <div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:12}}>
          <span style={{fontSize:7,fontWeight:800,color:GOLD,background:"rgba(212,175,55,0.12)",border:"1px solid rgba(212,175,55,0.3)",padding:"2px 6px",borderRadius:4}}>EDUCATIONAL DATA</span>
          <span style={{fontSize:8.5,color:T3}}>{d.symbol} {d.spot} &#8226; Not live</span>
        </div>
      </div>
    );
  }
...
export function GammaBlast(props){
  var theme = useTheme(); // reuses the existing ThemeProvider - no new theme system
  // Theme-sourced overrides - shadow the module-level hardcoded fallbacks above.
  var BD2 = theme.c.border2, BLUE = theme.c.blue, CARD = theme.c.card, GOLD = theme.c.gold, T2 = theme.c.text2, T3 = theme.c.text3, WARN = theme.c.warn; T1=theme.c.text1;
  ...
  return (
    <div style={{padding:"0 14px",marginTop:full?6:22}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:12}}>
        <span style={{fontSize:7,fontWeight:800,color:GOLD,background:"rgba(212,175,55,0.12)",border:"1px solid rgba(212,175,55,0.3)",padding:"2px 6px",borderRadius:4}}>EDUCATIONAL DATA</span>
        <span style={{fontSize:8.5,color:T3}}>{g.symbol} {g.spot} &#8226; Not live</span>
      </div>
      <div style={{background:CARD,border:"1px solid "+BD,borderRadius:13,padding:"4px 13px"}}>
...
function Sec(props){
 ...
 return <div style={{marginBottom:16}}><div style={{fontSize:11,fontWeight:800,color:T2,letterSpacing:0.4,marginBottom:8}}>{props.title}</div>{props.children}</div>; }
function Disc(){
  var theme = useTheme(); // reuses the existing ThemeProvider - no new theme system
  // Theme-sourced overrides - shadow the module-level hardcoded fallbacks above.
  var WARN = theme.c.warn;
 return <div style={{background:"rgba(249,115,22,0.06)",border:"1px solid rgba(249,115,22,0.15)",borderRadius:10,padding:11,marginBottom:16}}><div style={{fontSize:8.5,color:WARN,lineHeight:1.5}}>Educational Market Observation Only. Not Investment Advice.</div></div>; }
