...
function CandleSVG(props){
  var type=props.type;
  var col=type=="bullish"?G2:type=="bearish"?R:T2;
  var shapes={
...
export default function CandleLibrary(props){
  ...
  if(selected){
    ...
    var badgeCol=selected.type=="bullish"?G2:selected.type=="bearish"?R:BLUE;
    return (
      <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:32}}>
        <div style={{background:CB,padding:"12px 16px",borderBottom:"1px solid "+BD,display:"flex",alignItems:"center",gap:12}}>
          <button onClick={function(){setSelected(null);}} style={{background:"rgba(255,255,255,0.06)",border:"none",borderRadius:8,width:44,height:44,color:T1,fontSize:14,cursor:"pointer"}}>&#8592;</button>
          <div style={{fontSize:14,fontWeight:800,color:T1}}>{selected.name}</div>
        </div>
        <div style={{padding:16}}>
          {/* large animated SVG */}
          <div style={{background:CB,border:"1px solid "+BD,borderRadius:16,padding:24,display:"flex",justifyContent:"center",marginBottom:16}}>
            <CandleIcon name={selected.name} size={140} animate={true}/>
          </div>
          <div style={{textAlign:"center",marginBottom:16}}>
            <span style={{background:(selected.type=="bullish"?"rgba(0,200,83,0.1)":selected.type=="bearish"?"rgba(239,68,68,0.1)":"rgba(245,158,11,0.1)"),borderRadius:20,padding:"4px 16px",fontSize:12,fontWeight:700,color:badgeCol,textTransform:"capitalize"}}>{selected.type} Signal</span>
          </div>
          {det?[
            ...
            return (
              <div key={i} style={{background:CB,border:"1px solid "+BD,borderRadius:16,padding:12,marginBottom:8}}>
                <div style={{fontSize:12,fontWeight:800,color:T3,letterSpacing:0.4,marginBottom:4}}>{row[0].toUpperCase()}</div>
                <div style={{fontSize:12,color:T2,lineHeight:1.6}}>{row[1]}</div>
              </div>
            );
          }):<div style={{background:CB,border:"1px solid "+BD,borderRadius:16,padding:12,marginBottom:8}}><div style={{fontSize:12,color:T2,lineHeight:1.6}}>{selected.desc}</div></div>}
          <div style={{display:"flex",justifyContent:"space-between",background:CB,border:"1px solid "+BD,borderRadius:16,padding:12,marginBottom:8}}>
            <span style={{fontSize:12,color:T3}}>Reliability</span>
            <span style={{fontSize:12,fontWeight:800,color:selected.reliability=="High"?G2:selected.reliability=="Medium"?BLUE:T2}}>{selected.reliability}</span>
          </div>
  ...
  return (
    <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:32}}>
      <div style={{background:CB,padding:"12px 16px",borderBottom:"1px solid "+BD}}>
        <div style={{fontSize:16,fontWeight:900,color:T1,marginBottom:4}}>Candle <span style={{color:BLUE}}>Library</span></div>
        <div style={{fontSize:12,color:T2}}>{CANDLES.length} candlestick patterns - quick reference</div>
      </div>
      ...
      <div style={{padding:"12px 16px"}}>
        <div style={{display:"flex",gap:8,marginBottom:16}}>
          {[["all","All"],["bullish","Bullish"],["bearish","Bearish"],["neutral","Neutral"]].map(function(f){
            var act=filter==f[0];
            var col=f[0]=="bullish"?G2:f[0]=="bearish"?R:f[0]=="neutral"?BLUE:BLUE;
            return <button key={f[0]} onClick={function(){setFilter(f[0]);}} style={{flex:1,background:act?col+"18":"rgba(255,255,255,0.04)",border:"1px solid "+(act?col:BD),borderRadius:8,padding:"8px 4px",color:act?col:T2,fontSize:12,fontWeight:act?700:400,cursor:"pointer",fontFamily:"inherit"}}>{f[1]}</button>;
          })}
        </div>

        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {filtered.map(function(c){
            var col=c.type=="bullish"?G2:c.type=="bearish"?R:BLUE;
            return (
              <div key={c.name} onClick={function(){setSelected(c);}} style={{background:CB,border:"1px solid "+BD,borderRadius:16,padding:"12px 12px",cursor:"pointer",display:"flex",alignItems:"center",gap:12}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:12,color:col,fontWeight:700,textTransform:"capitalize",marginTop:4}}>{c.type}</div>
                  <div style={{fontSize:12,color:T2,marginTop:4,lineHeight:1.4,overflow:"hidden",textOverflow:"ellipsis",display:"-webkit-box",WebkitLineClamp:1,WebkitBoxOrient:"vertical"}}>{c.desc}</div>
                </div>
