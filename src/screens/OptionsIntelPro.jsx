...
function inputStyle(){
  return {width:"100%",background:"#0B0E13",border:"1px solid "+BD,borderRadius:9,padding:"9px 10px",color:T1,fontSize:12,fontFamily:"inherit",boxSizing:"border-box",marginBottom:10};
}
function labelStyle(){
  return {fontSize:12,color:T2,fontWeight:700,display:"block",marginBottom:5};
}
    ...
    return (
      <div style={{background:theme.c.bg,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
        <div style={{background:CARD,padding:"12px 14px",borderBottom:"1px solid "+BD,display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            <div style={{fontSize:15,fontWeight:800,color:T1}}>Options Intelligence Pro</div>
        ...
        <div style={{padding:14}}>
          {SECTIONS.map(function(s){
            return (
              <div key={s.id} onClick={function(){setSection(s.id);}} style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,marginBottom:10,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <div style={{background:"rgba(249,115,22,0.06)",border:"1px solid rgba(249,115,22,0.15)",borderRadius:10,padding:11,marginTop:10}}>
            <div style={{fontSize:12,color:WARN,lineHeight:1.5}}>Educational only. No live market data anywhere in this module. Not investment advice.</div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div>
      {GREEKS.map(function(g){
        ...
        return (
          <div key={g.id} style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,marginBottom:10}}>
            <div onClick={function(){setOpenId(open?null:g.id);}} style={{display:"flex",justifyContent:"space-between",cursor:"pointer"}}>
              <span style={{fontSize:13,fontWeight:800,color:T1}}>{g.name}</span>
...
function Field(props){
  ...
  return (
    <div style={{marginBottom:8}}>
      <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:3}}>{props.label.toUpperCase()}</div>
...
function GreeksSimulator(){
  ...
  return (
    <div>
      <div style={{fontSize:12,color:T3,marginBottom:14,lineHeight:1.5}}>Enter practice values below. This uses a standard simplified approximation formula for educational illustration only - never a live or guaranteed value.</div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:12}}>
        <button onClick={function(){setIsCall(true);}} style={{flex:1,background:isCall?BLUE:"transparent",border:"1px solid "+(isCall?BLUE:BD),borderRadius:9,padding:9,color:isCall?"#fff":T2,fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>Call</button>
        <button onClick={function(){setIsCall(false);}} style={{flex:1,background:!isCall?BLUE:"transparent",border:"1px solid "+(!isCall?BLUE:BD),borderRadius:9,padding:9,color:!isCall?"#fff":T2,fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>Put</button>
      </div>

      <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14}}>
        <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10}}>APPROXIMATE EDUCATIONAL IMPACT</div>
        {r ? (
          <div>
...
function MaxPainLearning(){
  var theme = useTheme(); // reuses the existing ThemeProvider - no new theme system
  // Theme-sourced overrides - shadow the module-level hardcoded fallbacks above.
  var BD = theme.c.border, CARD = theme.c.card, T2 = theme.c.text2, T3 = theme.c.text3, WARN = theme.c.warn;
  ...
  return (
    <div>
      ...
      <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,marginTop:10}}>
        <div style={{fontSize:12,fontWeight:800,color:WARN,marginBottom:8}}>{MAX_PAIN_SAMPLE.label.toUpperCase()}</div>
        {MAX_PAIN_SAMPLE.strikes.map(function(row){
          return (
            <div key={row.strike} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:"1px solid "+BD}}>
        ...
        <div style={{marginTop:10,fontSize:12,fontWeight:700,color:T1}}>Computed Max Pain strike (from this example only): {maxPainStrike}</div>
      </div>
    </div>
  );
}
...
function CallPutWalls(){
  var theme = useTheme(); // reuses the existing ThemeProvider - no new theme system
  // Theme-sourced overrides - shadow the module-level hardcoded fallbacks above.
  var BD = theme.c.border, CARD = theme.c.card, WARN = theme.c.warn;
...
function OIBuilder(){
  ...
  return (
    <div>
      <div style={{fontSize:12,color:T3,marginBottom:14,lineHeight:1.5}}>Build your own practice option chain below, then see the resulting educational read.</div>
      {rows.map(function(r,i){
        return (
          <div key={i} style={{display:"flex",gap:6,marginBottom:8}}>
            <input placeholder="Strike" style={Object.assign({},inputStyle(),{marginBottom:0})} type="number" value={r.strike} onChange={function(e){updateRow(i,"strike",e.target.value);}}/>
            <input placeholder="Call OI" style={Object.assign({},inputStyle(),{marginBottom:0})} type="number" value={r.callOI} onChange={function(e){updateRow(i,"callOI",e.target.value);}}/>
            <input placeholder="Put OI" style={Object.assign({},inputStyle(),{marginBottom:0})} type="number" value={r.putOI} onChange={function(e){updateRow(i,"putOI",e.target.value);}}/>
          </div>
        );
      })}
      <button onClick={addRow} style={{background:"transparent",border:"1px solid "+BD,borderRadius:9,padding:"8px 12px",color:T2,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",marginBottom:14,minHeight:44}}>+ Add Strike Row</button>
  ...
  return (
    <div>
      {STRATEGY_CARDS.map(function(s){
        return (
          <div key={s.name} style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,marginBottom:10}}>
            <div style={{fontSize:13,fontWeight:800,color:T1,marginBottom:8}}>{s.name}</div>
...
function MiniQuiz(props){
  ...
  if(submitted){
    ...
    return (
      <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:16,textAlign:"center"}}>
        <div style={{fontSize:14,fontWeight:800,color:BLUE,marginBottom:6}}>Quiz Complete</div>
  ...
  return (
    <div>
      <div style={{fontSize:12,color:T3,marginBottom:10}}>Question {idx+1} of {QUIZ_QUESTIONS.length}</div>
      <div style={{fontSize:13,fontWeight:700,color:T1,marginBottom:14}}>{q.q}</div>
      {q.options.map(function(opt,oi){
        var picked = answers[idx]==oi;
        return (
          <div key={oi} onClick={function(){pick(oi);}} style={{border:"1px solid "+(picked?BLUE:BD),background:picked?"rgba(59,130,246,0.1)":"transparent",borderRadius:9,padding:"11px 13px",marginBottom:8,fontSize:12,color:T1,cursor:"pointer"}}>{opt}</div>
        );
      })}
      <button onClick={next} disabled={answers[idx]==null} style={{width:"100%",background:answers[idx]!=null?BLUE:CARD,border:"none",borderRadius:10,padding:12,color:answers[idx]!=null?"#fff":T3,fontSize:12,fontWeight:800,cursor:answers[idx]!=null?"pointer":"default",fontFamily:"inherit",marginTop:6,minHeight:44}}>{idx+1<QUIZ_QUESTIONS.length?"Next":"Finish Quiz"}</button>
    </div>
  );
}
