...
export default function EquityHomeDesktop(props){
  ...
  var keyLevelsZones = useMemo(function(){
    ...
    var candles = generateDemoCandles(price, tierCfg.keyLevelsCandles, sym);
    ...
    return zones;
  }, [keyLevelsIdx, data.spineRows, tierCfg.keyLevelsCandles]);
  ...
  var scannerPreview = useMemo(function(){
    return DEMO_STOCKS.slice(0,tierCfg.scannerRows).map(function(s){
      ...
      return {sym:s.sym, verdict:sbi.verdict, confidence:confidence, zoneLevel:zoneLevel, zoneLabel:zoneLabel, riskLevel:riskLevel, volume:lastCandle.vol};
    });
  }, [tierCfg.scannerRows]);
    ...
    <div style={{background:BG,minHeight:"100vh",fontFamily:"'Inter',Arial,sans-serif",color:T1,overflowX:"hidden",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,display:"flex",minHeight:0,width:"100%"}}>
        ...
        <div style={{flex:1,minWidth:0,padding:"12px 16px",overflowY:"auto"}}>
          ...
          {/* Row 1: AI Market Mood | Breakout Intelligence (4-card strip) */}
          <div style={{display:"grid",gridTemplateColumns:"260px minmax(0, 1fr)",gap:tierCfg.gridGap,marginBottom:tierCfg.gridGap}}>
              ...
              {(function(){
                ...
                return (
                  <div>
                    ...
                    <div style={{padding:"4px 0"}}><div style={{fontSize:tierCfg.label,color:T3,marginBottom:2}}>Confidence Level</div><div style={{fontSize:tierCfg.secondaryText,fontWeight:800,color:T1}}>{mood.confidence}</div></div>
                  </div>
                );
              })()}
            </div>
          ...
          {/* Row 2: Scanner Summary | Key Levels - side by side, matching the reference layout, not stacked */}
          <div style={{display:"grid",gridTemplateColumns:"minmax(0, 2fr) minmax(0, 3fr)",gap:tierCfg.gridGap,marginBottom:tierCfg.gridGap}}>
            <div style={{background:CARD2,border:"1px solid "+BD,borderRadius:10,padding:Math.max(8,tierCfg.cardPadding-4),height:tierCfg.keyLevelsHeight,boxSizing:"border-box",overflowY:"auto"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <span style={{fontSize:tierCfg.widgetTitle,fontWeight:800,color:T1}}>Scanner Summary</span>
                  <ProvenanceBadge type="calculated"/>
                </div>
                <button onClick={function(){setTab("scan");}} style={{background:"none",border:"none",color:BLUE,fontSize:tierCfg.label,fontWeight:700,cursor:"pointer"}}>View Full Scanner &#8594;</button>
              </div>
              <table style={{width:"100%",borderCollapse:"collapse",tableLayout:"fixed"}}>
                <thead>
                  <tr style={{borderBottom:"1px solid "+BD2}}>
                    <th style={{width:"28%",textAlign:"left",fontSize:tierCfg.label,color:T3,fontWeight:700,padding:"4px 4px"}}>STOCK</th>
                    <th style={{width:"18%",textAlign:"right",fontSize:tierCfg.label,color:T3,fontWeight:700,padding:"4px 4px"}}>PRICE</th>
                    <th style={{width:"18%",textAlign:"right",fontSize:tierCfg.label,color:T3,fontWeight:700,padding:"4px 4px"}}>CHG%</th>
                    <th style={{width:"18%",textAlign:"right",fontSize:tierCfg.label,color:T3,fontWeight:700,padding:"4px 4px"}}>VOLUME</th>
                    <th style={{width:"18%",textAlign:"right",fontSize:tierCfg.label,color:T3,fontWeight:700,padding:"4px 4px"}}>CONFIDENCE</th>
                  </tr>
                </thead>
                <tbody>
                  {scannerPreview.map(function(sr){
                    var s = DEMO_STOCKS.filter(function(x){return x.sym===sr.sym;})[0];
                    var volDisplay = sr.volume>=10000000 ? (sr.volume/10000000).toFixed(1)+"Cr" : sr.volume>=100000 ? (sr.volume/100000).toFixed(1)+"L" : sr.volume.toLocaleString("en-IN");
                    return (
                      <tr key={sr.sym} onClick={function(){data.selectStock(sr.sym);}} style={{borderBottom:"1px solid "+BD2,cursor:"pointer",height:Math.max(24,tierCfg.tableRowHeight-8)}}>
                        <td style={{fontSize:tierCfg.secondaryText,fontWeight:700,color:T1,padding:"2px 4px"}}>{sr.sym}</td>
                        <td style={{fontSize:tierCfg.secondaryText,color:T1,textAlign:"right",fontFamily:"monospace",padding:"2px 4px"}}>{s?s.ltp.toLocaleString("en-IN"):"--"}</td>
                        <td style={{fontSize:tierCfg.secondaryText,fontWeight:700,textAlign:"right",color:s&&s.chgPct>=0?UP:DOWN,padding:"2px 4px"}}>{s?((s.chgPct>=0?"+":"")+s.chgPct+"%"):"--"}</td>
                        <td style={{fontSize:tierCfg.secondaryText,color:T1,textAlign:"right",fontFamily:"monospace",padding:"2px 4px"}}>{volDisplay}</td>
                        <td style={{fontSize:tierCfg.secondaryText,fontWeight:800,color:BLUE,textAlign:"right",padding:"2px 4px"}}>{sr.confidence}%</td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <div style={{background:CARD2,border:"1px solid "+BD,borderRadius:10,padding:tierCfg.cardPadding,height:tierCfg.keyLevelsHeight,boxSizing:"border-box",display:"flex",flexDirection:"column"}}>
              ...
              <div style={{height:tierCfg.keyLevelsHeight-90,marginBottom:10,overflow:"hidden"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(5, minmax(0, 1fr))",gap:6,textAlign:"center"}}>
          ...
          {/* Row 2b: Options Intelligence - compact summary, real underlying function shared with Mobile */}
          <div style={{background:CARD2,border:"1px solid "+BD,borderRadius:10,padding:tierCfg.cardPadding,marginBottom:tierCfg.gridGap}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:tierCfg.widgetTitle,fontWeight:800,color:T1}}>Options Intelligence</span>
                <ProvenanceBadge type="demo"/>
              </div>
              <button onClick={function(){setTab("optionsintelpro");}} style={{background:"none",border:"none",color:BLUE,fontSize:tierCfg.label,fontWeight:700,cursor:"pointer"}}>View Full Options Intelligence &#8594;</button>
            </div>
            {(function(){
              ...
              return (
                <div style={{display:"grid",gridTemplateColumns:"repeat("+picked.length+", minmax(0, 1fr))",gap:tierCfg.gridGap}}>
                  {picked.map(function(m){
                    return (
                      <div key={m.key}>
                        <div style={{fontSize:tierCfg.label,color:T3,marginBottom:3}}>{m.label}</div>
                        <div style={{fontSize:tierCfg.secondaryText,fontWeight:800,color:toneColor(m.tone)}}>{m.val}</div>
                      </div>
                    );
                  })}
                </div>
          ...
          {/* Row 3: Market Breadth (donut) | Sector Strength (toggle) | Market Heatmap */}
          <div style={{display:"grid",gridTemplateColumns:"minmax(0, 1fr) minmax(0, 1fr) minmax(0, 1fr)",gap:tierCfg.gridGap,marginBottom:tierCfg.gridGap}}>
            <div style={{background:CARD2,border:"1px solid "+BD,borderRadius:10,padding:tierCfg.cardPadding,height:tierCfg.supportingHeight,boxSizing:"border-box",overflow:"hidden"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
              ...
              {(function(){
                ...
                var r=34, circ=2*Math.PI*r; // fixed - Market Breadth's 3-metric donut is intentionally never-scaling per Blueprint v1.2
                ...
                return (
                  <div style={{display:"flex",alignItems:"center",gap:16}}>
                    ...
                    <div>
                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:5}}><span style={{width:8,height:8,borderRadius:"50%",background:UP,display:"inline-block"}}></span><span style={{fontSize:tierCfg.secondaryText,color:T2}}>Advancing {breadth.adv}</span></div>
                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:5}}><span style={{width:8,height:8,borderRadius:"50%",background:DOWN,display:"inline-block"}}></span><span style={{fontSize:tierCfg.secondaryText,color:T2}}>Declining {breadth.dec}</span></div>
                      <div style={{display:"flex",alignItems:"center",gap:6}}><span style={{width:8,height:8,borderRadius:"50%",background:T3,display:"inline-block"}}></span><span style={{fontSize:tierCfg.secondaryText,color:T2}}>Unchanged {breadth.unch}</span></div>
                    </div>
                  </div>
                );
              })()}
            ...
            <div style={{background:CARD2,border:"1px solid "+BD,borderRadius:10,padding:tierCfg.cardPadding,height:tierCfg.supportingHeight,boxSizing:"border-box",overflow:"hidden"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                <span style={{fontSize:tierCfg.widgetTitle,fontWeight:800,color:T1}}>Sector Strength</span>
                <ProvenanceBadge type="demo"/>
              </div>
              <div style={{display:"flex",gap:4,marginBottom:10}}>
                <button onClick={function(){setSectorView("strong");}} style={{flex:1,background:sectorView==="strong"?BLUE:CARD2,border:"1px solid "+(sectorView==="strong"?BLUE:BD2),borderRadius:6,padding:"5px 0",color:sectorView==="strong"?"#fff":T2,fontSize:tierCfg.label,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Strongest</button>
                <button onClick={function(){setSectorView("weak");}} style={{flex:1,background:sectorView==="weak"?BLUE:CARD2,border:"1px solid "+(sectorView==="weak"?BLUE:BD2),borderRadius:6,padding:"5px 0",color:sectorView==="weak"?"#fff":T2,fontSize:tierCfg.label,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Weakest</button>
              </div>
              {(sectorView==="strong"?sectorPerf.strong:sectorPerf.weak).map(function(sec){
                var up = sec.avgChg>=0;
                var barPct = Math.min(100, Math.abs(sec.avgChg)*30); // fixed - 5-sector list is intentionally never-scaling per Blueprint v1.2
                return (
                  <div key={sec.sector} style={{marginBottom:8}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:tierCfg.secondaryText,marginBottom:3}}>
            ...
            <div style={{background:CARD2,border:"1px solid "+BD,borderRadius:10,padding:tierCfg.cardPadding,height:tierCfg.supportingHeight,boxSizing:"border-box",overflow:"hidden"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                <span style={{fontSize:tierCfg.widgetTitle,fontWeight:800,color:T1}}>Market Heatmap</span>
                <button onClick={function(){setTab("heatmap");}} style={{background:"none",border:"none",color:BLUE,fontSize:tierCfg.label,fontWeight:700,cursor:"pointer"}}>View Full &#8594;</button>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(4, minmax(0, 1fr))",gap:6}}>
                {DEMO_STOCKS.slice(0,Math.min(tierCfg.heatmapTiles,DEMO_STOCKS.length)).map(function(s){
                  var up = s.chgPct>=0;
                  return (
