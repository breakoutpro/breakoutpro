...
export default function EquityHome(props) {
  ...
  return (
    <div style={{background:BG,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:80}}>
      ...
      {/* Session badge + AIPulseCard (already has One Tap Market in live session) */}
      <div style={{padding:"10px 16px 0"}}>
        ...
        {/* AI Markets Briefing — always visible on home */}
        <div style={{background:"linear-gradient(135deg,rgba(59,130,246,0.08),rgba(124,58,237,0.06))",border:"1px solid rgba(59,130,246,0.2)",borderRadius:16,padding:"14px",marginBottom:4}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
            <div style={{width:28,height:28,borderRadius:8,background:"linear-gradient(135deg,"+BLUE+","+PURPLE+")",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
              <span style={{fontSize:10,fontWeight:900,color:"#fff"}}>AI</span>
            </div>
            <div>
              <div style={{fontSize:14,fontWeight:800,color:T1}}>AI Markets Briefing</div>
              <div style={{fontSize:10,color:T2}}>{trendLabel} &bull; Confidence {mood.conf}%</div>
            </div>
            <div style={{marginLeft:"auto",background:mood.bull>=50?"rgba(34,197,94,0.12)":"rgba(239,68,68,0.12)",border:"1px solid "+(mood.bull>=50?UP:DOWN)+"44",borderRadius:20,padding:"3px 10px"}}>
              <span style={{fontSize:10,fontWeight:700,color:mood.bull>=50?UP:DOWN}}>{mood.bull>=50?"Bullish":"Bearish"} {Math.round(mood.bull)}%</span>
            </div>
          </div>
          {[
            ...
            ["Top Pick",      "ICICIBANK",                                                       BLUE2                  ],
            ["NIFTY Support", "23,850",                                                          UP                     ],
            ["Resistance",    "24,050",                                                          DOWN                   ],
            ["FII Flow",      "Net +Rs 2,847 Cr",                                               UP                     ],
          ].map(function(row, i, arr){
            return (
              <div key={row[0]} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:i<arr.length-1?"1px solid rgba(30,58,95,0.4)":"none"}}>
                <span style={{fontSize:11,color:T2}}>{row[0]}</span>
                <span style={{fontSize:12,fontWeight:700,color:row[2]}}>{row[1]}</span>
              </div>
            );
          })}
          <button onClick={function(){setTab("briefing");}} style={{width:"100%",marginTop:12,background:"linear-gradient(135deg,"+BLUE+","+PURPLE+")",border:"none",borderRadius:12,padding:"11px",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
            Full AI Briefing &#8594;
          </button>
        </div>
      </div>
