...
export function buildMT(theme) {
  return {
    ...
    SENT_GREEN: "#16A34A", SENT_RED: "#DC2626", SENT_YELLOW: "#EAB308"
  };
}
...
export function IndexRow(props){
  ...
  if(!row || !row.available){
    return (
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",rowGap:2,padding:"4px 8px",borderBottom:"1px solid "+MT.BD}}>
        <span style={{fontSize:10,fontWeight:700,color:MT.T1}}>{(row && row.name) || "--"}</span>
        <FreshnessPill status="UNAVAILABLE"/>
      </div>
    );
  }
  ...
  return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",rowGap:2,padding:"4px 8px",borderBottom:"1px solid "+MT.BD}}>
      <span style={{fontSize:10,fontWeight:700,color:MT.T1,flexShrink:0}}>{row.name}</span>
      <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap",justifyContent:"flex-end"}}>
        <span style={{fontSize:10,fontWeight:800,color:MT.T1}}>{row.ltp}</span>
        <span style={{fontSize:10,fontWeight:700,color:chgColor}}>{row.chgPct}</span>
      </div>
    </div>
  );
}
...
// Semi-circle mood gauge. Three fixed zone colors (Red/Yellow/Green) are
// always drawn - only the needle position is real data (mapped from the
// same 0-100 mood.score every other mood display on this page uses). The
// zones are a visual reference scale, not separately-sourced data.
export function Gauge(props){
  var theme = useTheme(); var MT = buildMT(theme);
  var mood = props.mood;
  var score = mood && mood.score!=null ? Math.max(0, Math.min(100, mood.score)) : 50;
  var cx = 90, cy = 90, r = 70;
  var needleLen = 55;
  // Exact requested rotation: angle = (score/100)*180, applied via SVG
  // rotate(angle, 90, 90). The needle is drawn once, unrotated, pointing
  // due WEST (into the red/bearish zone, score=0's position) - rotating it
  // clockwise by `angle` degrees around the pivot sweeps it from west (0)
  // through north/up (score=50, the bearish/bullish midpoint) to east
  // (score=100, deep into the green/bullish zone). A score of 74 rotates
  // 133.2 degrees, landing the tip in the upper-right (green) zone -
  // verified by computing the rotated tip position by hand.
  var angle = (score/100)*180;
  // Zone boundary points at 180/120/60/0 degrees along the arc.
  var zoneAngles = [Math.PI, Math.PI*2/3, Math.PI/3, 0];
  var pts = zoneAngles.map(function(a){ return [ (cx + r*Math.cos(a)).toFixed(1), (cy - r*Math.sin(a)).toFixed(1) ]; });
  return (
    <svg width="120" height="65" viewBox="0 0 180 100" style={{flexShrink:0}}>
      <path d={"M "+pts[0][0]+" "+pts[0][1]+" A "+r+" "+r+" 0 0 1 "+pts[1][0]+" "+pts[1][1]} fill="none" stroke="#DC2626" strokeWidth="12" strokeLinecap="round"/>
      <path d={"M "+pts[1][0]+" "+pts[1][1]+" A "+r+" "+r+" 0 0 1 "+pts[2][0]+" "+pts[2][1]} fill="none" stroke="#EAB308" strokeWidth="12"/>
      <path d={"M "+pts[2][0]+" "+pts[2][1]+" A "+r+" "+r+" 0 0 1 "+pts[3][0]+" "+pts[3][1]} fill="none" stroke="#16A34A" strokeWidth="12" strokeLinecap="round"/>
      <g transform={"rotate(" + angle + ", 90, 90)"} style={{transformOrigin:"90px 90px"}}>
        <polygon points={cx+","+(cy-4)+" "+(cx-needleLen)+","+cy+" "+cx+","+(cy+4)} fill="#FFFFFF"/>
      </g>
      <circle cx={cx} cy={cy} r="7" fill={MT.CARD} stroke="#FFFFFF" strokeWidth="2"/>
    </svg>
  );
}
...
export function EvolutionCard(props){
  ...
  if(props.compact){
    return (
      <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:8,padding:"6px 8px",overflow:"hidden"}}>
        <div style={{fontSize:10,fontWeight:800,color:ev.return3dPct.indexOf("-")==0?MT.SENT_RED:MT.SENT_GREEN,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
          NIFTY {ev.return3dPct} / {ev.sessions}d
        </div>
      </div>
    );
  }
...
export function StageTimeline(props){
  ...
  if(props.compact){
    return (
      <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:8,padding:"6px 10px",height:"100%",boxSizing:"border-box",display:"flex",flexDirection:"column",justifyContent:"center",overflow:"hidden",minWidth:0}}>
        <div style={{fontSize:10,fontWeight:800,color:MT.T1,marginBottom:3}}>Market Stage Timeline</div>
          ...
          {steps.map(function(s,i){
            return (
              <span key={i} style={{display:"flex",alignItems:"center",gap:4,flexShrink:0}}>
                {i>0 ? <span style={{color:MT.T3,fontSize:10}}>&#8594;</span> : null}
                <span style={{width:6,height:6,borderRadius:"50%",background:i==steps.length-1?MT.BLUE:MT.T3,flexShrink:0}}></span>
                <span style={{fontSize:10,color:i==steps.length-1?MT.T1:MT.T2,fontWeight:i==steps.length-1?800:600,whiteSpace:"nowrap"}}>{s}</span>
              </span>
            );
          })}
        </div>
        {summary ? <div style={{fontSize:9,color:MT.T3,marginTop:2,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{summary}</div> : null}
      </div>
    );
  }
  return (
    <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:14,padding:6,marginBottom:6,height:"100%",boxSizing:"border-box",overflowY:"auto",minWidth:0}}>
      <div style={{fontSize:11,fontWeight:800,color:MT.T1,marginBottom:2}}>Market Stage Timeline</div>
      {steps.map(function(s,i){
        return (
          <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"2px 0",borderTop:i>0?"1px solid "+MT.DIV:"none"}}>
      ...
      {summary ? <div style={{fontSize:10,color:MT.T3,marginTop:2,paddingTop:2,borderTop:"1px solid "+MT.DIV}}>{summary}</div> : null}
    </div>
  );
}
