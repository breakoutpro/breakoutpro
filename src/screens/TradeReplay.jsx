...
export default function TradeReplay(props){
  ...
  var BD = theme.c.border, BLUE = theme.c.blue, CARD = theme.c.card, BLUE = theme.c.gold, T2 = theme.c.text2, T3 = theme.c.text3; T1=theme.c.text1; UP=theme.c.up;
    ...
    <div style={{padding:16}}>
      ...
      {!decision ? (
        ...
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:16}}>
          ...
          <div style={{marginBottom:8}}><span style={{fontSize:12,fontWeight:800,color:T2}}>TREND: </span><span style={{fontSize:12,color:"#C9D4E5"}}>{current.trendNote}</span></div>
          <div style={{marginBottom:8}}><span style={{fontSize:12,fontWeight:800,color:T2}}>SUPPORT/RESISTANCE: </span><span style={{fontSize:12,color:"#C9D4E5"}}>{current.zone=="none"?"No nearby zone at this point.":("Price is near a "+current.zone+" zone.")}</span></div>
          <div style={{marginBottom:8}}><span style={{fontSize:12,fontWeight:800,color:T2}}>CANDLE PSYCHOLOGY: </span><span style={{fontSize:12,color:"#C9D4E5"}}>{CANDLE_PSYCHOLOGY[current.shapeId]}</span></div>
          <div style={{marginBottom:8}}><span style={{fontSize:12,fontWeight:800,color:T2}}>RISK: </span><span style={{fontSize:12,color:"#C9D4E5"}}>{current.riskNote}</span></div>
          <div><span style={{fontSize:12,fontWeight:800,color:T2}}>VERDICT: </span><span style={{fontSize:12,color:"#C9D4E5"}}>The ideal decision here was "{current.idealDecision}".</span></div>
        </div>
      )}
