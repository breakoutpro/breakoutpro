...
export default function IndexTabs(props) {
  ...
  BD=theme.c.border; CB=theme.c.card; G2=theme.c.up; R=theme.c.down; T1=theme.c.text1; T2=theme.c.text2;
  ...
  if(vt=="pattern") return (
    <div>
      {pat?(
        <div style={{background:CB,border:"1px solid "+(pat.type=="bullish"?BLUE:pat.type=="bearish"?R:BLUE)+"44",borderRadius:16,padding:16,marginBottom:12}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div style={{fontSize:18,fontWeight:900,color:T1}}>{pat.name}</div>
            <span style={{background:pat.type=="bullish"?"rgba(37,99,235,0.15)":pat.type=="bearish"?"rgba(239,68,68,0.15)":"rgba(37,99,235,0.15)",color:pat.type=="bullish"?BLUE:pat.type=="bearish"?R:BLUE,borderRadius:20,padding:"4px 12px",fontSize:12,fontWeight:700}}>{pat.signal}</span>
          </div>
        ...
        <div style={{background:CB,border:"1px solid rgba(37,99,235,0.2)",borderRadius:16,padding:"24px",textAlign:"center",marginBottom:12}}>
