...
// Desktop Home layout - independent from Mobile, Tablet, and Desktop.
// Production dashboard for 1920x1080 and larger desktop/ultrawide monitors: 4 explicit
// dashboard rows matching the approved row structure, desktop-tuned type
// scale (slightly larger than Laptop, matching the extra available width). Same cards, same shared
...
export default function EquityHomeDesktop(props){
  ...
  // Desktop type scale - a touch larger than Laptop, tuned for ~1920px+ width.
  var CARD_TITLE = 15, HERO_TITLE = 17, BODY = 13, LABEL = 12, PRICE = 19;
  ...
  return (
    <div style={{background:BG,minHeight:"100vh",fontFamily:"'Inter',Arial,sans-serif",paddingBottom:28,color:T1,overflowX:"hidden"}}>
      ...
      <div style={{maxWidth:1800,margin:"0 auto",width:"100%",boxSizing:"border-box"}}>
              ...
              <div>
                <div style={{display:"flex",gap:6,marginBottom:10}}>
                  {syms.map(function(s,i){
                    ...
                    return (
                      <button key={s[0]} onClick={function(){setLevelsIdx(i);}} style={{flex:1,minWidth:0,background:active?BLUE:theme.c.card2,border:"1px solid "+(active?BLUE:BD),borderRadius:8,padding:"6px 4px",color:active?"#fff":T2,fontSize:LABEL-1,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>{s[0]}</button>
                    );
                  })}
                </div>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                  <span style={{fontSize:PRICE-2,fontWeight:900,color:T1,fontFamily:"monospace"}}>Rs {z.currentPrice.toLocaleString("en-IN",{maximumFractionDigits:2})}</span>
                  <span style={{fontSize:LABEL-1,fontWeight:700,color:trendColor,background:theme.c.card2,border:"1px solid "+BD,borderRadius:14,padding:"2px 8px"}}>{z.trend}</span>
                </div>
                <div style={{width:"100%",boxSizing:"border-box"}}>
                  {z.resistance2 ? (
                    <div style={{display:"flex",justifyContent:"space-between",padding:"4px 8px"}}>
                      <span style={{fontSize:LABEL-1,color:T2,fontWeight:700}}>R2</span>
                      <span style={{fontSize:LABEL,fontWeight:800,color:DOWN}}>Rs {z.resistance2.price}</span>
                    </div>
                  ) : null}
                  {z.resistance ? (
                    <div style={{display:"flex",justifyContent:"space-between",padding:"4px 8px"}}>
                      <span style={{fontSize:LABEL-1,color:T2,fontWeight:700}}>R1</span>
                      <span style={{fontSize:LABEL+1,fontWeight:800,color:DOWN}}>Rs {z.resistance.price}</span>
                    </div>
                  ) : null}
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 8px",background:theme.c.gold+"1A",borderLeft:"3px solid "+theme.c.gold,borderRadius:4,margin:"3px 0"}}>
                    <span style={{fontSize:LABEL-1,color:theme.c.gold,fontWeight:800}}>CURRENT</span>
                    <span style={{fontSize:LABEL+1,fontWeight:900,color:theme.c.gold}}>Rs {z.currentPrice.toLocaleString("en-IN",{maximumFractionDigits:2})}</span>
                  </div>
                  {z.support ? (
                    <div style={{display:"flex",justifyContent:"space-between",padding:"4px 8px"}}>
                      <span style={{fontSize:LABEL-1,color:T2,fontWeight:700}}>S1</span>
                      <span style={{fontSize:LABEL+1,fontWeight:800,color:UP}}>Rs {z.support.price}</span>
                    </div>
                  ) : null}
                  {z.support2 ? (
                    <div style={{display:"flex",justifyContent:"space-between",padding:"4px 8px"}}>
                      <span style={{fontSize:LABEL-1,color:T2,fontWeight:700}}>S2</span>
                      <span style={{fontSize:LABEL,fontWeight:800,color:UP}}>Rs {z.support2.price}</span>
                    </div>
                  ) : null}
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginTop:8,marginBottom:10}}>
                  <div style={{fontSize:LABEL-1,color:T2}}>Strength: <span style={{fontWeight:700,color:T1}}>{z.support?z.support.strength:"--"}</span></div>
                  <div style={{fontSize:LABEL-1,color:T2}}>R:R: <span style={{fontWeight:700,color:T1}}>{z.riskReward!=null?z.riskReward+":1":"--"}</span></div>
                  <div style={{fontSize:LABEL-1,color:T2,gridColumn:"1 / -1"}}>Next Key Level: <span style={{fontWeight:700,color:T1}}>{z.nextKeyLevel!=null?("Rs "+z.nextKeyLevel):"--"}</span></div>
                </div>
                ...
                <div style={{display:"flex",gap:8}}>
                  ...
                  <button onClick={function(){setTab("pazones");}} style={{flex:1,background:BLUE,border:"none",borderRadius:9,padding:"9px",color:"#fff",fontSize:LABEL-1,fontWeight:700,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:5}}><span style={{fontSize:13}}>&#128202;</span>View Full Price Action &#8594;</button>
                </div>
              </div>
            );
          })()}
      ...
      <div style={{padding:"8px 18px 0",fontSize:LABEL-1,color:T3,lineHeight:1.4,textAlign:"center"}}>
