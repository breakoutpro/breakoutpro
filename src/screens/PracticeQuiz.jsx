  ...
  if(!q){
    return (
      <div style={{padding:16}}>
        <button onClick={onBack} style={{background:"none",border:"none",color:BLUE,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",padding:4,marginBottom:12}}>&#8592; Back</button>
  ...
  return (
    <div style={{padding:16}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        <span style={{fontSize:12,color:T3}}>Question {idx+1} of {bank.length}</span>
        <span style={{fontSize:12,color:T2}}>All-time: {score.correct}/{score.attempts}</span>
      </div>
      ...
      <div style={{fontSize:14,fontWeight:700,color:T1,marginBottom:16,lineHeight:1.5}}>{q.q}</div>
      ...
      {q.options.map(function(opt,oi){
        ...
        return (
          <div key={oi} onClick={function(){pick(oi);}} style={{border:"1px solid "+col,background:bg,borderRadius:10,padding:"12px 16px",marginBottom:8,fontSize:12,color:T1,cursor:answered?"default":"pointer",minHeight:44,display:"flex",alignItems:"center"}}>{opt}</div>
        );
      })}
      ...
      {answered ? (
        <div>
          <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:12,marginBottom:12}}>
            <div style={{fontSize:12,fontWeight:700,color:picked==q.correct?UP:R,marginBottom:8}}>{picked==q.correct?"Correct":"Not quite"}</div>
            <div style={{fontSize:12,color:T2,lineHeight:1.6}}>{q.explain}</div>
          </div>
          {!isLast ? (
            <button onClick={next} style={{width:"100%",background:BLUE,border:"none",borderRadius:12,padding:"12px 24px",color:"#fff",fontSize:12,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>Next Question &#8594;</button>
          ) : (
            <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:16,textAlign:"center"}}>
              <div style={{fontSize:14,fontWeight:800,color:UP,marginBottom:4}}>Practice Complete</div>
              <div style={{fontSize:12,color:T2}}>This session: {sessionCorrect} of {bank.length} correct</div>
            </div>
          )}
        </div>
      ) : null}
