    ...
    <div style={{background:BG,minHeight:"100vh",fontFamily:"'Inter',Arial,sans-serif",color:T1,overflowX:"hidden",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{display:"flex",borderBottom:"1px solid "+BD,background:CARD2}}>
        {data.spineRows.map(function(r,i){
          ...
          return (
            <div key={r.key} onClick={function(){data.selectIndex(r.key);}} style={{padding:"6px 12px",cursor:"pointer",borderRight:i<data.spineRows.length-1?"1px solid "+BD2:"none",background:active?"rgba(59,130,246,0.10)":"transparent",borderBottom:active?"2px solid "+BLUE:"2px solid transparent",display:"flex",alignItems:"center",gap:6}}>
      ...
      <div style={{flex:1,display:"flex",minHeight:0,width:"100%"}}>
        ...
        {/* CENTER WORKSPACE - primary 2x2 intelligence grid (AI Market Mood,
            ...
            by a secondary grid (Market Breadth, FII/DII Flow, Latest
            Alerts, Global Markets) and Key Levels. Each card's detailed
        ...
        <div style={{flex:1,minWidth:0,padding:"12px 16px",overflowY:"auto"}}>
          ...
          <div onClick={function(){setTab("marketmood");}} style={{background:CARD2+"cc",backdropFilter:"blur(12px)",border:"1px solid "+BLUE+"55",boxShadow:"0 0 24px "+BLUE+"22",borderRadius:14,padding:"8px 12px",boxSizing:"border-box",cursor:"pointer",maxHeight:400,overflowY:"auto",display:"flex",flexDirection:"column",height:"100%"}}>
                ...
                <div style={{display:"flex",gap:14,flex:1,minHeight:0}}>
                  <div style={{flex:"0 0 45%",minWidth:0,display:"flex",flexDirection:"column"}}>
                    ...
                    <div style={{marginBottom:6}}>
                      ...
                      <div style={{fontSize:tierCfg.secondaryText,color:ai&&ai.watchNext?T1:T2,lineHeight:1.4}}>{ai && ai.watchNext ? ai.watchNext : "No verified watch-level data available right now."}</div>
                    </div>
                  ...
                  {/* F&O TRADER PULSE - PCR, Max Pain, FII F&O Flow have no
                      real provider anywhere in this codebase (confirmed via
                      full audit: dhan.js's getOptionChain() is commented out
                      pending a paid subscription; OptionsIntelData.jsx is
                      explicitly "Mock now"). Shown honestly as "-" rather
                      than invented or omitted. India VIX is real - same
                      indices.VIX already used by Market Spine/Range
                      Intelligence, not a new data source. */}
                  {(function(){
                    var vix = data.mm && data.mm.data && data.mm.data.indices && data.mm.data.indices.VIX;
                    var hasVix = vix && vix.ltp!=null;
                    return (
                      <div style={{flex:"0 0 55%",minWidth:0,background:CARD2,border:"1px solid "+BD,borderRadius:10,padding:10,display:"flex",flexDirection:"column"}}>
                        <div style={{fontSize:9,fontWeight:800,color:T2,marginBottom:8}}>F&amp;O TRADER PULSE</div>
                        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,flex:1}}>
                          <div>
                            <div style={{fontSize:9,color:T3,marginBottom:2}}>PCR</div>
                            <div style={{fontSize:13,fontWeight:800,color:T2}}>&#8212;</div>
                          </div>
                          <div>
                            <div style={{fontSize:9,color:T3,marginBottom:2}}>Max Pain</div>
                            <div style={{fontSize:13,fontWeight:800,color:T2}}>&#8212;</div>
                          </div>
                          <div>
                            <div style={{fontSize:9,color:T3,marginBottom:2}}>India VIX</div>
                            <div style={{fontSize:13,fontWeight:800,color:T1}}>{hasVix ? vix.ltp+(vix.chgPct!=null?" ("+(vix.chgPct>=0?"+":"")+vix.chgPct+"%)":"") : "\u2014"}</div>
                          </div>
                          <div>
                            <div style={{fontSize:9,color:T3,marginBottom:2}}>FII F&amp;O Flow</div>
                            <div style={{fontSize:13,fontWeight:800,color:T2}}>&#8212;</div>
                          </div>
                        </div>
                        <div style={{fontSize:9,color:T3,marginTop:6}}>Connect a verified options-chain provider to activate PCR, Max Pain, and FII flow.</div>
                      </div>
                    );
                  })()}
                </div>
          ...
          {(function(){
            ...
            if(!hasRange){
              return (
                <div onClick={function(){setTab("rangeintel");}} style={{background:CARD2+"cc",backdropFilter:"blur(12px)",border:"1px solid "+BLUE+"55",boxShadow:"0 0 24px "+BLUE+"22",borderRadius:14,padding:"8px 12px",boxSizing:"border-box",cursor:"pointer",maxHeight:400,overflowY:"auto"}}>
            ...
            var bcRaw = (nifty.high + nifty.low) / 2;
            var tcRaw = (2 * pivot) - bcRaw;
            var cprWidthPct = (Math.abs(tcRaw - bcRaw) / pivot) * 100;
            var cprStatus = cprWidthPct < 0.15 ? "Narrow CPR" : "Wide CPR";
            var cprColor = cprStatus==="Narrow CPR" ? WARN : T2;
            return (
              <div onClick={function(){setTab("rangeintel");}} style={{background:CARD2+"cc",backdropFilter:"blur(12px)",border:"1px solid "+BLUE+"55",boxShadow:"0 0 24px "+BLUE+"22",borderRadius:14,padding:"8px 12px",boxSizing:"border-box",cursor:"pointer",maxHeight:400,overflowY:"auto",display:"flex",flexDirection:"column",height:"100%"}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                  <span style={{fontSize:tierCfg.widgetTitle,fontWeight:800,color:T1}}>Range Intelligence</span>
                  <ProvenanceBadge type="calculated"/>
                </div>
                ...
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                  ...
                  <span style={{fontSize:tierCfg.label,fontWeight:700,color:cprColor,background:cprColor+"14",border:"1px solid "+cprColor+"55",borderRadius:5,padding:"2px 7px"}}>{cprStatus}</span>
                </div>
                ...
                <div style={{textAlign:"right",fontSize:tierCfg.label,color:BLUE,fontWeight:700,marginTop:"auto",paddingTop:6}}>View Full Range Intelligence &#8594;</div>
              </div>
            );
          })()}
          {/* OPTIONS INTELLIGENCE - card 3 of 3. No live options-chain
              provider exists anywhere in this codebase - confirmed via full
              audit: api/market.js has dhan.js's real getOptionChain()
              commented out ("enable when Data API subscribed"), and
              OptionsIntelData.jsx's own header states its values are "Mock
              now". The full field structure is shown honestly with "--" so
              the card communicates what this feature will show once
              connected, rather than looking empty or being hidden. */}
          <div onClick={function(){ if(props.setShowOptions) props.setShowOptions(true); else setTab("optionsintelpro"); }} style={{background:CARD2+"cc",backdropFilter:"blur(12px)",border:"1px solid "+BLUE+"55",boxShadow:"0 0 24px "+BLUE+"22",borderRadius:14,padding:"8px 12px",boxSizing:"border-box",cursor:"pointer",maxHeight:400,overflowY:"auto",display:"flex",flexDirection:"column",height:"100%"}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
              <span style={{fontSize:tierCfg.widgetTitle,fontWeight:800,color:T1}}>Options Intelligence</span>
              <ProvenanceBadge type="unavailable"/>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:8}}>
              <div><div style={{fontSize:9,color:T3}}>Underlying</div><div style={{fontSize:12,fontWeight:700,color:T2}}>&#8212;</div></div>
              <div><div style={{fontSize:9,color:T3}}>Expiry</div><div style={{fontSize:12,fontWeight:700,color:T2}}>&#8212;</div></div>
              <div><div style={{fontSize:9,color:T3}}>ATM Strike</div><div style={{fontSize:12,fontWeight:700,color:T2}}>&#8212;</div></div>
              <div><div style={{fontSize:9,color:T3}}>Straddle Price</div><div style={{fontSize:12,fontWeight:700,color:T2}}>&#8212;</div></div>
              <div><div style={{fontSize:9,color:T3}}>PCR</div><div style={{fontSize:12,fontWeight:700,color:T2}}>&#8212;</div></div>
              <div><div style={{fontSize:9,color:T3}}>Max Pain</div><div style={{fontSize:12,fontWeight:700,color:T2}}>&#8212;</div></div>
              <div><div style={{fontSize:9,color:T3}}>IV</div><div style={{fontSize:12,fontWeight:700,color:T2}}>&#8212;</div></div>
              <div><div style={{fontSize:9,color:T3}}>Call OI / Change</div><div style={{fontSize:12,fontWeight:700,color:T2}}>&#8212;</div></div>
              <div><div style={{fontSize:9,color:T3}}>Put OI / Change</div><div style={{fontSize:12,fontWeight:700,color:T2}}>&#8212;</div></div>
              <div><div style={{fontSize:9,color:T3}}>Call Resistance</div><div style={{fontSize:12,fontWeight:700,color:T2}}>&#8212;</div></div>
              <div><div style={{fontSize:9,color:T3}}>Put Support</div><div style={{fontSize:12,fontWeight:700,color:T2}}>&#8212;</div></div>
              <div><div style={{fontSize:9,color:T3}}>CE / PE Dominance</div><div style={{fontSize:12,fontWeight:700,color:T2}}>&#8212;</div></div>
              <div><div style={{fontSize:9,color:T3}}>Delta / Gamma / Theta / Vega</div><div style={{fontSize:12,fontWeight:700,color:T2}}>&#8212;</div></div>
            </div>
            <div style={{fontSize:tierCfg.label,color:T3,lineHeight:1.5}}>Connect a verified options-chain provider to activate live PCR, Max Pain, OI, and Greeks here.</div>
          </div>

          {/* MARKET NEWS - compact, max 3 headlines, real JustIn data */}
          <div onClick={function(){setTab("news");}} style={{background:CARD2+"cc",backdropFilter:"blur(12px)",border:"1px solid "+BLUE+"55",boxShadow:"0 0 24px "+BLUE+"22",borderRadius:14,padding:"8px 12px",boxSizing:"border-box",cursor:"pointer",maxHeight:400,overflowY:"auto",display:"flex",flexDirection:"column",height:"100%"}}>
      ...
      {/* MARKET BREADTH / FII-DII FLOW / LATEST ALERTS / GLOBAL MARKETS - the
          first three have no real provider anywhere in this codebase, shown
      ...
      <div style={{padding:"0 20px 16px",display:"grid",gridTemplateColumns:"repeat(4, 1fr)",gap:8}}>
        <div style={{background:CARD2+"cc",backdropFilter:"blur(12px)",border:"1px solid "+BD,borderRadius:14,padding:tierCfg.cardPadding,boxSizing:"border-box"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
      ...
      {(function(){
        ...
        return (
          <div style={{padding:"0 20px 16px"}}>
            <div style={{background:CARD2+"cc",backdropFilter:"blur(12px)",border:"1px solid "+BLUE+"55",boxShadow:"0 0 24px "+BLUE+"22",borderRadius:14,padding:tierCfg.cardPadding,boxSizing:"border-box"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
