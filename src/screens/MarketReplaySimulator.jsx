  ...
  if(view=="history"){
    return (
      <div style={{padding:14}}>
        <button onClick={function(){setView("setup");}} style={{background:"none",border:"none",color:BLUE,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",padding:0,marginBottom:16,minHeight:44}}>&#8592; Back</button>
        <div style={{fontSize:18,fontWeight:800,color:T1,marginBottom:16}}>Replay History</div>
        {replayHistory.history.length==0 ? (
          <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:20,textAlign:"center",color:T3,fontSize:12}}>No saved replays yet.</div>
          ...
          return (
            <div key={s.id} style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,marginBottom:10}}>
              ...
              {s.notes ? <div style={{fontSize:12,color:T2,marginTop:6,fontStyle:"italic"}}>{s.notes}</div> : null}
              <button onClick={function(){replayHistory.deleteSession(s.id);}} style={{background:"none",border:"1px solid "+R,borderRadius:8,padding:"7px 12px",color:R,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",marginTop:8,minHeight:44}}>Delete</button>
            </div>
          );
        })}
      </div>
  ...
  if(view=="summary"){
    ...
    return (
      <div style={{padding:14}}>
        ...
        <label style={{fontSize:12,color:T2,fontWeight:700,display:"block",marginBottom:6}}>Notes</label>
        <textarea value={notes} onChange={function(ev2){setNotes(ev2.target.value);}} style={{width:"100%",background:"#0B0E13",border:"1px solid "+BD,borderRadius:10,padding:"11px 12px",color:T1,fontSize:14,fontFamily:"inherit",minHeight:70,boxSizing:"border-box",marginBottom:14}}/>
        <button onClick={saveToHistory} style={{width:"100%",background:BLUE,border:"none",borderRadius:11,padding:14,color:"#fff",fontSize:15,fontWeight:800,cursor:"pointer",fontFamily:"inherit",marginBottom:8,minHeight:44}}>Save to Replay History</button>
  ...
  if(view=="replay"){
    ...
    return (
      <div style={{padding:14}}>
        ...
        {status=="loading" ? (
          <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:20,textAlign:"center",color:T2,fontSize:12}}>Loading real historical data...</div>
        ) : status=="error" ? (
          <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:20,textAlign:"center",color:R,fontSize:12}}>Data unavailable. <button onClick={startReplay} style={{background:"none",border:"1px solid "+BLUE,borderRadius:8,padding:"7px 14px",color:BLUE,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",marginTop:8,minHeight:44}}>Retry</button></div>
        ) : status=="empty" ? (
          <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:20,textAlign:"center",color:T3,fontSize:12}}>No real data available for this symbol/timeframe.</div>
        ) : (
          <div>
            ...
            <div style={{display:"flex",gap:8,marginBottom:14}}>
              <button onClick={prevCandle} disabled={revealed<=1} style={{flex:1,background:"transparent",border:"1px solid "+BD,borderRadius:10,padding:10,color:revealed<=1?T3:T2,fontSize:12,fontWeight:700,cursor:revealed<=1?"default":"pointer",fontFamily:"inherit",minHeight:44}}>&#8592; Previous</button>
              {playing ? (
                <button onClick={pause} style={{flex:1,background:BLUE,border:"none",borderRadius:10,padding:10,color:"#fff",fontSize:12,fontWeight:800,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>Pause</button>
              ) : (
                <button onClick={play} style={{flex:1,background:BLUE,border:"none",borderRadius:10,padding:10,color:"#fff",fontSize:12,fontWeight:800,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>Play</button>
              )}
              <button onClick={nextCandle} style={{flex:1,background:"transparent",border:"1px solid "+BD,borderRadius:10,padding:10,color:T2,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>Next &#8594;</button>
            </div>
            <button onClick={restart} style={{width:"100%",background:"transparent",border:"1px solid "+BD,borderRadius:10,padding:10,color:T2,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",marginBottom:14,minHeight:44}}>Restart Replay</button>
            ...
            <div style={{display:"flex",gap:8}}>
              ...
              <button onClick={function(){decide("hold");}} style={{flex:1,background:decisions[revealed-1]=="hold"?T2:"transparent",border:"1px solid "+T2,borderRadius:10,padding:12,color:decisions[revealed-1]=="hold"?"#050505":T2,fontSize:12,fontWeight:800,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>Hold</button>
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <div style={{background:"#050505",minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
      <div style={{background:CARD,padding:"12px 14px",borderBottom:"1px solid "+BD,display:"flex",alignItems:"center",gap:10}}>
        ...
        <button onClick={function(){setView("history");}} style={{background:"transparent",border:"1px solid "+BD,borderRadius:9,padding:"9px 12px",color:T2,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>History</button>
      </div>

      <div style={{padding:14}}>
        <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10}}>SYMBOL</div>
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:16}}>
          {SYMBOLS.map(function(s,i){
            var act=i==symIdx;
            return <button key={s.sym} onClick={function(){setSymIdx(i);}} style={{background:act?BLUE:"transparent",border:"1px solid "+(act?BLUE:BD),borderRadius:9,padding:"9px 13px",color:act?"#fff":T2,fontSize:12,fontWeight:act?800:600,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>{s.sym}</button>;
          })}
        </div>

        <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10}}>TIMEFRAME</div>
        <div style={{display:"flex",gap:6,marginBottom:16}}>
          {Object.keys(TF_MAP).map(function(t){
            var act=t==tf;
            return <button key={t} onClick={function(){setTf(t);}} style={{flex:1,background:act?BLUE:"transparent",border:"1px solid "+(act?BLUE:BD),borderRadius:9,padding:"9px",color:act?"#fff":T2,fontSize:12,fontWeight:act?800:600,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>{t}</button>;
          })}
        </div>

        <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10}}>REPLAY SPEED</div>
        <div style={{display:"flex",gap:6,marginBottom:20}}>
          {SPEEDS.map(function(s){
            var act=s.id==speedId;
            return <button key={s.id} onClick={function(){setSpeedId(s.id);}} style={{flex:1,background:act?BLUE:"transparent",border:"1px solid "+(act?BLUE:BD),borderRadius:9,padding:"9px",color:act?"#fff":T2,fontSize:12,fontWeight:act?800:600,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>{s.label}</button>;
          })}
        </div>

        <button onClick={startReplay} style={{width:"100%",background:BLUE,border:"none",borderRadius:11,padding:14,color:"#fff",fontSize:15,fontWeight:800,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>Start Replay</button>

        <div style={{background:"rgba(212,175,55,0.06)",border:"1px solid rgba(212,175,55,0.15)",borderRadius:10,padding:11,marginTop:16}}>
          <div style={{fontSize:12,color:WARN,lineHeight:1.5}}>Real historical data only, via the same source used elsewhere in this app. Educational only - not investment advice. Correct/Wrong is graded against what actually happened next in this real data, not a prediction.</div>
        </div>
      </div>
    </div>
  );
...
function Row(props){
  return (
    <div style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:props.last?"none":"1px solid "+BD}}>
