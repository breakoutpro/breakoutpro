  ...
  return(
    <div style={PS}>
        ...
        {tab==="markets"&&(
          <div>
            <div style={{padding:"12px 12px 0 12px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                <div style={{fontSize:13,fontWeight:800,color:"#39FF14",display:"flex",alignItems:"center",gap:6}}>
                  <span style={{fontSize:16}}>&#11088;</span>
                  My Watchlist
                </div>
                <div style={{fontSize:9,color:"#666"}}>{watchlist.length} stocks</div>
              </div>
              {watchlist.length===0?(
                <div style={{background:"#0d0d0d",border:"1px dashed #1a1a1a",borderRadius:10,padding:"20px",textAlign:"center",marginBottom:10}}>
                  <div style={{fontSize:20,marginBottom:5}}>&#11088;</div>
                  <div style={{fontSize:11,color:"#888",fontWeight:600}}>No stocks in watchlist</div>
                  <div style={{fontSize:8,color:"#555",marginTop:3}}>Tap star icon on any stock to add</div>
                </div>
              ):(
                <div style={{display:"flex",gap:6,overflowX:"auto",marginBottom:10,paddingBottom:4}}>
                  {watchlist.map(function(sym){var st=stocks.find(function(s){return s.sym===sym;});if(!st)return null;return(
                    <div key={sym} style={{minWidth:120,background:"#0d0d0d",border:"1px solid #1a3a1a",borderRadius:10,padding:"8px 10px",cursor:"pointer"}} onClick={function(){setSelSt(st);}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
                        <div style={{fontSize:10,fontWeight:800,color:"#fff"}}>{st.sym}</div>
                        <button onClick={function(e){e.stopPropagation();toggleWatch(sym);}} style={{background:"transparent",border:"none",color:"#FFD700",fontSize:13,cursor:"pointer",padding:0}}>&#11088;</button>
                      </div>
                      <div style={{fontSize:11,fontWeight:700,color:"#fff"}}>{st.ltp}</div>
                      <div style={{fontSize:9,fontWeight:700,color:st.up?"#39FF14":"#ff4444"}}>{st.up?"+":""}{st.pct}%</div>
                    </div>
              ...
              {oiView==="chart"&&(
                <div style={{background:"#0d0d0d",border:"1px solid #1a1a1a",borderRadius:13,padding:"10px",marginBottom:10}}>
                  ...
                  <div style={{position:"relative",width:"100%",height:"400px",borderRadius:10,overflow:"hidden",background:"#000"}}>
                    <iframe src="https://www.tradingview-widget.com/embed-widget/advanced-chart/?locale=en#%7B%22autosize%22%3Atrue%2C%22symbol%22%3A%22NSE%3ANIFTY%22%2C%22interval%22%3A%225%22%2C%22timezone%22%3A%22Asia%2FKolkata%22%2C%22theme%22%3A%22dark%22%2C%22style%22%3A%221%22%7D" style={{width:"100%",height:"100%",border:"0"}} title="NIFTY Chart"></iframe>
                  </div>
                </div>
              )}
            </div>
          ...
          <div style={{paddingBottom:8}}>
            <div style={{padding:"10px 9px 4px"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                <div>
                  <div style={{fontSize:14,fontWeight:900,color:"#39FF14"}}>📡 Price Action Scanner</div>
                  <div style={{fontSize:8,color:"#556"}}>Educational Pattern Detection — Not Investment Advice</div>
                </div>
                <button onClick={function(){setScanRefreshing(true);setTimeout(function(){setScanRefreshing(false);},1200);}} style={{background:"#0d0d0d",border:"1px solid #39FF1433",borderRadius:9,padding:"5px 10px",color:"#39FF14",fontSize:9,cursor:"pointer",fontFamily:"inherit"}}>{scanRefreshing?"🔄...":"🔃 Scan"}</button>
              </div>
              <div style={{display:"flex",gap:4,overflowX:"auto",paddingBottom:4}}>
                {[["scanner","📡 Scanner"],["candles","🕯 Patterns"],["structure","🏗 Structure"],["alerts","⚡ Alerts"],["ai","🤖 AI Chart"]].map(function(item){return(<button key={item[0]} onClick={function(){setScanTab(item[0]);}} style={{background:scanTab===item[0]?"#39FF1418":"#0a0a0a",border:"1.5px solid "+(scanTab===item[0]?"#39FF14":"#1a1a1a"),borderRadius:16,padding:"5px 10px",color:scanTab===item[0]?"#39FF14":"#445",fontSize:9,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",fontFamily:"inherit"}}>{item[1]}</button>);})}
              </div>
            </div>
            ...
            {scanTab==="scanner"&&(
              <div style={{padding:"4px 9px"}}>
                <div style={{display:"flex",gap:4,marginBottom:8,overflowX:"auto",paddingBottom:2}}>
                  {[["breakout","⚡ Breakout"],["breakdown","⬇ Breakdown"],["volume","📊 Volume"]].map(function(item){return(<button key={item[0]} onClick={function(){setScanSub(item[0]);}} style={{background:scanSub===item[0]?"linear-gradient(135deg,#39FF14,#00b377)":"#0a0a0a",border:"1px solid "+(scanSub===item[0]?"#39FF14":"#1a1a1a"),borderRadius:12,padding:"5px 12px",color:scanSub===item[0]?"#000":"#445",fontSize:9,fontWeight:800,cursor:"pointer",whiteSpace:"nowrap",fontFamily:"inherit"}}>{item[1]}</button>);})}
                </div>

                <div style={{display:"flex",gap:4,marginBottom:8,overflowX:"auto"}}>
                  {["1m","5m","15m","30m","1h","4h","Daily","Weekly"].map(function(tf){return(<button key={tf} onClick={function(){setScanTF(tf);}} style={{background:scanTF===tf?"#1a2a1a":"#0a0a0a",border:"1px solid "+(scanTF===tf?"#39FF1444":"#111"),borderRadius:8,padding:"3px 8px",color:scanTF===tf?"#39FF14":"#334",fontSize:8,cursor:"pointer",fontFamily:"inherit",flexShrink:0}}>{tf}</button>);})}
                </div>

                {scanSub==="breakout"&&(
                  <div>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                      <div style={{fontSize:10,fontWeight:800,color:"#39FF14"}}>⚡ Breakout Stocks Detected</div>
                      <div style={{fontSize:7,color:"#556"}}>{SCANNER_DATA.BREAKOUT_STOCKS.length} found · {scanTF}</div>
                    </div>
                    {SCANNER_DATA.BREAKOUT_STOCKS.map(function(st){return(
                      <div key={st.sym} style={{background:"#0d0d0d",border:"1px solid #39FF1422",borderLeft:"3px solid #39FF14",borderRadius:12,padding:"11px",marginBottom:8}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:7}}>
                          <div>
                            <div style={{display:"flex",alignItems:"center",gap:6}}>
                              <span style={{fontSize:13,fontWeight:900,color:"#fff"}}>{st.sym}</span>
                              {st.gapType&&<span style={{background:"#39FF1420",border:"1px solid #39FF1444",borderRadius:5,padding:"1px 5px",fontSize:7,color:"#39FF14",fontWeight:700}}>{st.gapType}</span>}
                            </div>
                            <div style={{fontSize:8,color:"#556",marginTop:2}}>{st.sector} · {st.timeframe} · {st.candle}</div>
                          </div>
                          <div style={{textAlign:"right"}}>
                            <div style={{fontFamily:"monospace",fontSize:13,fontWeight:900,color:"#fff"}}>Rs{st.ltp}</div>
                            <div style={{fontSize:10,fontWeight:700,color:"#39FF14"}}>+{st.chg}%</div>
                          </div>
                        </div>
                        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:5,marginBottom:7}}>
                          <div style={{background:"#080808",borderRadius:7,padding:"6px",textAlign:"center"}}>
                            <div style={{fontSize:6,color:"#445"}}>Pattern</div>
                            <div style={{fontSize:8,fontWeight:700,color:"#39FF14"}}>{st.pattern}</div>
                          </div>
                          <div style={{background:"#080808",borderRadius:7,padding:"6px",textAlign:"center"}}>
                            <div style={{fontSize:6,color:"#445"}}>Volume</div>
                            <div style={{fontSize:8,fontWeight:700,color:"#60a5fa"}}>{st.vol}</div>
                          </div>
                          <div style={{background:"#080808",borderRadius:7,padding:"6px",textAlign:"center"}}>
                            <div style={{fontSize:6,color:"#445"}}>Strength</div>
                            <div style={{fontSize:8,fontWeight:700,color:st.strength>=80?"#39FF14":st.strength>=65?"#f59e0b":"#ff4444"}}>{st.strength}/100</div>
                          </div>
                        </div>
                        <div style={{height:4,background:"#111",borderRadius:3,overflow:"hidden",marginBottom:7}}>
                          <div style={{height:"100%",width:st.strength+"%",background:"linear-gradient(90deg,#39FF14,#00ff88)",borderRadius:3}}></div>
                        </div>
                        <div style={{background:"#0a1a0a",border:"1px solid #39FF1422",borderRadius:7,padding:"7px"}}>
                          <div style={{fontSize:7,color:"#39FF14",fontWeight:700,marginBottom:2}}>📚 EDUCATIONAL NOTE</div>
                          <div style={{fontSize:8,color:"#889",lineHeight:1.5}}>Market structure: {st.structure}. Breakout above Rs.{st.res} observed. This is pattern recognition for educational study only. Not a buy/sell recommendation.</div>
                        </div>
                      </div>
                    );})}
                  </div>
                ...
                {scanSub==="breakdown"&&(
                  <div>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                      <div style={{fontSize:10,fontWeight:800,color:"#ff4444"}}>⬇ Breakdown Stocks Detected</div>
                      <div style={{fontSize:7,color:"#556"}}>{SCANNER_DATA.BREAKDOWN_STOCKS.length} found · {scanTF}</div>
                    </div>
                    {SCANNER_DATA.BREAKDOWN_STOCKS.map(function(st){return(
                      <div key={st.sym} style={{background:"#0d0d0d",border:"1px solid #ff444422",borderLeft:"3px solid #ff4444",borderRadius:12,padding:"11px",marginBottom:8}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:7}}>
                          <div>
                            <div style={{fontSize:13,fontWeight:900,color:"#fff"}}>{st.sym}</div>
                            <div style={{fontSize:8,color:"#556",marginTop:2}}>{st.sector} · {st.timeframe} · {st.candle}</div>
                          </div>
                          <div style={{textAlign:"right"}}>
                            <div style={{fontFamily:"monospace",fontSize:13,fontWeight:900,color:"#fff"}}>Rs{st.ltp}</div>
                            <div style={{fontSize:10,fontWeight:700,color:"#ff4444"}}>{st.chg}%</div>
                          </div>
                        </div>
                        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:5,marginBottom:7}}>
                          <div style={{background:"#080808",borderRadius:7,padding:"6px",textAlign:"center"}}>
                            <div style={{fontSize:6,color:"#445"}}>Pattern</div>
                            <div style={{fontSize:8,fontWeight:700,color:"#ff4444"}}>{st.pattern}</div>
                          </div>
                          <div style={{background:"#080808",borderRadius:7,padding:"6px",textAlign:"center"}}>
                            <div style={{fontSize:6,color:"#445"}}>Volume</div>
                            <div style={{fontSize:8,fontWeight:700,color:"#60a5fa"}}>{st.vol}</div>
                          ...
                          <div style={{background:"#080808",borderRadius:7,padding:"6px",textAlign:"center"}}>
                            <div style={{fontSize:6,color:"#445"}}>Weakness</div>
                            <div style={{fontSize:8,fontWeight:700,color:st.strength>=80?"#ff4444":st.strength>=65?"#f59e0b":"#ff8888"}}>{st.strength}/100</div>
                          </div>
                        </div>
                        <div style={{height:4,background:"#111",borderRadius:3,overflow:"hidden",marginBottom:7}}>
                          <div style={{height:"100%",width:st.strength+"%",background:"linear-gradient(90deg,#ff8888,#ff4444)",borderRadius:3}}></div>
                        </div>
                        <div style={{background:"#1a0a0a",border:"1px solid #ff444422",borderRadius:7,padding:"7px"}}>
                          <div style={{fontSize:7,color:"#ff4444",fontWeight:700,marginBottom:2}}>📚 EDUCATIONAL NOTE</div>
                          <div style={{fontSize:8,color:"#889",lineHeight:1.5}}>Market structure: {st.structure}. Breakdown below support Rs.{st.sup} observed. Pattern study for educational purposes only. Not a sell recommendation.</div>
                        </div>
                      </div>
                    );})}
                  </div>
                ...
                {scanSub==="volume"&&(
                  <div>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                      <div style={{fontSize:10,fontWeight:800,color:"#60a5fa"}}>📊 Volume Intelligence</div>
                      <div style={{fontSize:7,color:"#556"}}>{SCANNER_DATA.VOLUME_STOCKS.length} signals</div>
                    </div>
                    {SCANNER_DATA.VOLUME_STOCKS.map(function(st){return(
                      <div key={st.sym} style={{background:"#0d0d0d",border:"1px solid #60a5fa22",borderLeft:"3px solid #60a5fa",borderRadius:12,padding:"11px",marginBottom:8}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:7}}>
                          <div>
                            <div style={{fontSize:13,fontWeight:900,color:"#fff"}}>{st.sym}</div>
                            <div style={{display:"flex",alignItems:"center",gap:5,marginTop:2}}>
                              <span style={{background:"#60a5fa22",border:"1px solid #60a5fa44",borderRadius:5,padding:"1px 5px",fontSize:7,color:"#60a5fa",fontWeight:700}}>{st.type}</span>
                              {st.volSpike&&<span style={{background:"#FFD70022",border:"1px solid #FFD70044",borderRadius:5,padding:"1px 5px",fontSize:7,color:"#FFD700",fontWeight:700}}>SPIKE</span>}
                            </div>
                          </div>
                          <div style={{textAlign:"right"}}>
                            <div style={{fontFamily:"monospace",fontSize:13,fontWeight:900,color:"#fff"}}>Rs{st.ltp}</div>
                            <div style={{fontSize:10,fontWeight:700,color:st.chg>=0?"#39FF14":"#ff4444"}}>{st.chg>=0?"+":""}{st.chg}%</div>
                          </div>
                        </div>
                        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:5,marginBottom:7}}>
                          {[["Volume",st.vol,"#60a5fa"],["Rel Vol",st.relVol,"#FFD700"],["Delivery",st.delivPct,"#a78bfa"]].map(function(item){return(<div key={item[0]} style={{background:"#080808",borderRadius:7,padding:"6px",textAlign:"center"}}><div style={{fontSize:6,color:"#445"}}>{item[0]}</div><div style={{fontSize:11,fontWeight:800,color:item[2]}}>{item[1]}</div></div>);})}
                        </div>
                        <div style={{background:"#0a0a1a",border:"1px solid #60a5fa22",borderRadius:7,padding:"7px"}}>
                          <div style={{fontSize:7,color:"#60a5fa",fontWeight:700,marginBottom:2}}>📚 VOLUME EDUCATION</div>
                          <div style={{fontSize:8,color:"#889",lineHeight:1.5}}>Relative volume {st.relVol} = {st.relVol} times the average volume. High delivery % ({st.delivPct}) indicates institutional participation. Educational reference only.</div>
                        </div>
                      </div>
                    );})}
                  </div>
                ...
                <div style={{background:"#080400",border:"1px solid #ff440018",borderRadius:9,padding:"8px",marginTop:4,fontSize:7,color:"#885522",lineHeight:1.7,textAlign:"center"}}>⚠️ All scanner results are EDUCATIONAL ONLY. Pattern detection does not guarantee future price movement. Not investment advice. SEBI disclaimer applies.</div>
              </div>
            )}
            ...
            {scanTab==="candles"&&(
              <div style={{padding:"4px 9px"}}>
                ...
                <div style={{fontSize:10,fontWeight:800,color:"#f59e0b",marginBottom:8}}>🕯 Live Candlestick Patterns · {scanTF}</div>
                {SCANNER_DATA.CANDLE_PATTERNS.map(function(p){
                  var bc=p.signal==="Bullish"?"#39FF14":p.signal==="Bearish"?"#ff4444":"#f59e0b";
                  var edu=CANDLE_EDU[p.pattern]||{};
                  return(
                    <div key={p.sym+p.pattern} style={{background:"#0d0d0d",border:"1px solid "+bc+"22",borderLeft:"3px solid "+bc,borderRadius:12,padding:"11px",marginBottom:8}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:7}}>
                        <div>
                          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
                            <span style={{fontSize:13,fontWeight:900,color:"#fff"}}>{p.sym}</span>
                            <span style={{background:bc+"20",border:"1px solid "+bc+"44",borderRadius:5,padding:"1px 6px",fontSize:7,color:bc,fontWeight:800}}>{p.signal}</span>
                            <span style={{fontSize:7,color:"#445"}}>{p.timeframe}</span>
                          </div>
                          <div style={{display:"flex",alignItems:"center",gap:5}}>
                            <span style={{fontSize:16}}>{edu.emoji||"🕯"}</span>
                            <span style={{fontSize:10,fontWeight:700,color:bc}}>{p.pattern}</span>
                          </div>
                        </div>
                        <div style={{textAlign:"right"}}>
                          <div style={{fontFamily:"monospace",fontSize:13,fontWeight:900,color:"#fff"}}>Rs{p.ltp}</div>
                          <div style={{fontSize:10,fontWeight:700,color:p.chg>=0?"#39FF14":"#ff4444"}}>{p.chg>=0?"+":""}{p.chg}%</div>
                        </div>
                      </div>
                      <div style={{background:"#080808",borderRadius:8,padding:"8px",marginBottom:6}}>
                        <div style={{fontSize:7,color:"#445",marginBottom:3}}>Pattern Strength: <span style={{color:p.strength==="Strong"?"#39FF14":p.strength==="Moderate"?"#f59e0b":"#888",fontWeight:700}}>{p.strength}</span></div>
                        <div style={{fontSize:9,color:"#aaa",lineHeight:1.6}}>{p.desc}</div>
                      </div>
                      {edu.te&&<div style={{background:"#0a0a14",border:"1px solid #60a5fa22",borderRadius:7,padding:"6px",marginBottom:5}}>
                        <div style={{fontSize:7,color:"#60a5fa",fontWeight:700,marginBottom:2}}>తెలుగు · {p.pattern}</div>
                        <div style={{fontSize:8,color:"#778",lineHeight:1.5}}>{edu.te}</div>
                      </div>}
                      {edu.hi&&<div style={{background:"#0a140a",border:"1px solid #a78bfa22",borderRadius:7,padding:"6px"}}>
                        <div style={{fontSize:7,color:"#a78bfa",fontWeight:700,marginBottom:2}}>हिंदी · {p.pattern}</div>
                        <div style={{fontSize:8,color:"#778",lineHeight:1.5}}>{edu.hi}</div>
                      </div>}
                    </div>
                  );
                })}

                <div style={{background:"#0a0a0a",border:"1px solid #1a2a1a",borderRadius:12,padding:"12px",marginTop:4}}>
                  <div style={{fontSize:10,fontWeight:800,color:"#f59e0b",marginBottom:8}}>📚 Candlestick Pattern Guide</div>
                  {Object.entries(CANDLE_EDU).map(function(entry){
                    var name=entry[0],edu=entry[1];
                    var bc=edu.signal==="Bullish"?"#39FF14":edu.signal==="Bearish"?"#ff4444":edu.signal==="Strong"?"#FFD700":"#f59e0b";
                    return(<div key={name} style={{background:"#080808",borderRadius:9,padding:"9px",marginBottom:6}}>
                      <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:4}}>
                        <span style={{fontSize:18}}>{edu.emoji}</span>
                        <div style={{flex:1}}>
                          <div style={{fontSize:10,fontWeight:800,color:"#fff"}}>{name}</div>
                          <div style={{fontSize:7,color:bc,fontWeight:700,background:bc+"18",borderRadius:4,padding:"1px 5px",display:"inline-block",marginTop:2}}>{edu.signal}</div>
                        </div>
                      </div>
                      <div style={{fontSize:8,color:"#667",lineHeight:1.6}}>{edu.desc}</div>
                    </div>);
                  })}
                </div>
              </div>
            ...
            {scanTab==="structure"&&(
              <div style={{padding:"4px 9px"}}>
                <div style={{fontSize:10,fontWeight:800,color:"#a78bfa",marginBottom:8}}>🏗 Market Structure Analysis</div>
                {SCANNER_DATA.STRUCTURE.map(function(st){
                  var isUp=st.type==="Uptrend";
                  var isSide=st.type==="Sideways";
                  var bc=isUp?"#39FF14":isSide?"#f59e0b":"#ff4444";
                  return(
                    <div key={st.sym} style={{background:"#0d0d0d",border:"1px solid "+bc+"22",borderLeft:"3px solid "+bc,borderRadius:12,padding:"11px",marginBottom:8}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:7}}>
                        <div>
                          <div style={{fontSize:13,fontWeight:900,color:"#fff"}}>{st.sym}</div>
                          <div style={{fontSize:8,color:"#556",marginTop:2}}>{st.sector}</div>
                        </div>
                        <div style={{textAlign:"right"}}>
                          <div style={{fontFamily:"monospace",fontSize:13,fontWeight:900,color:"#fff"}}>Rs{st.ltp}</div>
                          <div style={{background:bc+"20",border:"1px solid "+bc+"44",borderRadius:6,padding:"2px 7px",fontSize:8,fontWeight:800,color:bc,marginTop:3}}>{isUp?"📈":isSide?"➡":"📉"} {st.type}</div>
                        </div>
                      </div>
                      <div style={{background:"#080808",borderRadius:9,padding:"9px",marginBottom:7}}>
                        <div style={{fontSize:8,color:bc,fontWeight:700,marginBottom:6}}>{st.structure}</div>
                        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5}}>
                          {[["Prev High","Rs"+st.prev_high,"#888"],["Prev Low","Rs"+st.prev_low,"#888"],["Curr High","Rs"+st.curr_high,isUp?"#39FF14":"#ff4444"],["Curr Low","Rs"+st.curr_low,isUp?"#39FF14":"#ff4444"]].map(function(item){return(<div key={item[0]} style={{textAlign:"center"}}><div style={{fontSize:7,color:"#445"}}>{item[0]}</div><div style={{fontFamily:"monospace",fontSize:10,fontWeight:700,color:item[2]}}>{item[1]}</div></div>);})}
                        </div>
                      </div>
                      <div style={{background:bc+"0a",border:"1px solid "+bc+"22",borderRadius:7,padding:"7px"}}>
                        <div style={{fontSize:7,color:bc,fontWeight:700,marginBottom:2}}>📚 STRUCTURE EDUCATION</div>
                        <div style={{fontSize:8,color:"#889",lineHeight:1.5}}>
                          {isUp?"Higher Highs (HH) + Higher Lows (HL) = Uptrend structure. Price making progressively higher peaks and higher bases — classic bullish market structure.":isSide?"Lower Highs + Higher Lows = Consolidation / Sideways structure. Market in balance between buyers and sellers. Breakout direction determines next trend.":"Lower Highs (LH) + Lower Lows (LL) = Downtrend structure. Price making lower peaks and lower bases — classic bearish market structure."} Educational only.
                        </div>
                      </div>
                    </div>
                  );
                })}
                <div style={{background:"#0d0d0d",border:"1px solid #1a1a1a",borderRadius:12,padding:"11px",marginTop:4}}>
                  <div style={{fontSize:10,fontWeight:800,color:"#a78bfa",marginBottom:8}}>📚 Market Structure Basics</div>
                  {[
                    {ico:"📈",title:"Higher High - Higher Low (HH-HL)",color:"#39FF14",en:"Price making higher peaks and higher bases. Classic uptrend structure. Buyers are in control.",te:"ధర అధిక శిఖరాలు మరియు అధిక ఆధారాలను చేస్తోంది. క్లాసిక్ అప్‌ట్రెండ్ నిర్మాణం.",hi:"कीमत ऊंची चोटियां और ऊंचे आधार बना रही है। क्लासिक अपट्रेंड संरचना।"},
                    {ico:"📉",title:"Lower High - Lower Low (LH-LL)",color:"#ff4444",en:"Price making lower peaks and lower bases. Classic downtrend structure. Sellers are in control.",te:"ధర తక్కువ శిఖరాలు మరియు తక్కువ ఆధారాలను చేస్తోంది. క్లాసిక్ డౌన్‌ట్రెండ్ నిర్మాణం.",hi:"कीमत निचली चोटियां और निचले आधार बना रही है। क्लासिक डाउनट्रेंड संरचना।"},
                    {ico:"➡",title:"Lower High - Higher Low (Consolidation)",color:"#f59e0b",en:"Converging price action. Neither buyers nor sellers in control. Breakout expected soon.",te:"సంకోచించే ధర చర్య. కొనుగోలుదారులు లేదా అమ్మకందారులు నియంత్రణలో లేరు. బ్రేకౌట్ ఆశించబడుతోంది.",hi:"संकुचित मूल्य क्रिया। न खरीदार, न विक्रेता नियंत्रण में। जल्द ब्रेकआउट अपेक्षित।"},
                  ].map(function(item){return(<div key={item.title} style={{background:"#080808",borderRadius:9,padding:"9px",marginBottom:7}}>
                    <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:5}}><span style={{fontSize:18}}>{item.ico}</span><div style={{fontSize:10,fontWeight:800,color:item.color}}>{item.title}</div></div>
                    <div style={{fontSize:8,color:"#aaa",lineHeight:1.5,marginBottom:4}}>{item.en}</div>
                    <div style={{fontSize:7,color:"#60a5fa",marginBottom:2}}>తెలుగు: <span style={{color:"#778"}}>{item.te}</span></div>
                    <div style={{fontSize:7,color:"#a78bfa"}}>हिंदी: <span style={{color:"#778"}}>{item.hi}</span></div>
                  </div>);})}
                </div>
              </div>
            )}
            ...
            {scanTab==="alerts"&&(
              <div style={{padding:"4px 9px"}}>
                <div style={{fontSize:10,fontWeight:800,color:"#ff5500",marginBottom:8}}>⚡ Smart Educational Alerts</div>
                {SMART_ALERTS_DATA.map(function(a){
                  return(
                    <div key={a.id} style={{background:"#0d0d0d",border:"1px solid "+a.color+"22",borderLeft:"3px solid "+a.color,borderRadius:12,padding:"11px",marginBottom:8,position:"relative",opacity:isPrem?1:0.75}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:7}}>
                        <div style={{flex:1}}>
                          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>
                            <span style={{fontSize:16}}>{a.icon}</span>
                            <span style={{fontSize:9,fontWeight:800,color:a.color,letterSpacing:0.5}}>{a.type}</span>
                            <span style={{background:a.color+"18",border:"1px solid "+a.color+"33",borderRadius:4,padding:"1px 5px",fontSize:7,color:"#555"}}>{a.timeframe}</span>
                          </div>
                          <div style={{fontSize:12,fontWeight:800,color:"#fff",marginBottom:3}}>{a.sym}</div>
                          <div style={{fontSize:9,color:"#aaa",lineHeight:1.4}}>{a.msg}</div>
                        </div>
                        <div style={{textAlign:"right",flexShrink:0,marginLeft:8}}>
                          <div style={{fontSize:8,color:"#445",marginBottom:3}}>Strength</div>
                          <div style={{fontFamily:"monospace",fontSize:13,fontWeight:900,color:a.color}}>{a.strength}</div>
                          <div style={{fontSize:6,color:"#334"}}>/100</div>
                        </div>
                      </div>
                      <div style={{height:3,background:"#111",borderRadius:2,overflow:"hidden",marginBottom:7}}>
                        <div style={{height:"100%",width:a.strength+"%",background:a.color,borderRadius:2,opacity:0.7}}></div>
                      </div>
                      <div style={{background:a.color+"08",border:"1px solid "+a.color+"18",borderRadius:7,padding:"7px"}}>
                        <div style={{fontSize:7,color:a.color,fontWeight:700,marginBottom:2}}>📚 EDUCATIONAL NOTE</div>
                        <div style={{fontSize:8,color:"#778",lineHeight:1.5}}>{a.eduNote}</div>
                      </div>
                      {!isPrem&&(<div style={{position:"absolute",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.6)",backdropFilter:"blur(2px)",borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center"}}>
                        <button onClick={function(){setShowSub(true);}} style={{background:"linear-gradient(135deg,#FFD700,#FFA500)",border:"none",borderRadius:8,padding:"5px 14px",fontSize:9,fontWeight:800,color:"#000",cursor:"pointer",fontFamily:"inherit"}}>Unlock Premium</button>
                      </div>)}
                    </div>
                  );
                })}
                <div style={{background:"#080808",border:"1px solid #1a1a1a",borderRadius:10,padding:"10px",marginTop:4}}>
                  <div style={{fontSize:9,fontWeight:700,color:"#f59e0b",marginBottom:7}}>📖 Alert Types Explained</div>
                  {[
                    ["⚡","Breakout Alert","Price breaks above a key resistance level with volume. Study breakout patterns for education.","రెసిస్టెన్స్ స్థాయిని పైన వాల్యూమ్‌తో బ్రేక్ చేయడం. విద్య కోసం మాత్రమే.","प्रतिरोध स्तर के ऊपर वॉल्यूम के साथ ब्रेक।"],
                    ["⬇","Breakdown Alert","Price falls below a key support level. Learn about support/resistance for educational purposes.","కీలక సపోర్ట్ స్థాయి క్రింద ధర పడిపోవడం. విద్య కోసం నేర్చుకోండి.","कीमत मुख्य समर्थन स्तर से नीचे गिरती है।"],
                    ["📊","Volume Expansion","Significant increase in trading volume. Volume is the fuel of price movement — key to study.","ట్రేడింగ్ వాల్యూమ్‌లో గణనీయమైన పెరుగుదల. వాల్యూమ్ ధర కదలికకు ఇంధనం.","ट्रेडिंग वॉल्यूम में महत्वपूर्ण वृद्धि।"],
                    ["🔄","Trend Reversal","Potential change in trend direction detected. Market structure shifting — important to observe.","ట్రెండ్ దిశలో సంభావ్య మార్పు. మార్కెట్ నిర్మాణం మారుతోంది.","ट्रेंड दिशा में संभावित बदलाव।"],
                    ["🚀","Gap Up Alert","Opening price significantly above previous close. Pre-market and opening analysis topic.","ముందటి క్లోజ్ కంటే చాలా ఎక్కువగా ఓపెనింగ్ ధర.","पिछले क्लोज से काफी ऊपर खुलना।"],
                    ["🏆","New High Alert","Stock making a 52-week or all-time high. Momentum study topic in technical analysis.","52-వారాల లేదా సర్వకాల గరిష్ట స్థాయి చేరడం.","52-सप्ताह या सर्वकालिक उच्च बनाना।"],
                  ].map(function(item){return(<div key={item[0]} style={{display:"flex",gap:8,marginBottom:7,background:"#0a0a0a",borderRadius:8,padding:"8px"}}>
                    <span style={{fontSize:16,flexShrink:0}}>{item[0]}</span>
                    <div>
                      <div style={{fontSize:9,fontWeight:700,color:"#fff",marginBottom:2}}>{item[1]}</div>
                      <div style={{fontSize:8,color:"#667",lineHeight:1.5,marginBottom:3}}>{item[2]}</div>
                      <div style={{fontSize:7,color:"#60a5fa"}}>తె: <span style={{color:"#556"}}>{item[3]}</span></div>
                      <div style={{fontSize:7,color:"#a78bfa",marginTop:1}}>हि: <span style={{color:"#556"}}>{item[4]}</span></div>
                    </div>
                  </div>);})}
                </div>
                <div style={{background:"#080400",border:"1px solid #ff440018",borderRadius:9,padding:"8px",marginTop:6,fontSize:7,color:"#885522",lineHeight:1.7,textAlign:"center"}}>⚠️ All alerts are EDUCATIONAL ONLY. These are pattern observations, not trading signals or investment advice. Always consult a SEBI registered advisor.</div>
              </div>
            )}

            {scanTab==="ai"&&(
              <div style={{padding:"4px 9px"}}>
                <div style={{fontSize:11,fontWeight:800,color:"#fff",marginBottom:3}}>🤖 AI Chart Explanation</div>
                <div style={{fontSize:8,color:"#556",marginBottom:10}}>AI explains charts in Telugu, Hindi & English</div>

                <div style={{display:"flex",gap:4,marginBottom:8}}>
                  {[["en","English"],["hi","हिंदी"],["te","తెలుగు"]].map(function(item){return(<button key={item[0]} onClick={function(){setAiChartLang(item[0]);}} style={{background:aiChartLang===item[0]?"#39FF1418":"#0a0a0a",border:"1.5px solid "+(aiChartLang===item[0]?"#39FF14":"#1a1a1a"),borderRadius:16,padding:"4px 12px",color:aiChartLang===item[0]?"#39FF14":"#445",fontSize:9,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>{item[1]}</button>);})}
                </div>

                <div style={{background:"#0d0d0d",border:"1px solid #39FF1433",borderRadius:12,padding:"12px",marginBottom:8}}>
                  <div style={{fontSize:9,color:"#39FF14",fontWeight:700,marginBottom:6}}>Ask AI to Explain Chart (Educational)</div>
                  <textarea value={aiChartQuery} onChange={function(e){setAiChartQuery(e.target.value);}} placeholder={aiChartLang==="te"?"TATASTEEL చార్ట్ నిర్మాణం వివరించు\nNIFTY ట్రెండ్ ఎలా ఉంది?\nBullish Engulfing అంటే ఏమిటి?\nSupport Resistance ఎలా పని చేస్తాయి?":aiChartLang==="hi"?"TATASTEEL चार्ट संरचना समझाओ\nNIFTY ट्रेंड कैसा है?\nBullish Engulfing क्या है?\nSupport Resistance कैसे काम करते हैं?":"Explain TATASTEEL chart structure\nWhat is NIFTY trend direction?\nExplain Bullish Engulfing pattern\nHow does Support Resistance work?"} style={{width:"100%",background:"#0a0a0a",border:"1px solid #1a2a1a",borderRadius:9,padding:"10px",color:"#e8eaf0",fontSize:11,fontFamily:"inherit",outline:"none",boxSizing:"border-box",minHeight:80,resize:"none"}}/>
                  <button onClick={async function(){
                    if(!aiChartQuery.trim())return;
                    setAiChartLoading(true);setAiChartResult("");
                    var langName=aiChartLang==="te"?"Telugu (తెలుగు భాషలో సమాధానం ఇవ్వండి)":aiChartLang==="hi"?"Hindi (हिंदी में जवाब दें)":"English";
                    try{
                      var resp=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,messages:[{role:"user",content:"You are an expert stock market educator for Indian retail traders. STRICTLY educational only.\n\nIMPORTANT: Answer ONLY in "+langName+" language.\n\nUser asks: "+aiChartQuery+"\n\nProvide a clear educational explanation covering:\n1. What this pattern/concept means\n2. How to identify it on a chart\n3. What it tells us about market psychology\n4. Historical examples if relevant\n5. Important limitations and what NOT to assume\n\nALWAYS end with: This explanation is for educational purposes only. Not investment advice. Consult SEBI registered advisor before investing.\n\n"+(aiChartLang==="te"?"తెలుగులో సమాధానం ఇవ్వండి.":aiChartLang==="hi"?"हिंदी में जवाब दें।":"")}]})});
                      var data=await resp.json();
                      var txt=data.content&&data.content[0]?data.content[0].text:"AI response unavailable.";
                      setAiChartResult(txt);
                    }catch(e){setAiChartResult("AI Chart explanation temporarily unavailable. Please try again.");}
                    setAiChartLoading(false);
                  }} disabled={aiChartLoading||!aiChartQuery.trim()} style={{width:"100%",background:aiChartLoading?"#0d0d0d":"linear-gradient(135deg,#39FF14,#00b377)",border:"none",borderRadius:10,padding:"10px",fontSize:12,fontWeight:800,color:aiChartLoading?"#445":"#000",cursor:aiChartLoading?"not-allowed":"pointer",fontFamily:"inherit",marginTop:8}}>
                    {aiChartLoading?"🔄 AI Explaining Chart...":"🤖 Explain Chart"}
                  </button>
                </div>

                {aiChartResult&&(<div style={{background:"#0a0a0a",border:"1px solid #39FF1422",borderRadius:12,padding:"12px",marginBottom:8}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                    <div style={{fontSize:10,fontWeight:700,color:"#39FF14"}}>📊 AI Chart Explanation</div>
                    <div style={{fontSize:8,color:"#39FF14",background:"#39FF1415",borderRadius:5,padding:"2px 6px"}}>{aiChartLang==="te"?"తెలుగు":aiChartLang==="hi"?"हिंदी":"English"}</div>
                  </div>
                  <div style={{fontSize:10,color:"#ccc",lineHeight:1.7,whiteSpace:"pre-wrap"}}>{aiChartResult}</div>
                </div>)}

                <div style={{background:"#0d0d0d",border:"1px solid #1a1a1a",borderRadius:12,padding:"12px",marginBottom:8}}>
                  <div style={{fontSize:10,fontWeight:700,color:"#a78bfa",marginBottom:8}}>Quick Chart Questions</div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr",gap:5}}>
                    {[
                      {q_en:"Explain the current NIFTY chart structure and trend direction",q_te:"ప్రస్తుత NIFTY చార్ట్ నిర్మాణం మరియు ట్రెండ్ దిశ వివరించు",q_hi:"वर्तमान NIFTY चार्ट संरचना और ट्रेंड दिशा समझाओ",label:"📊 NIFTY Trend"},
                      {q_en:"Explain how to identify support and resistance levels on a chart",q_te:"చార్ట్‌లో సపోర్ట్ మరియు రెసిస్టెన్స్ స్థాయిలను ఎలా గుర్తించాలో వివరించు",q_hi:"चार्ट पर सपोर्ट और रेसिस्टेंस स्तर कैसे पहचानें",label:"📏 Support & Resistance"},
                      {q_en:"Explain volume behaviour and what high volume means for price action",q_te:"వాల్యూమ్ ప్రవర్తన మరియు అధిక వాల్యూమ్ ధర చర్యకు అర్థమేమిటో వివరించు",q_hi:"वॉल्यूम व्यवहार और उच्च वॉल्यूम का मतलब समझाओ",label:"📊 Volume Behaviour"},
                      {q_en:"Explain candlestick patterns and how to read them for market psychology",q_te:"కొవ్వొత్తి నమూనాలు మరియు మార్కెట్ మనస్తత్వం కోసం వాటిని చదవడం ఎలాగో వివరించు",q_hi:"कैंडलस्टिक पैटर्न और बाजार मनोविज्ञान के लिए उन्हें कैसे पढ़ें",label:"🕯 Candlestick Patterns"},
                      {q_en:"Explain multi-timeframe analysis and how different timeframes work together",q_te:"మల్టీ-టైమ్‌ఫ్రేమ్ విశ్లేషణ మరియు వివిధ టైమ్‌ఫ్రేమ్‌లు ఒకదానితో ఒకటి ఎలా పని చేస్తాయో వివరించు",q_hi:"मल्टी-टाइमफ्रेम एनालिसिस और अलग टाइमफ्रेम कैसे काम करते हैं",label:"⏱ Multi-Timeframe"},
                    ].map(function(item){
                      var q=aiChartLang==="te"?item.q_te:aiChartLang==="hi"?item.q_hi:item.q_en;
                      return(<button key={item.label} onClick={function(){setAiChartQuery(q);}} style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:9,padding:"10px",cursor:"pointer",textAlign:"left",display:"flex",justifyContent:"space-between",alignItems:"center",fontFamily:"inherit"}}>
                        <div style={{fontSize:10,color:"#fff",fontWeight:600}}>{item.label}</div>
                        <span style={{fontSize:8,color:"#39FF14"}}>Ask →</span>
                      </button>);
                    })}
                  </div>
                </div>

                <div style={{background:"#080400",border:"1px solid #ff440018",borderRadius:9,padding:"8px",fontSize:7,color:"#885522",lineHeight:1.7,textAlign:"center"}}>⚠️ AI explanations are for EDUCATIONAL PURPOSES ONLY. Not investment advice. Patterns don't guarantee future performance.</div>
              </div>
            )}
          </div>
        )}
      ...
      <div style={{position:"absolute",bottom:0,left:0,right:0,background:"rgba(0,0,0,0.98)",borderTop:"1px solid #0d0d0d",display:"flex",justifyContent:"space-around",padding:"5px 3px 10px",zIndex:100}}>
        {[["home","&#127968;","Home"],["watch","&#11088;","Watch"],["markets","&#128200;","Stocks"],["oi","&#128202;","OI"],["scanner","&#128209;","Scanner"],["news","&#128240;","News"],["learn","&#128218;","Learn"]].concat(isAdmin?[["admin","&#128081;","Admin"]]:[]).map(function(item){
