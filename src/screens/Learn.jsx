...
export default function LearnScreen() {
  ...
  if(sec=="risk") return (
    <div style={{background:DB,minHeight:"100vh",paddingBottom:80,fontFamily:"Inter,Arial,sans-serif"}}>
      <BackHeader onBack={function(){setSec("home");}} title="Risk Management" sub="Position size, SL, R:R ratio"/>
      <div style={{padding:14}}>
  ...
  if(sec=="psychology") return (
    <div style={{background:DB,minHeight:"100vh",paddingBottom:80,fontFamily:"Inter,Arial,sans-serif"}}>
      <BackHeader onBack={function(){setSec("home");}} title="Trading Psychology" sub="Mindset, emotions, discipline"/>
      <div style={{padding:14}}>
        {[
          {title:"Fear & Greed",body:"Fear makes you exit early. Greed makes you hold losers. Identify which emotion controls you and work on it daily."},
          {title:"Loss Aversion",body:"Losses hurt 2x more than gains feel good. This causes traders to hold losers too long and cut winners too early."},
          {title:"Confirmation Bias",body:"We tend to look for information that confirms what we already believe. Always look at both bullish and bearish cases."},
          {title:"Overtrading",body:"Boredom and revenge trading are deadly. The best traders often sit on their hands for hours waiting for the right setup."},
          {title:"Discipline > Intelligence",body:"A consistent trader with simple rules beats a smart trader who keeps changing strategies."},
          {title:"Journaling",body:"Write every trade down — entry, exit, reason, emotion. This is the single most powerful habit of professional traders."},
          {title:"Accept Losses",body:"Every trader has losing days. What separates pros is they keep losses small and let winners run."},
        ].map(function(t){return <TopicCard key={t.title} topic={t}/>;  })}
      </div>
    </div>
  );
  ...
  if(sec=="priceaction") return (
    <div style={{background:DB,minHeight:"100vh",paddingBottom:80,fontFamily:"Inter,Arial,sans-serif"}}>
      <BackHeader onBack={function(){setSec("home");}} title="Price Action" sub="Support, resistance, market structure"/>
      <div style={{padding:14}}>
        {[
          {title:"Support & Resistance",body:"Support is a price level where buyers historically step in. Resistance is where sellers dominate. These levels flip roles when broken."},
          {title:"Trend Structure",body:"Uptrend = Higher Highs + Higher Lows. Downtrend = Lower Highs + Lower Lows. Trade in the direction of the trend."},
          {title:"Breakout Trading",body:"When price breaks above resistance with volume, it signals a strong move. Wait for a candle close above the level before entering."},
          {title:"Pullback Entry",body:"The safest entries are on pullbacks to support in an uptrend. Risk is clearly defined with SL just below support."},
          {title:"Consolidation",body:"When price moves sideways in a tight range, it is consolidating energy. Breakout from consolidation often leads to strong moves."},
          {title:"Volume Analysis",body:"Volume confirms price moves. Breakout on high volume = valid. Breakout on low volume = likely to fail."},
        ].map(function(t){return <TopicCard key={t.title} topic={t}/>;  })}
      </div>
    </div>
  );
