  ...
  if (session == "live") {
    return (
      <div style={{background:"linear-gradient(135deg,"+CARD+",#0A1525)",border:"1px solid "+BD,borderRadius:18,overflow:"hidden",marginBottom:14,boxShadow:"0 4px 24px rgba(0,0,0,0.3)"}}>
        ...
        <div onClick={function(){setOpen(function(p){return !p;});}} style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:10,cursor:"pointer",userSelect:"none"}}>
          ...
          <span style={{fontSize:14,color:T3,display:"inline-block",transition:"transform 0.3s ease",transform:open?"rotate(180deg)":"rotate(0deg)"}}>&#9660;</span>
        </div>

        {/* Collapsible body — lazy rendered */}
        {open && (
          <div style={{borderTop:"1px solid "+BD,padding:"12px 16px",animation:"apc-slide 0.3s ease"}}>
        ...
        <style>{"@keyframes apc-slide{from{opacity:0;transform:translateY(-6px)}to{opacity:1;transform:translateY(0)}}"}</style>
      </div>
    );
  }
