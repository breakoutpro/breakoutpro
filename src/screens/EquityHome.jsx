...
export default function EquityHome(props){
  var TH=useTheme().c;
  var BG=TH.bg,CARD=TH.card,CARD2=TH.card2,BD=TH.border,CARDBORDER=TH.border;
  var BLUE=TH.blue,CYAN=TH.blue,PROBLUE=TH.blue;
  var UP=TH.up,DOWN=TH.down,GOLD=TH.gold;
  ...
  return (
    <div style={{background:BG,minHeight:"100vh",fontFamily:"'Inter',Arial,sans-serif",paddingBottom:84,color:T1}}>
      ...
      <div onClick={function(){setSelArticle(JUSTIN[tickerIdx]);}} style={{background:CARD2,borderBottom:"1px solid "+BD,display:"flex",alignItems:"stretch",overflow:"hidden",cursor:"pointer"}}>
        <div style={{background:DOWN,padding:"5px 8px",display:"flex",alignItems:"center",flexShrink:0}}>
        ...
        <div style={{flex:1,display:"flex",alignItems:"center",padding:"6px 11px",minWidth:0}}>
          <span key={tickerIdx} style={{fontSize:10.5,color:T1,fontWeight:600,lineHeight:1.35,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden",animation:"ticker-fade 0.4s ease"}}>{TICKER[tickerIdx]}</span>
        </div>
