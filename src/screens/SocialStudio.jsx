...
export default function SocialStudio(props){
  ...
  var BD2 = theme.c.border2, BLUE = theme.c.blue, CARD = theme.c.card, T2 = theme.c.text2, T3 = theme.c.text3, WARN = theme.c.warn; T1=theme.c.text1; UP=theme.c.up;
  ...
  return (
    <div style={{padding:"14px 14px 40px"}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
        {props.setTab?<button onClick={function(){props.setTab("home");}} style={{background:"none",border:"none",color:BLUE,fontSize:13,cursor:"pointer",fontFamily:"inherit",padding:0}}>&#8592;</button>:null}
        <span style={{fontSize:17,fontWeight:900,color:T1}}>Social Media Studio</span>
      </div>
      <div style={{fontSize:9,color:T3,marginBottom:14}}>Auto-generate branded posters from market data &#8226; Educational only</div>

      {/* template picker */}
      <div style={{fontSize:10,fontWeight:800,color:T2,marginBottom:8}}>POSTER TYPE</div>
      <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:8,marginBottom:12}}>
        {TEMPLATES.map(function(x){
          var act=tpl.id==x.id;
          return <button key={x.id} onClick={function(){setTpl(x);}} style={{flexShrink:0,background:act?x.accent:CARD,border:"1px solid "+(act?x.accent:BD),borderRadius:20,padding:"7px 13px",color:act?"#001910":T2,fontSize:10,fontWeight:act?800:600,cursor:"pointer",fontFamily:"inherit"}}>{x.label}</button>;
        })}
      </div>

      {/* ratio picker */}
      <div style={{fontSize:10,fontWeight:800,color:T2,marginBottom:8}}>SIZE</div>
      <div style={{display:"flex",gap:6,marginBottom:14}}>
        {RATIOS.map(function(r){
          var act=ratio.id==r.id;
          return <button key={r.id} onClick={function(){setRatio(r);}} style={{flex:1,background:act?BLUE:CARD,border:"1px solid "+(act?BLUE:BD),borderRadius:10,padding:"8px 4px",color:act?"#fff":T2,fontSize:9.5,fontWeight:act?800:600,cursor:"pointer",fontFamily:"inherit"}}>{r.label}<div style={{fontSize:7,opacity:0.8}}>{r.w}x{r.h}</div></button>;
        })}
      </div>

      {/* preview */}
      <div style={{background:CARD,border:"1px solid "+BD,borderRadius:14,padding:12,marginBottom:14,textAlign:"center"}}>
        {url?<img src={url} alt="poster" style={{maxWidth:"100%",maxHeight:420,borderRadius:8,border:"1px solid "+BD2}}/>:<div style={{color:T3,fontSize:11,padding:40}}>Generating...</div>}
        <canvas ref={canvasRef} style={{display:"none"}}></canvas>
      </div>

      {/* actions */}
      <div style={{display:"flex",gap:8,marginBottom:10}}>
        <button onClick={generate} style={{flex:1,background:CARD,border:"1px solid "+BD,borderRadius:11,padding:"12px",color:T1,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Regenerate</button>
        <button onClick={shareImg} style={{flex:1,background:"linear-gradient(135deg,#2563EB,#1D4ED8)",border:"none",borderRadius:11,padding:"12px",color:"#fff",fontSize:12,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>Share</button>
      </div>
      <div style={{display:"flex",gap:8}}>
        <button onClick={download} style={{flex:1,background:CARD,border:"1px solid "+BD,borderRadius:11,padding:"11px",color:T2,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Download PNG</button>
        <button onClick={copyImg} style={{flex:1,background:CARD,border:"1px solid "+BD,borderRadius:11,padding:"11px",color:T2,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Copy Image</button>
      </div>

      <div style={{background:"rgba(249,115,22,0.06)",border:"1px solid rgba(249,115,22,0.15)",borderRadius:10,padding:11,marginTop:14}}>
        <div style={{fontSize:8.5,color:WARN,lineHeight:1.5}}>Educational Market Observation Only. Not Investment Advice. Posters use observed market data for learning and sharing purposes.</div>
      </div>
    </div>
  );
}
