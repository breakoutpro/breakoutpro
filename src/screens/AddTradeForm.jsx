...
export default function AddTradeForm(props){
  ...
  var BD = theme.c.border, BLUE = theme.c.blue, T2 = theme.c.text2; T1=theme.c.text1; CARD=theme.c.card;
  ...
  return (
    <div style={{padding:16}}>
      ...
      <div style={{fontSize:16,fontWeight:800,color:T1,marginBottom:16}}>Add Trade</div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:12}}>
        <button onClick={function(){setSide("Buy");}} style={{flex:1,background:side=="Buy"?"#22C55E":"transparent",border:"1px solid "+(side=="Buy"?"#22C55E":BD),borderRadius:10,padding:12,color:side=="Buy"?"#05130a":T2,fontWeight:800,fontSize:12,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>Buy</button>
        <button onClick={function(){setSide("Sell");}} style={{flex:1,background:side=="Sell"?"#EF4444":"transparent",border:"1px solid "+(side=="Sell"?"#EF4444":BD),borderRadius:10,padding:12,color:side=="Sell"?"#fff":T2,fontWeight:800,fontSize:12,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>Sell</button>
      </div>
      ...
      <button onClick={submit} style={{width:"100%",background:BLUE,border:"none",borderRadius:12,padding:"12px 24px",color:"#fff",fontSize:14,fontWeight:800,cursor:"pointer",fontFamily:"inherit",marginTop:8,minHeight:44}}>Save Trade</button>
    </div>
  );
}
