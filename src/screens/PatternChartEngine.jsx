...
var UP="#22C55E",DOWN="#EF4444",WICK="#5B6472";
...
export default function PatternChartEngine(props){
  ...
  var GRID = theme.c.border2, T2 = theme.c.text2, WICK = theme.c.text3; UP=theme.c.up;
  ...
  return (
    <div>
      <div style={{background:CARD,border:"1px solid "+GRID,borderRadius:14,padding:"12px 8px 8px",overflow:"hidden"}}>
        <svg width="100%" viewBox={"0 0 "+W+" "+H} style={{display:"block"}}>
          ...
          {lines.map(function(ln,i){
            ...
            return (
              <g key={i}>
                <line x1={x1} y1={y1} x2={x2} y2={y2} stroke={ln.color||BLUE} strokeWidth="1.4" strokeDasharray={ln.dash||"5 4"} opacity="0.9"/>
                {ln.bandLabel?<text x={x1+6} y={y2-4} fontSize="7" fill={ln.color||BLUE} textAnchor="start" fontWeight="700">{ln.bandLabel}</text>:null}
                {ln.priceTag?(
                  <g>
                    <rect x={x2-tagW} y={y2-tagH/2} width={tagW} height={tagH} rx="2" fill={ln.color||BLUE}/>
                    <text x={x2-tagW/2} y={y2+3} fontSize="7" fill="#fff" textAnchor="middle" fontWeight="700">{ln.priceTag}</text>
                  </g>
                ):(ln.label?<text x={x2-2} y={y2-4} fontSize="7" fill={ln.color||BLUE} textAnchor="end">{ln.label}</text>:null)}
              </g>
            );
          })}
          ...
          {progress>=0.85?markers.map(function(m,i){
            ...
            var col=m.type=="sl"?DOWN:m.type=="target"?UP:BLUE;
            return (
              <g key={i}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,marginTop:12}}>
        ...
        {[1,2].map(function(sp){
          var act=speed==sp;
          return <button key={sp} onClick={function(){setSpeed(sp);}} style={{background:act?BLUE:"rgba(255,255,255,0.05)",border:"1px solid "+(act?BLUE:GRID),borderRadius:8,padding:"8px 12px",color:act?"#04060D":T2,fontSize:12,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>{sp}x</button>;
        })}
      </div>

      {/* progress bar */}
      <div style={{height:3,background:"rgba(255,255,255,0.06)",borderRadius:2,marginTop:8}}>
        <div style={{height:3,width:(progress*100)+"%",background:BLUE,borderRadius:2,transition:"width 0.1s"}}></div>
      </div>
    </div>
  );
}
