...
export default function StocksBuddy(){
  ...
  function getSess(){try{var s=localStorage.getItem("bp_sess");return s?JSON.parse(s):{};}catch(e){return{};}}
  var _sess=getSess();
  ...
  async function sendGemini(userMsg){
    ...
    try{
      var resp=await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key="+GEMINI_KEY,{
        method:"POST",
      ...
      if(data.candidates&&data.candidates[0]&&data.candidates[0].content&&data.candidates[0].content.parts){
        ...
        if(errMsg.includes("quota")||errMsg.includes("RESOURCE_EXHAUSTED")){
          aiText="⚠️ Gemini free tier limit reached for today.\n\nFree tier allows 15 requests/minute and 1500/day.\n\nPlease try again after a few minutes, or upgrade your Gemini API plan at:\naistudio.google.com\n\nEducational only. Not investment advice.";
        } else if(errMsg.includes("API_KEY")||errMsg.includes("invalid")){
          aiText="❌ Invalid Gemini API key.\n\nPlease check VITE_GEMINI_KEY in Vercel Settings → Environment Variables.\n\nGet free key at: aistudio.google.com/app/apikey";
        } else {
          aiText="API Error: "+errMsg+"\n\nPlease try again.";
        }
      } else {
