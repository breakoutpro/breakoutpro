...
// Colors now come from the theme via useTheme() inside each component.
...
export default function Guardian(props){
  var t=useTheme().c;
  var BG=t.bg,CARD=t.card,CARD2=t.card2,BD=t.border,BD2=t.border2;
  var UP=t.up,DOWN=t.down,BLUE=t.blue,CYAN="#60A5FA",GOLD=t.gold,WARN=t.warn;
  var T1=t.text1,T2=t.text2,T3=t.text3;
...
function ObsCard(props){
  ...
  var t=th.c;
  var dev=th.device;
  var big=(dev=="laptop"||dev=="desktop"||dev=="ultra");
  var mid=(dev=="tablet");
  var BG=t.bg,CARD=t.card,CARD2=t.card2,BD=t.border,BD2=t.border2;
  var UP=t.up,DOWN=t.down,BLUE=t.blue,CYAN="#60A5FA",GOLD=t.gold,WARN=t.warn;
  var T1=t.text1,T2=t.text2,T3=t.text3;
  ...
  return (
    <div onClick={props.onOpen} style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:"11px 12px",marginBottom:8,cursor:"pointer"}}>
      ...
      <div style={{display:"flex",gap:big?12:mid?9:7,marginTop:big?13:11,paddingTop:big?13:11,borderTop:"1px solid "+BD2,overflowX:"auto"}}>
        {mtf.map(function(m,i){
          var dc=m.dir>0?UP:m.dir<0?DOWN:T3;
          var ar=m.dir>0?"&#9650;":m.dir<0?"&#9660;":"&#9644;";
          var tfSize=big?11:mid?10:9;
          var arSize=big?10:mid?9:8;
          var lblSize=big?9.5:mid?8.5:8;
          var lblMax=big?60:mid?48:40;
          var minW=m.label?(big?64:mid?52:44):(big?38:mid?32:28);
          return (
            <div key={i} style={{flexShrink:0,textAlign:"center",minWidth:minW}}>
              <div style={{fontSize:tfSize,color:T2,fontWeight:800,letterSpacing:0.3}}>{m.tf}</div>
              <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:3,marginTop:big?5:3}}>
                <span style={{fontSize:arSize,color:dc}} dangerouslySetInnerHTML={{__html:ar}}/>
                {m.label?<span style={{fontSize:lblSize,color:dc,fontWeight:700,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:lblMax}}>{m.label.split(" ")[0]}</span>:null}
              </div>
            </div>
          );
        })}
