...
export default function IPOCenterPro(props){
  ...
  STATUS_COLOR["Upcoming"]=BLUE; STATUS_COLOR["Open"]=UP; STATUS_COLOR["Closed"]=WARN; STATUS_COLOR["Listed"]=T2;
        ...
        <div style={{padding:14}}>
          ...
          <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:16,marginBottom:14}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
              ...
              <span onClick={function(){bookmarks.toggleBookmark(ipo.id);}} style={{fontSize:20,color:saved?WARN:T3,cursor:"pointer",minWidth:44,minHeight:44,display:"flex",alignItems:"center",justifyContent:"center"}}>{saved?"\u2605":"\u2606"}</span>
            </div>
            <span style={{fontSize:12,fontWeight:700,color:STATUS_COLOR[ipo.status],background:STATUS_COLOR[ipo.status]+"18",border:"1px solid "+STATUS_COLOR[ipo.status]+"40",borderRadius:6,padding:"3px 9px"}}>{ipo.status}</span>
          ...
          <div style={{background:"rgba(212,175,55,0.06)",border:"1px solid rgba(212,175,55,0.15)",borderRadius:10,padding:11}}>
            <div style={{fontSize:12,color:WARN,lineHeight:1.5}}>Illustrative educational entry, not a confirmed live IPO status. Breakout Pro has no real IPO data feed. Never a listing-gain prediction.</div>
          </div>
        </div>
      </div>
    );
    ...
    <div style={{background:"#050505",minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
      ...
      <div style={{padding:14}}>
        <div style={{background:"rgba(212,175,55,0.06)",border:"1px solid rgba(212,175,55,0.15)",borderRadius:10,padding:11,marginBottom:14}}>
          <div style={{fontSize:12,color:WARN,lineHeight:1.5}}>Breakout Pro has no live IPO data feed connected. All entries below are illustrative examples for learning purposes, not a confirmed schedule. Never a listing-gain prediction, never investment advice.</div>
        </div>

        <input value={search} onChange={function(e){setSearch(e.target.value);}} placeholder="Search IPO..." style={{width:"100%",background:CARD,border:"1px solid "+BD,borderRadius:9,padding:"10px 12px",color:T1,fontSize:14,fontFamily:"inherit",boxSizing:"border-box",marginBottom:12}}/>
        ...
        <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10}}>IPO TIMELINE ({sorted.length})</div>
        {sorted.length==0 ? (
          <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:20,textAlign:"center",color:T3,fontSize:12}}>No IPOs match your filters.</div>
        ) : sorted.map(function(ipo){
          var saved = bookmarks.isBookmarked(ipo.id);
          return (
            <div key={ipo.id} onClick={function(){setDetailId(ipo.id);}} style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,marginBottom:10,cursor:"pointer"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                ...
                <span onClick={function(e2){ e2.stopPropagation(); bookmarks.toggleBookmark(ipo.id); }} style={{fontSize:18,color:saved?WARN:T3,minWidth:44,minHeight:44,display:"flex",alignItems:"center",justifyContent:"center"}}>{saved?"\u2605":"\u2606"}</span>
              </div>
              <span style={{fontSize:12,fontWeight:700,color:STATUS_COLOR[ipo.status],background:STATUS_COLOR[ipo.status]+"18",border:"1px solid "+STATUS_COLOR[ipo.status]+"40",borderRadius:6,padding:"3px 9px",marginTop:8,display:"inline-block"}}>{ipo.status}</span>
            </div>
          );
        })}
