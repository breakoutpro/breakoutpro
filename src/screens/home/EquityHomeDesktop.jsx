    ...
    <div style={{background:BG,minHeight:"100vh",fontFamily:"'Inter',Arial,sans-serif",color:T1,overflowX:"hidden",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,display:"flex",minHeight:0,width:"100%"}}>
        ...
        <div style={{flex:1,minWidth:0,padding:"8px 12px",overflowY:"auto"}}>
          ...
          {/* MARKET STRUCTURE - card 2 of 3. Combines Range Intelligence
              (real data, from api/market-mood-data.js's indices.NIFTY - the
              most recent daily candle IS today's session, so high/low are
              genuinely today's real range) with Options Intelligence
              (DEMO only - confirmed via full repository audit that no real
              options-chain provider exists anywhere: dhan.js's real
              getOptionChain() is commented out pending a paid subscription,
              OptionsIntelData.jsx's own header states "Mock now"). The
              options half is explicitly, prominently labeled DEMO per this
              round's instruction - never presented as live. Internal
              2-column split so both fit within the same shared card height
              as the other 3 primary cards, rather than needing double the
              height if stacked. */}
          {(function(){
              ...
              rangeContent = (
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
                    <span style={{fontSize:tierCfg.label,fontWeight:800,color:T1}}>Range</span>
            ...
            return (
              <div onClick={function(){setTab("rangeintel");}} style={{background:CARD2+"cc",backdropFilter:"blur(12px)",border:"1px solid "+BD,borderRadius:6,padding:"8px 12px",boxSizing:"border-box",cursor:"pointer",overflowY:"auto",display:"flex",flexDirection:"column"}}>
                <div style={{fontSize:tierCfg.widgetTitle,fontWeight:800,color:T1,marginBottom:6}}>Market Structure</div>
                <div style={{display:"flex",gap:10,flex:1,minHeight:0}}>
                  {rangeContent}
                  <div style={{width:1,background:BD,flexShrink:0}}></div>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
                      <span style={{fontSize:tierCfg.label,fontWeight:800,color:T1}}>Options</span>
                    ...
                    <div style={{fontSize:9,color:T3,marginBottom:2}}>PCR <span style={{color:T1,fontWeight:700}}>1.02</span></div>
                    <div style={{fontSize:9,color:T3,marginBottom:2}}>Max Pain <span style={{color:T1,fontWeight:700}}>24,600</span></div>
          ...
          {/* MARKET ALERTS - card 3 of 3. No verified alert-generation
              source exists anywhere in this codebase (same "Latest Alerts"
              provider status confirmed in earlier rounds) - shown as an
              honest unavailable state, not fabricated. */}
