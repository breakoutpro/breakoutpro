...
function MoreScreen(props) {
  ...
  var sections = [
    ...
    {title:"Account & Settings", col:GOLD, items:[
      ...
      {id:"ipo",        label:"Upcoming IPOs",       sub:"GMP, AI rating, lot size, dates",    col:GOLD     },
...
export default function App() {
  ...
  function renderScreen(){
    return (
      ...
      <Suspense fallback={<Loader/>}>
        ...
        {tab=="strategy" ? <StrategyBuilder setTab={setTab} onBack={function(){setTab("home");}} /> : null}
  ...
  var isDesktop = winW>=1024;
  var isTablet = winW>=768 && winW<1024;
  var centerW = isDesktop?"100%":(isTablet?"100%":430);

  // The mobile app column (unchanged on mobile).
  var appColumn = (
    <div style={{position:"relative",width:"100%",maxWidth:centerW,margin:"0 auto",height:isDesktop?"100%":"100vh",overflow:"hidden",fontFamily:"Inter,Arial,sans-serif",background:"#F8F9FA"}}>
  ...
  // DESKTOP (1024px+): premium 3-column dashboard around the app column.
  return (
    <div style={{width:"100%",minHeight:"100vh",background:"#050505",display:"flex",justifyContent:"center",fontFamily:"Inter,Arial,sans-serif"}}>
      <div style={{width:"100%",maxWidth:1600,display:"flex",gap:24,padding:24,boxSizing:"border-box",height:"100vh"}}>
        <DesktopLeftSidebar tab={tab} setTab={goTo} user={user} onLogout={logout}/>
        <div style={{flex:1,minWidth:0,height:"calc(100vh - 48px)",overflow:"hidden",borderRadius:20,border:"1px solid #1B2330",boxShadow:"0 8px 40px rgba(0,0,0,0.5)",background:"#F8F9FA"}}>
          <div style={{zoom:1.3,width:"100%",height:"100%"}}>
            {appColumn}
          </div>
        </div>
        <DesktopRightSidebar setTab={goTo} liveNews={LIVE_NEWS}/>
      </div>
    </div>
  );
}
...
function DesktopLeftSidebar(props){
  ...
  return (
    <div style={{width:280,flexShrink:0,height:"calc(100vh - 48px)",position:"sticky",top:24,background:BG,border:"1px solid "+BD,borderRadius:20,padding:18,boxSizing:"border-box",display:"flex",flexDirection:"column"}}>
...
function DesktopRightSidebar(props){
  ...
  return (
    <div style={{width:320,flexShrink:0,height:"calc(100vh - 48px)",position:"sticky",top:24,overflowY:"auto",paddingRight:2}}>
      ...
      <Card title="AI Alerts" action="Guardian" onAction={function(){props.setTab("guardian");}}>
        <div style={{fontSize:10.5,color:T2,lineHeight:1.7}}>
          <div style={{marginBottom:6}}><span style={{color:UP,fontWeight:700}}>RELIANCE</span> Higher High detected</div>
          <div style={{marginBottom:6}}><span style={{color:UP,fontWeight:700}}>TCS</span> Bullish Engulfing on 1H</div>
          <div><span style={{color:DOWN,fontWeight:700}}>SBIN</span> Below VWAP observed</div>
        </div>
      </Card>
