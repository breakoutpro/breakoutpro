...
export function useHomeData(){
  ...
  // Deterministic seeding (seedFromSymbol hashes only the symbol string)
  // meant Mobile and Desktop already produced identical values even when
  // duplicated, but the duplication itself was real - this removes it.
  var VERDICT_CATEGORY = {Healthy:"Strong Breakout", Weakening:"Momentum Building", Failed:"Watching", None:"Fresh Setup"};
  var breakoutCardsPool = useMemo(function(){
    ...
    return pool.sort(function(a,b){ return b.confidence-a.confidence; });
  }, []);
  ...
  function getKeyLevels(symIdx, candleCount){
    ...
    var realPrice = (row && row.live) ? row.ltp : fallback;
    ...
    zones.priceIsLive = !!(row && row.live);
