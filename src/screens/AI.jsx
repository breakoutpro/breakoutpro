...
export default function AIScreen() {
  ...
  return (
    <div style={{ background: "#0B0B0B", minHeight: "100%", display: "flex", flexDirection: "column", paddingBottom:40 }}>
      <div style={{ background: "#111", borderBottom: "1px solid #1E1E1E", padding: "12px 16px" }}>
      ...
      <div style={{ background: "#0F0A00", borderBottom: "1px solid #2A1E00", padding: "4px 16px" }}>
        ...
        {msgs.length == 0
          ? <div style={{ textAlign: "center", paddingTop:24 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap:8, marginTop: 12 }}>
                {chips.map(function(q) { return <button key={q} onClick={function() { send(q); }} style={{ background: "#161616", border: "1px solid #222", borderRadius: 10, padding: "8px", cursor: "pointer", textAlign: "left", fontFamily: "inherit", fontSize: 8, color: "#ccc" }}>{q}</button>; })}
              </div>
            </div>
          : null
        }
        {msgs.map(function(m, i) {
          ...
          return (
            <div key={i} style={{ marginBottom: 12, display: "flex", flexDirection: "column", alignItems: isu ? "flex-end" : "flex-start" }}>
              <div style={{ maxWidth: "88%", background: isu ? "#1E3A5F" : "#161616", border: isu ? "1px solid #1E4080" : "1px solid #222", borderRadius: isu ? "16px 16px 4px 16px" : "4px 16px 16px 16px", padding: "12px 12px" }}>
                <div style={{ fontSize: 11, color: "#e8e8e8", lineHeight: 1.75, whiteSpace: "pre-wrap" }}>{m.text}</div>
                <div style={{ fontSize: 7, color: "rgba(255,255,255,0.3)", marginTop:4, textAlign: "right" }}>{m.time}</div>
              </div>
            </div>
          );
        })}
        {loading ? <div style={{ display: "flex", marginBottom: 12 }}><div style={{ background: "#161616", border: "1px solid #222", borderRadius: "4px 16px 16px 16px", padding: "12px 16px" }}><div style={{ display: "flex", gap: 4 }}>{[0, 1, 2].map(function(i) { return <div key={i} style={{ width: 7, height: 7, borderRadius: "50%", background: theme.c.blue, opacity: 0.8 }}></div>; })}</div></div></div> : null}
        <div ref={endRef}></div>
      </div>
      <div style={{ borderTop: "1px solid #1A1A1A", padding: "8px 12px 12px", background: "#0F0F0F" }}>
        <div style={{ display: "flex", gap:4, overflowX: "auto", marginBottom: 8, paddingBottom:4 }}>
          {chips.map(function(q) { return <button key={q} disabled={loading} onClick={function() { send(q); }} style={{ background: "#161616", border: "1px solid #222", borderRadius: 20, padding: "4px 12px", color: loading ? "#333" : theme.c.blue, fontSize: 8, cursor: loading ? "not-allowed" : "pointer", whiteSpace: "nowrap", flexShrink: 0, fontFamily: "inherit", fontWeight: 600 }}>{q}</button>; })}
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
          <textarea value={input} onChange={function(e) { setInput(e.target.value); }} onKeyDown={function(e) { if (e.key == "Enter" && !e.shiftKey) { e.preventDefault(); send(input); } }} placeholder="Ask about any market concept..." rows={1} disabled={loading} style={{ flex: 1, background: "#161616", border: "1px solid #222", borderRadius: 11, padding: "12px 12px", color: "#fff", fontSize: 11, fontFamily: "inherit", outline: "none", resize: "none", lineHeight: 1.5, maxHeight: 80, overflowY: "auto", opacity: loading ? 0.6 : 1 }} />
          <button onClick={function() { send(input); }} disabled={!input.trim() || loading} style={{ background: (!input.trim() || loading) ? "#1A1A1A" : "#3B82F6", border: "none", borderRadius: 11, width: 42, height: 42, display: "flex", alignItems: "center", justifyContent: "center", cursor: (!input.trim() || loading) ? "not-allowed" : "pointer", fontSize: 14, color: "#fff", flexShrink: 0 }}>&#10148;</button>
        </div>
      </div>
    </div>
  );
