...
export default function MarketsScreen(props) {
  ...
  function fetchLive(){
    ...
    fetch("/api/nse?index=NIFTY500")
      .then(function(r){return r.json();})
      .then(function(data){
        if(data && data.error){
          console.log("NSE error:", data.error);
        ...
        if(data && data.data && Array.isArray(data.data)){
          ...
          data.data.forEach(function(q){
            if(!q.symbol || !q.lastPrice) return;
            var chg = q.change || (q.lastPrice - (q.previousClose||q.lastPrice));
            combinedMap[q.symbol] = {
              ltp: q.lastPrice,
              chg: chg,
              chgPct: q.pChange!=null ? Math.abs(parseFloat(q.pChange)).toFixed(2) : "0.00",
              up: chg>=0,
              high: q.dayHigh||0,
              low: q.dayLow||0,
              vol: q.totalTradedVolume||0,
            };
            anyLive = true;
          });
          if(anyLive){
            setLiveData(Object.assign({},combinedMap));
            setLiveStatus(marketHoursNow?"live":"closed");
          } else {
            setLiveStatus(function(prev){ return prev=="live"||prev=="closed" ? prev : "unavailable"; });
          }
        } else {
