    ...
    <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:80}}>
      ...
      <div style={{background:CB,borderBottom:"1px solid "+BD,display:"flex",gap:0}}>
        {[["chart","Chart"],["stocks","Stocks"],["zones","Zones"],["pattern","Pattern"],["analysis","Analysis"]].map(function(t){
          var act=viewTab==t[0];
          return <button key={t[0]} onClick={function(){setViewTab(t[0]);}} style={{flex:1,background:"none",border:"none",borderBottom:"2px solid "+(act?BLUE:"transparent"),padding:"10px 4px",color:act?BLUE:T2,fontSize:9,fontWeight:act?700:400,cursor:"pointer",fontFamily:"inherit"}}>{t[1]}</button>;
        })}
      </div>
      <div style={{padding:"12px 14px 0"}}>
        ...
        {viewTab=="stocks"?(
          <div>
            <div style={{fontSize:9,color:T2,marginBottom:10}}>{stockList.length} stocks in {idx.label||"index"}  Tap a stock to view chart &amp; history</div>
            {stockList.length==0?(
              <div style={{background:CB,border:"1px solid "+BD,borderRadius:12,padding:20,textAlign:"center"}}>
                <div style={{fontSize:10,color:T2}}>Constituent list not available for this index yet.</div>
              </div>
            ):(
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {stockList.map(function(s){
                  return (
                    <div key={s.sym} onClick={function(){setSelStock(s);}} style={{background:CB,border:"1px solid "+BD,borderRadius:12,padding:"10px 12px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer"}}>
                      <div>
                        <div style={{fontSize:11,fontWeight:700,color:T1}}>{s.sym}</div>
                        <div style={{fontSize:8,color:T2,marginTop:1}}>{s.name}</div>
                      </div>
                      <div style={{textAlign:"right"}}>
                        <div style={{fontSize:11,fontWeight:700,color:T1}}>{s.ltp.toLocaleString("en-IN",{maximumFractionDigits:2})}</div>
                        <div style={{fontSize:9,fontWeight:700,color:s.up?G2:R}}>{s.up?"+":"-"}{Math.abs(s.chgPct)}%</div>
                      </div>
                    </div>
                  );
                })}
