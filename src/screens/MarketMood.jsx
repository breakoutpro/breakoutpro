...
import {
  ...
  StageTimeline, EvolutionCard, Sparkline, MiniHLCChart, buildMT
      ...
      <div style={{padding:wideLayout?"4px 10px":"16px 24px 32px",maxWidth:1440,margin:"0 auto",width:"100%",boxSizing:"border-box"}}>
        ...
        <div style={{display:wideLayout?"grid":"block",gridTemplateColumns:wideLayout?"1fr 1fr":undefined,gap:gapV,marginBottom:gapV,alignItems:"start"}}>
          <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:10,padding:padV}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
              ...
              <span style={{fontSize:18,fontWeight:900,color:moodColor}}>{mood.label.toUpperCase()}</span>
            </div>
            <div style={{fontSize:12,color:MT.T2}}>{mood.stage} &nbsp;\u00b7&nbsp; {mood.confidence} Confidence &nbsp;\u00b7&nbsp; AI Score: {mood.score}/100</div>
        ...
        <div style={{display:wideLayout?"grid":"block",gridTemplateColumns:wideLayout?"1fr 3fr":undefined,gap:gapV,marginBottom:gapV,alignItems:"start"}}>
          ...
          {(function(){
            ...
            if(!wideLayout && !anyReal){
              // Mobile, nothing real yet: one compact row instead of 4 stacked unavailable cards.
              return (
                <div style={{background:MT.CARD2,border:"1px solid "+MT.BD,borderRadius:12,padding:padV,boxSizing:"border-box"}}>
                  <div style={{fontSize:11,fontWeight:800,color:MT.T2,marginBottom:2}}>MARKET INTELLIGENCE</div>
        ...
        <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:14,padding:padV,marginBottom:gapV}}>
          <div style={{fontSize:12,fontWeight:800,color:MT.T1,marginBottom:6}}>MARKET SNAPSHOT</div>
          <div style={{display:wideLayout?"grid":"block",gridTemplateColumns:wideLayout?"repeat(5, 1fr)":undefined,gap:wideLayout?8:8}}>
              ...
              if(key==="GIFTNIFTY"){
                return (
                  <div key={i} style={{marginBottom:wideLayout?0:8}}>
                    <div style={{fontSize:11,fontWeight:700,color:MT.T2,marginBottom:4}}>GIFT NIFTY</div>
                    <div style={{fontSize:12,color:MT.T2}}>Unavailable</div>
                ...
                <div key={i} style={{marginBottom:wideLayout?0:8}}>
                  ...
                  {row.available ? (
                    <div>
                      ...
                      <div style={{fontSize:12,fontWeight:700,color:row.up==null?MT.T2:(row.up?MT.SENT_GREEN:MT.SENT_RED)}}>{row.up==null?"":(row.up?"\u25b2 ":"\u25bc ")}{row.absChange ? row.absChange+" " : ""}({row.chgPct})</div>
                      {row.dayLow && row.dayHigh ? <div style={{fontSize:10,color:MT.T3,marginTop:2}}>Low: {row.dayLow} &nbsp; High: {row.dayHigh}</div> : null}
                      {!usingDemo && data && data.indexHistory && data.indexHistory[key] && data.indexHistory[key].candles && data.indexHistory[key].candles.length>=2 ? (
                        <div style={{marginTop:4}}><MiniHLCChart candles={data.indexHistory[key].candles} height={28}/></div>
          ...
          <div style={{marginBottom:wideLayout?0:8}}>
            ...
            <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:10,padding:padV}}>
              {mood && mood.score!=null ? (
                <div style={{fontSize:14,fontWeight:800,color:mood.label.indexOf("Bullish")>=0?MT.SENT_GREEN:(mood.label.indexOf("Bearish")>=0?MT.SENT_RED:MT.SENT_YELLOW)}}>{usingDemo ? DEMO_TRADE_BIAS.label : mood.stage}</div>
