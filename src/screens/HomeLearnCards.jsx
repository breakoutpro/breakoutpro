...
export default function HomeLearnCards(props){
  ...
  var CARD = theme.c.card, CARD2 = theme.c.card2;
  ...
  return (
    <div style={{padding:"6px 0 0"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 14px 9px"}}>
        <span style={{fontSize:13,fontWeight:800,color:T1}}>Learn &amp; Invest</span>
        <button onClick={function(){setTab("learn");}} style={{background:"none",border:"none",color:BLUE,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>View All &#8594;</button>
      </div>
      <div style={{display:"flex",gap:8,overflowX:"auto",padding:"0 14px 4px",WebkitOverflowScrolling:"touch"}}>
        {LEARN_CARDS.map(function(c){
          return (
            <div key={c.id} onClick={function(){onTopic(c.id);}} style={{background:CARD,border:"1px solid "+BD,borderRadius:11,padding:"11px 12px",minWidth:104,flexShrink:0,cursor:"pointer"}}>
              <div style={{width:30,height:30,borderRadius:8,background:CARD2,border:"1px solid "+BD,display:"flex",alignItems:"center",justifyContent:"center",marginBottom:8}}>
                <span style={{fontSize:14,color:BLUE}} dangerouslySetInnerHTML={{__html:c.ic}}/>
              </div>
              <div style={{fontSize:10,fontWeight:700,color:T1,lineHeight:1.25}}>{c.title}</div>
              <div style={{fontSize:8.5,color:BLUE,fontWeight:600,marginTop:5}}>Learn &#8594;</div>
            </div>
          );
        })}
      </div>
