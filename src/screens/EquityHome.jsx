...
export default function EquityHome(props){
  ...
  var UP=theme.c.up, DOWN=theme.c.down, GOLD=theme.c.gold;
  ...
  return (
    <div style={{background:BG,minHeight:"100vh",fontFamily:"'Inter',Arial,sans-serif",paddingBottom:84,color:T1}}>
      ...
      <div style={{background:BG,borderBottom:"1px solid "+BD,padding:"12px 14px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:14,flexShrink:0}}>
            <button onClick={function(){setTab("search");}} style={{background:"none",border:"none",padding:3,cursor:"pointer",minWidth:44,minHeight:44}}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={T2} strokeWidth="2.2" strokeLinecap="round">
                <circle cx="11" cy="11" r="8"/>
                <line x1="21" y1="21" x2="16.65" y2="16.65"/>
              </svg>
            </button>
            <button onClick={function(){setTab("alerts");}} style={{background:"none",border:"none",padding:3,cursor:"pointer",position:"relative",minWidth:44,minHeight:44}}>
            ...
            <button onClick={function(){setTab("profile");}} style={{background:"none",border:"none",padding:3,cursor:"pointer",minWidth:44,minHeight:44}}>
      ...
      <div onClick={function(){setSelArticle(JUSTIN[tickerIdx]);}} style={{background:theme.c.card2,borderBottom:"1px solid "+BD,display:"flex",alignItems:"stretch",overflow:"hidden",cursor:"pointer"}}>
        ...
        <span style={{fontSize:14,color:T3,display:"flex",alignItems:"center",paddingRight:10,flexShrink:0}}>&#8250;</span>
      </div>

      {/* STAGE 1 TABLET REFLOW - md only, 2 columns. xs/sm/lg/xl/xxl/tv/tv4k stay 1 column (unchanged behavior). */}
      <div style={{display:"grid",gridTemplateColumns:responsive.isTablet?"1fr 1fr":"1fr",gap:"18px 16px"}}>
        ...
        {/* QUICK ACTIONS */}
        <div style={{gridColumn:"1 / -1",display:"flex",gap:8,padding:"12px 14px 0"}}>
          {[["Scanner","scan","&#128269;"],["Watchlist","watchlist","&#11088;"],["Alerts","alerts","&#128276;"],["News","news","&#128240;"]].map(function(q){
            return (
              <button key={q[1]} onClick={function(){setTab(q[1]);}} style={{flex:1,background:CARD,border:"1px solid "+BD,borderRadius:12,padding:"11px 4px",display:"flex",flexDirection:"column",alignItems:"center",gap:5,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>
            ...
            return (
              <div style={{display:"flex",gap:10,overflowX:"auto",padding:"0 14px 4px",WebkitOverflowScrolling:"touch"}}>
                {rows.map(function(r){
                  ...
                  return (
                    <div key={r.key} style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:"12px",flexShrink:0,minWidth:120}}>
                      <div style={{fontSize:12,color:T2,marginBottom:5,fontWeight:600}}>{r.label}</div>
      ...
      <div style={{marginTop:18,padding:"0 14px"}}>
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:14,padding:14}}>
          ...
          {!wl.hasStoredWatchlist ? (
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
              {wl.list.slice(0,4).map(function(sym){
                return <span key={sym} style={{fontSize:12,fontWeight:700,color:T1,background:theme.c.card2,border:"1px solid "+BD,borderRadius:8,padding:"7px 11px"}}>{sym}</span>;
              })}
            </div>
          )}
        </div>
      ...
      <div style={{marginTop:18,padding:"0 14px"}}>
        ...
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,overflow:"hidden"}}>
          {JUSTIN.slice(0,5).map(function(n,i){
            ...
            return (
              <div key={n.id} onClick={function(){setSelArticle(n);}} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",borderBottom:i<4?"1px solid "+BD:"none",cursor:"pointer"}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:12,color:T3,marginTop:3}}>{n.source}  &#8226;  {n.time}</div>
                </div>
              </div>
            );
          })}
...
function MarketBadge(){
  ...
  var BD = theme.c.border, BLUE = theme.c.blue, GOLD = theme.c.gold;
