    ...
    <div style={{background:BG,minHeight:"100vh",fontFamily:"'Inter',Arial,sans-serif",color:T1,overflowX:"hidden",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,display:"flex",minHeight:0,width:"100%"}}>
        ...
        <div style={{flex:1,minWidth:0,padding:"12px 16px",overflowY:"auto"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"240px 1fr",gap:10,marginBottom:10}}>
            <div style={{background:CARD2,border:"1px solid "+BD,borderRadius:10,padding:11}}>
              ...
              {(function(){
                ...
                return (
                  <div>
                    <div style={{fontSize:12,color:T1,lineHeight:1.6,marginBottom:12}}>{symLabel} is {bi.trend.toLowerCase()}, volume is {bi.volumeConfirmed?"confirmed":"average"}, price is {bi.aboveVwap===true?"above":(bi.aboveVwap===false?"below":"near")} VWAP.</div>
                    <div style={{display:"flex",justifyContent:"space-between",padding:"4px 0",fontSize:11}}><span style={{color:T3}}>Risk Level</span><span style={{fontWeight:700,color:riskLevel?riskColor:T3}}>{riskLevel||"N/A"}</span></div>
                    <div style={{display:"flex",justifyContent:"space-between",padding:"4px 0",fontSize:11}}><span style={{color:T3}}>Trading Bias</span><span style={{fontWeight:700,color:biasColor}}>{bi.trend}</span></div>
                    <div style={{display:"flex",justifyContent:"space-between",padding:"4px 0",fontSize:11}}><span style={{color:T3}}>Events</span><span style={{fontWeight:700,color:T1}}>RBI Today</span></div>
                  </div>
                );
              })()}
            </div>
            ...
            <div style={{background:CARD2,border:"1px solid "+BD,borderRadius:10,padding:11}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <span style={{fontSize:12,fontWeight:800,color:T1}}>Breakout Intelligence</span>
                  <ProvenanceBadge type="calculated"/>
                </div>
                <button onClick={function(){setTab("scan");}} style={{background:"none",border:"none",color:BLUE,fontSize:11,fontWeight:700,cursor:"pointer"}}>View All Opportunities &#8594;</button>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(4, 1fr)",gap:10}}>
                {breakoutCards.map(function(card){
                  ...
                  var w=70,h=22;
                  var pts = card.sparkline.map(function(v,i){ return (i/(card.sparkline.length-1))*w+","+(h-((v-min)/range)*(h-3)+1); }).join(" ");
                  return (
                    <div key={card.sym} onClick={function(){data.selectStock(card.sym);}} style={{background:BG,border:"1px solid "+BD,borderRadius:9,padding:10,cursor:"pointer"}}>
                      ...
                      <div style={{fontSize:13,fontWeight:800,color:T1}}>{card.sym}</div>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginTop:2}}>
                        <div>
                          <div style={{fontSize:12,fontWeight:700,color:T1,fontFamily:"monospace"}}>{card.ltp.toLocaleString("en-IN")}</div>
                          <div style={{fontSize:10,fontWeight:700,color:up?UP:DOWN}}>{up?"+":""}{card.chgPct}%</div>
                        </div>
                        <svg width={w} height={h}><polyline points={pts} fill="none" stroke={up?UP:DOWN} strokeWidth="1.3"/></svg>
                      </div>
                      <div style={{borderTop:"1px solid "+BD2,marginTop:6,paddingTop:6,fontSize:9,color:T3}}>
          ...
          {/* Row 2: Scanner Summary (tabbed table) | Key Levels (toggle + real chart) */}
              ...
              <table style={{width:"100%",borderCollapse:"collapse"}}>
                <thead>
                  <tr style={{borderBottom:"1px solid "+BD2}}>
                    <th style={{textAlign:"left",fontSize:10,color:T3,fontWeight:700,padding:"4px 4px"}}>STOCK</th>
                    <th style={{textAlign:"right",fontSize:10,color:T3,fontWeight:700,padding:"4px 4px"}}>PRICE</th>
                    <th style={{textAlign:"right",fontSize:10,color:T3,fontWeight:700,padding:"4px 4px"}}>CHG%</th>
                    <th style={{textAlign:"right",fontSize:10,color:T3,fontWeight:700,padding:"4px 4px"}}>CONFIDENCE</th>
                  </tr>
                </thead>
                <tbody>
          ...
          {/* Quick Actions */}
          <div style={{display:"flex",gap:20,padding:"12px 4px",borderTop:"1px solid "+BD2,flexWrap:"wrap"}}>
            <span style={{fontSize:10,color:T3,fontWeight:700,alignSelf:"center"}}>QUICK ACTIONS</span>
            <button onClick={function(){setTab("scan");}} style={{background:"none",border:"none",color:T2,fontSize:11,fontWeight:600,cursor:"pointer"}}>&#128269; New Scanner</button>
            <button onClick={function(){setTab("optionsintelpro");}} style={{background:"none",border:"none",color:T2,fontSize:11,fontWeight:600,cursor:"pointer"}}>&#128202; Option Chain</button>
            <button onClick={function(){setTab("stockprofile");}} style={{background:"none",border:"none",color:T2,fontSize:11,fontWeight:600,cursor:"pointer"}}>&#128200; Stock Detail</button>
            <button onClick={function(){setTab("alerts");}} style={{background:"none",border:"none",color:T2,fontSize:11,fontWeight:600,cursor:"pointer"}}>&#128276; Add Alert</button>
            <button onClick={function(){setTab("journal");}} style={{background:"none",border:"none",color:T2,fontSize:11,fontWeight:600,cursor:"pointer"}}>&#128221; Trading Journal</button>
          </div>
