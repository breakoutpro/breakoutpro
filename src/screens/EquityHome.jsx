...
import { MarketHeatmapCard, LiveNewsCards, AIBriefingCard, TrendingStocksCard, TopBreakoutsCard, AITradeSetupCard, SmartAlertsCard, ExploreShortcuts, AINewsSummaryCard } from "../components/HomeNewsHeatmap";
    ...
    <div style={{background:BG,minHeight:"100vh",fontFamily:"'Inter',Arial,sans-serif",paddingBottom:84,color:T1}}>
      ...
      <div style={{padding:"18px 18px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{display:"flex",alignItems:"center",gap:9}}>
          <div style={{width:38,height:38,borderRadius:11,background:CARD,border:"1px solid "+BD,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
            <svg width="21" height="21" viewBox="0 0 24 24" fill="none">
              <line x1="6" y1="9" x2="6" y2="20" stroke={CYAN} strokeWidth="1.6"/>
              <rect x="3.5" y="9" width="5" height="6" rx="1" fill={CYAN}/>
              <line x1="13" y1="4" x2="13" y2="20" stroke="#fff" strokeWidth="1.6"/>
              <rect x="10.5" y="9" width="5" height="7" rx="1" fill="none" stroke="#fff" strokeWidth="1.6"/>
              <path d="M3 7L9 2L14 5L21 0" stroke={UP} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" fill="none"/>
              <path d="M17 0L21 0L21 4" stroke={UP} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" fill="none"/>
            </svg>
          </div>
          <div style={{fontSize:18,fontWeight:800,letterSpacing:-0.3}}><span style={{color:"#FFFFFF"}}>Breakout</span><span style={{color:CYAN}}>Pro</span></div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <button onClick={function(){setTab("markets");}} style={{background:CARD,border:"1px solid "+BD,borderRadius:10,width:36,height:36,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}>
            <span style={{fontSize:14,color:T2}}>&#128269;</span>
          </button>
          <button style={{background:CARD,border:"1px solid "+BD,borderRadius:10,padding:"0 10px",height:36,display:"flex",alignItems:"center",gap:5,cursor:"pointer",flexShrink:0}}>
      ...
      {/* ===== LIVE TICKER STRIP + VOICE TOGGLE ===== */}
      <div style={{padding:"4px 18px 0",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
        <div style={{display:"flex",alignItems:"center",gap:6,overflow:"hidden",flex:1,minWidth:0}}>
          ...
          <span key={tickerIdx} style={{fontSize:9.5,color:T3,fontWeight:500,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",animation:"ticker-fade 0.4s ease"}}>{TICKER[tickerIdx]}</span>
        </div>
        <button onClick={voiceAlerts.toggleMute} style={{background:CARD,border:"1px solid "+BD,borderRadius:9,width:30,height:30,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}>
          <span style={{fontSize:13}}>{voiceAlerts.muted?"\uD83D\uDD07":"\uD83D\uDD0A"}</span>
        </button>
      </div>

      {voiceAlerts.lastAlert&&(
        <div style={{padding:"8px 18px 0"}}>
          <div style={{background:"rgba(59,130,246,0.08)",border:"1px solid "+BD,borderRadius:10,padding:"7px 11px",display:"flex",alignItems:"center",gap:7}}>
            <span style={{fontSize:11}}>&#127908;</span>
            <span style={{fontSize:9,color:T2,flex:1}}>{voiceAlerts.lastAlert.sentence}</span>
            <span style={{fontSize:8,color:T3,flexShrink:0}}>{voiceAlerts.lastAlert.time}</span>
          </div>
        </div>
      )}
