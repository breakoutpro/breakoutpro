...
export default function EquityHomeDesktop(props){
  ...
  // PRIMARY_CARD_HEIGHT - the ONE number that controls the primary 2x2
  // grid's row height. Both rows use this same value, so all four primary
  // cards (AI Market Mood, Range Intelligence, Options Intelligence,
  // Market News) are always identical in height regardless of how much
  // content each one has. Content that does not fit scrolls inside its
  // own card (overflowY:auto on each card) rather than growing the row -
  // this is what keeps Row 1 and Row 2 the same height instead of
  // AI Market Mood or Market News stretching their row taller than the
  // other. Tune this one value if a tier needs a different compact height.
  var PRIMARY_CARD_HEIGHT = 240;
    ...
    <div style={{background:BG,minHeight:"100vh",fontFamily:"'Inter',Arial,sans-serif",color:T1,overflowX:"hidden",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,display:"flex",minHeight:0,width:"100%"}}>
        ...
        <div style={{flex:1,minWidth:0,padding:"8px 12px",overflowY:"auto"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gridTemplateRows:"repeat(2, "+PRIMARY_CARD_HEIGHT+"px)",gap:8,alignItems:"stretch"}}>
