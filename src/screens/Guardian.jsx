...
import { ALERT_TYPES, alertType, alertCategory, getAlertEdu, getAiInsights, loadObservation, saveObservation, ASSET_SUGGEST, getAlertFeed, NIFTY50, BANKNIFTY, addBasket } from "./GuardianData";
    ...
    <div style={{background:BG,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:90}}>
      <div style={{background:BG,padding:"14px 14px 12px",borderBottom:"1px solid "+BD,position:"sticky",top:0,zIndex:10}}>
        ...
        <div style={{display:"flex",gap:6,marginTop:12,overflowX:"auto",WebkitOverflowScrolling:"touch",paddingBottom:2}}>
          {[["watch","My Watchlist"],["smart","Smart Alerts"],["technical","Technical Alerts"],["options","Options Alerts"],["news","News Alerts"],["insights","AI Insights"],["settings","Alert Settings"]].map(function(t){
            var act=tab==t[0];
      ...
      <div style={{padding:14}}>
        {/* ALERTS FEED */}
        {/* ALERT FEEDS - Smart (all) / Technical / Options / News */}
        {(tab=="smart"||tab=="technical"||tab=="options"||tab=="news")?(
          <div>
            {watch.length==0?(
              ...
              return list.length?list.map(function(f,i){
                ...
                <div style={{background:CARD,border:"1px solid "+BD,borderRadius:13,padding:20,textAlign:"center"}}>
                  <div style={{fontSize:11,color:T2}}>No alerts in this category right now. Enable more alert types in Alert Settings.</div>
                </div>
              );
            })()}
            <Disclaimer/>
