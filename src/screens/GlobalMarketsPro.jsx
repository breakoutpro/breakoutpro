  ...
  return (
    <div style={{background:CARD,border:"1px solid "+(c==null?BD:col),borderRadius:10,padding:12,minHeight:64}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div style={{fontSize:12,fontWeight:700,color:T1,marginBottom:6,flex:1}}>{props.item.name}</div>
        <span onClick={function(e){ e.stopPropagation(); props.onBookmark(); }} style={{fontSize:14,color:props.saved?WARN:T3,cursor:"pointer",minWidth:32,minHeight:32,display:"flex",alignItems:"center",justifyContent:"center"}}>{props.saved?"\u2605":"\u2606"}</span>
      </div>
      <div style={{fontSize:14,fontWeight:800,color:col}}>{c==null?"--":((c>=0?"+":"")+c.toFixed(2)+"%")}</div>
      {!props.isReal ? <div style={{fontSize:12,color:WARN,marginTop:2}}>Demo</div> : <div style={{fontSize:12,color:UP,marginTop:2}}>Real</div>}
    </div>
  );
}
  ...
  return (
    <div style={{background:"#050505",minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
      ...
      <div style={{padding:14}}>
        <input value={search} onChange={function(e){setSearch(e.target.value);}} placeholder="Search instrument..." style={{width:"100%",background:CARD,border:"1px solid "+BD,borderRadius:9,padding:"10px 12px",color:T1,fontSize:14,fontFamily:"inherit",boxSizing:"border-box",marginBottom:12}}/>

        <div style={{display:"flex",gap:6,marginBottom:16,flexWrap:"wrap"}}>
          {REGIONS.map(function(r){
            var act=r==region;
            return <button key={r} onClick={function(){setRegion(r);}} style={{background:act?BLUE:"transparent",border:"1px solid "+(act?BLUE:BD),borderRadius:9,padding:"9px 12px",color:act?"#fff":T2,fontSize:12,fontWeight:act?800:600,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>{r}</button>;
          })}
        </div>

        {realStatus=="error" ? (
          <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,textAlign:"center",color:R,fontSize:12,marginBottom:14}}>Real data temporarily unavailable. <button onClick={loadReal} style={{background:"none",border:"1px solid "+BLUE,borderRadius:8,padding:"7px 14px",color:BLUE,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",marginTop:8,minHeight:44}}>Retry</button></div>
        ) : null}

        {(demoTiles.length>0) ? (
          <div style={{background:"rgba(212,175,55,0.08)",border:"1px solid rgba(212,175,55,0.2)",borderRadius:8,padding:"6px 10px",marginBottom:12,display:"inline-block"}}>
            <span style={{fontSize:12,fontWeight:700,color:WARN}}>{DEMO_LABEL.toUpperCase()} (for instruments marked Demo below)</span>
          </div>
        ) : null}
        ...
        <div style={{background:"rgba(249,115,22,0.06)",border:"1px solid rgba(249,115,22,0.15)",borderRadius:10,padding:11,marginTop:6}}>
          <div style={{fontSize:12,color:"#F97316",lineHeight:1.5}}>Educational only. Not investment advice. Never a market-direction prediction.</div>
        </div>
      </div>
    </div>
  );
