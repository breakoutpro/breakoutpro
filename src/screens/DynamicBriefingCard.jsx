...
function IndexLine(props){
  ...
  return (
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0"}}>
      <span style={{fontSize:12.5,fontWeight:700,color:MT.T1}}>{props.label}</span>
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        <span style={{fontSize:13,fontWeight:800,color:MT.T1}}>{row.ltp}</span>
...
export default function DynamicBriefingCard(props){
  ...
  MT.BLUE=theme.c.blue; MT.GREEN=theme.c.up; MT.RED=theme.c.down; MT.GOLD=theme.c.gold;
  var mm = props.mm || {};
  ...
  var freshColor = freshStatus=="LIVE" ? MT.GREEN : (freshStatus=="STALE" ? theme.c.warn : MT.GOLD);
  ...
  if(loadingOrError){
    ...
    body = (
      <div>
        ...
        {mood && mood.label ? (
          <div style={{fontSize:12,color:MT.T2,marginTop:6}}>Market mood is {mood.label}.</div>
        ) : (
    ...
    <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:16,padding:16,minHeight:150,boxSizing:"border-box"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <span style={{fontSize:13.5,fontWeight:800,color:MT.T1}}>{meta.title}</span>
        </div>
