...
import { MiniChart, genSpark, INDICES, COMM_IDX, GLOBAL, EVENTS, NEWS_TICKER, WATCHLIST, GAINERS, LOSERS, LARGECAP, MIDCAP, SECTORS, getSession, SESSION_META } from "./HomeData";
  ...
  return (
    <div style={{background:BG,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:80}}>
      ...
      <div style={{padding:"12px 16px 0"}}>
        ...
        {/* Sector Performance */}
        <div style={{fontSize:14,fontWeight:800,color:T1,letterSpacing:0.3,marginBottom:10}}>Sector Performance</div>
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:18,overflow:"hidden",marginBottom:14}}>
          {SECTORS.map(function(s,i){
            var barW = Math.min(100,Math.abs(s.pct)*20);
            return (
              <div key={s.name} style={{padding:"10px 14px",borderBottom:i<SECTORS.length-1?"1px solid rgba(30,58,95,0.4)":"none",cursor:"pointer"}} onClick={function(){setTab("markets");}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                  <div>
                    <span style={{fontSize:12,fontWeight:700,color:T1}}>{s.name}</span>
                    <span style={{fontSize:9,color:T3,marginLeft:6}}>{s.stocks?s.stocks.slice(0,2).join(", "):"--"}</span>
                  </div>
                  <span style={{fontSize:12,fontWeight:800,color:s.up?UP:DOWN}}>{s.up?"+":""}{s.pct}%</span>
                </div>
                <div style={{height:3,background:"rgba(255,255,255,0.05)",borderRadius:2}}>
                  <div style={{height:3,background:s.up?UP:DOWN,borderRadius:2,width:barW+"%",opacity:0.7}}></div>
                </div>
              </div>
            );
          })}
