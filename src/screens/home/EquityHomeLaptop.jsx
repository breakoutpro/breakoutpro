      ...
      <div style={{flex:1,display:"flex",minHeight:0,width:"100%"}}>
        ...
        <div style={{flex:1,minWidth:0,padding:"12px 16px",overflowY:"auto"}}>
          ...
          {(function(){
            ...
            var state, watchText;
            if(posPct>=90){ state="Breakout Watch"; watchText="Price approaching today's high. A sustained move above it would indicate potential range expansion."; }
            else if(posPct<=10){ state="Breakdown Watch"; watchText="Price approaching today's low. A sustained move below it would indicate potential range expansion downward."; }
            else if(posPct>=60){ state="Upper Range"; watchText="Price trading in the upper half of today's range."; }
            else if(posPct<=40){ state="Lower Range"; watchText="Price trading in the lower half of today's range."; }
            else { state="Range"; watchText="Price trading near the middle of today's range."; }
            ...
            return (
              <div onClick={function(){setTab("rangeintel");}} style={{background:CARD2+"cc",backdropFilter:"blur(12px)",border:"1px solid "+BLUE+"55",boxShadow:"0 0 24px "+BLUE+"22",borderRadius:14,padding:tierCfg.cardPadding,boxSizing:"border-box",cursor:"pointer",maxHeight:400,overflowY:"auto"}}>
                ...
                <div style={{fontSize:tierCfg.secondaryText,color:T2,marginBottom:4}}>NIFTY 50 &middot; Range: {nifty.low.toLocaleString("en-IN")} &ndash; {nifty.high.toLocaleString("en-IN")}</div>
                <div style={{fontSize:tierCfg.secondaryText,color:T2,marginBottom:6}}>CMP: <b style={{color:T1}}>{nifty.ltp.toLocaleString("en-IN")}</b> &middot; Position: {posPct}% of range</div>
                <div style={{fontSize:tierCfg.secondaryText,fontWeight:800,color:stateColor,marginBottom:6}}>{state}</div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:6}}>
                  <div><div style={{fontSize:tierCfg.label,color:T3}}>Resistance</div><div style={{fontSize:tierCfg.secondaryText,color:T1,fontWeight:700}}>{nifty.high.toLocaleString("en-IN")} <span style={{color:T3,fontWeight:400}}>({distToHigh} away)</span></div></div>
                  <div><div style={{fontSize:tierCfg.label,color:T3}}>Support</div><div style={{fontSize:tierCfg.secondaryText,color:T1,fontWeight:700}}>{nifty.low.toLocaleString("en-IN")} <span style={{color:T3,fontWeight:400}}>({distToLow} away)</span></div></div>
                </div>
                <div style={{fontSize:tierCfg.label,color:T2,lineHeight:1.5}}>{watchText}</div>
              </div>
            );
          })()}
