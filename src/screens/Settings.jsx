...
export default function SettingsScreen(props) {
  ...
  var [bubbleOn, setBubbleOn] = useState(function(){ try{ return localStorage.getItem("bp_bubble")=="1"; }catch(e){ return false; } });
  var [watchOnly, setWatchOnly] = useState(function(){ try{ return localStorage.getItem("bp_watch_only")=="1"; }catch(e){ return false; } });
  function toggleBubble(){
    var nv=!bubbleOn;
    if(nv){ var ok=true; try{ ok=window.confirm("Enable floating bubble? On Android this needs Display-over-other-apps permission. Shows educational observations only, never blocks touches."); }catch(e){} if(!ok){ return; } }
    setBubbleOn(nv);
    try{ localStorage.setItem("bp_bubble", nv?"1":"0"); }catch(e){}
    try{ window.dispatchEvent(new CustomEvent("bp_bubble_change",{detail:nv})); }catch(e){}
  }
  function toggleWatchOnly(){
    var nv=!watchOnly;
    setWatchOnly(nv);
    try{ localStorage.setItem("bp_watch_only", nv?"1":"0"); }catch(e){}
    try{ window.dispatchEvent(new CustomEvent("bp_watch_only_change",{detail:nv})); }catch(e){}
  }
  var [smartAlerts, setSmartAlerts] = useState(function(){ try{ return localStorage.getItem("bp_smart_alerts")!="0"; }catch(e){ return true; } });
  function toggleSmartAlerts(){
    var nv=!smartAlerts;
    setSmartAlerts(nv);
    try{ localStorage.setItem("bp_smart_alerts", nv?"1":"0"); }catch(e){}
    try{ window.dispatchEvent(new CustomEvent("bp_smart_alerts_change",{detail:nv})); }catch(e){}
  }
  ...
  return (
    <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:80}}>
      ...
      <div style={{background:CB,border:"1px solid "+BD,borderRadius:14,margin:"0 14px",overflow:"hidden"}}>
        ...
        <div onClick={toggleSmartAlerts} style={{padding:"13px 14px",borderBottom:"1px solid "+BD,display:"flex",alignItems:"center",gap:10,cursor:"pointer"}}>
          <span style={{fontSize:14}} dangerouslySetInnerHTML={{__html:"&#128276;"}}/>
          <div style={{flex:1}}>
            <div style={{fontSize:12,fontWeight:700,color:T1}}>Smart Alerts</div>
            <div style={{fontSize:9,color:T2}}>Master switch for scanning, voice, notifications and bubble</div>
          </div>
          <div style={{width:44,height:24,borderRadius:12,background:smartAlerts?G:"#2A3550",position:"relative"}}>
            <div style={{width:20,height:20,borderRadius:"50%",background:"#fff",position:"absolute",top:2,left:smartAlerts?22:2}}></div>
          </div>
        </div>
        <div onClick={toggleBubble} style={{padding:"13px 14px",borderBottom:"1px solid "+BD,display:"flex",alignItems:"center",gap:10,cursor:"pointer"}}>
          <span style={{fontSize:14}} dangerouslySetInnerHTML={{__html:"&#128737;"}}/>
          <div style={{flex:1}}>
            <div style={{fontSize:12,fontWeight:600,color:T1}}>Floating Bubble</div>
            <div style={{fontSize:9,color:T2}}>Draggable educational observation bubble</div>
          </div>
          <div style={{width:40,height:22,borderRadius:11,background:bubbleOn?G:"#2A3550",position:"relative"}}>
            <div style={{width:18,height:18,borderRadius:"50%",background:"#fff",position:"absolute",top:2,left:bubbleOn?20:2}}></div>
          </div>
        </div>
        <div onClick={toggleWatchOnly} style={{padding:"13px 14px",borderBottom:"1px solid "+BD,display:"flex",alignItems:"center",gap:10,cursor:"pointer"}}>
          <span style={{fontSize:14}} dangerouslySetInnerHTML={{__html:"&#11088;"}}/>
          <div style={{flex:1}}>
            <div style={{fontSize:12,fontWeight:600,color:T1}}>Watchlist Alerts Only</div>
            <div style={{fontSize:9,color:T2}}>Alerts only for watchlist stocks</div>
          </div>
          <div style={{width:40,height:22,borderRadius:11,background:watchOnly?G:"#2A3550",position:"relative"}}>
            <div style={{width:18,height:18,borderRadius:"50%",background:"#fff",position:"absolute",top:2,left:watchOnly?20:2}}></div>
          </div>
        </div>
