...
var G = "#22C55E", R = "#EF4444", BLUE = "#3B82F6", GOLD = "#D4AF37";
...
var TEMPLATES = [
  ...
  {id:"blue",  label:"Blue Corporate",bg:"#0B1E3D", fg:"#FFFFFF", accent:GOLD}
];
...
export default function NewsStudio(props){
  ...
  var BLUE = theme.c.blue, GOLD = theme.c.gold, T3 = theme.c.text3; T1=theme.c.text1;
    ...
    return (
      <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
        <div style={{background:CB,padding:"12px 14px",borderBottom:"1px solid "+BD,display:"flex",alignItems:"center",gap:10}}>
          <button onClick={function(){setSelected(null);}} style={{background:"rgba(255,255,255,0.06)",border:"none",borderRadius:8,width:40,height:40,color:T1,fontSize:16,cursor:"pointer"}}>&#8592;</button>
          <div style={{fontSize:15,fontWeight:800,color:T1}}>News Card Studio</div>
        </div>

        <div style={{padding:14}}>
          <div style={{fontSize:13,fontWeight:700,color:T1,lineHeight:1.5,marginBottom:8}}>{selected.title}</div>
          <div style={{fontSize:10,color:T2,marginBottom:14}}>{selected.source} &#8226; {selected.time}</div>

          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16}}>
            {analyzing ? (
              <span style={{fontSize:11,color:T2}}>Analyzing with AI...</span>
            ) : (
              <span>
                <span style={{fontSize:11,fontWeight:800,color:SENTIMENT_COLOR[meta.sentiment]}}>{meta.sentiment}</span>
                <span style={{fontSize:11,color:T2}}> &#8226; Impact: {meta.impact}</span>
              </span>
            )}
          </div>

          <div style={{fontSize:11,fontWeight:800,color:T2,marginBottom:8}}>TEMPLATE</div>
          <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
            {TEMPLATES.map(function(t){
              var act = t.id==tmplId;
              return (
                <button key={t.id} onClick={function(){setTmplId(t.id);}} style={{background:t.bg,border:"2px solid "+(act?t.accent:BD),borderRadius:10,padding:"10px 14px",color:t.fg,fontSize:10,fontWeight:700,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>{t.label}</button>
              );
            })}
          </div>

          {/* Lightweight DOM preview - the real PNG is only generated on tap */}
          <div style={{background:tmpl.bg,border:"1px solid "+BD,borderRadius:14,padding:20,marginBottom:16,minHeight:180}}>
            <div style={{width:40,height:4,background:tmpl.accent,marginBottom:10}}></div>
            <div style={{fontSize:12,fontWeight:800,color:tmpl.fg,marginBottom:8}}>Breakout Pro{tmplId=="red"?" - BREAKING":""}</div>
            <div style={{fontSize:16,fontWeight:700,color:tmpl.fg,lineHeight:1.4,marginBottom:10}}>{selected.title}</div>
            <div style={{fontSize:11,fontWeight:700,color:SENTIMENT_COLOR[meta.sentiment]}}>{meta.sentiment.toUpperCase()}</div>
            <div style={{fontSize:9,color:tmpl.fg,opacity:0.6,marginTop:10}}>Educational Market Observation Only. Not Investment Advice.</div>
          </div>
          ...
          <div style={{fontSize:9,color:T3,textAlign:"center",marginBottom:10}}>Share opens your device's share sheet (Instagram, WhatsApp, X, Telegram - whichever apps you have installed).</div>
          {shareMsg ? <div style={{fontSize:10,color:GOLD,textAlign:"center"}}>{shareMsg}</div> : null}
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
      <div style={{background:CB,padding:"12px 14px",borderBottom:"1px solid "+BD,display:"flex",alignItems:"center",gap:10}}>
        ...
        <div>
          <div style={{fontSize:15,fontWeight:800,color:T1}}>AI News Studio</div>
          <div style={{fontSize:9,color:T2}}>Pick a real headline to create a shareable card</div>
        </div>
      </div>
      <div style={{padding:14}}>
        {news.length==0 ? (
          <div style={{fontSize:11,color:T2,textAlign:"center",padding:30}}>Loading news...</div>
        ) : news.map(function(a,i){
          return (
            <div key={i} onClick={function(){selectArticle(a);}} style={{background:CB,border:"1px solid "+BD,borderRadius:12,padding:12,marginBottom:8,cursor:"pointer"}}>
              <div style={{fontSize:12,fontWeight:700,color:T1,lineHeight:1.4,marginBottom:4}}>{a.title}</div>
              <div style={{fontSize:9,color:T2}}>{a.source} &#8226; {a.time}</div>
            </div>
          );
        })}
      </div>
