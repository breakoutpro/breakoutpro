...
export default function MarketsScreen(props) {
  ...
  function fetchLive(){
    var syms = "RELIANCE.NS,TCS.NS,HDFCBANK.NS,ICICIBANK.NS,INFY.NS,WIPRO.NS,SBIN.NS,TATAMOTORS.NS,AXISBANK.NS,BAJFINANCE.NS,MARUTI.NS,SUNPHARMA.NS,LT.NS,ADANIENT.NS,ITC.NS,NESTLEIND.NS,HINDUNILVR.NS,KOTAKBANK.NS,HCLTECH.NS,TITAN.NS,^NSEI,^BSESN,^NSEBANK,NIFTY_FIN_SERVICE.NS,^CRSLDX";
    fetch("/api/yahoo?symbols=" + syms)
      .then(function(r){return r.json();})
      .then(function(data){
        if(data && data.quoteResponse && data.quoteResponse.result){
          var map = {};
          data.quoteResponse.result.forEach(function(q){
            var sym = q.symbol.replace(".NS","").replace("^NSEI","NIFTY").replace("^BSESN","SENSEX").replace("^NSEBANK","BANKNIFTY").replace("NIFTY_FIN_SERVICE","FINNIFTY").replace("^CRSLDX","MIDCAP");
            map[sym] = {
              ltp: q.regularMarketPrice||0,
              chg: q.regularMarketChange||0,
              chgPct: Math.abs(q.regularMarketChangePercent||0).toFixed(2),
      ...
      .catch(function(){setLiveStatus("demo");});
  }
