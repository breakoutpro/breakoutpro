    ...
    return (
      <div style={{padding:14}}>
        ...
        {CHECKLIST_ITEMS.map(function(item){
          ...
          return (
            <div key={item} onClick={function(){toggleCheck(item);}} style={{display:"flex",alignItems:"center",gap:10,background:CARD,border:"1px solid "+(isChecked?BLUE:BD),borderRadius:10,padding:"12px 14px",marginBottom:8,cursor:"pointer"}}>
        ...
        <div style={{background:allChecked?"rgba(37,99,235,0.08)":"rgba(249,115,22,0.06)",border:"1px solid "+(allChecked?"rgba(37,99,235,0.2)":"rgba(249,115,22,0.15)"),borderRadius:10,padding:11,marginTop:12}}>
          <div style={{fontSize:9.5,color:allChecked?BLUE:WARN,lineHeight:1.5}}>{allChecked?"All checks complete - this is a personal readiness checklist, not a trade recommendation.":"Complete all checks before proceeding, or proceed at your own judgement. This checklist is educational only."}</div>
        </div>
      </div>
    );
  }
