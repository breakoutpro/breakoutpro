        ...
        {data.spineRows.map(function(r,i){
          ...
          return (
            <div key={r.key} onClick={function(){data.selectIndex(r.key);}} style={{padding:"6px 16px",cursor:"pointer",borderRight:i<data.spineRows.length-1?"1px solid "+BD2:"none",background:active?"rgba(59,130,246,0.10)":"transparent",borderBottom:active?"2px solid "+BLUE:"2px solid transparent",display:"flex",alignItems:"center",gap:8}}>
              ...
              {r.live ? (
                <span style={{fontSize:tierCfg.primaryNumber,fontWeight:800,fontFamily:"monospace",color:T1}}>{r.ltp.toLocaleString("en-IN",{maximumFractionDigits:2})}</span>
              ) : mm.status=="loading" ? (
            ...
            <table style={{width:"100%",borderCollapse:"collapse",tableLayout:"fixed"}}>
              ...
              <tbody>
                {scannerPreview.map(function(sr){
                  ...
                  return (
                    <tr key={sr.sym} onClick={function(){data.selectStock(sr.sym);}} style={{borderBottom:"1px solid "+BD2,cursor:"pointer",height:tierCfg.tableRowHeight}}>
                      <td style={{fontSize:tierCfg.secondaryText,fontWeight:700,color:T1,padding:"4px 4px"}}>{sr.sym}</td>
                      <td style={{fontSize:tierCfg.secondaryText,color:T1,textAlign:"right",fontFamily:"monospace",padding:"4px 4px"}}>{s?s.ltp.toLocaleString("en-IN"):"--"}</td>
                      <td style={{fontSize:tierCfg.secondaryText,fontWeight:700,textAlign:"right",color:s&&s.chgPct>=0?UP:DOWN,padding:"4px 4px"}}>{s?((s.chgPct>=0?"+":"")+s.chgPct+"%"):"--"}</td>
                      <td style={{fontSize:tierCfg.secondaryText,color:T2,textAlign:"right",fontFamily:"monospace",padding:"4px 4px"}}>{volDisplay}</td>
                      <td style={{fontSize:tierCfg.secondaryText,fontWeight:800,color:BLUE,textAlign:"right",padding:"4px 4px"}}>{sr.confidence}%</td>
                    </tr>
                  );
                })}
              </tbody>
