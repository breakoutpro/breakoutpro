      ...
      <div style={{padding:wideLayout?"4px 8px":"10px 12px 16px",maxWidth:1800,margin:"0 auto",width:"100%",boxSizing:"border-box"}}>
        ...
        <div style={{display:wideLayout?"grid":"block",gridTemplateColumns:wideLayout?"1fr 1fr":undefined,gridTemplateRows:ROW1_H?ROW1_H+"px":undefined,gap:gapV,marginBottom:gapV,alignItems:"stretch"}}>
          <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:8,padding:padV,height:"100%",boxSizing:"border-box",display:"flex",flexDirection:"row",alignItems:"center",gap:8,overflow:"hidden",minWidth:0}}>
            ...
            <svg width="80" height="44" viewBox="0 0 80 44" style={{flexShrink:0}}>
              <path d="M 6 36 A 28 28 0 0 1 74 36" fill="none" stroke={MT.BD} strokeWidth="12" strokeLinecap="round"/>
              <path d={(function(){
                var pct = Math.max(0, Math.min(100, mood.score))/100;
                var ang = Math.PI - pct*Math.PI;
                var x = 40 + 28*Math.cos(ang), y = 36 - 28*Math.sin(ang);
                var largeArc = pct>0.5 ? 1 : 0;
                return "M 6 36 A 28 28 0 " + largeArc + " 1 " + x.toFixed(1) + " " + y.toFixed(1);
              })()} fill="none" stroke={moodColor} strokeWidth="12" strokeLinecap="round"/>
              <circle cx={(function(){ var pct=Math.max(0,Math.min(100,mood.score))/100; var ang=Math.PI-pct*Math.PI; return (40+28*Math.cos(ang)).toFixed(1); })()} cy={(function(){ var pct=Math.max(0,Math.min(100,mood.score))/100; var ang=Math.PI-pct*Math.PI; return (36-28*Math.sin(ang)).toFixed(1); })()} r="4" fill={MT.T1} stroke={moodColor} strokeWidth="2"/>
            </svg>
                  ...
                  {row.available ? (
                    <div style={{display:"flex",flexDirection:"column",gap:2}}>
                      ...
                      <div style={{display:"flex",alignItems:"baseline",gap:5,lineHeight:1.2}}>
                        ...
                        <span style={{fontSize:9,fontWeight:700,color:chgColor,whiteSpace:"nowrap"}}>{row.up==null?"":(row.up?"\u25b2 ":"\u25bc ")}{row.chgPct}</span>
                      </div>
                      {!usingDemo && data && data.indexHistory && data.indexHistory[key] && data.indexHistory[key].points && data.indexHistory[key].points.length>=2 ? (
                        <Sparkline points={data.indexHistory[key].points} inverse={key==="VIX"} height={12}/>
                      ) : null}
                    </div>
                  ) : (
                    <div style={{display:"flex",flexDirection:"column",gap:2}}>
