...
export default function MarketsScreen(props) {
  ...
  function fetchLive(){
    var allSyms = ALL_NSE_STOCKS.map(function(s){ return s.sym; });
    var batchSize = 50;
    var batches = [];
    ...
    var marketHoursNow = mins>=555 && mins<930;

    batches.forEach(function(batch){
      fetch("/api/angelone?action=quote&symbols=" + batch.join(","))
        .then(function(r){return r.json();})
        .then(function(data){
          if(data && data.data && data.data.fetched){
            data.data.fetched.forEach(function(q){
              if(!q.ourSymbol || !q.ltp) return;
              var chg = q.ltp - (q.close||q.ltp);
              combinedMap[q.ourSymbol] = {
                ltp: q.ltp,
                chg: chg,
                chgPct: q.close ? Math.abs((chg/q.close)*100).toFixed(2) : "0.00",
                up: chg>=0,
                high: q.high||0,
                low: q.low||0,
                vol: q.tradeVolume||0,
          ...
          if(data && data.error){
            console.log("Angel One error:", data.error);
          }
        })
        .catch(function(e){
