      ...
      <div style={{background:CB,padding:"12px 14px",borderBottom:"1px solid "+BD}}>
        ...
        <div style={{display:"flex",gap:6,overflowX:"auto"}}>
          {TABS.map(function(t){
            ...
            return <button key={t.id} onClick={function(){setActiveTab(t.id);}} style={{background:act?G:"rgba(255,255,255,0.06)",border:"1px solid "+(act?G:BD),borderRadius:20,padding:"5px 12px",color:act?"#fff":T2,fontSize:12,fontWeight:act?700:500,cursor:"pointer",fontFamily:"inherit",flexShrink:0}}>{t.label}</button>;
          })}
          <div style={{flex:1}}></div>
          {activeTab=="stocks"?[["pct","% Chg"],["ltp","Price"]].map(function(s){
            var act=sort==s[0];
            return <button key={s[0]} onClick={function(){setSort(s[0]);}} style={{background:act?"rgba(0,200,83,0.15)":"rgba(255,255,255,0.06)",border:"1px solid "+(act?G:BD),borderRadius:20,padding:"5px 10px",color:act?G:T2,fontSize:12,cursor:"pointer",fontFamily:"inherit",flexShrink:0}}>{s[1]}</button>;
          }):null}
        </div>
      </div>
      ...
      {activeTab=="stocks"?(
        <div style={{margin:"10px 14px 0"}}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}>
            <span style={{fontSize:12,fontWeight:800,color:GOLD,background:"rgba(212,175,55,0.12)",border:"1px solid rgba(212,175,55,0.3)",padding:"2px 7px",borderRadius:5,letterSpacing:0.5}}>DEMO DATA</span>
      ...
      {activeTab=="indices"?(
        <div style={{padding:"10px 14px"}}>
          {INDICES.map(function(idx){
            ...
            return (
              <div key={idx.label} style={{background:CB,border:"1px solid "+BD,borderRadius:12,padding:16,marginBottom:10,cursor:"pointer"}} onClick={function(){setSelIdx(idx.label);}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                    <div style={{fontSize:12,color:T2,marginBottom:2}}>{idx.label}</div>
                    ...
                    <span style={{background:idx.up?"rgba(0,200,83,0.15)":"rgba(239,68,68,0.15)",color:idx.up?G2:R,borderRadius:6,padding:"2px 8px",fontSize:12,fontWeight:700,marginTop:4,display:"inline-block"}}>{idx.up?"+":""}{idx.pct.toFixed(2)}%</span>
                  </div>
            ...
            return (
              <div key={s.name} style={{background:CB,border:"1px solid "+BD,borderRadius:12,padding:"12px 14px",marginBottom:8,display:"flex",alignItems:"center",gap:12,cursor:"pointer"}} onClick={function(){setSelSector(s);}}>
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:12,color:T2,marginTop:2}}>{s.stocks.slice(0,3).join(", ")}</div>
                </div>
              ...
              return (
                <div key={s.sym} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",borderBottom:"1px solid "+BD,cursor:"pointer"}} onClick={function(){setSelCom(s);}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:12,color:T2,marginTop:1}}>{s.sect} - per {s.unit}</div>
                  </div>
                  ...
                  <div style={{textAlign:"right",minWidth:80}}>
                    ...
                    <div style={{background:s.up?"rgba(0,200,83,0.15)":"rgba(239,68,68,0.15)",borderRadius:5,padding:"1px 6px",fontSize:12,fontWeight:700,color:s.up?G2:R,marginTop:2}}>{s.up?"+":""}{s.chgPct.toFixed(2)}%</div>
                  </div>
      ...
      {activeTab=="circuit"?(
        <div style={{padding:"10px 14px"}}>
          ...
          <div style={{background:CB,border:"1px solid "+BD,borderRadius:12,overflow:"hidden",marginBottom:14}}>
            {CIRCUIT_UP.length>0?CIRCUIT_UP.map(function(s,i){
              return (
                <div key={s.sym} style={{display:"flex",alignItems:"center",padding:"10px 14px",borderBottom:i<CIRCUIT_UP.length-1?"1px solid "+BD:"none",cursor:"pointer"}} onClick={function(){setSelStock(s);}}>
                  <div style={{background:"rgba(0,200,83,0.15)",border:"1px solid rgba(0,200,83,0.3)",borderRadius:6,padding:"2px 8px",marginRight:10,flexShrink:0}}>
            ...
            }):<div style={{padding:20,textAlign:"center",color:T2,fontSize:12}}>No upper circuit stocks today</div>}
          </div>
          ...
          <div style={{background:CB,border:"1px solid "+BD,borderRadius:12,overflow:"hidden"}}>
            {CIRCUIT_DOWN.length>0?CIRCUIT_DOWN.map(function(s,i){
              return (
                <div key={s.sym} style={{display:"flex",alignItems:"center",padding:"10px 14px",borderBottom:i<CIRCUIT_DOWN.length-1?"1px solid "+BD:"none",cursor:"pointer"}} onClick={function(){setSelStock(s);}}>
                  <div style={{background:"rgba(239,68,68,0.15)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:6,padding:"2px 8px",marginRight:10,flexShrink:0}}>
            ...
            }):<div style={{padding:20,textAlign:"center",color:T2,fontSize:12}}>No lower circuit stocks today</div>}
          </div>
        </div>
      ):null}
