  ...
  return (
    <div style={{background:BG,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
      <div style={{background:BG,padding:"12px 14px",borderBottom:"1px solid "+BD,display:"flex",alignItems:"center",gap:10,position:"sticky",top:0,zIndex:10}}>
        ...
        <button onClick={speak} style={{background:"rgba(59,130,246,0.12)",border:"1px solid rgba(59,130,246,0.3)",borderRadius:9,padding:"7px 11px",color:BLUE,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>&#128266;</button>
      </div>

      <div style={{padding:14}}>
        {/* explanation */}
        <div style={{background:theme.c.card,border:"1px solid rgba(59,130,246,0.25)",borderRadius:13,padding:14,marginBottom:16}}>
          <div style={{fontSize:11.5,color:T1,lineHeight:1.65}}>{d.explanation}</div>
        </div>

        <Sec t="WHY IT IS IMPORTANT"><Box><div style={{fontSize:11.5,color:T1,lineHeight:1.65}}>{d.why}</div></Box></Sec>
        ...
        <Sec t="BEST PRACTICES">
          <Box>
            {d.best.map(function(b,i){
              return <div key={i} style={{fontSize:11,color:T1,lineHeight:1.5,padding:"5px 0",borderBottom:i<d.best.length-1?"1px solid "+BD2:"none"}}><span style={{color:UP}}>&#10003;</span>  {b}</div>;
            })}
          </Box>
        </Sec>
        ...
        {quiz?(
          <Sec t="QUICK QUIZ">
            <Box>
              <div style={{fontSize:11.5,fontWeight:700,color:T1,marginBottom:10,lineHeight:1.5}}>{quiz.q}</div>
              {quiz.opts.map(function(o,i){
                ...
                return <button key={i} onClick={function(){if(pick==null)setPick(i);}} style={{display:"block",width:"100%",textAlign:"left",background:bg,border:"1px solid "+bd,borderRadius:9,padding:"11px",marginBottom:7,color:tc,fontSize:11,fontWeight:600,cursor:pick==null?"pointer":"default",fontFamily:"inherit"}}>{o}</button>;
              })}
              {pick!=null?<div style={{fontSize:10,color:pick==quiz.a?UP:DOWN,fontWeight:700,marginTop:4}}>{pick==quiz.a?"Correct!":"Review the explanation above."}</div>:null}
            </Box>
          </Sec>
        ):null}
          ...
          <Sec t="RELATED TOPICS">
            <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
              {d.related.map(function(r,i){
                return <span key={i} style={{fontSize:10,color:BLUE,background:"rgba(59,130,246,0.1)",border:"1px solid rgba(59,130,246,0.25)",borderRadius:14,padding:"6px 12px"}}>{r}</span>;
              })}
            </div>
          </Sec>
        ):null}
        ...
        <div style={{background:"rgba(249,115,22,0.06)",border:"1px solid rgba(249,115,22,0.15)",borderRadius:10,padding:11}}>
          <div style={{fontSize:8.5,color:theme.c.warn,lineHeight:1.5}}>Educational Market Intelligence Only. This content is designed to help users understand options markets. It is not investment advice or a trading recommendation.</div>
        </div>
      </div>
    </div>
  );
