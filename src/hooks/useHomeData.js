...
export function useHomeData(sharedMm){
  // Prefer a shared useMarketMood() instance passed from the parent (the
  // same one App.jsx already uses for the full AI Market Mood page) so
  // Home and the full page poll the same data once, not twice
  // independently. Falls back to its own call only if no shared instance
  // is provided, for backward compatibility with any caller not yet
  // updated to pass one down.
  var ownMm = useMarketMood();
  var mm = sharedMm || ownMm;
  var mkt = useMarketSnapshot(); // centralized real-price source - Spine/Key Levels/AI Mood gating use THIS, not mm.data
  var wl = useWatchlist();
  ...
  var niftyLive = !!(mkt.snapshot && mkt.snapshot.indices && mkt.snapshot.indices.NIFTY && mkt.snapshot.indices.NIFTY.status==="ok");
  var bankLive = !!(mkt.snapshot && mkt.snapshot.indices && mkt.snapshot.indices.BANKNIFTY && mkt.snapshot.indices.BANKNIFTY.status==="ok");
  var niftyLtp = niftyLive ? mkt.snapshot.indices.NIFTY.price : null;
  var bankLtp = bankLive ? mkt.snapshot.indices.BANKNIFTY.price : null;
  ...
  var spineRows = useMemo(function(){
    var mktData = mkt.snapshot && mkt.snapshot.indices;
    var moodData = mm.data && mm.data.indices;
    return SPINE_INDICES.map(function(s){
      var e = mktData && mktData[s.key];
      var hasLive = e && e.status==="ok" && e.price!=null;
      var ltp = hasLive ? e.price : null;
      var chgNum = hasLive ? Number(e.changePercent) : null;
      var chgOk = hasLive && e.changePercent!=null && isFinite(chgNum);
      if(!hasLive){
        // Fall back to the same real mm.data.indices source that already
        // powers the working AI Market Mood card/Market Snapshot, instead
        // of leaving this row "unavailable" while proven-real data for the
        // same index exists elsewhere on the page.
        var me = moodData && moodData[s.key];
        if(me && me.ltp!=null){
          hasLive = true;
          ltp = me.ltp;
          var mChg = Number(me.chgPct);
          chgOk = me.chgPct!=null && isFinite(mChg);
          chgNum = chgOk ? mChg : null;
        }
      }
      var dir = chgOk ? (chgNum>0?"up":(chgNum<0?"down":"neutral")) : "neutral";
      return {
