...
export default function OptionsIntel(props){
  ...
  var BD2 = theme.c.border2, BLUE = theme.c.blue, CARD = theme.c.card, CARD2 = theme.c.card2, GOLD = theme.c.gold, T2 = theme.c.text2, T3 = theme.c.text3; T1=theme.c.text1; UP=theme.c.up;
    ...
    <div style={{padding:"0 14px",marginTop:full?6:22}}>
      ...
      {!full?(
        <div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
            ...
            <span style={{fontSize:8,color:T3}}>Simulated for preview. Not live options data.</span>
          </div>
      ...
      {!full?(
        ...
        <div>
          ...
          {/* GREEKS */}
          <div style={{fontSize:10.5,fontWeight:800,color:T2,letterSpacing:0.5,marginBottom:9}}>GREEKS (SIMULATED)</div>
      ...
      <div style={{background:theme.c.card,border:"1px solid rgba(59,130,246,0.25)",borderRadius:13,padding:14}}>
        <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:8}}>
          ...
          <span style={{fontSize:7.5,fontWeight:800,color:GOLD,background:"rgba(212,175,55,0.12)",border:"1px solid rgba(212,175,55,0.3)",padding:"2px 7px",borderRadius:5,letterSpacing:0.5}}>SAMPLE</span>
        </div>
        <div style={{fontSize:8,color:T3,marginBottom:8}}>Sample commentary - not live AI analysis.</div>
...
function MetricCard(props){
  ...
  return (
    <div onClick={props.onClick} style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:11,cursor:"pointer"}}>
      ...
      <div style={{fontSize:8.5,color:BLUE,marginTop:5}}>Tap to learn</div>
    </div>
  );
}
...
function MetricDetail(props){
  ...
  return (
    <div onClick={props.onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:100,display:"flex",alignItems:"flex-end"}}>
      <div onClick={function(e){e.stopPropagation();}} style={{background:theme.c.card2,borderTop:"1px solid "+BD,borderRadius:"18px 18px 0 0",width:"100%",maxHeight:"85vh",overflowY:"auto",padding:"16px 16px 30px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
          ...
          <button onClick={speak} style={{background:"rgba(59,130,246,0.12)",border:"1px solid rgba(59,130,246,0.3)",borderRadius:9,padding:"8px 12px",color:BLUE,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>&#128266; Listen</button>
        </div>
        ...
        <div style={{background:"rgba(249,115,22,0.06)",border:"1px solid rgba(249,115,22,0.15)",borderRadius:9,padding:10,marginTop:6}}>
          <div style={{fontSize:8.5,color:theme.c.warn}}>Educational only. Not a buy or sell recommendation.</div>
        </div>
      </div>
    </div>
  );
