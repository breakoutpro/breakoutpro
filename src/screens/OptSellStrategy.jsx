...
function PayoffGraph(props){
  ...
  var BD2 = theme.c.border2, BLUE=theme.c.blue, T3 = theme.c.text3; UP=theme.c.up;
  ...
  return (
    <svg width="100%" viewBox={"0 0 "+W+" "+H} style={{display:"block"}}>
      ...
      {s.legs.map(function(l,i){
        return <line key={i} x1={x(l.strike)} y1={PAD} x2={x(l.strike)} y2={H-PAD} stroke={l.dir=="short"?BLUE:T3} strokeWidth="0.8" strokeDasharray="2 2" opacity="0.5"/>;
      })}
    </svg>
  );
}
...
export default function OptSellStrategy(props){
  ...
  var BLUE=theme.c.blue; 
  // Theme-sourced overrides - shadow the module-level hardcoded fallbacks above.
  var BD2 = theme.c.border2, BG = theme.c.bg, CARD = theme.c.card, T2 = theme.c.text2, T3 = theme.c.text3; T1=theme.c.text1; UP=theme.c.up;
  ...
  function Row(p){ return <div style={{display:"flex",justifyContent:"space-between",padding:"12px 12px",borderBottom:p.last?"none":"1px solid "+BD2}}><span style={{fontSize:12,color:p.c||T2,fontWeight:600}}>{p.k}</span><span style={{fontSize:12,color:T1,fontWeight:600,textAlign:"right",maxWidth:190}}>{p.v}</span></div>; }
  ...
  return (
    <div style={{background:BG,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
      <div style={{background:BG,padding:"12px 16px",borderBottom:"1px solid "+BD,display:"flex",alignItems:"center",gap:12,position:"sticky",top:0,zIndex:10}}>
        ...
        <div style={{flex:1}}>
          <div style={{fontSize:18,fontWeight:900,color:T1}}>{s.name}</div>
          <div style={{fontSize:12,fontWeight:700,color:BLUE}}>{s.bias}</div>
        </div>
      ...
      <div style={{padding:16}}>
        ...
        <Sec t="PAYOFF AT EXPIRY">
          <div style={{background:"#08090D",border:"1px solid "+BD,borderRadius:13,padding:"12px 8px"}}>
            <PayoffGraph strat={s}/>
            <div style={{fontSize:12,color:T3,textAlign:"center",marginTop:4}}>Blue line is profit and loss across price. Gold lines are short strikes.</div>
          </div>
        </Sec>

        <Sec t="OVERVIEW"><div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:12}}><div style={{fontSize:12,color:T1,lineHeight:1.6}}>{s.overview}</div></div></Sec>
        ...
        <Sec t="GREEKS IMPACT"><div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:12}}><div style={{fontSize:12,color:T1,lineHeight:1.6}}>{s.greeks}</div></div></Sec>
        <Sec t="EXPIRY BEHAVIOUR"><div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:12}}><div style={{fontSize:12,color:T1,lineHeight:1.6}}>{s.expiry}</div></div></Sec>

        <Sec t="COMMON MISTAKES">
          <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:12}}>
            {s.mistakes.map(function(m,i){ return <div key={i} style={{fontSize:12,color:T1,lineHeight:1.5,padding:"4px 0"}}><span style={{color:DOWN}}>&#33;</span>  {m}</div>; })}
          </div>
        </Sec>
