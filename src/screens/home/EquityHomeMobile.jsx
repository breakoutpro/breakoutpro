    ...
    <div style={{background:BG,minHeight:"100vh",width:"100%",boxSizing:"border-box",overflowX:"hidden",fontFamily:"'Inter',Arial,sans-serif",paddingBottom:84,color:T1}}>
      ...
      <div style={{width:"100%",boxSizing:"border-box",padding:"7px 9px 0"}}>
        ...
        {(function(){
          ...
          return (
            <Card title="Range Intelligence" icon="&#128640;" onDetails={function(){setTab("rangeintel");}}>
              <div style={{fontSize:14,fontWeight:900,color:T1,marginBottom:3}}>{nifty.ltp.toLocaleString("en-IN")}</div>
              ...
              <div style={{fontSize:12,fontWeight:800,color:stateColor,marginBottom:6}}>{posPct}% &middot; {zone} &middot; {state}</div>
            ...
            return (
              <div key={r.sym} style={{padding:"4px 0",borderBottom:"1px solid "+BD}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  <span style={{fontSize:12,fontWeight:800,color:T1}}>{r.sym}</span>
                  <span style={{fontSize:11,fontWeight:700,color:tagColor}}>{tagLabel}</span>
                </div>
        ...
        {/* MARKET PULSE - broad market context only, compact and scrollable
            rather than more full cards. Market Breadth/FII/DII are literal
            "UNAVAILABLE" in api/market-mood-data.js's own response - no
            real provider exists for either. Global Markets IS real
            (mm.data.global). Commodities/Economic Calendar are confirmed
            DEMO-only elsewhere in this codebase, so honestly Not connected
            here too rather than silently omitted. */}
        {(function(){
          ...
          return (
            <div style={{background:CARD,border:"1px solid "+BD,borderRadius:10,padding:"8px 10px",marginBottom:7,overflowX:"auto"}}>
              <div style={{fontSize:9,fontWeight:800,color:T2,letterSpacing:0.5,marginBottom:5}}>MARKET PULSE</div>
              <div style={{display:"flex",flexDirection:"column",gap:4}}>
                {pulseItems.map(function(it){
                  return (
                    <div key={it.label} style={{display:"flex",alignItems:"center",justifyContent:"space-between",fontSize:10}}>
