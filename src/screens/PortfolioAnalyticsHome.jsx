...
export default function PortfolioAnalyticsHome(props){
  ...
  HEALTH_COLOR["Excellent"]=UP; HEALTH_COLOR["Good"]=UP; HEALTH_COLOR["Average"]=WARN; HEALTH_COLOR["Needs Improvement"]=R;
  RISK_COLOR["Low"]=UP; RISK_COLOR["Medium"]=WARN; RISK_COLOR["High"]=R;
  ...
  return (
    <div style={{background:theme.c.bg,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
      <div style={{background:CARD,padding:"12px 14px",borderBottom:"1px solid "+BD,display:"flex",alignItems:"center",gap:10}}>
        ...
        <button onClick={function(){setMode("create");}} style={{background:BLUE,border:"none",borderRadius:9,padding:"9px 14px",color:"#fff",fontSize:15,fontWeight:800,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>+ Add</button>
      </div>

      <div style={{padding:14}}>
        <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10}}>PORTFOLIO DASHBOARD</div>
        ...
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:12,marginBottom:16,textAlign:"center"}}>
          <div style={{fontSize:14,fontWeight:800,color:T1}}>{dash.count}</div>
          <div style={{fontSize:12,color:T2,marginTop:2}}>Number of Holdings</div>
        </div>
        ...
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,marginBottom:14}}>
          <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10}}>PORTFOLIO HEALTH</div>
        ...
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,marginBottom:14}}>
          <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10}}>RISK METER</div>
        ...
        {sectorAlloc.length>0 ? (
          <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,marginBottom:14}}>
            <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10}}>SECTOR ALLOCATION</div>
        ...
        {assetAlloc.length>0 ? (
          <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,marginBottom:14}}>
            <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10}}>ASSET ALLOCATION (PER HOLDING)</div>
            {assetAlloc.map(function(a){
              return (
                <div key={a.symbol} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:"1px solid "+BD}}>
        ...
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,marginBottom:16}}>
          <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10}}>PORTFOLIO INSIGHTS</div>
          {insights.map(function(line,i){
            return <div key={i} style={{fontSize:12,color:"#C9D4E5",lineHeight:1.6,marginBottom:6}}>{line}</div>;
          })}
        </div>

        <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
          <input value={search} onChange={function(ev){setSearch(ev.target.value);}} placeholder="Search holdings..." style={{flex:1,minWidth:140,background:CARD,border:"1px solid "+BD,borderRadius:9,padding:"9px 11px",color:T1,fontSize:12,fontFamily:"inherit"}}/>
          <select value={sortBy} onChange={function(ev){setSortBy(ev.target.value);}} style={{background:CARD,border:"1px solid "+BD,borderRadius:9,padding:"9px 11px",color:T1,fontSize:12,fontFamily:"inherit"}}>
            {SORT_OPTIONS.map(function(o){ return <option key={o} value={o}>{o}</option>; })}
          </select>
        </div>

        <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10}}>HOLDINGS ({sorted.length})</div>
        {sorted.length==0 ? (
          <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:20,textAlign:"center",color:T3,fontSize:12}}>No holdings yet. Add your first one to get started.</div>
        ) : sorted.map(function(h){
          return (
            <div key={h.id} style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,marginBottom:10}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                <div>
                  <div style={{fontSize:16,fontWeight:700,color:T1}}>{h.symbol}</div>
                  <div style={{fontSize:12,color:T3,marginTop:2}}>{h.sector} &#8226; Qty {h.qty} &#8226; Avg Rs{fmt(h.avgBuyPrice)}</div>
                </div>
                <div style={{textAlign:"right"}}>
                  <div style={{fontSize:14,fontWeight:800,color:h.pnlPct>=0?UP:R}}>{(h.pnlPct>=0?"+":"")+h.pnlPct.toFixed(2)}%</div>
                  <div style={{fontSize:12,color:T2,marginTop:2}}>Rs {fmt(h.value)}</div>
                </div>
              </div>
              <div style={{display:"flex",gap:6}}>
                <button onClick={function(){setEditingId(h.id); setMode("edit");}} style={{flex:1,background:"transparent",border:"1px solid "+BD,borderRadius:8,padding:"9px",color:T2,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>Edit</button>
                <button onClick={function(){portfolio.deleteHolding(h.id);}} style={{flex:1,background:"transparent",border:"1px solid "+R,borderRadius:8,padding:"9px",color:R,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>Delete</button>
              </div>
            </div>
          );
        })}
        ...
        <div style={{background:"rgba(212,175,55,0.06)",border:"1px solid rgba(212,175,55,0.15)",borderRadius:10,padding:11,marginTop:16}}>
          <div style={{fontSize:12,color:WARN,lineHeight:1.5}}>Educational tool only. Current prices are values you entered yourself - not live market data. Not investment advice.</div>
        </div>
      </div>
    </div>
  );
...
function StatCard(props){
  return (
    <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:12}}>
      ...
      <div style={{fontSize:12,color:T2,marginTop:2}}>{props.label}</div>
    </div>
  );
}
function MiniCard(props){
  ...
  return (
    <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:12}}>
      ...
      <div style={{fontSize:12,fontWeight:700,color:props.color||T2,marginTop:2}}>{props.sub}</div>
    </div>
  );
}
