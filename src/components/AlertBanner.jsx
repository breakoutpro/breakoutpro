    ...
    <div style={{
      ...
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
          ...
          <span style={{fontSize:11,fontWeight:700,color:"#fff"}}>{current.sym}</span>
        </div>
        <div style={{fontSize:9,color:"#8899BB",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{current.msg}</div>
      </div>
      <div style={{textAlign:"right",flexShrink:0}}>
        ...
        <div style={{fontSize:7,color:"#4A5A7A"}}>{current.time}</div>
      </div>
      <button onClick={function(){setVisible(false);}} style={{background:"none",border:"none",color:"#4A5A7A",fontSize:16,cursor:"pointer",flexShrink:0,padding:4}}>X</button>
    </div>
  );
}
