...
function OneTapMarket(props) {
  var onClose = props.onClose || function(){};
  var [data, setData] = useState(props.initialData || []);
  ...
  function refresh() {
    ...
    setData(function(prev) {
      return prev.map(function(idx) {
        var chg = (Math.random()-0.48)*idx.ltp*0.003;
        var nl = parseFloat((idx.ltp+chg).toFixed(2));
        var up = nl >= idx.ltp;
        return Object.assign({}, idx, { ltp: nl, pct: parseFloat(((nl-idx.base)/idx.base*100).toFixed(2)), up: up, spark: idx.spark.slice(-19).concat([nl]) });
      });
    });
  ...
  return (
    <div style={{position:"fixed",inset:0,background:BG,zIndex:999,overflowY:"auto",paddingBottom:80}}>
      {/* Header */}
      <div style={{background:CARD,borderBottom:"1px solid "+BD,padding:"14px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:10}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <button onClick={onClose} style={{background:"rgba(255,255,255,0.06)",border:"none",borderRadius:8,width:32,height:32,color:T1,fontSize:14,cursor:"pointer"}}>&#8592;</button>
          <div>
            <div style={{fontSize:18,fontWeight:900,color:T1}}>One Tap Market</div>
            <div style={{fontSize:12,color:T2}}>Auto refresh every 5 sec</div>
          </div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          <div style={{width:8,height:8,borderRadius:"50%",background:UP,boxShadow:"0 0 8px "+UP,animation:"otm-pulse 1.5s infinite"}}></div>
          <span style={{fontSize:10,fontWeight:700,color:UP}}>LIVE</span>
          {refreshing && <span style={{fontSize:9,color:T3}}>...</span>}
        </div>
      </div>

      {/* Last updated */}
      {lastUpdate && (
        <div style={{background:"rgba(34,197,94,0.06)",borderBottom:"1px solid rgba(34,197,94,0.15)",padding:"5px 16px"}}>
          <span style={{fontSize:10,color:T3}}>Last updated: </span>
          <span style={{fontSize:10,fontWeight:600,color:UP}}>{lastUpdate}</span>
        </div>
      )}

      {/* Index cards grid */}
      <div style={{padding:"14px 16px 0"}}>
        <div style={{fontSize:12,fontWeight:700,color:T3,letterSpacing:0.8,marginBottom:10}}>INDICES</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
          {data.map(function(idx) {
            return (
              <div key={idx.label} onClick={function(){onClose(); props.onSelectIdx && props.onSelectIdx(idx);}} style={{background:CARD,border:"1px solid "+(idx.up?UP+"44":DOWN+"44"),borderRadius:16,padding:"14px",cursor:"pointer",position:"relative",overflow:"hidden"}}>
                ...
                <div style={{fontSize:12,fontWeight:700,color:T3,marginBottom:4}}>{idx.label}</div>
                <div style={{fontSize:20,fontWeight:900,color:T1,fontFamily:"monospace",marginBottom:3}}>{idx.ltp.toLocaleString("en-IN",{maximumFractionDigits:2})}</div>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span style={{fontSize:12,fontWeight:700,color:idx.up?UP:DOWN}}>{idx.up?"+":""}{idx.pct}%</span>
                  <MiniChart d={idx.spark} col={idx.up?UP:DOWN} w={50} h={22}/>
                </div>
              </div>
            );
          })}
        ...
        {/* Top Gainers live */}
        <div style={{fontSize:12,fontWeight:700,color:T3,letterSpacing:0.8,marginBottom:10}}>TOP GAINERS</div>
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,overflow:"hidden",marginBottom:14}}>
          {GAINERS.map(function(s, i) {
            return (
              <div key={s.sym} style={{padding:"11px 14px",borderBottom:i<GAINERS.length-1?"1px solid rgba(30,58,95,0.4)":"none",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div>
                  <div style={{fontSize:13,fontWeight:700,color:T1}}>{s.sym}</div>
                  <div style={{fontSize:11,color:T3}}>{s.name||""}</div>
                </div>
                <div style={{textAlign:"right"}}>
                  ...
                  <div style={{fontSize:11,fontWeight:700,color:UP}}>+{s.pct}%</div>
                </div>
              </div>
            );
          })}
        ...
        {/* Top Losers live */}
        <div style={{fontSize:12,fontWeight:700,color:T3,letterSpacing:0.8,marginBottom:10}}>TOP LOSERS</div>
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,overflow:"hidden",marginBottom:14}}>
          {LOSERS.map(function(s, i) {
            return (
              <div key={s.sym} style={{padding:"11px 14px",borderBottom:i<LOSERS.length-1?"1px solid rgba(30,58,95,0.4)":"none",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div>
                  <div style={{fontSize:13,fontWeight:700,color:T1}}>{s.sym}</div>
                  <div style={{fontSize:11,color:T3}}>{s.name||""}</div>
                </div>
                <div style={{textAlign:"right"}}>
                  <div style={{fontSize:13,fontWeight:700,color:T1,fontFamily:"monospace"}}>{s.ltp ? s.ltp.toLocaleString("en-IN",{maximumFractionDigits:2}) : "--"}</div>
                  <div style={{fontSize:11,fontWeight:700,color:DOWN}}>{s.pct}%</div>
                </div>
              </div>
            );
          })}
      ...
      <style>{"@keyframes otm-pulse{0%,100%{opacity:1}50%{opacity:0.3}}"}</style>
    </div>
  );
}
...
export default function EquityHome(props) {
  ...
  var [showOneTap, setShowOneTap] = useState(false);
  ...
  if (showOneTap) return <OneTapMarket onClose={function(){setShowOneTap(false);}} initialData={indices} onSelectIdx={function(idx){setSelIdx(idx);}} setTab={setTab}/>;
    ...
    <div style={{background:BG,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:80}}>
      ...
      <div style={{padding:"10px 16px 0"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          ...
          {/* One Tap Market button */}
          <button onClick={function(){setShowOneTap(true);}} style={{background:"linear-gradient(135deg,"+BLUE+","+PURPLE+")",border:"none",borderRadius:12,padding:"7px 14px",color:"#fff",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",gap:6}}>
            <span>&#9889;</span>
            <span>One Tap Market</span>
          </button>
        </div>
