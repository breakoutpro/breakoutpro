...
import { MiniChart, genSpark, INDICES, LARGECAP, MIDCAP, SECTORS, getSession, SESSION_META } from "./HomeData";
...
export default function EquityHome(props){
  ...
  return (
    <div style={{background:BG,minHeight:"100vh",fontFamily:"'Inter',Arial,sans-serif",paddingBottom:84,color:T1}}>
      ...
      {/* ===== TOP HEADER (candlestick logo, single, no dup time) ===== */}
      <div style={{padding:"20px 18px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            <div style={{fontSize:16,fontWeight:800,letterSpacing:-0.2}}><span style={{color:"#FFFFFF"}}>Breakout</span><span style={{color:CYAN}}>Pro</span></div>
            <div style={{fontSize:8.5,color:T3,fontWeight:600,letterSpacing:0.3}}>Catch Every Breakout</div>
          </div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:7}}>
          <button onClick={function(){setTab("alerts");}} style={{background:CARD,border:"1px solid "+BD,borderRadius:10,width:36,height:36,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",position:"relative"}}>
            <span style={{fontSize:14}}>&#128276;</span>
            <div style={{position:"absolute",top:6,right:7,width:5,height:5,borderRadius:"50%",background:DOWN,border:"1.5px solid "+CARD}}/>
          </button>
          <button onClick={toggleLang} style={{background:CARD,border:"1px solid "+BD,borderRadius:10,width:36,height:36,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>
            <span style={{fontSize:9.5,fontWeight:800,color:T2}}>{lang=="te"?"\u0c24\u0c46":"EN"}</span>
          </button>
          <button onClick={function(){setTab("sub");}} style={{background:"linear-gradient(135deg,"+GOLD+",#D97706)",border:"none",borderRadius:10,padding:"8px 13px",display:"flex",alignItems:"center",gap:4,cursor:"pointer"}}>
      ...
      <div style={{padding:"0 18px 14px"}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:6,background:CARD,border:"1px solid "+(isLive?"rgba(34,197,94,0.25)":BD),borderRadius:20,padding:"5px 12px"}}>
      ...
      <div style={{padding:"0 0 18px"}}>
        <div style={{padding:"0 18px",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:11}}>
          <span style={{fontSize:14.5,fontWeight:800,color:T1,letterSpacing:0.2}}>Live Markets</span>
          <span style={{fontSize:9.5,color:T3}}>Tap for chart &#8594;</span>
        </div>
      ...
      <div style={{padding:"0 18px"}}>
        ...
        <div style={{display:"flex",gap:9,overflowX:"auto",marginBottom:18,paddingBottom:2,WebkitOverflowScrolling:"touch"}}>
          {SECTORS.slice(0,6).map(function(s){
            return (
              <div key={s.name} onClick={function(){setTab("markets");}} style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:"13px",minWidth:100,flexShrink:0,cursor:"pointer"}}>
                <span style={{fontSize:17}} dangerouslySetInnerHTML={{__html:SECTOR_ICONS[s.name]||"&#128202;"}}/>
                <div style={{fontSize:10.5,fontWeight:700,color:T1,marginTop:7,marginBottom:4}}>{s.name}</div>
