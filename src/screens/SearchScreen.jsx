...
export default function SearchScreen(props){
  ...
  var BD2 = theme.c.border2, BG = theme.c.bg, BLUE = theme.c.blue, CARD = theme.c.card, T2 = theme.c.text2, T3 = theme.c.text3;
  ...
  function Row(props2){
    var s=props2.s;
    return (
      <div onClick={function(){choose(s);}} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 13px",borderBottom:props2.last?"none":"1px solid "+BD2,cursor:"pointer"}}>
        <div>
          <div style={{fontSize:12.5,fontWeight:700,color:T1}}>{s.sym}</div>
          <div style={{fontSize:8.5,color:T3,marginTop:1}}>{s.type}</div>
        </div>
  ...
  return (
    <div style={{background:BG,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:90}}>
      <div style={{background:BG,padding:"12px 14px",borderBottom:"1px solid "+BD,display:"flex",alignItems:"center",gap:10,position:"sticky",top:0,zIndex:10}}>
        ...
        <input autoFocus value={q} onChange={function(e){setQ(e.target.value);}} placeholder="Search stocks, indices, ETFs" style={{flex:1,background:CARD,border:"1px solid "+BD,borderRadius:10,padding:"10px 13px",color:T1,fontSize:12.5,fontFamily:"inherit",outline:"none"}}/>
      </div>
      ...
      <div style={{padding:14}}>
        {q?(
          results.length?(
            ...
            <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:20,textAlign:"center"}}>
              <div style={{fontSize:11,color:T2}}>No results for "{q}". Try another symbol.</div>
            </div>
          )
        ):(
          <div>
            {recentItems.length?(
              <div style={{marginBottom:18}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:9}}>
                  <span style={{fontSize:10.5,fontWeight:800,color:T2,letterSpacing:0.4}}>RECENT</span>
                  <button onClick={clearRecent} style={{background:"none",border:"none",color:BLUE,fontSize:10,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Clear</button>
                </div>
                <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,overflow:"hidden"}}>
            ...
            <div style={{fontSize:10.5,fontWeight:800,color:T2,letterSpacing:0.4,marginBottom:9}}>TRENDING</div>
            <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,overflow:"hidden"}}>
            ...
            <div style={{background:"rgba(249,115,22,0.06)",border:"1px solid rgba(249,115,22,0.15)",borderRadius:10,padding:11,marginTop:16}}>
              <div style={{fontSize:8.5,color:theme.c.warn}}>Educational Market Intelligence Only. Not Investment Advice.</div>
            </div>
          </div>
        )}
      </div>
