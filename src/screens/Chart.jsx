...
function CandleChart(props) {
  ...
  BD=theme.c.border; R=theme.c.down; T2=theme.c.text2;
...
export default function ChartScreen() {
  ...
  G=theme.c.up; G2=theme.c.up; R=theme.c.down; BLUE=theme.c.blue;
  ...
  var rsiColor = rsi==null ? T2 : (rsi > 70 ? R : rsi < 30 ? G2 : BLUE);
  ...
  var indicators = [
    ...
    {label:"EMA 9", val: (ema9.length? (ema9[ema9.length-1]==null?"--":ema9[ema9.length-1]) : "--"), color:BLUE, sub: (ema9.length && ema9[ema9.length-1]!=null) ? (lastC.close > ema9[ema9.length-1] ? "Bullish" : "Bearish") : "Insufficient data"},
    ...
    <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:80}}>
      ...
      <div style={{background:CB,padding:"12px 16px",borderBottom:"1px solid "+BD}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <div>
            <div style={{fontSize:18,fontWeight:900,color:T1}}>Chart <span style={{color:BLUE}}>Engine</span></div>
            <div style={{fontSize:12,color:T2}}>Historical candles</div>
          </div>
          <div style={{textAlign:"right"}}>
            ...
            <div style={{fontSize:12,fontWeight:700,color:hasData?(bull?G2:R):T2}}>{hasData ? ((bull?"+":"")+chgPct+"%") : "--"}</div>
          </div>
        </div>
        ...
        <div style={{display:"flex",gap:8,overflowX:"auto",marginBottom:8}}>
          {SYMBOLS.map(function(s,i){
            var act=symIdx==i;
            return <button key={s.sym} onClick={function(){setSymIdx(i);}} style={{background:act?BLUE:"rgba(255,255,255,0.06)",border:"1px solid "+(act?BLUE:BD),borderRadius:20,padding:"4px 12px",color:act?"#fff":T2,fontSize:12,fontWeight:act?700:500,cursor:"pointer",fontFamily:"inherit",flexShrink:0}}>{s.sym}</button>;
          })}
        </div>
        ...
        <div style={{display:"flex",gap:8}}>
          {TIMEFRAMES.map(function(t){
            var act=tf==t;
            return <button key={t} onClick={function(){setTf(t);}} style={{background:act?"rgba(245,158,11,0.15)":"rgba(255,255,255,0.04)",border:"1px solid "+(act?BLUE:BD),borderRadius:8,padding:"4px 12px",color:act?BLUE:T2,fontSize:12,fontWeight:act?700:400,cursor:"pointer",fontFamily:"inherit"}}>{t}</button>;
          })}
          <div style={{flex:1}}></div>
          <button onClick={load} style={{background:"rgba(37,99,235,0.1)",border:"1px solid rgba(37,99,235,0.3)",borderRadius:8,padding:"4px 12px",color:BLUE,fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>Refresh</button>
        </div>
      </div>
      ...
      <div style={{padding:16}}>
        ...
        {!hasData ? (
          <div style={{background:CB,border:"1px solid "+BD,borderRadius:16,padding:24,marginBottom:12,textAlign:"center"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8,marginBottom:12}}>
          {[["O",lastC.open],["H",lastC.high],["L",lastC.low],["C",lastC.close],["V",(lastC.vol/1000).toFixed(0)+"K"]].map(function(r){
            return (
              <div key={r[0]} style={{background:CB,border:"1px solid "+BD,borderRadius:16,padding:"8px",textAlign:"center"}}>
                <div style={{fontSize:12,color:T2,marginBottom:4}}>{r[0]}</div>
                <div style={{fontFamily:"monospace",fontSize:12,fontWeight:700,color:T1}}>{typeof r[1]=="number"?r[1].toLocaleString("en-IN",{maximumFractionDigits:0}):r[1]}</div>
              </div>
            );
          })}
        </div>
        ...
        {/* Chart */}
        <div style={{background:CB,border:"1px solid "+BD,borderRadius:16,padding:12,marginBottom:12,overflow:"hidden"}}>
        ...
        <div style={{display:"flex",gap:8,marginBottom:12}}>
          {[["EMA 9/21",showEMA,function(){setShowEMA(!showEMA);},BLUE],["VWAP",showVWAP,function(){setShowVWAP(!showVWAP);},"#8B5CF6"],["RSI",showRSI,function(){setShowRSI(!showRSI);},rsiColor]].map(function(item){
            return (
              <button key={item[0]} onClick={item[2]} style={{background:item[1]?"rgba(255,255,255,0.1)":"rgba(255,255,255,0.04)",border:"1px solid "+(item[1]?item[3]:BD),borderRadius:20,padding:"4px 12px",color:item[1]?item[3]:T2,fontSize:12,fontWeight:item[1]?700:400,cursor:"pointer",fontFamily:"inherit"}}>
                {item[0]}
              </button>
            );
          })}
        ...
        {showRSI ? (
          <div style={{background:CB,border:"1px solid "+BD,borderRadius:16,padding:12,marginBottom:12}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
              <div style={{fontSize:12,fontWeight:700,color:T1}}>RSI (14)</div>
            ...
            {rsi!=null ? (
              <div>
                ...
                <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}>
                  <span style={{fontSize:12,color:G2}}>30 Oversold</span>
                  <span style={{fontSize:12,color:R}}>70 Overbought</span>
                </div>
              </div>
            ) : (
              <div style={{fontSize:12,color:T2}}>Not enough historical candles for this symbol/timeframe to compute RSI.</div>
            )}
          </div>
        ) : null}
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
          {indicators.map(function(ind){
            return (
              <div key={ind.label} style={{background:CB,border:"1px solid "+BD,borderRadius:16,padding:12}}>
                <div style={{fontSize:12,color:T2,marginBottom:4}}>{ind.label}</div>
                ...
                <div style={{fontSize:12,color:ind.color,marginTop:4}}>{ind.sub}</div>
              </div>
            );
          })}
        </div>
        ...
        {/* Legend */}
        <div style={{background:CB,border:"1px solid "+BD,borderRadius:16,padding:12,marginBottom:12}}>
          <div style={{fontSize:12,fontWeight:700,color:T1,marginBottom:8}}>Chart Legend</div>
          <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
            {[["Green candle","Bullish (Close > Open)",G],[" Red candle","Bearish (Close < Open)",R],["Yellow line","EMA 9",BLUE],["Blue line","EMA 21",theme.c.blue],["Purple dash","VWAP","#8B5CF6"]].map(function(l){
              return <div key={l[0]} style={{fontSize:12,color:T2}}><span style={{color:l[2],fontWeight:700}}>{l[0]}</span> = {l[1]}</div>;
            })}
          </div>
        </div>
        </div>
        ...
        <div style={{background:"rgba(249,115,22,0.08)",border:"1px solid rgba(249,115,22,0.2)",borderRadius:10,padding:12}}>
          <div style={{fontSize:12,color:theme.c.warn}}>
