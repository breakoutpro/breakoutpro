    ...
    <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:80}}>
      ...
      <div style={{background:CB,padding:"12px 14px",borderBottom:"1px solid "+BD}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
          <div>
            <div style={{fontSize:16,fontWeight:900,color:T1}}>Paper <span style={{color:G}}>Trading</span></div>
            <div style={{fontSize:8,color:"#F97316",fontWeight:600}}>Virtual Money Only  Educational Practice</div>
          </div>
        ...
        <div style={{display:"flex",gap:5,marginBottom:8}}>
          {[["trade","Trade"],["positions","Positions("+positions.length+")"],["history","History"]].map(function(t){
            var act=tab==t[0];
            return <button key={t[0]} onClick={function(){setTab(t[0]);}} style={{flex:1,background:act?G:"rgba(255,255,255,0.05)",border:"1px solid "+(act?G:BD),borderRadius:20,padding:"5px",color:act?"#fff":T2,fontSize:8,fontWeight:act?700:400,cursor:"pointer",fontFamily:"inherit"}}>{t[1]}</button>;
          })}
        </div>
      ...
      {tab=="trade"?(
        <div style={{padding:"12px 14px 0"}}>
          ...
          <div style={{background:"rgba(249,115,22,0.06)",border:"1px solid rgba(249,115,22,0.15)",borderRadius:10,padding:10}}>
            <div style={{fontSize:8,color:"#F97316",lineHeight:1.7}}>DISCLAIMER: This is a paper trading simulator for educational practice only. All money is virtual. No real money is involved. This is NOT investment advice. Not SEBI registered. Past simulated performance does not guarantee real market results. Always consult a SEBI registered advisor before real trading.</div>
          </div>
        </div>
      ):null}
      ...
      {tab=="positions"?(
        <div style={{padding:"12px 14px 0"}}>
          {positions.length==0?(
            ...
            var mktColor=MARKETS[p.market]?MARKETS[p.market].color:G;
            return (
      ...
      {tab=="history"?(
        <div style={{padding:"12px 14px 0"}}>
          ...
          {data.history.length==0?(
            ...
            var won=h.pnl>=0;
            var mktColor=MARKETS[h.market]?MARKETS[h.market].color:G;
            return (
