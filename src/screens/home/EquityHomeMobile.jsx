  ...
  return (
    <div style={{background:BG,minHeight:"100vh",width:"100%",boxSizing:"border-box",overflowX:"hidden",fontFamily:"'Inter',Arial,sans-serif",paddingBottom:84,color:T1}}>
      ...
      <div style={{width:"100%",boxSizing:"border-box",padding:"7px 9px 0"}}>
        ...
        <Card title="Breakout Intelligence" icon="&#128640;" onDetails={function(){setTab("scan");}}>
          ...
          <div style={{display:"flex",gap:8,overflowX:"auto",WebkitOverflowScrolling:"touch",width:"100%",boxSizing:"border-box"}}>
            {breakoutCards.map(function(c){
              ...
              var tone = c.category=="Strong Breakout"?UP:(c.category=="Watching"?DOWN:theme.c.gold);
              return (
                <div key={c.sym} onClick={function(e){e.stopPropagation(); data.selectStock(c.sym);}} style={{flexShrink:0,minWidth:120,background:theme.c.card2,border:"1px solid "+BD,borderRadius:10,padding:8,cursor:"pointer"}}>
        ...
        <Card title="Today's Key Levels" icon="&#128200;" hero={true} onDetails={function(){setTab("pazones");}}>
          {(function(){
            ...
            return (
              <div>
                {/* Tabs */}
                <div style={{display:"flex",gap:6,marginBottom:10}} onClick={function(e){e.stopPropagation();}}>
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:2}}>
                  <span style={{fontSize:15,fontWeight:900,color:T1,fontFamily:"monospace"}}>{formatMarketPrice(z.currentPrice)}</span>
                ...
                <div style={{width:"100%",boxSizing:"border-box"}}>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 8px",background:theme.c.gold+"1A",borderLeft:"3px solid "+theme.c.gold,borderRadius:4,margin:"3px 0"}}>
                    ...
                    <span style={{fontSize:12,fontWeight:900,color:theme.c.gold}}>{formatMarketPrice(z.currentPrice)}</span>
                  </div>
                  {z.support ? (
                    <div style={{display:"flex",justifyContent:"space-between",padding:"4px 8px"}}>
                ...
                <div style={{display:"flex",gap:8,width:"100%",boxSizing:"border-box"}} onClick={function(e){e.stopPropagation();}}>
        ...
        <Card title="Market Breadth" icon="&#128202;" onDetails={function(){setTab("markets");}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3, 1fr)",gap:6,textAlign:"center"}}>
            <div><div style={{fontSize:16,fontWeight:900,color:UP}}>{breadth.adv}</div><div style={{fontSize:9,color:T2}}>Advances</div></div>
            <div><div style={{fontSize:16,fontWeight:900,color:DOWN}}>{breadth.dec}</div><div style={{fontSize:9,color:T2}}>Declines</div></div>
            <div><div style={{fontSize:16,fontWeight:900,color:T1}}>{breadth.flat}</div><div style={{fontSize:9,color:T2}}>Unchanged</div></div>
          </div>
        </Card>

        <Card title="FII / DII Flow" icon="&#128176;">
          <div style={{display:"grid",gridTemplateColumns:"repeat(3, 1fr)",gap:6,textAlign:"center"}}>
            <div><div style={{fontSize:14,fontWeight:900,color:UP}}>+3,245</div><div style={{fontSize:9,color:T2}}>FII (Cr)</div></div>
            <div><div style={{fontSize:14,fontWeight:900,color:DOWN}}>-1,890</div><div style={{fontSize:9,color:T2}}>DII (Cr)</div></div>
            <div><div style={{fontSize:14,fontWeight:900,color:UP}}>+1,355</div><div style={{fontSize:9,color:T2}}>Net (Cr)</div></div>
          </div>
        </Card>

        <Card title="Top Gainers" icon="&#128200;">
          {GAINERS.slice(0,4).map(function(s){
            return (
              <div key={s.sym} style={{display:"flex",justifyContent:"space-between",padding:"4px 0",borderBottom:"1px solid "+BD}}>
        ...
        <Card title="Sector Performance" icon="&#127970;" onDetails={function(){setTab("markets");}}>
          {SECTORS.slice(0,5).map(function(s){
            var w = Math.min(100, Math.abs(s.pct)*20);
            return (
              <div key={s.name} style={{marginBottom:6}}>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:T2,marginBottom:2}}><span>{s.name}</span><span style={{color:s.up?UP:DOWN,fontWeight:700}}>{s.up?"+":""}{s.pct}%</span></div>
                <div style={{height:5,background:BD,borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",width:w+"%",background:s.up?UP:DOWN}}/></div>
              </div>
            );
          })}
        </Card>

        <Card title="NIFTY 50 Heatmap" icon="&#128293;">
          <div style={{display:"grid",gridTemplateColumns:"repeat(4, 1fr)",gap:6}}>
            {DEMO_STOCKS.slice(0,8).map(function(s){
              var up = s.chgPct>=0;
              return (
                <div key={s.sym} style={{background:up?"rgba(34,197,94,0.15)":"rgba(239,68,68,0.15)",border:"1px solid "+(up?UP:DOWN),borderRadius:6,padding:5,textAlign:"center"}}>
                  <div style={{fontSize:9,fontWeight:700,color:T1}}>{s.sym}</div>
