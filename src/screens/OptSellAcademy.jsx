...
export default function OptSellAcademy(props){
  ...
  var BD2 = theme.c.border2, BG = theme.c.bg, BLUE = theme.c.blue, CARD = theme.c.card, GOLD = theme.c.gold, T2 = theme.c.text2, T3 = theme.c.text3; T1=theme.c.text1; UP=theme.c.up;
      ...
      <div style={{background:BG,padding:"14px 14px 12px",borderBottom:"1px solid "+BD,position:"sticky",top:0,zIndex:10}}>
        ...
        <div style={{display:"flex",gap:6,marginTop:12}}>
          {[["dash","Dashboard"],["strat","Strategies"],["learn","Learn"]].map(function(t){
            var act=tab==t[0];
            return <button key={t[0]} onClick={function(){setTab(t[0]);}} style={{background:act?BLUE:"rgba(255,255,255,0.05)",border:"1px solid "+(act?BLUE:BD),borderRadius:9,padding:"6px 13px",color:act?"#04060D":T2,fontSize:11,fontWeight:act?800:600,cursor:"pointer",fontFamily:"inherit"}}>{t[1]}</button>;
          })}
        </div>
      </div>
      ...
      <div style={{padding:14}}>
        {tab=="dash"?(
          <div>
            ...
            <div style={{background:theme.c.card,border:"1px solid rgba(59,130,246,0.25)",borderRadius:13,padding:14,marginBottom:14}}>
            ...
            {/* Daily Hub entry */}
            <button onClick={function(){props.setTab&&props.setTab("optsellhub");}} style={{width:"100%",background:theme.c.card,border:"1px solid rgba(212,175,55,0.3)",borderRadius:13,padding:14,marginBottom:14,cursor:"pointer",fontFamily:"inherit",textAlign:"left",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <div>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:11}}>
              {Object.keys(CONDITIONS).map(function(c){
                var act=cond==c;
                return <button key={c} onClick={function(){setCond(c);}} style={{background:act?"rgba(59,130,246,0.15)":"rgba(255,255,255,0.05)",border:"1px solid "+(act?BLUE:BD),borderRadius:16,padding:"6px 12px",color:act?BLUE:T2,fontSize:10.5,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>{c}</button>;
              })}
            </div>
              ...
              {STRATEGIES.map(function(s){
                return (
                  <button key={s.id} onClick={function(){setSel(s.id);}} style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:13,cursor:"pointer",fontFamily:"inherit",textAlign:"left"}}>
                    ...
                    <div style={{fontSize:9,color:BLUE,marginTop:3,fontWeight:600}}>{s.bias}</div>
...
function Disclaimer(){
  ...
  return (
    <div style={{background:"rgba(249,115,22,0.06)",border:"1px solid rgba(249,115,22,0.15)",borderRadius:10,padding:11}}>
      <div style={{fontSize:8.5,color:theme.c.warn,lineHeight:1.5}}>Educational Content Only. This platform is for learning and market intelligence. It does not provide investment advice or trading recommendations. Please consult a SEBI-registered investment adviser before making investment decisions.</div>
    </div>
  );
                                           }
