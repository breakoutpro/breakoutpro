    ...
    <div style={{background:theme.c.bg,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
      ...
      <div style={{padding:14}}>
        ...
        <div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap"}}>
          <select value={filterStrategy} onChange={function(ev){setFilterStrategy(ev.target.value);}} style={{background:theme.c.card2,border:"1px solid "+BD,borderRadius:8,padding:"7px 8px",color:T1,fontSize:10,fontFamily:"inherit"}}>
            <option value="All">All Strategies</option>
            {STRATEGIES.map(function(s){ return <option key={s} value={s}>{s}</option>; })}
          </select>
          <select value={filterResult} onChange={function(ev){setFilterResult(ev.target.value);}} style={{background:theme.c.card2,border:"1px solid "+BD,borderRadius:8,padding:"7px 8px",color:T1,fontSize:10,fontFamily:"inherit"}}>
            <option value="All">Win/Loss: All</option>
            <option value="Win">Win Only</option>
            <option value="Loss">Loss Only</option>
          </select>
          <input value={filterSymbol} onChange={function(ev){setFilterSymbol(ev.target.value);}} placeholder="Symbol" style={{background:theme.c.card2,border:"1px solid "+BD,borderRadius:8,padding:"7px 8px",color:T1,fontSize:10,fontFamily:"inherit",width:90}}/>
          <input value={filterDate} onChange={function(ev){setFilterDate(ev.target.value);}} type="date" style={{background:theme.c.card2,border:"1px solid "+BD,borderRadius:8,padding:"7px 8px",color:T1,fontSize:10,fontFamily:"inherit"}}/>
        </div>
        ...
        <div style={{background:"rgba(249,115,22,0.06)",border:"1px solid rgba(249,115,22,0.15)",borderRadius:10,padding:11,marginTop:16}}>
          <div style={{fontSize:8.5,color:theme.c.warn,lineHeight:1.5}}>Educational journal only. All statistics are computed from your own logged trades. Not investment advice.</div>
        </div>
      </div>
    </div>
  );
