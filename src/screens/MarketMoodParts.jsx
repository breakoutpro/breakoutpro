...
export function SectionHead(props){
  ...
  return (
    <div style={{fontSize:12,color:MT.T2,fontWeight:800,margin:wide?"12px 0 6px":"22px 0 10px",letterSpacing:0.5}}>
      {props.children}
    </div>
  );
}
...
export function UnavailableCard(props){
  var theme = useTheme(); var MT = buildMT(theme);
  return (
    <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:14,padding:16,marginBottom:16}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",rowGap:6,marginBottom:8}}>
        <span style={{fontSize:12,fontWeight:800,color:MT.T1}}>{props.title}</span>
...
export function IndexRow(props){
  ...
  if(!row || !row.available){
    return (
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",rowGap:6,padding:"12px 16px",borderBottom:"1px solid "+MT.BD}}>
...
export function StageTimeline(props){
  ...
  return (
    <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:14,padding:16,marginBottom:16}}>
      <div style={{fontSize:12,fontWeight:800,color:MT.T1,marginBottom:12}}>Market Stage Timeline</div>
      {steps.map(function(s,i){
        return (
          <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"8px 0",borderTop:i>0?"1px solid "+MT.DIV:"none"}}>
            <div style={{width:8,height:8,borderRadius:"50%",background:i==steps.length-1?MT.BLUE:MT.T3,flexShrink:0}}></div>
            <span style={{fontSize:12,color:i==steps.length-1?MT.T1:MT.T2,fontWeight:i==steps.length-1?800:600}}>{s}</span>
          </div>
        );
      })}
    </div>
...
export function SectionHeadWithPill(props){
  ...
  var wide = responsive.isDesktop || responsive.isTV;
  return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",rowGap:6,margin:wide?"12px 0 6px":"22px 0 10px"}}>
...
export function AiCommentaryBlock(props){
  ...
  if(!ai){
    return (
      <div style={{background:MT.CARD2,border:"1px solid "+MT.BD,borderRadius:12,padding:16,marginBottom:16}}>
...
export function SemicircleGauge(props){
  ...
  var label = score!=null ? mood.label : "Unavailable";
  var col = MT.T2;
  if(score!=null){
    if(label.indexOf("Bull")>=0) col = MT.GREEN;
    else if(label.indexOf("Bear")>=0) col = MT.RED;
  ...
  // Needle: -90deg at score 0 (full left/BEARISH) to +90deg at score 100 (full right/BULLISH).
  var angle = score!=null ? -90 + (score/100)*180 : -90;
  // Strength: distance from the 50 midpoint, using the same real score - not a new data field.
  var strength = null;
  if(score!=null){
    var dist = Math.abs(score-50);
    strength = dist>=30 ? "Strong" : (dist>=12 ? "Moderate" : "Weak");
  }
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",width:"100%"}}>
      <div style={{position:"relative",width:180,height:96,overflow:"hidden"}}>
        <div style={{position:"absolute",top:0,left:0,width:180,height:180,borderRadius:"50%",background:"conic-gradient(from 180deg, "+MT.RED+" 0deg, "+MT.WARN+" 90deg, "+MT.GREEN+" 180deg, transparent 180deg)"}}/>
        <div style={{position:"absolute",top:14,left:14,width:152,height:152,borderRadius:"50%",background:MT.CARD}}/>
        <div style={{position:"absolute",bottom:0,left:"50%",width:2,height:78,background:MT.T1,transformOrigin:"bottom center",transform:"translateX(-50%) rotate("+angle+"deg)"}}/>
        <div style={{position:"absolute",bottom:-4,left:"50%",width:10,height:10,borderRadius:"50%",background:MT.T1,transform:"translateX(-50%)"}}/>
        <div style={{position:"absolute",top:8,left:8,fontSize:9,fontWeight:900,color:MT.BLUE,background:MT.BLUE+"18",border:"1px solid "+MT.BLUE+"55",borderRadius:5,padding:"2px 5px"}}>AI</div>
      </div>
      <div style={{display:"flex",justifyContent:"space-between",width:180,fontSize:9,color:MT.T3,fontWeight:700,marginTop:-4}}>
        <span>BEARISH</span><span>SIDEWAYS</span><span>BULLISH</span>
      </div>
      <div style={{fontSize:20,fontWeight:900,color:col,marginTop:6}}>{label.toUpperCase()}</div>
      <div style={{fontSize:12,color:MT.T2,marginTop:2}}>Confidence: {mood ? mood.confidence : "--"}</div>
      {strength ? <div style={{fontSize:12,color:MT.T3,marginTop:2}}>{strength}</div> : null}
    </div>
  );
}
