...
export default function PatternEduPro(props){
  ...
  var CARD = theme.c.card, GOLD = theme.c.gold, T2 = theme.c.text2, T3 = theme.c.text3; T1=theme.c.text1; UP=theme.c.up;
  ...
  return (
    <div style={{background:BG,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
      <div style={{background:BG,padding:"12px 14px",borderBottom:"1px solid "+BD,display:"flex",alignItems:"center",gap:10,position:"sticky",top:0,zIndex:10}}>
        ...
        <button onClick={voiceExplain} style={{background:"rgba(59,130,246,0.12)",border:"1px solid rgba(59,130,246,0.3)",borderRadius:9,padding:"7px 11px",color:BLUE,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>&#128266; Listen</button>
      </div>
      ...
      <div style={{padding:14}}>
        ...
        <div style={{background:"rgba(249,115,22,0.06)",border:"1px solid rgba(249,115,22,0.15)",borderRadius:10,padding:11}}>
          <div style={{fontSize:9,color:theme.c.warn}}>Educational only. Patterns are not guarantees. Not investment advice.</div>
        </div>
      </div>
    </div>
  );
...
function Row(props){
  return (
    <div style={{display:"flex",justifyContent:"space-between",padding:"11px 13px",borderBottom:props.last?"none":"1px solid "+BD}}>
      ...
      <span style={{fontSize:10.5,color:T1,textAlign:"right",maxWidth:200}}>{props.val}</span>
    </div>
  );
}
function tabBtn(act,col){
  return {flex:1,background:act?(col==UP?"rgba(0,143,57,0.12)":"rgba(220,38,38,0.12)"):"rgba(120,120,120,0.06)",border:"1px solid "+(act?col:BD),borderRadius:9,padding:"8px",color:act?col:T2,fontSize:10.5,fontWeight:700,cursor:"pointer",fontFamily:"inherit"};
}
function statBox(){ return {background:CARD,border:"1px solid "+BD,borderRadius:10,padding:"10px 6px",textAlign:"center"}; }
