...
export default function RiskCalcPractice(props){
  ...
  return (
    <div style={{padding:16}}>
      ...
      <div style={{fontSize:16,fontWeight:800,color:T1,marginBottom:8}}>Risk Management Calculator Practice</div>
      <div style={{fontSize:12,color:T3,marginBottom:16,lineHeight:1.5}}>Enter your own practice numbers below - these are not real market prices, just values for you to experiment with.</div>

      <label style={{fontSize:12,color:T2,fontWeight:700}}>Practice Capital (Rs)</label>
      <input style={inputStyle()} type="number" value={capital} onChange={function(ev){setCapital(ev.target.value);}}/>

      <label style={{fontSize:12,color:T2,fontWeight:700}}>Risk per Trade (%)</label>
      ...
      <label style={{fontSize:12,color:T2,fontWeight:700}}>Practice Target Level (optional)</label>
      ...
      <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:16,marginTop:8}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid "+BD}}>
          <span style={{fontSize:12,color:T2}}>Amount at risk</span>
          <span style={{fontSize:12,fontWeight:700,color:R}}>Rs {riskAmount.toFixed(2)}</span>
        </div>
        <div style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid "+BD}}>
          <span style={{fontSize:12,color:T2}}>Suggested max quantity</span>
          <span style={{fontSize:12,fontWeight:700,color:T1}}>{qty} units</span>
        </div>
        <div style={{display:"flex",justifyContent:"space-between",padding:"8px 0"}}>
          <span style={{fontSize:12,color:T2}}>Risk : Reward ratio</span>
