        ...
        <div style={{padding:14}}>
          ...
          <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:16,marginBottom:14}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
              ...
              <span onClick={function(){bookmarks.toggleBookmark(d.id);}} style={{fontSize:20,color:saved?WARN:T3,cursor:"pointer",minWidth:44,minHeight:44,display:"flex",alignItems:"center",justifyContent:"center"}}>{saved?"\u2605":"\u2606"}</span>
            </div>
            <span style={{fontSize:12,fontWeight:700,color:STATUS_COLOR[d.status],background:STATUS_COLOR[d.status]+"18",border:"1px solid "+STATUS_COLOR[d.status]+"40",borderRadius:6,padding:"3px 9px"}}>{d.status}</span>
          ...
          <div style={{background:"rgba(212,175,55,0.06)",border:"1px solid rgba(212,175,55,0.15)",borderRadius:10,padding:11}}>
            <div style={{fontSize:12,color:WARN,lineHeight:1.5}}>Illustrative educational entry, not a confirmed live dividend announcement. Breakout Pro has no real dividend data feed. Never a prediction of future dividends.</div>
          </div>
        </div>
      </div>
    );
    ...
    <div style={{background:theme.c.bg,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
      <div style={{background:CARD,padding:"12px 14px",borderBottom:"1px solid "+BD,display:"flex",alignItems:"center",gap:10}}>
        ...
        <div>
          ...
          <div style={{fontSize:12,color:WARN}}>{DEMO_LABEL}</div>
        </div>
      </div>
      ...
      <div style={{padding:14}}>
        <div style={{background:"rgba(212,175,55,0.06)",border:"1px solid rgba(212,175,55,0.15)",borderRadius:10,padding:11,marginBottom:14}}>
          <div style={{fontSize:12,color:WARN,lineHeight:1.5}}>Breakout Pro has no live dividend data feed connected. All entries below are illustrative examples for learning purposes, not a confirmed announcement. Never a prediction of future dividends, never investment advice.</div>
        </div>

        <input value={search} onChange={function(e){setSearch(e.target.value);}} placeholder="Search company..." style={{width:"100%",background:CARD,border:"1px solid "+BD,borderRadius:9,padding:"10px 12px",color:T1,fontSize:14,fontFamily:"inherit",boxSizing:"border-box",marginBottom:12}}/>

        <div style={{display:"flex",gap:6,marginBottom:16,flexWrap:"wrap"}}>
          {["All","Upcoming","Recent","History"].map(function(s){
            var act=s==filter;
            return <button key={s} onClick={function(){setFilter(s);}} style={{background:act?BLUE:"transparent",border:"1px solid "+(act?BLUE:BD),borderRadius:9,padding:"9px 12px",color:act?"#fff":T2,fontSize:12,fontWeight:act?800:600,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>{s}</button>;
          })}
        </div>
        ...
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,marginBottom:16}}>
          ...
          <div style={{fontSize:12,color:"#C9D4E5",lineHeight:1.6}}>{generateInsight(DIVIDENDS)}</div>
        </div>

        <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10}}>DIVIDEND TIMELINE ({sorted.length})</div>
        {sorted.length==0 ? (
          <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:20,textAlign:"center",color:T3,fontSize:12}}>No entries match your filters.</div>
        ) : sorted.map(function(d){
          var saved = bookmarks.isBookmarked(d.id);
          return (
            <div key={d.id} onClick={function(){setDetailId(d.id);}} style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,marginBottom:10,cursor:"pointer"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                ...
                <span onClick={function(e2){ e2.stopPropagation(); bookmarks.toggleBookmark(d.id); }} style={{fontSize:18,color:saved?WARN:T3,minWidth:44,minHeight:44,display:"flex",alignItems:"center",justifyContent:"center"}}>{saved?"\u2605":"\u2606"}</span>
              </div>
              <span style={{fontSize:12,fontWeight:700,color:STATUS_COLOR[d.status],background:STATUS_COLOR[d.status]+"18",border:"1px solid "+STATUS_COLOR[d.status]+"40",borderRadius:6,padding:"3px 9px",marginTop:8,display:"inline-block"}}>{d.status}</span>
            </div>
          );
        })}

        <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10,marginTop:16}}>EDUCATIONAL SECTION</div>
