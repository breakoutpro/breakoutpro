...
// Home trading signals. Neutral icon box, price-only colors, tap for pattern detail.
...
// Pattern detail overlay - explains the pattern and what may come next.
function PatternDetail(props){
  var s=props.s;
  var info=getPattern(s.pattern);
  var bc=biasColor(info.bias);
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.7)",zIndex:200,display:"flex",alignItems:"flex-end"}} onClick={props.onClose}>
      <div onClick={function(e){e.stopPropagation();}} style={{background:BG,borderTop:"1px solid "+BD,borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"80vh",overflowY:"auto",padding:"18px 18px 30px"}}>
        <div style={{width:38,height:4,borderRadius:2,background:BD,margin:"0 auto 16px"}}></div>

        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
          <div style={{width:50,height:50,borderRadius:12,background:CARD2,border:"1px solid "+BD,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
            {s.ptype=="candle"?<CandleIcon bull={s.bull}/>:<ChartIcon bull={s.bull}/>}
          </div>
          <div style={{flex:1}}>
            <div style={{fontSize:17,fontWeight:900,color:T1}}>{s.sym}</div>
            <div style={{fontSize:10,color:T3,marginTop:2}}>Frame: {s.frame}  &#8226;  {s.time}</div>
          </div>
          <span style={{background:bc+"22",color:bc,border:"1px solid "+bc+"44",borderRadius:8,padding:"4px 11px",fontSize:11,fontWeight:800}}>{info.bias}</span>
        </div>

        <div style={{fontSize:15,fontWeight:800,color:T1,marginBottom:14}}>{s.pattern}</div>

        <div style={{marginBottom:14}}>
          <div style={{fontSize:10,color:CYAN,fontWeight:700,marginBottom:5}}>WHAT IS IT</div>
        ...
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,marginBottom:14}}>
          <div style={{fontSize:10,color:bc,fontWeight:700,marginBottom:5}}>WHAT MAY HAPPEN NEXT</div>
          <div style={{fontSize:12.5,color:T1,lineHeight:1.7}}>{info.next}</div>
        </div>

        <div style={{marginBottom:16}}>
          <div style={{fontSize:10,color:GOLD,fontWeight:700,marginBottom:5}}>HOW TO USE</div>
          <div style={{fontSize:12.5,color:"#D4D8E0",lineHeight:1.7}}>{info.action}</div>
        </div>

        <button onClick={function(){props.setTab("scan");}} style={{width:"100%",background:"rgba(59,130,246,0.12)",border:"1px solid "+BLUE,borderRadius:12,padding:12,color:CYAN,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",marginBottom:10}}>Open Chart &#8594;</button>

        <div style={{background:"rgba(249,115,22,0.06)",border:"1px solid rgba(249,115,22,0.15)",borderRadius:10,padding:10}}>
          <div style={{fontSize:8,color:"#F97316",lineHeight:1.6}}>Educational only. Not SEBI registered. Not investment advice.</div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{padding:"14px 14px 0"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:9}}>
        ...
        <button onClick={function(){setTab("scan");}} style={{background:"none",border:"none",color:CYAN,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>View All &#8594;</button>
      </div>

      <div style={{background:CARD,border:"1px solid "+BD,borderRadius:14,overflow:"hidden"}}>
        {SIGNALS.map(function(s,i){
          return (
            <div key={s.sym+i} onClick={function(){setSel(s);}} style={{display:"flex",alignItems:"center",gap:12,padding:"13px 13px",borderBottom:i<SIGNALS.length-1?"1px solid "+BD:"none",cursor:"pointer"}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:11,fontWeight:700,color:T2,display:"flex",alignItems:"center",gap:3}}>
                  {s.pattern}
                  <span style={{color:s.bull?UP:DOWN}} dangerouslySetInnerHTML={{__html:s.bull?"&#8599;":"&#8600;"}}/>
                </div>
              </div>

              {/* PRICE + CHANGE - colors only here */}
      ...
      {sel?<PatternDetail s={sel} onClose={function(){setSel(null);}} setTab={setTab}/>:null}
    </div>
  );
}
