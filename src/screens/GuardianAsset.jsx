...
export default function GuardianAsset(props){
  var _dev=useTheme().device;
  var _big=(_dev=="laptop"||_dev=="desktop"||_dev=="ultra");
  var _mid=(_dev=="tablet");
  var a=props.asset;
    ...
    <div style={{background:BG,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
      ...
      <div style={{padding:14}}>
        ...
        <Sec t="MULTI-TIMEFRAME OBSERVATION">
          <div style={{background:CARD,border:"1px solid "+BD,borderRadius:13,padding:_big?"10px 18px":"8px 14px"}}>
            {mtf.map(function(m,i){
              var dc=m.dir>0?UP:m.dir<0?DOWN:T3;
              var ar=m.dir>0?"&#9650;":m.dir<0?"&#9660;":"&#9644;";
              return (
                <div key={i} style={{display:"flex",alignItems:"center",gap:_big?16:13,padding:_big?"13px 0":"11px 0",borderBottom:i<mtf.length-1?"1px solid "+BD2:"none"}}>
                  <span style={{fontSize:_big?14:_mid?12.5:11.5,fontWeight:800,color:T2,width:_big?52:42,flexShrink:0}}>{m.tf}</span>
                  <span style={{fontSize:_big?12:10,color:dc,flexShrink:0}} dangerouslySetInnerHTML={{__html:ar}}/>
                  <span style={{flex:1,fontSize:_big?13.5:_mid?12.5:12,fontWeight:600,color:m.label?dc:T3}}>{m.label||"Neutral"}</span>
                  {m.time?<span style={{fontSize:_big?11:9.5,color:T3,fontFamily:"monospace"}}>{m.time}</span>:null}
                </div>
              );
            })}
          </div>
