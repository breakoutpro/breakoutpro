    ...
    <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:80}}>
      ...
      <div style={{padding:"12px 14px"}}>
        ...
        <div style={{marginBottom:12}}>
          <div style={{fontSize:10,fontWeight:700,color:T3,letterSpacing:0.8,marginBottom:8}}>STRATEGY</div>
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            {STRATEGIES.map(function(s){
              var act=strategy==s.id;
              return (
                <div key={s.id} onClick={function(){setStrategy(s.id);setResult(null);}} style={{background:act?"rgba(59,130,246,0.12)":CB,border:"1px solid "+(act?BLUE:BD),borderRadius:12,padding:"10px 14px",cursor:"pointer",display:"flex",alignItems:"center",gap:10}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:9,color:T3,marginTop:2}}>{s.desc}</div>
                  </div>
                </div>
              );
            })}
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
          <div>
            <div style={{fontSize:10,fontWeight:700,color:T3,letterSpacing:0.8,marginBottom:6}}>SYMBOL</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
              {SYMBOLS.map(function(s){
                var act=symbol==s;
                return <button key={s} onClick={function(){setSymbol(s);setResult(null);}} style={{background:act?"rgba(245,158,11,0.2)":"rgba(255,255,255,0.04)",border:"1px solid "+(act?GOLD:BD),borderRadius:6,padding:"3px 8px",color:act?GOLD:T2,fontSize:8,fontWeight:act?700:400,cursor:"pointer",fontFamily:"inherit"}}>{s}</button>;
              })}
            </div>
          </div>
          <div>
            <div style={{fontSize:10,fontWeight:700,color:T3,letterSpacing:0.8,marginBottom:6}}>PERIOD</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
              {TIMEFRAMES.map(function(t){
                var act=timeframe==t;
                return <button key={t} onClick={function(){setTimeframe(t);setResult(null);}} style={{background:act?"rgba(139,92,246,0.2)":"rgba(255,255,255,0.04)",border:"1px solid "+(act?PURPLE:BD),borderRadius:6,padding:"3px 8px",color:act?"#A78BFA":T2,fontSize:8,fontWeight:act?700:400,cursor:"pointer",fontFamily:"inherit"}}>{t}</button>;
              })}
            </div>
          </div>
        </div>
        ...
        {/* Run Button */}
        <button onClick={runTest} disabled={running} style={{width:"100%",background:running?"rgba(59,130,246,0.3)":"linear-gradient(135deg,"+BLUE+","+PURPLE+")",border:"none",borderRadius:14,padding:"14px",color:"#fff",fontSize:13,fontWeight:700,cursor:running?"not-allowed":"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:14}}>
          {running?(
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
              {[
                ...
                return (
                  <div key={r[0]} style={{background:CB,border:"1px solid "+BD,borderRadius:12,padding:"12px 14px"}}>
            ...
            {/* Win/Loss bar */}
            <div style={{background:CB,border:"1px solid "+BD,borderRadius:14,padding:"12px 14px",marginBottom:12}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                <span style={{fontSize:10,fontWeight:700,color:UP}}>Wins: {result.wins}</span>
                <span style={{fontSize:10,fontWeight:700,color:DOWN}}>Losses: {result.losses}</span>
              </div>
              ...
              <div style={{display:"flex",justifyContent:"space-between",marginTop:6}}>
                <span style={{fontSize:9,color:UP}}>Avg Win: +{result.avgWin}%</span>
                <span style={{fontSize:9,color:DOWN}}>Avg Loss: -{result.avgLoss}%</span>
              </div>
            </div>

            {/* Trade Log */}
            <button onClick={function(){setShowTrades(function(p){return !p;});}} style={{width:"100%",background:"rgba(255,255,255,0.04)",border:"1px solid "+BD,borderRadius:10,padding:"10px",color:T2,fontSize:10,fontWeight:600,cursor:"pointer",fontFamily:"inherit",marginBottom:showTrades?8:12}}>
            ...
            {showTrades&&(
              <div style={{background:CB,border:"1px solid "+BD,borderRadius:12,overflow:"hidden",marginBottom:12}}>
                <div style={{display:"grid",gridTemplateColumns:"0.5fr 1fr 1fr 1fr",padding:"6px 12px",borderBottom:"1px solid "+BD}}>
                  {["#","Entry","Exit","P&L"].map(function(h){return <div key={h} style={{fontSize:8,fontWeight:700,color:T3}}>{h}</div>;})}
                </div>
                {result.trades.slice(0,20).map(function(t,i){
                  return (
                    <div key={i} style={{display:"grid",gridTemplateColumns:"0.5fr 1fr 1fr 1fr",padding:"7px 12px",borderBottom:i<result.trades.length-1?"1px solid rgba(30,42,77,0.4)":"none",alignItems:"center"}}>
                      <div style={{fontSize:9,color:T3}}>#{i+1}</div>
                      <div style={{fontSize:9,color:T1,fontFamily:"monospace"}}>{fmtN(t.entry)}</div>
                      <div style={{fontSize:9,color:T1,fontFamily:"monospace"}}>{fmtN(t.exit)}</div>
                      <div style={{fontSize:9,fontWeight:700,color:t.win?UP:DOWN}}>{t.win?"+":""}{t.pnlPct}%</div>
                    </div>
                  );
                })}
                {result.trades.length>20&&<div style={{padding:"8px 12px",fontSize:9,color:T3,textAlign:"center"}}>...and {result.trades.length-20} more trades</div>}
              </div>
            )}

            <div style={{background:"rgba(245,158,11,0.08)",border:"1px solid rgba(245,158,11,0.2)",borderRadius:10,padding:"8px 12px"}}>
              <div style={{fontSize:8,color:GOLD}}>&#9888; Educational simulation only. Past performance does not guarantee future results. Not SEBI registered. Not investment advice.</div>
            </div>
          </div>
        )}
      </div>
