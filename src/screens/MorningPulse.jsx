      ...
      <div style={{background:"linear-gradient(135deg,#050816,#0A1020)",padding:"14px 16px",borderBottom:"1px solid "+BD}}>
        ...
        <div style={{display:"flex",gap:5}}>
          {[["overview","Overview"],["global","Global"],["stocks","Stocks"],["ai","AI Analysis"]].map(function(t){
            var act=tab==t[0];
            return <button key={t[0]} onClick={function(){setTab2(t[0]);}} style={{flex:1,background:act?"rgba(0,200,83,0.15)":"rgba(255,255,255,0.04)",border:"1px solid "+(act?G:BD),borderRadius:8,padding:"6px 4px",color:act?G2:T2,fontSize:9,fontWeight:act?700:400,cursor:"pointer",fontFamily:"inherit"}}>{t[1]}</button>;
          })}
        </div>
      </div>
          ...
          <div>
            {!pulse&&!loading?(
              <div style={{textAlign:"center",padding:"40px 20px"}}>
                ...
                <button onClick={getPulse} style={{background:G,border:"none",borderRadius:14,padding:"14px 30px",color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Get AI Pulse</button>
              </div>
            ):null}
