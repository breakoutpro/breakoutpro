...
export default function MarketMood(props){
  ...
  var contentMax = responsive.isDesktop || responsive.isTV ? Math.max(1160, responsive.shell.centerMax) : responsive.shell.centerMax;
  ...
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:MT.BG,zIndex:350,overflowY:"auto"}}>
      ...
      <div style={{background:MT.CARD,borderBottom:"1px solid "+MT.BD,position:"sticky",top:0,zIndex:5}}>
        <div style={{display:"flex",alignItems:"center",gap:12,padding:"16px",maxWidth:contentMax,margin:"0 auto",boxSizing:"border-box"}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:22,fontWeight:800,color:MT.T1}}>AI Market Mood</div>
            <div style={{fontSize:12,color:MT.T2,marginTop:4,display:"flex",alignItems:"center",gap:4,flexWrap:"wrap"}}>
      ...
      <div style={{padding:"18px " + (responsive.shell.pad>16?responsive.shell.pad:16) + "px 40px",maxWidth:contentMax,margin:"0 auto",width:"100%",boxSizing:"border-box"}}>
        ...
        {showLoading ? (
          ...
          <>
        {/* WHAT: current mood, score, strength, confidence, stage - the primary signal */}
        <SectionHead>AI MARKET MOOD</SectionHead>
        <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:16,padding:"24px 16px 16px",marginBottom:8}}>
          <Gauge mood={mood}/>
        </div>

        <button onClick={onVoice} style={{width:"100%",background:MT.BLUE,border:"none",borderRadius:13,padding:16,color:"#fff",fontSize:14,fontWeight:800,cursor:"pointer",fontFamily:"inherit",marginBottom:16,display:"flex",alignItems:"center",justifyContent:"center",gap:8,minHeight:44}}>
        ...
        <div style={{display:wideLayout?"grid":"block",gridTemplateColumns:wideLayout?"1fr 1fr":undefined,gap:wideLayout?16:0,marginBottom:16}}>
        ...
        <div style={{fontSize:11,color:MT.T3,marginBottom:16}}>
        ...
        <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:14,overflow:"hidden",marginBottom:16}}>
          <IndexRow row={niftyRow}/>
          <IndexRow row={sensexRow}/>
          <IndexRow row={bankRow}/>
        </div>
        <div style={{fontSize:12,color:MT.T3,marginTop:-8,marginBottom:16}}>
        ...
        {vixHist.available ? (
          <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderTop:"none",borderRadius:"0 0 14px 14px",padding:"12px 16px 16px",marginBottom:16}}>
        ...
        {/* RISK LEVEL - genuinely derived from the real VIX price above, not invented */}
        ...
        <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:14,padding:16,marginBottom:16}}>
          {mood && mood.score!=null ? (
            <div style={{fontSize:14,fontWeight:800,color:mood.label.indexOf("Bullish")>=0?MT.GREEN:(mood.label.indexOf("Bearish")>=0?MT.RED:MT.WARN)}}>{mood.label}</div>
          ) : <div style={{fontSize:12,color:MT.T2}}>No verified mood data available right now.</div>}
          <div style={{fontSize:12,color:MT.T3,marginTop:6}}>Educational market bias only. Not a trading instruction.</div>
        </div>
        ...
        <div style={{background:MT.CARD2,border:"1px solid "+MT.BD,borderRadius:14,padding:16,marginBottom:16}}>
