...
export default function Guardian(props){
  ...
  var BG = theme.c.bg, CARD = theme.c.card, GOLD = theme.c.gold, T2 = theme.c.text2; T1=theme.c.text1; UP=theme.c.up;
  ...
  return (
    <div style={{background:theme.c.bg,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",display:"flex",flexDirection:"column"}}>
      <div style={{padding:"14px 14px 12px",borderBottom:"1px solid #1B2330",display:"flex",alignItems:"center",gap:10}}>
        {props.onBack?<button onClick={props.onBack} style={{background:"rgba(255,255,255,0.06)",border:"none",borderRadius:8,width:32,height:32,color:"#FFFFFF",fontSize:16,cursor:"pointer"}}>&#8592;</button>:null}
        <span style={{fontSize:15,fontWeight:900,color:"#FFFFFF"}}>AI Market Guardian</span>
      </div>
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:24,textAlign:"center"}}>
        <div style={{maxWidth:320}}>
          <div style={{fontSize:13,color:theme.c.text2,lineHeight:1.7}}>This feature is being rebuilt with real data - check back soon.</div>
        </div>
      </div>
    </div>
  );
...
function ObsCard(props){
  var theme = useTheme(); // reuses the existing ThemeProvider - no new theme system
  // Theme-sourced overrides - shadow the module-level hardcoded fallbacks above.
  var BD2 = theme.c.border2, CARD = theme.c.card, CARD2 = theme.c.card2, GOLD = theme.c.gold, T2 = theme.c.text2, T3 = theme.c.text3; T1=theme.c.text1; UP=theme.c.up;
...
function Disclaimer(){
  ...
  return (
    <div style={{background:"rgba(249,115,22,0.06)",border:"1px solid rgba(249,115,22,0.15)",borderRadius:10,padding:11}}>
      <div style={{fontSize:8.5,color:theme.c.warn,lineHeight:1.5}}>Educational Market Observation Only. Breakout Pro organizes observed market structure for learning purposes. It does not provide investment advice, trading recommendations or future price predictions.</div>
    </div>
  );
}
