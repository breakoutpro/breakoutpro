  ...
  return (
    <div style={{background:BG,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:80}}>
      ...
      <div style={{padding:"10px 0 0"}}>
        <div style={{display:"flex",gap:8,overflowX:"auto",padding:"0 16px",WebkitOverflowScrolling:"touch"}}>
          {indices.map(function(idx){
            return (
              <div key={idx.label} onClick={function(){setSelIdx(idx);}} style={{background:CARD,border:"1px solid "+BD,borderRadius:14,padding:"10px 12px",flexShrink:0,minWidth:108,cursor:"pointer"}}>
                <div style={{fontSize:8,fontWeight:700,color:T3,marginBottom:4,whiteSpace:"nowrap"}}>{idx.label}</div>
                <div style={{fontSize:13,fontWeight:800,color:T1,marginBottom:2}}>{idx.ltp.toLocaleString("en-IN",{maximumFractionDigits:2})}</div>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span style={{fontSize:9,fontWeight:700,color:idx.up?UP:DOWN}}>{idx.up?"+":""}{idx.pct}%</span>
                  <MiniChart d={idx.spark} col={idx.up?UP:DOWN} w={36} h={16}/>
                </div>
              </div>
            );
          })}
        ...
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,overflow:"hidden"}}>
          <div style={{padding:"8px 12px",borderBottom:"1px solid "+BD,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              <div style={{width:6,height:6,borderRadius:"50%",background:UP,boxShadow:"0 0 6px "+UP}}></div>
              <span style={{fontSize:10,fontWeight:800,color:T1}}>NIFTY 50</span>
              <span style={{fontSize:8,color:T2}}>Live Chart</span>
            </div>
            <span style={{fontSize:8,color:BLUE2,fontWeight:600}}>TradingView</span>
          </div>
          <div style={{height:300,position:"relative"}}>
            <iframe
              src="https://s.tradingview.com/widgetembed/?frameElementId=tv_chart_nifty&symbol=CAPITALCOM%3AIN50&interval=D&hidesidetoolbar=1&hidetoptoolbar=0&symboledit=0&saveimage=0&toolbarbg=101B2E&studies=%5B%5D&theme=dark&style=1&timezone=Asia%2FKolkata&locale=en&withdateranges=1&hide_legend=0&utm_source=breakoutpro.in&utm_medium=widget&utm_campaign=chart"
              style={{width:"100%",height:"100%",border:"none",display:"block"}}
              title="NIFTY 50 TradingView Chart"
              loading="lazy"
            />
          </div>
        </div>
