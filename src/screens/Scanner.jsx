  ...
  return (
    <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:20}}>
      ...
      <div style={{padding:14}}>
        <div style={{background:CB,border:"1px solid "+BD,borderRadius:12,padding:16,marginBottom:14}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
            <div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:6,marginTop:4}}>
                <span style={{background:s.up?"rgba(0,200,83,0.15)":"rgba(239,68,68,0.15)",color:s.up?G2:R,borderRadius:6,padding:"2px 8px",fontSize:12,fontWeight:700}}>
        ...
        <div style={{background:CB,border:"1px solid "+BD,borderRadius:12,padding:14,marginBottom:14}}>
          <div style={{fontSize:12,fontWeight:700,color:T1,marginBottom:10}}>Scanner Signal</div>
        ...
        <div style={{background:"rgba(249,115,22,0.08)",border:"1px solid rgba(249,115,22,0.2)",borderRadius:10,padding:10}}>
          <div style={{fontSize:12,color:"#F97316",lineHeight:1.7}}>Educational only. Not SEBI registered. Not investment advice. Do your own research before trading.</div>
        </div>
      </div>
    </div>
  );
...
export default function ScannerScreen(props) {
  ...
  var G=theme.c.up, G2=theme.c.up, R=theme.c.down, GOLD=theme.c.gold;
    ...
    <div style={{background:DB,minHeight:"100%",paddingBottom:80,fontFamily:"Inter,Arial,sans-serif"}}>
      ...
      <div style={{display:"flex",gap:6,overflowX:"auto",padding:"12px 14px",background:CB,borderBottom:"1px solid "+BD}}>
        {SCANS.map(function(sc){
          ...
          return (
            <button key={sc.id} style={{background:act?G:"rgba(255,255,255,0.06)",border:"1px solid "+(act?G:BD),borderRadius:20,padding:"7px 16px",color:act?"#fff":T2,fontSize:12,fontWeight:act?700:500,cursor:"pointer",fontFamily:"inherit",flexShrink:0}} onClick={function(){runScan(sc.id);}}>
              {sc.label}
            </button>
          );
        })}
      ...
      <div style={{background:"rgba(0,200,83,0.05)",borderBottom:"1px solid rgba(0,200,83,0.1)",padding:"8px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <span style={{fontSize:12,fontWeight:700,color:G}}>{results.length} found</span>
      </div>

      <div style={{padding:"10px 14px"}}>
        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10}}>
          <span style={{fontSize:12,fontWeight:800,color:"#F59E0B",background:"rgba(245,158,11,0.12)",border:"1px solid rgba(245,158,11,0.3)",padding:"2px 7px",borderRadius:5,letterSpacing:0.5}}>DEMO DATA</span>
              ...
              return (
                <div key={s.sym} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",borderBottom:"1px solid "+BD,cursor:"pointer"}} onClick={function(){setSelStock(s);}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:12,color:T2,marginTop:1}}>{s.sect}</div>
                  </div>
                  ...
                  <div style={{textAlign:"right",minWidth:80}}>
                    ...
                    <div style={{background:s.up?"rgba(0,200,83,0.15)":"rgba(239,68,68,0.15)",borderRadius:5,padding:"1px 6px",fontSize:12,fontWeight:700,color:s.up?G2:R,marginTop:2}}>
                      {s.up?"+":""}{s.chgPct.toFixed(2)}%
                    </div>
                  </div>
