...
export default function MarketMood(props){
  ...
  var padV = wideLayout ? "4px 6px" : 10;
  // Explicit, fixed row heights matching the ultra-compact 100%-zoom spec.
  var ROW1_H = wideLayout ? 90 : null;
  var ROW2_H = wideLayout ? 55 : null;
  var ROW3_H = wideLayout ? 50 : null;
  var ROW4_H = wideLayout ? 55 : null;
  var ROW5_H = wideLayout ? 45 : null;
    ...
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:MT.BG,zIndex:350,overflowY:"auto"}}>
      ...
      <div style={{padding:wideLayout?"4px 8px":"10px 12px 16px",maxWidth:1800,margin:"0 auto",width:"100%",boxSizing:"border-box"}}>
        ...
        <div style={{display:wideLayout?"grid":"block",gridTemplateColumns:wideLayout?"1fr 1fr":undefined,gridTemplateRows:ROW1_H?ROW1_H+"px":undefined,gap:gapV,marginBottom:gapV,alignItems:"stretch"}}>
          <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:8,padding:padV,height:"100%",boxSizing:"border-box",display:"flex",flexDirection:"row",alignItems:"center",gap:8,overflow:"hidden",minWidth:0}}>
            {/* Compact semi-circle gauge - needle angle mapped from the real 0-100 mood score, arc colored by the same real sentiment as everywhere else on this page. */}
            <svg width="80" height="40" viewBox="0 0 80 40" style={{flexShrink:0}}>
              <path d="M 6 38 A 34 34 0 0 1 74 38" fill="none" stroke={MT.BD} strokeWidth="6" strokeLinecap="round"/>
              <path d={(function(){
                var pct = Math.max(0, Math.min(100, mood.score))/100;
                var ang = Math.PI - pct*Math.PI;
                var x = 40 + 34*Math.cos(ang), y = 38 - 34*Math.sin(ang);
                var largeArc = pct>0.5 ? 1 : 0;
                return "M 6 38 A 34 34 0 " + largeArc + " 1 " + x.toFixed(1) + " " + y.toFixed(1);
              })()} fill="none" stroke={moodColor} strokeWidth="6" strokeLinecap="round"/>
              <circle cx={(function(){ var pct=Math.max(0,Math.min(100,mood.score))/100; var ang=Math.PI-pct*Math.PI; return (40+34*Math.cos(ang)).toFixed(1); })()} cy={(function(){ var pct=Math.max(0,Math.min(100,mood.score))/100; var ang=Math.PI-pct*Math.PI; return (38-34*Math.sin(ang)).toFixed(1); })()} r="3.5" fill={moodColor}/>
            </svg>
            <div style={{minWidth:0,overflow:"hidden"}}>
              <div style={{fontSize:12,fontWeight:900,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}><span style={{color:MT.T1}}>Market Mood: </span><span style={{color:moodColor}}>{mood.label.toUpperCase()}</span></div>
              <div style={{fontSize:10,color:MT.T2,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{mood.stage} &nbsp;&#183;&nbsp; {mood.confidence} {t("confidence")} &nbsp;&#183;&nbsp; {t("ai_score")}: {mood.score}/100</div>
            </div>
          </div>
        ...
        <div style={{display:wideLayout?"grid":"block",gridTemplateColumns:wideLayout?"1fr 3fr":undefined,gridTemplateRows:ROW2_H?ROW2_H+"px":undefined,gap:gapV,marginBottom:gapV,alignItems:"stretch"}}>
          <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:8,padding:"4px 6px",height:"100%",boxSizing:"border-box",display:"flex",flexDirection:"column",justifyContent:"center",overflow:"hidden",minWidth:0}}>
              {(function(){
          ...
          {(function(){
            ...
            return (
          <div style={{background:MT.CARD2,border:"1px solid "+MT.BD,borderRadius:8,padding:"4px 6px",height:"100%",boxSizing:"border-box",display:wideLayout?"grid":"block",gridTemplateColumns:wideLayout?"repeat(4, 1fr)":undefined,gap:6,overflowY:"auto",minWidth:0}}>
          {wideLayout || haveWhy ? (
          <div style={{minWidth:0,overflow:"hidden"}}>
        ...
        <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:8,padding:"4px 6px",marginBottom:gapV,height:ROW3_H?ROW3_H+"px":undefined,boxSizing:"border-box",overflow:"hidden"}}>
          <div style={{display:wideLayout?"grid":"block",gridTemplateColumns:wideLayout?"repeat(4, 1fr)":"repeat(2, minmax(0, 1fr))",gap:6,height:"100%"}}>
            {[["NIFTY", niftyRow], ["BANKNIFTY", bankRow], ["SENSEX", sensexRow], ["VIX", vixRow]].map(function(pair,i){
              ...
              return (
                <div key={i} onClick={clickable?function(){setSelectedIdx(key);}:undefined} style={{background:MT.CARD2,border:"1px solid "+MT.BD,borderRadius:6,padding:"3px 5px",boxSizing:"border-box",minWidth:0,overflow:"hidden",cursor:clickable?"pointer":"default",display:"flex",flexDirection:"column",justifyContent:"center"}}>
                  {row.available ? (
                    <div>
                      <div style={{display:"flex",alignItems:"baseline",justifyContent:"space-between",gap:4}}>
                        <span style={{fontSize:10,fontWeight:700,color:MT.T2,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{row.name}</span>
                        <span style={{fontSize:11,fontWeight:800,color:MT.T1,whiteSpace:"nowrap"}}>{row.ltp}</span>
                      </div>
                      <div style={{fontSize:9,fontWeight:700,color:chgColor}}>{row.up==null?"":(row.up?"\u25b2 ":"\u25bc ")}{row.chgPct}</div>
                      {!usingDemo && data && data.indexHistory && data.indexHistory[key] && data.indexHistory[key].points && data.indexHistory[key].points.length>=2 ? (
                        <Sparkline points={data.indexHistory[key].points} inverse={key==="VIX"} height={25}/>
                      ) : null}
                    </div>
                  ) : (
                    <div>
                      <div style={{fontSize:10,fontWeight:700,color:MT.T2}}>{row.name}</div>
                      <div style={{fontSize:9,color:MT.T2}}>{t("data_unavailable")}</div>
                    </div>
                  )}
                </div>
              );
        ...
        <div style={{display:wideLayout?"grid":"block",gridTemplateColumns:wideLayout?"repeat(4, 1fr)":undefined,gridTemplateRows:ROW4_H?ROW4_H+"px":undefined,gap:gapV,marginBottom:gapV,alignItems:"stretch"}}>
          <div style={{marginBottom:wideLayout?0:8,minWidth:0,height:wideLayout?"100%":undefined,display:wideLayout?"flex":undefined,flexDirection:wideLayout?"column":undefined,overflow:"hidden"}}>
            ...
            <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:6,overflow:wideLayout?"auto":"hidden",flex:wideLayout?1:undefined,minHeight:wideLayout?0:undefined}}>
            ...
            {sectorSection.available ? (
              <div style={{flex:wideLayout?1:undefined,minHeight:wideLayout?0:undefined,overflowY:wideLayout?"auto":undefined,display:"grid",gridTemplateColumns:"1fr 1fr",gap:3}}>
                {rankedSectors.map(function(s,i){
                  ...
                  return (
                    <div key={i} style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:5,padding:"2px 4px",minWidth:0,overflow:"hidden"}}>
                      <div style={{fontSize:9,color:MT.T1,fontWeight:700,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{s.name}</div>
                      <div style={{fontSize:9,color:color}}>{s.chgPct!=null ? (s.chgPct>=0?"+":"")+s.chgPct+"%" : "--"}</div>
                    </div>
                  );
                })}
              </div>
          ...
          <div style={{marginBottom:wideLayout?0:8,minWidth:0,height:wideLayout?"100%":undefined,display:wideLayout?"flex":undefined,flexDirection:wideLayout?"column":undefined,overflow:"hidden"}}>
            ...
            {usingDemo ? (
              <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:6,padding:"3px 4px",flex:wideLayout?1:undefined,minHeight:wideLayout?0:undefined,overflow:"hidden"}}>
        ...
        <div style={{display:wideLayout?"grid":"block",gridTemplateColumns:wideLayout?"repeat(4, 1fr)":undefined,gridTemplateRows:ROW5_H?ROW5_H+"px":undefined,gap:gapV,marginBottom:gapV,alignItems:"stretch"}}>
          <div style={{marginBottom:wideLayout?0:8,minWidth:0,height:wideLayout?"100%":undefined,display:wideLayout?"flex":undefined,flexDirection:wideLayout?"column":undefined,overflow:"hidden"}}>
            ...
            {usingDemo ? (
              <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:6,padding:"3px 5px",overflow:"hidden"}}>
          ...
          <div style={{marginBottom:wideLayout?0:8,minWidth:0,height:wideLayout?"100%":undefined,display:wideLayout?"flex":undefined,flexDirection:wideLayout?"column":undefined,overflow:"hidden"}}>
            ...
            <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:6,padding:"3px 5px",flex:wideLayout?1:undefined,minHeight:wideLayout?0:undefined,overflow:"hidden"}}>
              {usingDemo ? (
          ...
          <div style={{background:MT.WARN+"14",border:"1px solid "+MT.WARN+"55",borderRadius:6,padding:"3px 5px",minWidth:0,boxSizing:"border-box",overflow:"hidden",display:"flex",flexDirection:"column",justifyContent:"center"}}>
