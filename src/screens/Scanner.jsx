...
var SCANS = [
  ...
  {id:"vwap",      label:"VWAP",       desc:"Trading above VWAP with strength"},
  {id:"emacross",  label:"EMA Cross",  desc:"Short-term EMA crossed above long-term EMA"},
  {id:"rsi",       label:"RSI",        desc:"RSI in overbought or oversold zone"},
  {id:"macd",      label:"MACD",       desc:"MACD line crossed above signal line"},
  {id:"supertrend",label:"Supertrend", desc:"Supertrend indicator flipped bullish"},
  {id:"high52",    label:"52W High",   desc:"Trading near its 52-week high"},
  {id:"low52",     label:"52W Low",    desc:"Trading near its 52-week low"},
  {id:"orb",       label:"ORB",        desc:"Broke out of the opening 15-min range"},
  {id:"momentum",  label:"Momentum",   desc:"Strong same-direction intraday momentum"},
  {id:"swing",     label:"Swing Breakout", desc:"Multi-day base breakout, swing setup"},
  {id:"delivery",  label:"Delivery Volume", desc:"High delivery percentage - genuine buying/selling"},
];

// Deterministic per-symbol pseudo-random seed, so simulated indicator values
// (RSI, MACD, Supertrend, 52-week proximity, ORB, swing base) stay stable
// across re-renders instead of flickering on every scan - same symbol always
// produces the same demo indicator reading. Educational/demo only.
function symSeed(sym, salt){
  var s = sym + "_" + salt;
  var h = 0;
...
function simRSI(s){
  // biased toward the direction of today's move, but with spread
  var base = s.up ? 55 : 45;
  var spread = (symSeed(s.sym,"rsi")-0.5) * 50;
  var v = base + spread + s.chgPct*3;
  return Math.max(5, Math.min(95, Math.round(v)));
}
function simMacdBullish(s){ return (symSeed(s.sym,"macd") > (s.up?0.35:0.65)); }
function simSupertrendUp(s){ return (symSeed(s.sym,"st") > (s.up?0.3:0.7)); }
function sim52wPct(s, which){
  // 0 = at the low, 1 = at the high
  var v = symSeed(s.sym, which);
  if(which=="high") return s.up ? 0.75+v*0.25 : v*0.6;
  return s.up ? v*0.4 : 0.6+v*0.4;
}
function simORBBreak(s){ return symSeed(s.sym,"orb") > (s.up?0.4:0.75) && Math.abs(s.chgPct) >= 1.0; }
function simSwingBase(s){ return symSeed(s.sym,"swing") > 0.55 && s.up && s.chgPct >= 1.2; }
...
function StockDetail(props) {
  ...
  var spark = genSpark(s.ltp,{points:14,additive:true,bias:s.up?0.4:0.6});
...
export default function ScannerScreen(props) {
  ...
  function runScan(type) {
    ...
    if(type=="vwap")      res = src.filter(function(s){return s.up && s.chgPct >= 0.8;});
    if(type=="emacross")  res = src.filter(function(s){return s.up && symSeed(s.sym,"ema") > 0.45;});
    if(type=="rsi")       res = src.filter(function(s){var r=simRSI(s); return r>=70 || r<=30;});
    if(type=="macd")      res = src.filter(function(s){return simMacdBullish(s);});
    if(type=="supertrend")res = src.filter(function(s){return simSupertrendUp(s);});
    if(type=="high52")    res = src.filter(function(s){return sim52wPct(s,"high") >= 0.85;});
    if(type=="low52")     res = src.filter(function(s){return sim52wPct(s,"low") <= 0.15;});
    if(type=="orb")       res = src.filter(function(s){return simORBBreak(s);});
    if(type=="momentum")  res = src.filter(function(s){return Math.abs(s.chgPct) >= 2.0;});
    if(type=="swing")     res = src.filter(function(s){return simSwingBase(s);});
    if(type=="delivery")  res = src.filter(function(s){return symSeed(s.sym,"delivery") > 0.5 && s.up;});
    ...
    <div style={{background:DB,minHeight:"100%",paddingBottom:80,fontFamily:"Inter,Arial,sans-serif"}}>
      ...
      <div style={{padding:"12px 16px"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
          ...
          <span style={{fontSize:12,color:T3}}>Simulated scan results for preview. Not live market values.</span>
        </div>
        {results.length==0?(
          <div style={{textAlign:"center",padding:"40px 0"}}>
            <div style={{fontSize:14,color:T2,marginBottom:8}}>No stocks match this scanner</div>
            <div style={{fontSize:12,color:T3}}>Try a different scan type</div>
          </div>
        ):(
