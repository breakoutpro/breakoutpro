...
export function SemicircleGauge(props){
  ...
  // Strength: distance from the 50 midpoint, using the real score - not a new data field.
  var strength = null;
  if(score!=null){
  ...
  function arcPath(cx,cy,r,a0,a1){
    var s = polarToCartesian(cx,cy,r,a0), e = polarToCartesian(cx,cy,r,a1);
    var largeArc = (a1-a0) <= 180 ? 0 : 1;
    return "M "+s.x+" "+s.y+" A "+r+" "+r+" 0 "+largeArc+" 1 "+e.x+" "+e.y;
  }
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",width:"100%"}}>
      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4,width:180}}>
        <span style={{fontSize:9,fontWeight:900,color:MT.BLUE,background:MT.BLUE+"18",border:"1px solid "+MT.BLUE+"55",borderRadius:5,padding:"2px 5px"}}>AI</span>
        <span style={{fontSize:10,fontWeight:800,color:MT.T2,letterSpacing:0.3}}>AI MARKET MOOD</span>
      </div>
      <svg width="180" height="94" viewBox="0 0 180 94">
        <path d={arcPath(90,90,80,0,60)} stroke={MT.RED} strokeWidth="14" fill="none" strokeLinecap="round"/>
        <path d={arcPath(90,90,80,60,120)} stroke={MT.WARN} strokeWidth="14" fill="none"/>
        <path d={arcPath(90,90,80,120,180)} stroke={MT.GREEN} strokeWidth="14" fill="none" strokeLinecap="round"/>
        <g transform={"rotate("+needleRot+" 90 90)"}>
          <line x1="90" y1="90" x2="20" y2="90" stroke={MT.T1} strokeWidth="2.5" strokeLinecap="round"/>
        </g>
        <circle cx="90" cy="90" r="5" fill={MT.T1}/>
      </svg>
      <div style={{display:"flex",justifyContent:"space-between",width:180,fontSize:9,color:MT.T3,fontWeight:700,marginTop:2}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,marginTop:2}}>
        <span style={{fontSize:12,color:MT.T2}}>Confidence: {mood ? mood.confidence : "--"}</span>
        {strength ? <span style={{fontSize:10,fontWeight:800,color:col,background:col+"18",border:"1px solid "+col+"55",borderRadius:20,padding:"2px 9px"}}>{strength}</span> : null}
      </div>
      <div style={{fontSize:9,color:MT.T3,marginTop:6,textAlign:"center"}}>Educational market intelligence only. Not investment advice.</div>
    </div>
  );
}
