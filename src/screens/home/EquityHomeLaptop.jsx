...
export default function EquityHomeLaptop(props){
  ...
  // Market Breadth - real aggregation (count of up/down/flat) over the demo
  // stock set. This is a genuine calculation, not a fabricated number - but
  // the underlying stock set is demo data, so tagged accordingly.
      ...
      <div style={{flex:1,display:"flex",minHeight:0,width:"100%"}}>
        ...
        <div style={{flex:1,minWidth:0,padding:"12px 16px",overflowY:"auto"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 2fr",gap:tierCfg.gridGap,marginBottom:tierCfg.gridGap}}>
            <div onClick={function(){setTab("marketmood");}} style={{background:CARD2,border:"1px solid "+BD,borderRadius:10,padding:tierCfg.cardPadding,height:tierCfg.aiBriefHeight,overflowY:"auto",boxSizing:"border-box",cursor:"pointer"}}>
            ...
            <div onClick={function(){setTab("alerts");}} style={{background:CARD2,border:"1px solid "+BD,borderRadius:10,padding:tierCfg.cardPadding,height:tierCfg.aiBriefHeight,overflowY:"auto",boxSizing:"border-box",cursor:"pointer"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
            ...
            <div onClick={function(){setTab("scan");}} style={{background:CARD2,border:"1px solid "+BD,borderRadius:10,padding:tierCfg.cardPadding,height:tierCfg.breakoutIntelHeight,overflowY:"auto",boxSizing:"border-box",cursor:"pointer"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat("+tierCfg.breakoutCards+", minmax(0, 1fr))",gap:tierCfg.gridGap}}>
                {breakoutCards.map(function(card){
                  ...
                  return (
                    <div key={card.sym} onClick={function(e){e.stopPropagation(); data.selectStock(card.sym);}} style={{background:BG,border:"1px solid "+BD,borderRadius:9,padding:10,cursor:"pointer",minWidth:0,overflow:"hidden"}}>
                      ...
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginTop:2,gap:8}}>
                        <div style={{flexShrink:0}}>
                          <div style={{fontSize:tierCfg.primaryNumber,fontWeight:900,color:T1,fontFamily:"monospace"}}>{formatMarketPrice(card.ltp,{maximumFractionDigits:0})}</div>
                      ...
                      <div style={{borderTop:"1px solid "+BD2,marginTop:6,paddingTop:6,fontSize:tierCfg.label,color:T3}}>
                        {card.nearLabel} {formatMarketPrice(card.nearLevel,{maximumFractionDigits:0})} &middot; Vol {card.volumeConfirmed?"Confirmed":"Avg"}
                      </div>
                    </div>
                  );
                })}
          ...
          {/* Row 1.5: Market Breadth (50%) | FII/DII Flow (50%) - real aggregation + demo-labeled flow */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:tierCfg.gridGap,marginBottom:tierCfg.gridGap}}>
            <div onClick={function(){setTab("markets");}} style={{background:CARD2,cursor:"pointer",border:"1px solid "+BD,borderRadius:10,padding:tierCfg.cardPadding,boxSizing:"border-box"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                <span style={{fontSize:tierCfg.widgetTitle,fontWeight:800,color:T1}}>Market Breadth</span>
                <ProvenanceBadge type="calculated"/>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(3, 1fr)",gap:8,textAlign:"center"}}>
                <div><div style={{fontSize:tierCfg.primaryNumber,fontWeight:900,color:UP}}>{breadth.adv}</div><div style={{fontSize:tierCfg.label,color:T3}}>Advances</div></div>
                <div><div style={{fontSize:tierCfg.primaryNumber,fontWeight:900,color:DOWN}}>{breadth.dec}</div><div style={{fontSize:tierCfg.label,color:T3}}>Declines</div></div>
                <div><div style={{fontSize:tierCfg.primaryNumber,fontWeight:900,color:T2}}>{breadth.flat}</div><div style={{fontSize:tierCfg.label,color:T3}}>Unchanged</div></div>
              </div>
              <div style={{fontSize:tierCfg.label,color:T3,marginTop:8}}>Breadth: {breadth.total>0?Math.round(breadth.adv/breadth.total*100):0}% advancing</div>
            </div>
            <div onClick={function(){setTab("fiidii");}} style={{background:CARD2,cursor:"pointer",border:"1px solid "+BD,borderRadius:10,padding:tierCfg.cardPadding,boxSizing:"border-box"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                <span style={{fontSize:tierCfg.widgetTitle,fontWeight:800,color:T1}}>FII / DII Flow</span>
                <ProvenanceBadge type="demo"/>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(3, 1fr)",gap:8,textAlign:"center"}}>
                <div><div style={{fontSize:tierCfg.primaryNumber,fontWeight:900,color:UP}}>+3,245</div><div style={{fontSize:tierCfg.label,color:T3}}>FII (Cr)</div></div>
                <div><div style={{fontSize:tierCfg.primaryNumber,fontWeight:900,color:DOWN}}>-1,890</div><div style={{fontSize:tierCfg.label,color:T3}}>DII (Cr)</div></div>
                <div><div style={{fontSize:tierCfg.primaryNumber,fontWeight:900,color:UP}}>+1,355</div><div style={{fontSize:tierCfg.label,color:T3}}>Net (Cr)</div></div>
              </div>
            </div>
          </div>

          {/* Row 2.5: Top Gainers (50%) | Top Losers (50%) - demo-labeled */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:tierCfg.gridGap,marginBottom:tierCfg.gridGap}}>
            <div onClick={function(){setTab("markets");}} style={{background:CARD2,cursor:"pointer",border:"1px solid "+BD,borderRadius:10,padding:tierCfg.cardPadding,boxSizing:"border-box"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                <span style={{fontSize:tierCfg.widgetTitle,fontWeight:800,color:T1}}>Top Gainers</span>
                <ProvenanceBadge type="demo"/>
              </div>
              {GAINERS.slice(0,4).map(function(s){
                return (
                  <div key={s.sym} style={{display:"flex",justifyContent:"space-between",padding:"4px 0",borderBottom:"1px solid "+BD2}}>
                    <span style={{fontSize:tierCfg.secondaryText,fontWeight:700,color:T1}}>{s.sym}</span>
                    <span style={{fontSize:tierCfg.secondaryText,fontWeight:700,color:UP}}>+{s.pct}%</span>
                  </div>
                );
              })}
            </div>
            <div onClick={function(){setTab("markets");}} style={{background:CARD2,cursor:"pointer",border:"1px solid "+BD,borderRadius:10,padding:tierCfg.cardPadding,boxSizing:"border-box"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                <span style={{fontSize:tierCfg.widgetTitle,fontWeight:800,color:T1}}>Top Losers</span>
                <ProvenanceBadge type="demo"/>
              </div>
              {LOSERS.slice(0,4).map(function(s){
                return (
                  <div key={s.sym} style={{display:"flex",justifyContent:"space-between",padding:"4px 0",borderBottom:"1px solid "+BD2}}>
                    <span style={{fontSize:tierCfg.secondaryText,fontWeight:700,color:T1}}>{s.sym}</span>
                    <span style={{fontSize:tierCfg.secondaryText,fontWeight:700,color:DOWN}}>{s.pct}%</span>
                  </div>
                );
              })}
            </div>
          ...
          {/* Row 3.5: Sector Performance (50%) | Market Heatmap (50%) - demo-labeled */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:tierCfg.gridGap,marginBottom:tierCfg.gridGap}}>
            <div onClick={function(){setTab("markets");}} style={{background:CARD2,cursor:"pointer",border:"1px solid "+BD,borderRadius:10,padding:tierCfg.cardPadding,boxSizing:"border-box"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                <span style={{fontSize:tierCfg.widgetTitle,fontWeight:800,color:T1}}>Sector Performance</span>
                <ProvenanceBadge type="demo"/>
              </div>
              {SECTORS.slice(0,5).map(function(s){
                ...
                return (
                  <div key={s.name} style={{marginBottom:6}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:tierCfg.label,color:T2,marginBottom:2}}><span>{s.name}</span><span style={{color:s.up?UP:DOWN,fontWeight:700}}>{s.up?"+":""}{s.pct}%</span></div>
                    <div style={{height:5,background:BD2,borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",width:w+"%",background:s.up?UP:DOWN}}/></div>
                  </div>
                );
              })}
            </div>
            <div onClick={function(){setTab("heatmap");}} style={{background:CARD2,cursor:"pointer",border:"1px solid "+BD,borderRadius:10,padding:tierCfg.cardPadding,boxSizing:"border-box"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                <span style={{fontSize:tierCfg.widgetTitle,fontWeight:800,color:T1}}>NIFTY 50 Heatmap</span>
                <ProvenanceBadge type="demo"/>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(4, 1fr)",gap:6}}>
                {DEMO_STOCKS.slice(0,8).map(function(s){
                  var up = s.chgPct>=0;
                  return (
                    <div key={s.sym} style={{background:up?"rgba(34,197,94,0.15)":"rgba(239,68,68,0.15)",border:"1px solid "+(up?UP:DOWN),borderRadius:6,padding:6,textAlign:"center"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"11fr 9fr",gap:tierCfg.gridGap,marginBottom:tierCfg.gridGap}}>
            <div onClick={function(){setTab("pazones");}} style={{background:CARD2,border:"1px solid "+BD,borderRadius:10,padding:tierCfg.cardPadding,boxSizing:"border-box",cursor:"pointer"}}>
              ...
              {keyLevelsZones.currentPrice==null ? (
                <div style={{fontSize:tierCfg.secondaryText,color:T3,marginBottom:8}}>{UNAVAILABLE_TEXT}</div>
              ) : (
              <>
              <div style={{fontSize:tierCfg.primaryNumber,fontWeight:900,color:GOLD,fontFamily:"monospace",marginBottom:2}}>{formatMarketPrice(keyLevelsZones.currentPrice)}</div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(5, minmax(0, 1fr))",gap:6,textAlign:"center",marginBottom:10}}>
                ...
                <div><div style={{fontSize:tierCfg.label,color:GOLD}}>PIVOT</div><div style={{fontSize:tierCfg.secondaryText,fontWeight:800,color:GOLD,fontFamily:"monospace"}}>{formatMarketPrice(keyLevelsZones.currentPrice,{maximumFractionDigits:0})}</div></div>
            ...
            <div onClick={function(){setTab("news");}} style={{background:CARD2,cursor:"pointer",border:"1px solid "+BD,borderRadius:10,padding:tierCfg.cardPadding,boxSizing:"border-box"}}>
          ...
          {/* Range Intelligence - compact Home preview, full feature lives at tab="rangeintel" */}
          <div onClick={function(){setTab("rangeintel");}} style={{background:CARD2,border:"1px solid "+BD,borderRadius:10,padding:tierCfg.cardPadding,boxSizing:"border-box",marginBottom:tierCfg.gridGap,cursor:"pointer"}}>
            <span style={{fontSize:tierCfg.widgetTitle,fontWeight:800,color:T1}}>Range Intelligence</span>
            <div style={{fontSize:tierCfg.secondaryText,color:T3,marginTop:4}}>Multi-timeframe compression/expansion analysis.</div>
          </div>

          {/* Row 3: Options Intelligence (50%) | Futures Pulse (50%) */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:tierCfg.gridGap,marginBottom:tierCfg.gridGap}}>
            <div onClick={function(){setTab("optionsintelpro");}} style={{background:CARD2,cursor:"pointer",border:"1px solid "+BD,borderRadius:10,padding:tierCfg.cardPadding,boxSizing:"border-box"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
