...
export default function StockDetail(props){
  ...
  var BG = theme.c.bg, CARD = theme.c.card, T2 = theme.c.text2;
    ...
    <div style={{background:BG,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
      ...
      <div style={{background:BG,padding:"12px 14px",borderBottom:"1px solid "+BD,position:"sticky",top:0,zIndex:10}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div style={{flex:1}}>
            <div style={{fontSize:16,fontWeight:900,color:T1}}>{sym}</div>
            <div style={{fontSize:9,color:T2}}>{d.name}  &#8226;  {d.sector}</div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:18,fontWeight:900,fontFamily:"monospace",color:up?UP:DOWN}}>Rs {ltpNum.toLocaleString("en-IN")}</div>
            <div style={{fontSize:10,fontWeight:700,color:up?UP:DOWN}}>{up?"+":"-"}{Math.abs(pct).toFixed(2)}%</div>
          </div>
        </div>
      </div>
      ...
      <div style={{padding:"12px 14px 0"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
          {[["Open","Rs "+openP.toLocaleString("en-IN")],["High","Rs "+highP.toLocaleString("en-IN")],["Low","Rs "+lowP.toLocaleString("en-IN")],["Prev Close","Rs "+prevP.toLocaleString("en-IN")],["Mkt Cap",d.fundamentals[0].val],["Volume",d.delivery.volume]].map(function(r,i){
            return (
              <div key={i} style={{background:CARD,border:"1px solid "+BD,borderRadius:10,padding:"9px 8px"}}>
                <div style={{fontSize:8,color:T2}}>{r[0]}</div>
                <div style={{fontSize:11,fontWeight:700,color:T1,marginTop:2,fontFamily:"monospace"}}>{r[1]}</div>
              </div>
            );
          })}
        </div>
      ...
      {/* TAB SWITCHER */}
      <div style={{display:"flex",gap:6,overflowX:"auto",padding:"14px 14px 10px",position:"sticky",top:57,background:BG,zIndex:9}}>
        {TABS.map(function(t){
          var act=tab==t[0];
          return (
            <button key={t[0]} onClick={function(){setTab(t[0]);}} style={{background:act?CYAN:"rgba(255,255,255,0.05)",border:"1px solid "+(act?CYAN:BD),borderRadius:18,padding:"6px 13px",color:act?"#04060D":T2,fontSize:11,fontWeight:act?800:600,cursor:"pointer",fontFamily:"inherit",flexShrink:0,whiteSpace:"nowrap"}}>{t[1]}</button>
          );
        })}
      </div>
