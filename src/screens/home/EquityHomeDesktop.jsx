            ...
            <div style={{background:CARD2,border:"1px solid "+BD,borderRadius:10,padding:"8px 12px"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(4, 1fr)",gap:10}}>
                {breakoutCards.map(function(card){
                  ...
                  var h=22;
                  var pts = card.sparkline.map(function(v,i){ return (i/(card.sparkline.length-1))*100+","+(h-((v-min)/range)*(h-3)+1); }).join(" ");
                  return (
                    <div key={card.sym} onClick={function(){data.selectStock(card.sym);}} style={{background:BG,border:"1px solid "+BD,borderRadius:9,padding:10,cursor:"pointer"}}>
                      ...
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginTop:2,gap:8}}>
                        ...
                        <svg style={{flex:1,minWidth:0,height:h,display:"block"}} viewBox={"0 0 100 "+h} preserveAspectRatio="none"><polyline points={pts} fill="none" stroke={up?UP:DOWN} strokeWidth="1.3" vectorEffect="non-scaling-stroke"/></svg>
                      </div>
