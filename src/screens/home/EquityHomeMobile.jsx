...
export default function EquityHomeMobile(props){
  ...
  // Called DIRECTLY here - same reasoning as Desktop/Laptop - not routed
  // through useHomeData.js's own internal call, which was the actual break
  // point.
    ...
    <div style={{background:BG,minHeight:"100vh",width:"100%",boxSizing:"border-box",overflowX:"hidden",fontFamily:"'Inter',Arial,sans-serif",paddingBottom:84,color:T1}}>
      ...
      <div style={{width:"100%",boxSizing:"border-box",padding:"7px 9px 0"}}>
        ...
        {(function(){
          ...
          return (
            <div style={{background:CARD,border:"1px solid "+BD,borderRadius:8,padding:"7px 10px",marginBottom:9,display:"flex",flexWrap:"nowrap",overflowX:"auto",alignItems:"center",gap:14,whiteSpace:"nowrap"}}>
              <span style={{fontSize:10,fontWeight:800,color:T2,letterSpacing:0.5,flexShrink:0}}>MARKET PULSE</span>
              {pulseItems.map(function(it){
                return (
                  <div key={it.label} style={{display:"flex",alignItems:"center",gap:4,fontSize:11,flexShrink:0}}>
        ...
        {/* 4. AI MARKET MOOD */}
        <Card title={<span style={{display:"inline-flex",alignItems:"center",gap:5}}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={T1} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" style={{flexShrink:0}}><polyline points="2 14 8 14 10 8 14 18 16 11 19 11 22 14"/></svg>AI Market Mood</span>} onDetails={function(){setTab("marketmood");}}>
        ...
        {(function(){
          ...
          if(!hasRange){
            return (
              <Card title="Range Intelligence" onDetails={function(){setTab("rangeintel");}}>
          ...
          return (
            <Card title="Range Intelligence" onDetails={function(){setTab("rangeintel");}}>
              ...
              <div style={{fontSize:10,color:T3,marginBottom:4}}>VWAP: Not available (no real intraday tick feed connected)</div>
              {(function(){
                var widthPct = (Math.abs(tc - bc) / pivot) * 100;
                var cprStatus = widthPct < 0.15 ? "Narrow CPR" : "Wide CPR";
                return (
                  <span style={{fontSize:9,fontWeight:800,color:cprStatus==="Narrow CPR"?WARN:UP,background:(cprStatus==="Narrow CPR"?WARN:UP)+"18",border:"1px solid "+(cprStatus==="Narrow CPR"?WARN:UP)+"55",borderRadius:5,padding:"2px 6px",display:"inline-block",marginBottom:6}}>{cprStatus}</span>
                );
              })()}
              <div style={{fontSize:11,fontWeight:700,color:BLUE}}>Open Candlestick Range Chart &#8594;</div>
            </Card>
          );
        })()}
        ...
        <Card title="Market News" onDetails={function(){setTab("news");}}>
          <div style={{marginBottom:6}}><ProvenanceBadge type="calculated"/></div>
          {Array.isArray(safeLiveNews) && safeLiveNews.length > 0 ? safeLiveNews.filter(Boolean).slice(0,3).map(function(n){
            ...
            return (
              <div key={n.id} onClick={function(e){ if(n.link){ e.stopPropagation(); window.open(n.link, "_blank", "noopener"); } }} style={{padding:"5px 0",borderBottom:"1px solid "+BD,cursor:n.link?"pointer":"default"}}>
            ...
            <div style={{fontSize:11,color:T2}}>{liveNewsHook?.loading ? "Loading market news..." : "Market news unavailable"}</div>
          )}
        </Card>
        ...
        {/* LIVE BREAKOUTS - card 5 of 6. Same real /api/scanner-data source
            ...
            like the other five. */}
        <Card title="Live Breakouts" onDetails={function(){setTab("breakoutscan");}}>
