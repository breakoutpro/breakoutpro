      ...
      <div style={{background:CB,border:"1px solid "+BD,borderRadius:14,margin:"0 14px",overflow:"hidden"}}>
        ...
        <div style={{padding:"13px 14px",borderBottom:"1px solid "+BD}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:9}}>
            ...
            <div>
              <div style={{fontSize:12,fontWeight:600,color:T1}}>Theme</div>
              <div style={{fontSize:9,color:T2}}>Appearance of the app</div>
            </div>
          </div>
          <div style={{display:"flex",gap:6}}>
            {[["dark","Dark"],["light","Light"],["system","System"]].map(function(opt){
              var act=theme.mode==opt[0];
              return <button key={opt[0]} onClick={function(){theme.setTheme(opt[0]);}} style={{flex:1,background:act?BLUE:"rgba(255,255,255,0.05)",border:"1px solid "+(act?BLUE:BD),borderRadius:10,padding:"9px 6px",color:act?"#fff":T2,fontSize:11,fontWeight:act?700:500,cursor:"pointer",fontFamily:"inherit"}}>{opt[1]}</button>;
            })}
          </div>
        </div>
