...
import { ALERT_TYPES, alertType, alertCategory, alertMeta, priorityColor, getAlertEdu, getAiInsights, getMarketStatus, getTimeline, loadObservation, saveObservation, ASSET_SUGGEST, getAlertFeed, NIFTY50, BANKNIFTY, POPULAR_INDICES, POPULAR_OPTIONS, parseOptionContract, addBasket } from "./GuardianData";
      ...
      <div style={{padding:14}}>
        ...
        {tab=="watch"?(
          <div>
            ...
            {/* QUICK ADD - popular indices */}
            <div style={{fontSize:10.5,fontWeight:800,color:T2,letterSpacing:0.4,marginBottom:8}}>QUICK ADD &#8226; POPULAR INDICES</div>
            <div style={{display:"flex",gap:7,flexWrap:"wrap",marginBottom:13}}>
              {POPULAR_INDICES.map(function(a){
                var added=watch.filter(function(w){return w.sym==a.sym;}).length>0;
                return <button key={a.sym} onClick={function(){addAsset(a);}} disabled={added} style={{background:added?"rgba(34,197,94,0.1)":"rgba(59,130,246,0.1)",border:"1px solid "+(added?"rgba(34,197,94,0.3)":"rgba(59,130,246,0.25)"),borderRadius:16,padding:"7px 13px",color:added?UP:CYAN,fontSize:10.5,fontWeight:700,cursor:added?"default":"pointer",fontFamily:"inherit"}}>{added?"\u2713 ":"+ "}{a.sym}</button>;
              })}
            </div>

            {/* QUICK ADD - popular options */}
            <div style={{fontSize:10.5,fontWeight:800,color:T2,letterSpacing:0.4,marginBottom:8}}>QUICK ADD &#8226; POPULAR OPTIONS</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:7,marginBottom:13}}>
              {POPULAR_OPTIONS.map(function(a){
                var added=watch.filter(function(w){return w.sym==a.sym;}).length>0;
                return (
                  <button key={a.sym} onClick={function(){addAsset(a);}} disabled={added} style={{background:added?"rgba(34,197,94,0.08)":CARD,border:"1px solid "+(added?"rgba(34,197,94,0.3)":BD),borderRadius:10,padding:"9px 6px",cursor:added?"default":"pointer",fontFamily:"inherit",textAlign:"center"}}>
                    <div style={{fontSize:10,fontWeight:800,color:added?UP:T1}}>{a.sym.replace("NIFTY ","")}</div>
                    <div style={{fontSize:7,color:T3,marginTop:2}}>{a.note}</div>
                  </button>
                );
              })}
            </div>
            ...
            {/* typed option contract suggestion */}
            {(function(){
              var oc=parseOptionContract(query);
              if(!oc) return null;
              var added=watch.filter(function(w){return w.sym==oc.sym;}).length>0;
              return (
                <button onClick={function(){if(!added)addAsset(oc);}} disabled={added} style={{width:"100%",boxSizing:"border-box",background:added?"rgba(34,197,94,0.1)":"rgba(59,130,246,0.12)",border:"1px solid "+(added?"rgba(34,197,94,0.3)":"rgba(59,130,246,0.3)"),borderRadius:10,padding:"12px 13px",cursor:added?"default":"pointer",fontFamily:"inherit",textAlign:"left",marginBottom:10,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  <div>
                    <div style={{fontSize:12,fontWeight:800,color:added?UP:CYAN}}>{oc.sym}</div>
                    <div style={{fontSize:8,color:T3,marginTop:1}}>Option contract</div>
                  </div>
                  <span style={{fontSize:11,fontWeight:700,color:added?UP:CYAN}}>{added?"Added":"+ Add"}</span>
                </button>
              );
            })()}
