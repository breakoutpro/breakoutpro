...
function Tile(props){
  ...
  return (
    <div style={{background:CARD,border:"1px solid "+(c==null?BD:col),borderRadius:16,padding:12,minHeight:64}}>
      <div style={{fontSize:12,fontWeight:700,color:T1,marginBottom:8}}>{props.item.sym||props.item.name}</div>
      <div style={{fontSize:14,fontWeight:800,color:col}}>{c==null?"--":((c>=0?"+":"")+c.toFixed(2)+"%")}</div>
      <div style={{fontSize:12,color:T3,marginTop:4}}>{c==null?"Unavailable":(c>=0?"Up":"Down")}</div>
    </div>
  );
}
      ...
      <div style={{padding:16}}>
        <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
          {[{id:"nifty50",label:"Nifty 50"},{id:"banknifty",label:"Bank Nifty"},{id:"sectors",label:"Sectors"}].map(function(v){
            var act = v.id==view;
            return <button key={v.id} onClick={function(){setView(v.id);}} style={{flex:1,background:act?BLUE:"transparent",border:"1px solid "+(act?BLUE:BD),borderRadius:9,padding:"8px 4px",color:act?"#fff":T2,fontSize:12,fontWeight:act?800:600,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>{v.label}</button>;
          })}
        </div>
        ...
        <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
          <input value={search} onChange={function(e){setSearch(e.target.value);}} placeholder="Search symbol..." style={{flex:1,minWidth:140,background:CARD,border:"1px solid "+BD,borderRadius:16,padding:"8px 12px",color:T1,fontSize:12,fontFamily:"inherit"}}/>
          {["Default","Gainers","Losers","Alphabetical"].map(function(f){
            var act=f==filter;
            return <button key={f} onClick={function(){setFilter(f);}} style={{background:act?BLUE:"transparent",border:"1px solid "+(act?BLUE:BD),borderRadius:8,padding:"8px 12px",color:act?"#fff":T2,fontSize:12,fontWeight:act?800:600,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>{f}</button>;
          })}
        </div>
        ...
        {view=="sectors" ? (
          <div>
            {sectorStatus=="loading" ? (
              <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:24,textAlign:"center",color:T2,fontSize:12}}>Loading real sector data...</div>
            ) : sectorStatus=="error" ? (
              <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:24,textAlign:"center",color:R,fontSize:12}}>Data temporarily unavailable. <button onClick={loadSectors} style={{background:"none",border:"1px solid "+BLUE,borderRadius:16,padding:"8px 16px",color:BLUE,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",marginTop:8,minHeight:44}}>Retry</button></div>
            ) : (
              <div>
                {realDisplay.length>0 ? (
                  ...
                  <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:24,textAlign:"center",color:T3,fontSize:12,marginBottom:16}}>No real sector data available right now.</div>
                )}
                <div style={{background:"rgba(37,99,235,0.08)",border:"1px solid rgba(37,99,235,0.2)",borderRadius:8,padding:"8px 12px",marginBottom:12,display:"inline-block"}}>
                  <span style={{fontSize:12,fontWeight:700,color:T2}}>FINANCIAL / ENERGY - {DEMO_LABEL.toUpperCase()}</span>
                </div>
          ...
          <div>
            {demoDisplay.length==0 ? (
              <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:24,textAlign:"center",color:T3,fontSize:12}}>No entries match your search.</div>
            ) : (
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
        ...
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:16,marginBottom:16,marginTop:8}}>
          <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:8}}>RULE-BASED INSIGHT</div>
          <div style={{fontSize:12,color:T1,lineHeight:1.6}}>{generateInsight(view=="sectors"?realDisplay:demoDisplay)}</div>
        </div>
