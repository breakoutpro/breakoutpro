      ...
      <div style={{background:CARD,border:"1px solid "+(va.enabled?"rgba(34,197,94,0.3)":BD),borderRadius:14,padding:"14px"}}>
        ...
        {va.enabled?(
          <div>
              ...
              {[["normal","&#128276;","Normal"],["voice","&#128266;","Voice"],["priority","&#128680;","Priority"]].map(function(opt){
                ...
                return (
                  <button key={opt[0]} onClick={function(){va.setMode(opt[0]);}} style={{flex:1,background:act?"rgba(34,197,94,0.14)":"rgba(255,255,255,0.04)",border:"1px solid "+(act?"rgba(34,197,94,0.4)":BD),borderRadius:9,padding:"8px 4px",cursor:"pointer",fontFamily:"inherit",textAlign:"center"}}>
                    <div style={{fontSize:13}} dangerouslySetInnerHTML={{__html:opt[1]}}/>
                    <div style={{fontSize:9,color:act?UP:T3,fontWeight:act?700:600,marginTop:2}}>{opt[2]}</div>
                  </button>
                );
              })}
            </div>
                ...
                {LANGS.map(function(l){
                  ...
                  return (
                    <button key={l.id} onClick={function(){va.setLang(l.id);setShowLang(false);if(va.voiceOn)va.speak(l.native);}} style={{background:act?"rgba(59,130,246,0.15)":"rgba(255,255,255,0.04)",border:"1px solid "+(act?BLUE:BD),borderRadius:8,padding:"6px 11px",cursor:"pointer",fontFamily:"inherit"}}>
                      <span style={{fontSize:10,color:act?BLUE:T2,fontWeight:600}} dangerouslySetInnerHTML={{__html:l.native}}/>
                    </button>
                  );
                })}
              </div>
            ...
            {/* TEST BUTTON */}
            <button onClick={function(){va.fire(SIGNAL_FEED[0]);}} style={{width:"100%",marginTop:10,background:"rgba(255,255,255,0.04)",border:"1px solid "+BD,borderRadius:9,padding:"9px",color:BLUE,fontSize:10.5,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Test Alert</button>
          </div>
        ):null}
