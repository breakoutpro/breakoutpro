...
export function TechnicalTab(props){
  ...
  BLUE=theme.c.blue; 
  // Theme-sourced overrides - shadow the module-level hardcoded fallbacks above.
  var CARD = theme.c.card, BLUE = theme.c.gold, T2 = theme.c.text2; T1=theme.c.text1; UP=theme.c.up;
    ...
    <div>
      <Sec title="TREND">
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:16,textAlign:"center"}}>
          <div style={{fontSize:18,fontWeight:900,color:t.trend=="Uptrend"?UP:t.trend=="Downtrend"?DOWN:BLUE}}>{t.trend}</div>
        </div>
      </Sec>
      ...
      <Sec title="KEY LEVELS TODAY">
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,overflow:"hidden"}}>
          {levels.map(function(l,i){
            ...
            return (
              <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"12px 12px",borderBottom:i<levels.length-1?"1px solid "+BD:"none"}}>
                <span style={{fontSize:12,color:isR?DOWN:isS?UP:T2,fontWeight:700}}>{l[0]}</span>
          ...
          {[["EMA 20","Rs "+t.ema20],["EMA 50","Rs "+t.ema50],["VWAP","Rs "+t.vwap],["RSI",t.rsi],["MACD",t.macd]].map(function(r,i){
            return (
              <div key={i} style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:"12px 12px"}}>
                ...
                <div style={{fontSize:12,fontWeight:700,color:T1,marginTop:4}}>{r[1]}</div>
              </div>
            );
          })}
        </div>
...
export function OptionsTab(props){
  ...
  BLUE=theme.c.blue; 
  // Theme-sourced overrides - shadow the module-level hardcoded fallbacks above.
  var CARD = theme.c.card, BLUE = theme.c.gold, T2 = theme.c.text2; T1=theme.c.text1;
      ...
      <Sec title="ADVANCED OPTION SIGNALS">
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,overflow:"hidden"}}>
          {o.signals.map(function(s,i){
            return (
              <div key={i} style={{padding:"12px 12px",borderBottom:i<o.signals.length-1?"1px solid "+BD:"none"}}>
                <div style={{fontSize:12,fontWeight:700,color:BLUE,marginBottom:4}}>{s.label}</div>
                <div style={{fontSize:12,color:T2,lineHeight:1.5}}>{s.note}</div>
              </div>
            );
          })}
        </div>
      ...
      <Sec title="DELIVERY ANALYSIS">
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:8}}>
          {[["Delivery %",props.d.delivery.delPct],["Volume",props.d.delivery.volume],["Avg Vol",props.d.delivery.avgVol]].map(function(r,i){
            return (
              <div key={i} style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:"12px 8px",textAlign:"center"}}>
                <div style={{fontSize:12,color:T2}}>{r[0]}</div>
                <div style={{fontSize:12,fontWeight:800,color:T1,marginTop:4}}>{r[1]}</div>
              </div>
            );
          })}
        </div>
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:12}}>
          <div style={{fontSize:12,fontWeight:700,color:BLUE,marginBottom:4}}>Phase: {props.d.delivery.phase}</div>
          <div style={{fontSize:12,color:T2,lineHeight:1.5}}>{props.d.delivery.note}</div>
        </div>
      </Sec>
    </div>
  );
...
export function AITab(props){
  ...
  var d=props.d;
  var ai=d.ai;
  var probs=[["Bullish",ai.bull,UP],["Bearish",ai.bear,DOWN],["Sideways",ai.side,BLUE]];
  return (
    <div>
            ...
            return (
              <div key={i} style={{marginBottom:i<probs.length-1?12:0}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                  <span style={{fontSize:12,color:T1,fontWeight:600}}>{p[0]}</span>
      ...
      <Sec title="TRADING STYLE SCORE">
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,overflow:"hidden"}}>
          {d.scores.map(function(s,i){
            ...
            return (
              <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 12px",borderBottom:i<d.scores.length-1?"1px solid "+BD:"none"}}>
                <span style={{fontSize:12,color:T1}}>{s.label}</span>
                <span style={{fontSize:14,color:BLUE}} dangerouslySetInnerHTML={{__html:stars}}/>
              </div>
            );
          })}
        </div>
      ...
      <Sec title="STRENGTHS">
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:12}}>
          {d.strengths.map(function(x,i){
            return <div key={i} style={{fontSize:12,color:T1,padding:"4px 0"}}><span style={{color:UP}}>&#10003;</span>  {x}</div>;
          })}
        </div>
      </Sec>
      ...
      <Sec title="RISKS">
        <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:12}}>
          {d.risks.map(function(x,i){
            return <div key={i} style={{fontSize:12,color:T1,padding:"4px 0"}}><span style={{color:DOWN}}>&#33;</span>  {x}</div>;
          })}
        </div>
      </Sec>
          ...
          {[["Volatility",d.opinion.longterm],["Liquidity",d.opinion.swing],["Trend Style",d.opinion.intraday]].map(function(r,i){
            return (
              <div key={i} style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:"12px 8px",textAlign:"center"}}>
                ...
                <div style={{fontSize:12,fontWeight:800,color:T1}}>{r[1]}</div>
              </div>
            );
          })}
        </div>
      ...
      <div style={{background:"rgba(249,115,22,0.06)",border:"1px solid rgba(249,115,22,0.15)",borderRadius:10,padding:12,marginTop:4}}>
        <div style={{fontSize:12,color:theme.c.warn,lineHeight:1.5}}>Educational analysis only. Not SEBI registered. Not investment advice. No buy or sell recommendation.</div>
      </div>
    </div>
  );
}
