...
export default function MarketMood(props){
  var phase=getPhase();
  var mm=props.mm||null;
  // Prefer the real engine result from the card; fall back to static only if absent.
  var realMood = mm && mm.mood;
  var mood = realMood && realMood.score!=null ? { score:realMood.score, label:realMood.label, color:(realMood.label.indexOf("Bull")>=0?"#22C55E":realMood.label.indexOf("Bear")>=0?"#EF4444":"#F59E0B") } : computeMood();
  var ai = mm && mm.ai;
  var verdict = (ai && ai.now) ? ai.now : getVerdict();
    ...
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:MT.BG,zIndex:350,overflowY:"auto"}}>
      ...
      <div style={{padding:"18px 16px 36px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,margin:"4px 0 6px"}}>
          <span style={{fontSize:7,fontWeight:800,color:"#D4AF37",background:"rgba(212,175,55,0.12)",border:"1px solid rgba(212,175,55,0.3)",padding:"2px 6px",borderRadius:4}}>EDUCATIONAL DATA</span>
          <span style={{fontSize:8.5,color:MT.T3||"#5B6472"}}>Reference context, not live</span>
        </div>
