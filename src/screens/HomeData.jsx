...
function getSession(){
  ...
  if(mins>=0&&mins<9*60) return "globalEarly";
  if(mins>=9*60&&mins<9*60+15) return "premarket";
  if(mins>=9*60+15&&mins<15*60+30) return "live";
  if(mins>=15*60+30&&mins<17*60) return "closing";
  if(mins>=17*60&&mins<23*60+30) return "mcx";
  return "globalLate";
}

var SESSION_META={
  globalEarly:{label:"Global Markets",icon:"GM",col:BLUE},
  premarket:  {label:"Pre Market",     icon:"PM",col:GOLD},
  live:       {label:"Live Market",    icon:"LM",col:UP},
  closing:    {label:"Closing Summary",icon:"CS",col:BLUE2},
  mcx:        {label:"MCX Live",       icon:"MX",col:GOLD},
  globalLate: {label:"Global Preview", icon:"GP",col:BLUE},
  weekend:    {label:"Weekend Mode",   icon:"WK",col:T3},
};
