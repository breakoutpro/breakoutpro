      ...
      <div style={{padding:"18px " + (responsive.shell.pad>16?responsive.shell.pad:16) + "px 40px",maxWidth:contentMax,margin:"0 auto",width:"100%",boxSizing:"border-box"}}>
        ...
        <div style={{display:wideLayout?"grid":"block",gridTemplateColumns:wideLayout?"1fr 1fr":undefined,gap:wideLayout?16:0,marginBottom:16}}>
            ...
            <div style={{background:MT.CARD2,border:"1px solid "+MT.BD,borderRadius:12,padding:16,height:"100%",boxSizing:"border-box"}}>
              {!ai ? (
                ...
                <>
                  ...
                  {!ai.now && !ai.whatChanged && (!ai.keyDrivers || !ai.keyDrivers.length) ? <div style={{fontSize:12,color:MT.T2}}>No verified driver data available right now.</div> : null}
                </>
              )}
            </div>
          </div>
          <div>
            ...
            <div style={{background:MT.CARD2,border:"1px solid "+MT.BD,borderRadius:12,padding:16,height:"100%",boxSizing:"border-box"}}>
              {ai && ai.watchNext ? (
                <div style={{fontSize:12,color:MT.T1,lineHeight:1.6,marginBottom:ai.riskNote?8:0}}>{ai.watchNext}</div>
              ) : (
                <div style={{fontSize:12,color:MT.T2}}>No verified watch-level data available right now.</div>
              )}
              {ai && ai.riskNote ? <div style={{fontSize:12,color:MT.WARN,lineHeight:1.6}}>{ai.riskNote}</div> : null}
            </div>
          </div>
        </div>
