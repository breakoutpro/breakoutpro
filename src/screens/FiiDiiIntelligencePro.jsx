...
function NetCard(props){
  ...
  return (
    <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,flex:1}}>
      <div style={{fontSize:12,color:T2,marginBottom:6}}>{props.label}</div>
    ...
    <div style={{background:theme.c.bg,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
      <div style={{background:CARD,padding:"12px 14px",borderBottom:"1px solid "+BD,display:"flex",alignItems:"center",gap:10}}>
        ...
        <div>
          ...
          <div style={{fontSize:12,color:WARN}}>{DATA_LABEL}</div>
        </div>
      </div>
      ...
      <div style={{padding:14}}>
        <div style={{background:"rgba(212,175,55,0.06)",border:"1px solid rgba(212,175,55,0.15)",borderRadius:10,padding:11,marginBottom:14}}>
          <div style={{fontSize:12,color:WARN,lineHeight:1.5}}>Breakout Pro has no live FII/DII data feed connected. All figures below are fixed illustrative examples for learning purposes, not live institutional data. Never investment advice, never a market-direction prediction.</div>
        </div>

        <div style={{display:"flex",gap:6,marginBottom:16}}>
          {["Daily","Weekly","Monthly"].map(function(p){
            var act=p==period;
            return <button key={p} onClick={function(){setPeriod(p);}} style={{flex:1,background:act?BLUE:"transparent",border:"1px solid "+(act?BLUE:BD),borderRadius:9,padding:"9px 4px",color:act?"#fff":T2,fontSize:12,fontWeight:act?800:600,cursor:"pointer",fontFamily:"inherit",minHeight:44}}>{p} Flow</button>;
          })}
        </div>
        ...
        <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10}}>DII DASHBOARD</div>
        <div style={{display:"flex",gap:10,marginBottom:14}}>
        ...
        <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10}}>BUY VS SELL COMPARISON</div>
        ...
        <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10,marginTop:8}}>TIMELINE</div>
        <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>
          <input value={search} onChange={function(e){setSearch(e.target.value);}} placeholder="Search date (e.g. Mon, Tue)..." style={{flex:1,minWidth:160,background:CARD,border:"1px solid "+BD,borderRadius:9,padding:"9px 11px",color:T1,fontSize:12,fontFamily:"inherit"}}/>
          <input value={dateFilter} onChange={function(e){setDateFilter(e.target.value);}} type="date" style={{background:CARD,border:"1px solid "+BD,borderRadius:9,padding:"9px 11px",color:T1,fontSize:12,fontFamily:"inherit"}}/>
        </div>
