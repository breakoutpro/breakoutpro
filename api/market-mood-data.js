...
export default async function handler(req, res){
  ...
  for(var hk in histSymbols){
    ...
    try{
      ...
      if(hc && hc.length>=2){
        var hcloses = hc.map(function(x){ return Math.round(x.c*100)/100; });
        indexHistory[hk] = { status:"DELAYED", points:hcloses.slice(-10), sessions:hcloses.length };
      } else {
  ...
  if(vixHistory && vixHistory.points) indexHistory.VIX = { status:vixHistory.status, points:vixHistory.points, sessions:vixHistory.sessions };
