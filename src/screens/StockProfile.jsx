    ...
    <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:30}}>
      ...
      <div style={{display:"flex",background:CB,borderBottom:"1px solid "+BD,padding:"0 8px"}}>
        {[["overview","Overview"],["financials","Financials"],["news","News"],["technical","Technical"]].map(function(t){
          var act=tab==t[0];
          return <button key={t[0]} onClick={function(){setTab(t[0]);}} style={{flex:1,background:"none",border:"none",borderBottom:act?"2px solid "+BLUE:"2px solid transparent",padding:"11px 4px",color:act?BLUE:T2,fontSize:12,fontWeight:act?700:400,cursor:"pointer",fontFamily:"inherit"}}>{t[1]}</button>;
        })}
      </div>
      ...
      <div style={{padding:14}}>
          ...
          <div>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                ...
                <div style={{display:"flex",gap:5}}>
                  {[["5d","5D"],["1mo","1M"],["6mo","6M"],["1y","1Y"],["5y","5Y"],["max","All"]].map(function(p){
                    var act=range==p[0];
                    return <button key={p[0]} onClick={function(){setRange(p[0]);}} style={{background:act?"rgba(59,130,246,0.15)":"transparent",border:"1px solid "+(act?BLUE:BD),borderRadius:6,padding:"3px 7px",color:act?BLUE:T2,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>{p[1]}</button>;
                  })}
                </div>
              </div>

              {loading?(
                <div style={{background:CB,border:"1px solid "+BD,borderRadius:12,height:200,display:"flex",alignItems:"center",justifyContent:"center"}}>
                ...
                <div style={{background:CB,border:"1px solid "+BD,borderRadius:12,height:160,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:8}}>
                  ...
                  <button onClick={function(){fetchHistory(range);}} style={{marginTop:4,background:"transparent",border:"1px solid "+BLUE,borderRadius:10,padding:"7px 16px",color:BLUE,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Retry</button>
                </div>
              )}
            </div>
            ...
            {ipoInfo?(
              <div style={{background:"rgba(124,58,237,0.08)",border:"1px solid rgba(124,58,237,0.2)",borderRadius:10,padding:"8px 12px",marginBottom:12,display:"flex",justifyContent:"space-between"}}>
                ...
                <span style={{fontSize:12,fontWeight:700,color:PURPLE}}>{ipoInfo}</span>
              </div>
            ):null}
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:12}}>
              {[["52W High","Rs"+meta.high52],["52W Low","Rs"+meta.low52],["Mkt Cap",meta.mcap],["P/E Ratio",meta.pe],["P/B Ratio",meta.pb],["Sector",s.sect]].map(function(r){
                return (
                  <div key={r[0]} style={{background:CB,border:"1px solid "+BD,borderRadius:12,padding:"10px 8px",textAlign:"center"}}>
                    <div style={{fontSize:12,color:T2,marginBottom:3}}>{r[0]}</div>
            ...
            <div style={{background:"rgba(249,115,22,0.08)",border:"1px solid rgba(249,115,22,0.2)",borderRadius:10,padding:10}}>
              <div style={{fontSize:12,color:"#F97316"}}>Educational only. Not SEBI registered. Not investment advice.</div>
            </div>
          </div>
        ):null}
        ...
        {tab=="financials"?(
          <div>
            <div style={{fontSize:12,fontWeight:700,color:T2,marginBottom:10,letterSpacing:0.5}}>QUARTERLY RESULTS (Rs Cr)</div>
              ...
              return (
                <div key={i} style={{background:CB,border:"1px solid "+BD,borderRadius:12,padding:"12px 14px",marginBottom:8}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                    ...
                    <span style={{fontSize:12,fontWeight:700,color:q.up?G2:R,background:(q.up?"rgba(0,200,83,0.1)":"rgba(239,68,68,0.1)"),borderRadius:6,padding:"2px 8px"}}>{q.up?"+":""}{q.yoy}% YoY</span>
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                    <div>
                      <div style={{fontSize:12,color:T2,marginBottom:2}}>Revenue</div>
                      <div style={{fontSize:12,fontWeight:700,color:T1}}>Rs{q.revenue} Cr</div>
                    </div>
                    <div>
                      <div style={{fontSize:12,color:T2,marginBottom:2}}>Net Profit</div>
            ...
            <div style={{background:"rgba(249,115,22,0.08)",border:"1px solid rgba(249,115,22,0.2)",borderRadius:10,padding:10,marginTop:4}}>
              <div style={{fontSize:12,color:"#F97316"}}>Simulated data for demonstration. Verify with official filings before trading decisions.</div>
            </div>
          </div>
        ):null}
        ...
        {tab=="news"?(
          <div>
            <div style={{fontSize:12,fontWeight:700,color:T2,marginBottom:10,letterSpacing:0.5}}>LATEST NEWS - {s.sym}</div>
            {news.map(function(n,i){
              return (
                <div key={i} style={{background:CB,border:"1px solid "+BD,borderRadius:12,padding:"12px 14px",marginBottom:8}}>
                  <div style={{display:"flex",gap:6,marginBottom:6}}>
                    <span style={{fontSize:12,fontWeight:700,color:n.sentiment=="good"?G2:n.sentiment=="bad"?R:GOLD,background:(n.sentiment=="good"?"rgba(0,200,83,0.1)":n.sentiment=="bad"?"rgba(239,68,68,0.1)":"rgba(245,158,11,0.1)"),borderRadius:6,padding:"2px 8px"}}>{n.sentiment=="good"?"Positive":n.sentiment=="bad"?"Negative":"Neutral"}</span>
                  </div>
        ...
        {tab=="technical"?(
          <div>
            <div style={{background:CB,border:"1px solid "+BD,borderRadius:12,padding:14,marginBottom:12}}>
              <div style={{fontSize:12,fontWeight:700,color:T2,marginBottom:10}}>PRICE ACTION ({range.toUpperCase()})</div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
              {[["RSI (14)",(45+Math.random()*30).toFixed(1)],["EMA 21","Rs"+(s.ltp*0.98).toFixed(2)],["EMA 50","Rs"+(s.ltp*0.95).toFixed(2)],["Volume Trend",s.up?"Above Avg":"Below Avg"]].map(function(r){
                return (
                  <div key={r[0]} style={{background:CB,border:"1px solid "+BD,borderRadius:12,padding:"10px 12px"}}>
                    <div style={{fontSize:12,color:T2,marginBottom:3}}>{r[0]}</div>
                    <div style={{fontSize:12,fontWeight:700,color:T1}}>{r[1]}</div>
                  </div>
                );
            ...
            <div style={{background:"rgba(124,58,237,0.08)",border:"1px solid rgba(124,58,237,0.2)",borderRadius:12,padding:12}}>
              <div style={{fontSize:12,fontWeight:700,color:PURPLE,marginBottom:4}}>AI Technical Read</div>
