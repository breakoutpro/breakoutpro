...
export function AlertHistory(props){
  var theme = useTheme(); // reuses the existing ThemeProvider - no new theme system
  // Theme-sourced overrides - shadow the module-level hardcoded fallbacks above.
  var BLUE = theme.c.blue, CARD = theme.c.card, T2 = theme.c.text2, T3 = theme.c.text3, WARN = theme.c.warn; T1=theme.c.text1;
    ...
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,zIndex:10000,background:"rgba(0,0,0,0.85)",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{width:"100%",maxWidth:340,background:CARD,border:"1px solid "+BD,borderRadius:18,padding:22}}>
        ...
        <div style={{display:"flex",gap:8}}>
          ...
          <button onClick={function(){ if(i<STEPS.length-1){ setI(i+1); } else { done(); } }} style={{flex:2,background:BLUE,border:"none",borderRadius:11,padding:"11px",color:"#001910",fontSize:12,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>{i<STEPS.length-1?"Next":"Get Started"}</button>
        </div>
      </div>
    </div>
  );
