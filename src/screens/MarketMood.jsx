...
import {
  SectionHead, IndexRow, SemicircleGauge, EvolutionCard, buildMT
    ...
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:MT.BG,zIndex:350,overflowY:"auto"}}>
      ...
      <div style={{padding:wideLayout?"10px 24px 20px":"16px 24px 32px",maxWidth:1440,margin:"0 auto",width:"100%",boxSizing:"border-box"}}>
        ...
        {showLoading ? (
          ...
          <>
        {/* CURRENT MOOD - single semicircular gauge, no duplicate label rendering.
            Previously mood.label appeared 3 times (inside the old Gauge, in a
            separate "mood/stage" column, and again under "AI Bias") and
            confidence/stage were each duplicated once - all from the same
            single mood object. The gauge below is now the only place score,
            label and confidence render. "AI Bias" was removed entirely - it
            repeated the identical mood.label value already shown as the
            gauge's own center text; the gauge's center label IS the AI
            market bias, not a separate fact. Only Stage remains beside it,
            since that's genuinely distinct information the gauge doesn't
            show. */}
        <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:14,padding:padV,marginBottom:gapV,display:wideLayout?"flex":"block",alignItems:"center",justifyContent:"space-between",gap:24}}>
          <SemicircleGauge mood={mood}/>
          <div>
            <div style={{fontSize:10,color:MT.T3,fontWeight:700}}>STAGE</div>
            <div style={{fontSize:13,fontWeight:800,color:MT.T1}}>{mood.stage}</div>
          </div>
        </div>

        <button onClick={onVoice} style={{width:wideLayout?"auto":"100%",background:"transparent",border:"1px solid "+MT.BLUE,borderRadius:9,padding:"6px 14px",color:MT.BLUE,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",marginBottom:gapV,display:"inline-flex",alignItems:"center",justifyContent:"center",gap:6,minHeight:30}}>
        ...
        <div style={{display:wideLayout?"grid":"block",gridTemplateColumns:wideLayout?"repeat(4, 1fr)":undefined,gap:wideLayout?12:8,marginBottom:gapV,alignItems:"start"}}>
