...
export default function StocksBuddy(){
  ...
  const [watchlist,setWatchlist]=useState(function(){try{var s=localStorage.getItem("bp_watch");return s?JSON.parse(s):[];
  const [grkPrem,setGrkPrem]=useState(150);
  const [grkDelta,setGrkDelta]=useState(0.52);
  const [grkGamma,setGrkGamma]=useState(0.04);
  const [grkTheta,setGrkTheta]=useState(-12);
  const [grkVega,setGrkVega]=useState(8.5);
  const [rrEntry,setRrEntry]=useState(100);
  const [rrSL,setRrSL]=useState(95);
  const [rrTarget,setRrTarget]=useState(115);
  const [psCapital,setPsCapital]=useState(100000);
  const [psRisk,setPsRisk]=useState(1);
  const [psEntry,setPsEntry]=useState(100);
  const [psSL,setPsSL]=useState(95);
  const [journal,setJournal]=useState(function(){try{var s=localStorage.getItem("bp_journal");return s?JSON.parse(s):[];}catch(e){return [];}});
  const [jrnStock,setJrnStock]=useState("");
  const [jrnEntry,setJrnEntry]=useState("");
  const [jrnExit,setJrnExit]=useState("");
  const [jrnNotes,setJrnNotes]=useState("");
  const [aiQuery,setAiQuery]=useState("");
  const [aiResult,setAiResult]=useState("");
  const [aiLoading,setAiLoading]=useState(false);}catch(e){return [];}});
  ...
  function playBreakoutSound(){
    try{
      ...
      osc.frequency.setValueAtTime(1100,ctx.currentTime+0.22);
      osc.type="sine";
  ...
  function playBreakdownSound(){
    try{
      ...
      osc.frequency.setValueAtTime(220,ctx.currentTime+0.28);
      osc.type="sawtooth";
  ...
  function saveTrade(){
    if(!jrnStock||!jrnEntry||!jrnExit){alert("Fill all fields!");return;}
    var entry=parseFloat(jrnEntry);
    var exit=parseFloat(jrnExit);
    var profit=(exit-entry).toFixed(2);
    var pct=(((exit-entry)/entry)*100).toFixed(2);
    var newTrade={id:Date.now(),stock:jrnStock.toUpperCase(),entry:entry,exit:exit,profit:profit,pct:pct,notes:jrnNotes,date:new Date().toLocaleDateString("en-IN")};
    var updated=[newTrade].concat(journal);
    setJournal(updated);
    try{localStorage.setItem("bp_journal",JSON.stringify(updated));}catch(e){}
    setJrnStock("");setJrnEntry("");setJrnExit("");setJrnNotes("");
  }

  function delTrade(id){
    var updated=journal.filter(function(t){return t.id!==id;});
    setJournal(updated);
    try{localStorage.setItem("bp_journal",JSON.stringify(updated));}catch(e){}
  }

  function askAI(){
    if(!aiQuery.trim()){alert("Enter stock name!");return;}
    var prompt="Explain "+aiQuery+" stock: Business Model, Revenue Growth, Profit, Risks, Competitors, Latest News. Educational purpose only.";
    var url="https://chat.openai.com/?model=gpt-4&q="+encodeURIComponent(prompt);
            ...
            <div style={{padding:"14px 12px 4px 12px"}}>
              ...
              <div style={{background:"linear-gradient(135deg,#0a1a3a,#001a4a)",border:"1px solid #4488ff44",borderRadius:13,padding:"14px",marginBottom:10}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                  ...
                  <div>
                    <div style={{fontSize:12,fontWeight:800,color:"#88aaff"}}>AI Stock Explainer</div>
                    <div style={{fontSize:8,color:"#666"}}>Ask AI about any stock - Free!</div>
                  </div>
                </div>
                <input type="text" value={aiQuery} onChange={function(e){setAiQuery(e.target.value);}} placeholder="Enter stock name (e.g., RELIANCE, TCS)" style={{width:"100%",background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:9,padding:"10px",color:"#fff",fontSize:11,outline:"none",fontFamily:"inherit",marginBottom:8,boxSizing:"border-box"}}/>
                <button onClick={askAI} style={{width:"100%",background:"linear-gradient(135deg,#4488ff,#0066cc)",color:"#fff",border:"none",borderRadius:9,padding:"10px",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>Ask AI (Opens ChatGPT) \u25B8</button>
                <div style={{fontSize:8,color:"#888",marginTop:6,textAlign:"center",lineHeight:1.5}}>Educational AI explanation - Business model, Revenue, Risks, Competitors</div>
              </div>
              ...
              <div style={{background:"linear-gradient(135deg,#3a0a1a,#4a001a)",border:"1px solid #ff448844",borderRadius:13,padding:"14px",marginBottom:10}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                  <span style={{fontSize:22}}>\uD83D\uDD3A</span>
                  <div>
                    <div style={{fontSize:12,fontWeight:800,color:"#ff88aa"}}>Greeks Calculator</div>
                    <div style={{fontSize:8,color:"#666"}}>Options Greeks education</div>
                  </div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:8}}>
                  <div style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:7,padding:"7px"}}>
                    <div style={{fontSize:8,color:"#666"}}>Premium (Rs)</div>
                    <input type="number" value={grkPrem} onChange={function(e){setGrkPrem(parseFloat(e.target.value)||0);}} style={{width:"100%",background:"transparent",border:"none",color:"#fff",fontSize:13,fontWeight:700,outline:"none",fontFamily:"inherit"}}/>
                  </div>
                  <div style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:7,padding:"7px"}}>
                    <div style={{fontSize:8,color:"#666"}}>Delta</div>
                    <input type="number" step="0.01" value={grkDelta} onChange={function(e){setGrkDelta(parseFloat(e.target.value)||0);}} style={{width:"100%",background:"transparent",border:"none",color:"#39FF14",fontSize:13,fontWeight:700,outline:"none",fontFamily:"inherit"}}/>
                  </div>
                  <div style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:7,padding:"7px"}}>
                    <div style={{fontSize:8,color:"#666"}}>Gamma</div>
                    <input type="number" step="0.01" value={grkGamma} onChange={function(e){setGrkGamma(parseFloat(e.target.value)||0);}} style={{width:"100%",background:"transparent",border:"none",color:"#FFD700",fontSize:13,fontWeight:700,outline:"none",fontFamily:"inherit"}}/>
                  </div>
                  <div style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:7,padding:"7px"}}>
                    <div style={{fontSize:8,color:"#666"}}>Theta</div>
                    <input type="number" step="0.1" value={grkTheta} onChange={function(e){setGrkTheta(parseFloat(e.target.value)||0);}} style={{width:"100%",background:"transparent",border:"none",color:"#ff4444",fontSize:13,fontWeight:700,outline:"none",fontFamily:"inherit"}}/>
                  </div>
                  <div style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:7,padding:"7px"}}>
                    <div style={{fontSize:8,color:"#666"}}>Vega</div>
                    <input type="number" step="0.1" value={grkVega} onChange={function(e){setGrkVega(parseFloat(e.target.value)||0);}} style={{width:"100%",background:"transparent",border:"none",color:"#88aaff",fontSize:13,fontWeight:700,outline:"none",fontFamily:"inherit"}}/>
                  </div>
                </div>
                <div style={{background:"#1a0500",border:"1px solid #ff444422",borderRadius:8,padding:"9px",fontSize:8,color:"#aaa",lineHeight:1.6}}>
                  <b style={{color:"#FFD700"}}>Greek Meanings:</b><br/>
                  Delta: Price change per Rs.1 underlying move<br/>
                  Gamma: Delta change rate<br/>
                  Theta: Daily premium decay<br/>
              ...
              <div style={{background:"linear-gradient(135deg,#0a3a1a,#003a2a)",border:"1px solid #44ff8844",borderRadius:13,padding:"14px",marginBottom:10}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                  ...
                  <div>
                    <div style={{fontSize:12,fontWeight:800,color:"#88ffaa"}}>Risk:Reward Calculator</div>
                    <div style={{fontSize:8,color:"#666"}}>Trade planning tool</div>
                  </div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:8}}>
                  <div style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:7,padding:"7px"}}>
                    <div style={{fontSize:8,color:"#666"}}>Entry</div>
                    <input type="number" value={rrEntry} onChange={function(e){setRrEntry(parseFloat(e.target.value)||0);}} style={{width:"100%",background:"transparent",border:"none",color:"#fff",fontSize:12,fontWeight:700,outline:"none",fontFamily:"inherit"}}/>
                  </div>
                  <div style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:7,padding:"7px"}}>
                    <div style={{fontSize:8,color:"#666"}}>Stop Loss</div>
                    <input type="number" value={rrSL} onChange={function(e){setRrSL(parseFloat(e.target.value)||0);}} style={{width:"100%",background:"transparent",border:"none",color:"#ff4444",fontSize:12,fontWeight:700,outline:"none",fontFamily:"inherit"}}/>
                  </div>
                  <div style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:7,padding:"7px"}}>
                    <div style={{fontSize:8,color:"#666"}}>Target</div>
                    <input type="number" value={rrTarget} onChange={function(e){setRrTarget(parseFloat(e.target.value)||0);}} style={{width:"100%",background:"transparent",border:"none",color:"#39FF14",fontSize:12,fontWeight:700,outline:"none",fontFamily:"inherit"}}/>
                  </div>
                </div>
                <div style={{background:"#001a00",border:"1px solid #39FF1444",borderRadius:9,padding:"10px"}}>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,fontSize:10}}>
                    <div>
                      <div style={{color:"#666",fontSize:8}}>Risk</div>
                      <div style={{color:"#ff4444",fontWeight:800,fontSize:14}}>Rs.{Math.abs(rrEntry-rrSL).toFixed(2)}</div>
                    </div>
                    <div>
                      <div style={{color:"#666",fontSize:8}}>Reward</div>
                      <div style={{color:"#39FF14",fontWeight:800,fontSize:14}}>Rs.{Math.abs(rrTarget-rrEntry).toFixed(2)}</div>
                    </div>
                    <div>
                      <div style={{color:"#666",fontSize:8}}>R:R Ratio</div>
                      <div style={{color:"#FFD700",fontWeight:800,fontSize:14}}>1:{(Math.abs(rrTarget-rrEntry)/Math.max(0.01,Math.abs(rrEntry-rrSL))).toFixed(2)}</div>
                    </div>
                  </div>
                  <div style={{marginTop:8,fontSize:8,color:"#888",lineHeight:1.5}}>
                    {(Math.abs(rrTarget-rrEntry)/Math.max(0.01,Math.abs(rrEntry-rrSL)))>=2?"\u2705 Good R:R (2:1 or better)":"\u26A0\uFE0F Low R:R - Risky trade"}
                  </div>
                </div>
              </div>
              ...
              <div style={{background:"linear-gradient(135deg,#3a2a0a,#4a3a00)",border:"1px solid #ffaa4444",borderRadius:13,padding:"14px",marginBottom:10}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                  <span style={{fontSize:22}}>\uD83D\uDCCF</span>
                  <div>
                    <div style={{fontSize:12,fontWeight:800,color:"#ffcc88"}}>Position Size Calculator</div>
                    <div style={{fontSize:8,color:"#666"}}>How many shares to buy</div>
                  </div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:8}}>
                  <div style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:7,padding:"7px"}}>
                    <div style={{fontSize:8,color:"#666"}}>Capital (Rs)</div>
                    <input type="number" value={psCapital} onChange={function(e){setPsCapital(parseFloat(e.target.value)||0);}} style={{width:"100%",background:"transparent",border:"none",color:"#fff",fontSize:12,fontWeight:700,outline:"none",fontFamily:"inherit"}}/>
                  </div>
                  <div style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:7,padding:"7px"}}>
                    <div style={{fontSize:8,color:"#666"}}>Risk %</div>
                    <input type="number" step="0.1" value={psRisk} onChange={function(e){setPsRisk(parseFloat(e.target.value)||0);}} style={{width:"100%",background:"transparent",border:"none",color:"#FFD700",fontSize:12,fontWeight:700,outline:"none",fontFamily:"inherit"}}/>
                  </div>
                  <div style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:7,padding:"7px"}}>
                    <div style={{fontSize:8,color:"#666"}}>Entry</div>
                    <input type="number" value={psEntry} onChange={function(e){setPsEntry(parseFloat(e.target.value)||0);}} style={{width:"100%",background:"transparent",border:"none",color:"#fff",fontSize:12,fontWeight:700,outline:"none",fontFamily:"inherit"}}/>
                  </div>
                  <div style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:7,padding:"7px"}}>
                    <div style={{fontSize:8,color:"#666"}}>Stop Loss</div>
                    <input type="number" value={psSL} onChange={function(e){setPsSL(parseFloat(e.target.value)||0);}} style={{width:"100%",background:"transparent",border:"none",color:"#ff4444",fontSize:12,fontWeight:700,outline:"none",fontFamily:"inherit"}}/>
                  </div>
                </div>
                <div style={{background:"#1a1500",border:"1px solid #ffaa4422",borderRadius:9,padding:"10px"}}>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,fontSize:10}}>
                    <div>
                      <div style={{color:"#666",fontSize:8}}>Risk Amount</div>
                      <div style={{color:"#ff4444",fontWeight:800,fontSize:13}}>Rs.{(psCapital*psRisk/100).toFixed(0)}</div>
                    </div>
                    <div>
                      <div style={{color:"#666",fontSize:8}}>Quantity</div>
                      <div style={{color:"#39FF14",fontWeight:800,fontSize:13}}>{Math.floor((psCapital*psRisk/100)/Math.max(0.01,Math.abs(psEntry-psSL)))} shares</div>
                    </div>
                  </div>
                  <div style={{marginTop:8,padding:"6px",background:"#0a0a0a",borderRadius:6,fontSize:8,color:"#aaa",lineHeight:1.5}}>
                    Total Investment: Rs.{(Math.floor((psCapital*psRisk/100)/Math.max(0.01,Math.abs(psEntry-psSL)))*psEntry).toFixed(0)}
                  </div>
                </div>
              </div>
              ...
              <div style={{background:"linear-gradient(135deg,#2a0a3a,#3a0a4a)",border:"1px solid #aa44ff44",borderRadius:13,padding:"14px",marginBottom:10}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                  <span style={{fontSize:22}}>\uD83D\uDCD3</span>
                  <div>
                    <div style={{fontSize:12,fontWeight:800,color:"#cc88ff"}}>Trading Journal</div>
                    <div style={{fontSize:8,color:"#666"}}>Track your trades</div>
                  </div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:5,marginBottom:6}}>
                  <input type="text" value={jrnStock} onChange={function(e){setJrnStock(e.target.value);}} placeholder="Stock" style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:7,padding:"7px",color:"#fff",fontSize:10,outline:"none",fontFamily:"inherit"}}/>
                  <input type="number" value={jrnEntry} onChange={function(e){setJrnEntry(e.target.value);}} placeholder="Entry" style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:7,padding:"7px",color:"#fff",fontSize:10,outline:"none",fontFamily:"inherit"}}/>
                  <input type="number" value={jrnExit} onChange={function(e){setJrnExit(e.target.value);}} placeholder="Exit" style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:7,padding:"7px",color:"#fff",fontSize:10,outline:"none",fontFamily:"inherit"}}/>
                </div>
                <input type="text" value={jrnNotes} onChange={function(e){setJrnNotes(e.target.value);}} placeholder="Notes (optional)" style={{width:"100%",background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:7,padding:"7px",color:"#fff",fontSize:10,outline:"none",fontFamily:"inherit",marginBottom:6,boxSizing:"border-box"}}/>
                <button onClick={saveTrade} style={{width:"100%",background:"#aa44ff",color:"#fff",border:"none",borderRadius:8,padding:"8px",fontSize:10,fontWeight:800,cursor:"pointer",fontFamily:"inherit",marginBottom:8}}>+ Save Trade</button>
                <div style={{maxHeight:180,overflowY:"auto"}}>
                  {journal.length===0?(
                    <div style={{textAlign:"center",padding:"15px",color:"#666",fontSize:9}}>No trades logged yet</div>
                  ):journal.map(function(t){return(
                    <div key={t.id} style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:8,padding:"8px",marginBottom:5}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                        <div style={{fontSize:11,fontWeight:800,color:"#fff"}}>{t.stock}</div>
                        <button onClick={function(){delTrade(t.id);}} style={{background:"transparent",border:"none",color:"#ff4444",fontSize:11,cursor:"pointer",padding:0}}>\u2715</button>
                      </div>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:5,fontSize:8}}>
                        <div><span style={{color:"#666"}}>Entry:</span> <span style={{color:"#fff",fontWeight:700}}>{t.entry}</span></div>
                        <div><span style={{color:"#666"}}>Exit:</span> <span style={{color:"#fff",fontWeight:700}}>{t.exit}</span></div>
                        <div><span style={{color:"#666"}}>P/L:</span> <span style={{color:parseFloat(t.profit)>=0?"#39FF14":"#ff4444",fontWeight:800}}>{parseFloat(t.profit)>=0?"+":""}{t.profit} ({t.pct}%)</span></div>
                      </div>
                      {t.notes&&<div style={{marginTop:4,fontSize:8,color:"#aaa"}}>{t.notes}</div>}
                      <div style={{marginTop:3,fontSize:7,color:"#555"}}>{t.date}</div>
                    </div>
                  );})}
                </div>
              </div>

              <div style={{background:"linear-gradient(135deg,#0a3a3a,#003a3a)",border:"1px solid #44ffff44",borderRadius:13,padding:"14px",marginBottom:10}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                  <span style={{fontSize:22}}>\uD83D\uDCC5</span>
                  <div>
                    <div style={{fontSize:12,fontWeight:800,color:"#88ffff"}}>Economic Calendar</div>
                    <div style={{fontSize:8,color:"#666"}}>Major upcoming events</div>
                  </div>
                </div>
                {[{d:"2026-06-10",e:"RBI Policy Meeting",i:"High",c:"#ff4444"},{d:"2026-06-12",e:"CPI Inflation Data",i:"High",c:"#ff4444"},{d:"2026-06-15",e:"US Fed Meeting",i:"Very High",c:"#ff0000"},{d:"2026-06-20",e:"GDP Q1 Data",i:"High",c:"#ff4444"},{d:"2026-06-25",e:"WPI Inflation",i:"Medium",c:"#ffaa44"},{d:"2026-06-30",e:"Industrial Production",i:"Medium",c:"#ffaa44"}].map(function(ev,i){return(
                  <div key={i} style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:8,padding:"8px 10px",marginBottom:5,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div>
                      <div style={{fontSize:10,fontWeight:700,color:"#fff"}}>{ev.e}</div>
                      <div style={{fontSize:8,color:"#888"}}>{ev.d}</div>
                    </div>
                    <div style={{background:ev.c+"22",border:"1px solid "+ev.c+"55",borderRadius:6,padding:"3px 7px",fontSize:8,fontWeight:700,color:ev.c}}>{ev.i}</div>
                  </div>
                );})}
              </div>

            </div>
"#445",marginBottom:1}}>FII F&O</div>
