  ...
  return (
    <div style={{background:BG,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
      <div style={{background:BG,padding:"12px 16px",borderBottom:"1px solid "+BD,display:"flex",alignItems:"center",gap:12,position:"sticky",top:0,zIndex:10}}>
        ...
        <div style={{flex:1}}>
          <div style={{fontSize:18,fontWeight:900,color:T1}}>My Watchlist</div>
          <div style={{fontSize:12,color:T2}}>{wl.length} of {MAX} added</div>
        </div>
        <button onClick={function(){setAdding(!adding);}} style={{background:adding?"rgba(255,255,255,0.08)":"rgba(59,130,246,0.15)",border:"1px solid "+(adding?BD:"rgba(59,130,246,0.4)"),borderRadius:9,padding:"8px 12px",color:adding?T2:BLUE,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>{adding?"Close":"+ Add"}</button>
      </div>
      ...
      {adding?(
        <div style={{padding:"12px 16px",borderBottom:"1px solid "+BD}}>
          ...
          {wl.length>=MAX?<div style={{fontSize:12,color:DOWN,marginBottom:8}}>Watchlist full. Remove one to add more.</div>:null}
          <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,overflow:"hidden",maxHeight:260,overflowY:"auto"}}>
            {filtered.length==0?<div style={{padding:16,textAlign:"center",color:T2,fontSize:12}}>No matches</div>:filtered.map(function(s,i){
              return (
                <div key={s.sym} onClick={function(){add(s.sym);}} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 12px",borderBottom:i<filtered.length-1?"1px solid "+BD:"none",cursor:"pointer"}}>
                  <div><div style={{fontSize:12,fontWeight:700,color:T1}}>{s.sym}</div><div style={{fontSize:12,color:T2}}>{s.name}</div></div>
      ...
      <div style={{padding:16}}>
        {wl.length==0?(
          <div style={{textAlign:"center",padding:"40px 24px"}}>
            <div style={{fontSize:14,color:T2,marginBottom:8}}>Your watchlist is empty</div>
            <div style={{fontSize:12,color:T3}}>Tap "+ Add" to track stocks and indices</div>
          </div>
        ):(
          <div style={{background:CARD,border:"1px solid "+BD,borderRadius:16,overflow:"hidden"}}>
            {wl.map(function(sym,i){
              var m=mockPrice(sym);
              return (
                <div key={sym} style={{display:"flex",alignItems:"center",padding:"12px 12px",borderBottom:i<wl.length-1?"1px solid "+BD:"none"}}>
                  <div onClick={function(){if(props.onStock)props.onStock({sym:sym,ltp:m.ltp,up:m.up,chgPct:parseFloat(m.pct)});}} style={{flex:1,cursor:"pointer"}}>
                    <div style={{fontSize:14,fontWeight:700,color:T1}}>{sym}</div>
                    <div style={{fontSize:12,color:T2,marginTop:4}}>Alerts active</div>
                  </div>
                  <div style={{textAlign:"right",marginRight:12}}>
                    <div style={{fontSize:14,fontWeight:800,color:m.up?UP:DOWN,fontFamily:"monospace"}}>Rs {m.ltp}</div>
                    <div style={{fontSize:12,fontWeight:700,color:m.up?UP:DOWN}}>{m.up?"+":""}{m.pct}%</div>
                  </div>
        ...
        <div style={{background:CARD2,border:"1px solid "+BD,borderRadius:16,padding:12,marginTop:16}}>
          <div style={{fontSize:12,fontWeight:700,color:BLUE,marginBottom:8}}>Watchlist Alerts</div>
          <div style={{fontSize:12,color:T2,lineHeight:1.6}}>You will get breakout, volume spike, and pattern alerts for all stocks in your watchlist. Voice and push alerts can be set in the Signals page.</div>
        </div>
      </div>
    </div>
  );
