...
function UnavailableCard(props){
  ...
  return (
    <div style={{background:CARD,border:"1px dashed "+BD,borderRadius:12,padding:14,marginBottom:12}}>
...
export default function StockResearchPro(props){
  var theme = useTheme(); // reuses the existing ThemeProvider - no new theme system
  // Theme-sourced overrides - shadow the module-level hardcoded fallbacks above.
  var BD = theme.c.border, BLUE = theme.c.blue, CARD = theme.c.card, R = theme.c.down, T3 = theme.c.text3, UP = theme.c.up, WARN = theme.c.gold; T1=theme.c.text1;
  ...
  return (
    <div style={{background:theme.c.bg,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
      ...
      <div style={{padding:14}}>
        <div style={{background:"rgba(212,175,55,0.06)",border:"1px solid rgba(212,175,55,0.15)",borderRadius:10,padding:12,marginBottom:14}}>
          <div style={{fontSize:12,color:WARN,lineHeight:1.6}}>Breakout Pro does not have a real company-fundamentals data source. Company Overview, Financials, Shareholding, Valuation, SWOT, Timeline, Competitors, Similar Stocks, and Sector Comparison are shown as honestly unavailable below rather than invented. Only the symbols already proven real for this app (the same 8 used in Chart) can be researched here.</div>
        </div>

        <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10}}>SELECT SYMBOL</div>
        ...
        {status=="loading" ? (
          ...
          <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:20,textAlign:"center",color:R,fontSize:12}}>Data temporarily unavailable. <button onClick={load} style={{background:"none",border:"1px solid "+BLUE,borderRadius:8,padding:"7px 14px",color:BLUE,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",marginTop:8,minHeight:44}}>Retry</button></div>
        ) : status=="empty" ? (
          <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:20,textAlign:"center",color:T3,fontSize:12}}>No real data available for this symbol right now.</div>
        ) : (
          <div>
            <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,marginBottom:14}}>
              <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10}}>TECHNICAL ANALYSIS (real, computed from actual price history)</div>
            ...
            <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,marginBottom:14}}>
              <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10}}>TECHNICAL VOLATILITY (real, computed - not a fundamentals risk score)</div>
              {vol ? (
                <div>
                  <div style={{fontSize:16,fontWeight:800,color:vol.label=="Low"?UP:vol.label=="High"?R:WARN}}>{vol.label}</div>
            ...
            <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:14,marginBottom:14}}>
              <div style={{fontSize:12,fontWeight:800,color:T2,marginBottom:10}}>AI SUMMARY (rule-based, only from the real data above)</div>
