...
export function useHomeData(){
  ...
  var wl = useWatchlist();
  var liveNewsData = useLiveNews(); // real ET Markets RSS feed, via api/news.js -> rss2json, 3-minute cache

  // JUSTIN (the old fake static news array) is completely removed - LIVE_NEWS
  // below is mapped from useLiveNews()'s real fetched items into the exact
  // same field shape (headline/body/time/source/impact) the Market News
  // card already renders, so no Home layout file needed a rendering change,
  // only this data source underneath it.
  var LIVE_NEWS = useMemo(function(){
    return liveNewsData.news.map(function(n, i){
      var impact = n.sentiment==="good" ? "Bullish" : (n.sentiment==="bad" ? "Bearish" : "Neutral");
      return {
        id: n.link || i,
        headline: n.title,
        body: n.description ? [n.description] : [],
        time: n.time,
        source: n.source,
        impact: impact,
        link: n.link,
  ...
  var TICKER = LIVE_NEWS.length ? LIVE_NEWS.map(function(n){ return n.headline; }) : ["Market news unavailable right now."];

  var [tickerIdx, setTickerIdx] = useState(0);
  var tickerLenRef = useRef(TICKER.length);
  tickerLenRef.current = TICKER.length;
  ...
  useEffect(function(){
    var tk = setInterval(function(){
      setTickerIdx(function(i){ return (i+1) % tickerLenRef.current; });
    }, 10000);
  ...
  var spineRows = useMemo(function(){
    var idxData = mkt.snapshot && mkt.snapshot.indices;
  ...
  return {
    ...
    ticker: TICKER,
    liveNews: LIVE_NEWS,
    liveNewsLoading: liveNewsData.loading,
