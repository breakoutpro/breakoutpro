...
export default function NewsScreen(props) {
  ...
  BD=theme.c.border; CB=theme.c.card; DB=theme.c.bg; G2="#16A34A"; R="#DC2626"; T2=theme.c.text2;
  // Theme-sourced overrides - shadow the module-level hardcoded fallbacks above.
  var BLUE = theme.c.blue, G = theme.c.brand; T1=theme.c.text1;

  var allNews = Array.isArray(props.news) ? props.news : [];
  var overnightNews = Array.isArray(props.overnightNews) ? props.overnightNews : [];
  ...
  var showOvernightToggle = nowHour < 9 && Array.isArray(overnightNews) && overnightNews.length > 0;
  var news = filter=="overnight" ? overnightNews : filter=="good" ? allNews.filter(function(n){return n && n.sentiment=="good";}) : filter=="bad" ? allNews.filter(function(n){return n && n.sentiment=="bad";}) : allNews;
  news = Array.isArray(news) ? news.filter(Boolean) : [];
    ...
    <div style={{background:DB, minHeight:"100vh", fontFamily:"Inter,Arial,sans-serif", paddingBottom:80}}>
      ...
      <div style={{padding:"12px 16px 0"}}>
        ...
        {news.map(function(n, i) {
          ...
          return (
            <div key={i} style={{background:CB, border:"1px solid "+BD, borderRadius:16, margin:"6px 0", overflow:"hidden", width:"100%", boxSizing:"border-box"}}>
                ...
                <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:12}}>
                  <div style={{flex:1}}>
                    <div style={{display:"flex",gap:6,marginBottom:5,flexWrap:"wrap"}}>
                      {n.sentiment=="good"?<span style={{fontSize:10,fontWeight:700,color:"#16A34A",background:"#16A34A18",border:"1px solid #16A34A55",borderRadius:5,padding:"2px 6px",display:"flex",alignItems:"center",gap:3,textTransform:"uppercase",lineHeight:1.4}}><span style={{width:5,height:5,borderRadius:"50%",background:"#16A34A",display:"inline-block"}}></span>BULLISH</span>:null}
                      {n.sentiment=="bad"?<span style={{fontSize:10,fontWeight:700,color:"#DC2626",background:"#DC262618",border:"1px solid #DC262655",borderRadius:5,padding:"2px 6px",display:"flex",alignItems:"center",gap:3,textTransform:"uppercase",lineHeight:1.4}}><span style={{width:5,height:5,borderRadius:"50%",background:"#DC2626",display:"inline-block"}}></span>BEARISH</span>:null}
                      {n.sentiment=="neutral"?<span style={{fontSize:10,fontWeight:700,color:"#EAB308",background:"#EAB30818",border:"1px solid #EAB30855",borderRadius:5,padding:"2px 6px",display:"flex",alignItems:"center",gap:3,textTransform:"uppercase",lineHeight:1.4}}><span style={{width:5,height:5,borderRadius:"50%",background:"#EAB308",display:"inline-block"}}></span>NEUTRAL</span>:null}
                      {n.stock?<span style={{fontSize:10,fontWeight:700,color:BLUE,background:"rgba(37,99,235,0.15)",borderRadius:5,padding:"2px 6px",lineHeight:1.4}}>{n.stock}</span>:null}
                    </div>
                    <div style={{fontSize:14, fontWeight:600, color:T1, lineHeight:1.3, marginBottom:4}}>{n.title}</div>
                    <div style={{display:"flex", gap:8, alignItems:"center", flexWrap:"wrap"}}>
                      <span style={{fontSize:11, color:"#64748B", fontWeight:600, background:"rgba(59,130,246,0.1)", borderRadius:10, padding:"3px 7px"}}>{n.source||"ET Markets"}</span>
                      {n.pubDate?<span style={{fontSize:11, color:"#64748B"}}>{n.pubDate}</span>:null}
                      {n.link?<a href={n.link} target="_blank" rel="noopener noreferrer" style={{fontSize:11,color:"#64748B",fontWeight:600,textDecoration:"underline"}}>Read Full Article &#8599;</a>:null}
                    </div>
                  </div>
                  {/* Share button - Admin only */}
                  {isAdmin?(
                    <button onClick={function(){setShareId(isShareOpen?null:i);}} style={{background:isShareOpen?"rgba(59,130,246,0.2)":"rgba(255,255,255,0.06)",border:"1px solid "+(isShareOpen?BLUE:BD),borderRadius:8,padding:"6px 7px",cursor:"pointer",flexShrink:0,fontFamily:"inherit",display:"flex",alignItems:"center",gap:4}}>
                      <span style={{fontSize:11}}>&#8679;</span>
                      <span style={{fontSize:11,fontWeight:700,color:isShareOpen?BLUE:T2}}>Share</span>
                    </button>
                  ):null}
                </div>
              </div>
