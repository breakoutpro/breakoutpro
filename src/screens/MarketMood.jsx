...
import {
  SectionHead, SectionHeadWithPill, UnavailableCard, GridWrap,
  SemicircleGauge, EvolutionCard, Sparkline, buildMT
  ...
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:MT.BG,zIndex:350,overflowY:"auto"}}>
      ...
      <div style={{padding:wideLayout?"4px 10px":"16px 24px 32px",maxWidth:1440,margin:"0 auto",width:"100%",boxSizing:"border-box"}}>
        ...
        {/* CURRENT MOOD - compact, single-source hero. Avoid duplicate mood labels and unused timeline space. */}
        <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:10,padding:wideLayout?"5px 8px":padV,marginBottom:gapV,display:"flex",alignItems:"center",gap:10,minHeight:0,boxSizing:"border-box"}}>
          <SemicircleGauge mood={mood}/>
          <div style={{minWidth:0,flex:1}}>
            <div style={{fontSize:13,fontWeight:900,color:moodColor,lineHeight:1.15}}>Market Mood: {mood.label.toUpperCase()}</div>
            <div style={{fontSize:10,color:MT.T2,marginTop:2}}>
              Stage: <b style={{color:MT.T1}}>{mood.stage}</b>
              <span style={{marginLeft:8}}>Confidence: <b style={{color:moodColor}}>{mood.confidence}</b></span>
            </div>
        ...
        <div style={{display:wideLayout?"grid":"block",gridTemplateColumns:wideLayout?"1fr 3fr":undefined,gap:gapV,marginBottom:gapV,alignItems:"start"}}>
          <div style={{marginBottom:wideLayout?0:8}}>
            ...
            <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:10,padding:"4px 6px"}}>
              {(function(){
                ...
                return rows.map(function(r){
                  return (
                    <div key={r[0]} style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}>
                      <div style={{width:64,fontSize:10,fontWeight:700,color:r[1]}}>{r[0]}</div>
                      <div style={{flex:1,height:7,background:MT.BD,borderRadius:4,overflow:"hidden"}}>
        ...
        <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:10,padding:"4px 6px",marginBottom:gapV}}>
          <div style={{fontSize:12,fontWeight:800,color:MT.T1,marginBottom:3}}>MARKET SNAPSHOT</div>
          <div style={{display:wideLayout?"grid":"block",gridTemplateColumns:wideLayout?"repeat(4, 1fr)":undefined,gap:wideLayout?6:6}}>
            {[["NIFTY", niftyRow], ["BANKNIFTY", bankRow], ["SENSEX", sensexRow], ["VIX", vixRow]].map(function(pair,i){
              ...
              return (
                <div key={i} style={{marginBottom:wideLayout?0:8}}>
                  <div style={{fontSize:11,fontWeight:700,color:MT.T2,marginBottom:2}}>{row.name}</div>
                  {row.available ? (
                    <div>
                      <div style={{fontSize:14,fontWeight:800,color:MT.T1}}>{row.ltp}</div>
                      <div style={{fontSize:12,fontWeight:700,color:row.up==null?MT.T2:(row.up?MT.SENT_GREEN:MT.SENT_RED)}}>{row.absChange ? row.absChange+" " : ""}({row.chgPct})</div>
                      {row.dayLow && row.dayHigh ? <div style={{fontSize:10,color:MT.T3,marginTop:1}}>Low: {row.dayLow} &nbsp; High: {row.dayHigh}</div> : null}
                      {!usingDemo && data && data.indexHistory && data.indexHistory[key] && data.indexHistory[key].points && data.indexHistory[key].points.length>=2 ? (
                        <div style={{marginTop:2}}><Sparkline points={data.indexHistory[key].points} inverse={key==="VIX"} height={20}/></div>
                      ) : null}
                      {usingDemo && lvl ? (
        ...
        {/* SECTOR ROTATION / GLOBAL MARKETS - compact final intelligence row. */}
              ...
              <div>
                ...
                <GridWrap columns={2}>
                  {rankedSectors.map(function(s,i){
                    ...
                    return (
                      <div key={i} style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:8,padding:"5px 7px"}}>
        ...
        {/* 3-DAY EVOLUTION / RISK LEVEL / AI TRADE BIAS / DISCLAIMER - compact final intelligence row. */}        <div style={{display:wideLayout?"grid":"block",gridTemplateColumns:wideLayout?"repeat(4, 1fr)":undefined,gap:gapV,marginBottom:gapV,alignItems:"start"}}>
          <div style={{marginBottom:wideLayout?0:8}}>
            <SectionHead>3-DAY EVOLUTION</SectionHead>
            {usingDemo ? (
              <div style={{background:MT.CARD,border:"1px solid "+MT.BD,borderRadius:10,padding:padV}}>
          ...
          <div style={{background:MT.WARN+"14",border:"1px solid "+MT.WARN+"55",borderRadius:10,padding:padV}}>
