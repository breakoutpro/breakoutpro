    ...
    <div style={{background:BG,minHeight:"100vh",fontFamily:"'Inter',Arial,sans-serif",paddingBottom:40,color:T1,overflowX:"hidden"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(300px, 1fr))",gap:16,padding:"16px 16px 0",minWidth:0,boxSizing:"border-box"}}>
        ...
        {(function(){
          ...
          var posPct = Math.round(((nifty.ltp - nifty.low) / width) * 100);
          var distToHigh = Math.round((nifty.high - nifty.ltp)*100)/100;
          var distToLow = Math.round((nifty.ltp - nifty.low)*100)/100;
          var state = posPct>=90?"Breakout Watch":(posPct<=10?"Breakdown Watch":(posPct>=60?"Upper Range":(posPct<=40?"Lower Range":"Range")));
          var stateColor = state.indexOf("Breakout")>=0?UP:(state.indexOf("Breakdown")>=0?DOWN:T1);
          return (
            <SummaryCard title="Range Intelligence" icon="&#128640;" onDetails={function(){setTab("rangeintel");}}>
              <div style={{fontSize:12,color:T2,marginBottom:6}}>NIFTY 50 &middot; CMP: <b style={{color:T1}}>{nifty.ltp.toLocaleString("en-IN")}</b> &middot; Position: {posPct}%</div>
              <div style={{fontSize:13,fontWeight:800,color:stateColor,marginBottom:8}}>{state}</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,textAlign:"center"}}>
                <div><div style={{fontSize:10,color:T2}}>Support</div><div style={{fontSize:12,color:T1,fontWeight:700}}>{nifty.low.toLocaleString("en-IN")}</div></div>
                <div><div style={{fontSize:10,color:T2}}>Resistance</div><div style={{fontSize:12,color:T1,fontWeight:700}}>{nifty.high.toLocaleString("en-IN")}</div></div>
              </div>
            </SummaryCard>
          );
        })()}
