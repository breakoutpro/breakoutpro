...
export default function EquityHomeDesktop(props){
  ...
  var SENT_GREEN="#16A34A", SENT_RED="#DC2626", SENT_YELLOW="#EAB308";
  var T1="#111827", T2="#4B5563", T3="#9CA3AF";
  var setTab = props.setTab || function(){};
  var data = useHomeData();
  // Called DIRECTLY here - same hook, same /api/news source News.jsx
  // already proves works - not routed through useHomeData.js's own
  // internal call, since that indirection was the actual break point
  // (this exact "fix landed in the wrong/unused copy of a file" failure
  // mode has happened before in this codebase with useHomeData.js and
  // JustInData.jsx both, earlier in this project's history).
  var liveNewsHook = useLiveNews();
  var safeLiveNews = Array.isArray(liveNewsHook?.news) ? liveNewsHook.news.map(function(n, i){
    var impact = n.sentiment=="good" ? "Bullish" : (n.sentiment=="bad" ? "Bearish" : "Neutral");
    return { id: n.link || i, headline: n.title, body: n.description ? [n.description] : [], time: n.time, source: n.source, impact: impact, link: n.link, stock: n.stock };
  }) : [];
  var mm = data.mm;
    ...
    <div style={{background:BG,minHeight:"100vh",fontFamily:"'Inter',Arial,sans-serif",color:T1,overflowX:"hidden",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,display:"flex",minHeight:0,width:"100%"}}>
        ...
        <div style={{flex:1,minWidth:0,padding:"8px 12px",overflowY:"auto"}}>
          ...
          <div onClick={function(){setTab("marketmood");}} style={{background:CARD2+"cc",backdropFilter:"blur(12px)",border:"1px solid "+BD,borderRadius:10,padding:"8px 12px",boxSizing:"border-box",cursor:"pointer",overflowY:"auto",display:"flex",flexDirection:"column",height:"100%"}}>
          ...
          {(function(){
            ...
            if(!hasRange){
              ...
              var cprStatus = widthPct < 0.15 ? "Narrow CPR" : "Wide CPR";
              var distToR1 = Math.round((r1 - nifty.ltp)*100)/100;
              var distToS1 = Math.round((nifty.ltp - s1)*100)/100;
              rangeContent = (
                <div>
                  ...
                  <div style={{fontSize:8,color:T3,marginBottom:2}}>{distToR1} pts to R1 &#183; {distToS1} pts to S1</div>
                  <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:2}}>
                    <span style={{fontSize:8,fontWeight:800,color:cprStatus==="Narrow CPR"?WARN:UP,background:(cprStatus==="Narrow CPR"?WARN:UP)+"18",border:"1px solid "+(cprStatus==="Narrow CPR"?WARN:UP)+"55",borderRadius:4,padding:"1px 5px"}}>{cprStatus}</span>
                    <span style={{fontSize:8,color:T3}}>CPR: BC {bc.toLocaleString("en-IN")} &ndash; TC {tc.toLocaleString("en-IN")}</span>
                  </div>
                  <div style={{fontSize:8,color:T3,marginBottom:6}}>VWAP: Not available (no real intraday tick feed connected)</div>
                  <div style={{fontSize:9,fontWeight:700,color:BLUE,marginTop:"auto",paddingTop:2}}>Open Candlestick Range Chart &#8594;</div>
                </div>
              );
            }
            return (
              <div onClick={function(){setTab("rangeintel");}} style={{background:CARD2+"cc",backdropFilter:"blur(12px)",border:"1px solid "+BD,borderRadius:10,padding:"8px 12px",boxSizing:"border-box",cursor:"pointer",overflowY:"auto",display:"flex",flexDirection:"column",height:"100%"}}>
                {rangeContent}
              </div>
            );
          })()}
          ...
          <div style={{background:CARD2+"cc",backdropFilter:"blur(12px)",border:"1px solid "+BD,borderRadius:10,padding:"8px 12px",boxSizing:"border-box",overflowY:"auto",display:"flex",flexDirection:"column",height:"100%"}}>
          ...
          <div onClick={function(){setTab("news");}} style={{background:CARD2+"cc",backdropFilter:"blur(12px)",border:"1px solid "+BD,borderRadius:10,padding:"8px 12px",boxSizing:"border-box",cursor:"pointer",overflowY:"auto",display:"flex",flexDirection:"column",height:"100%"}}>
            ...
            {Array.isArray(safeLiveNews) && safeLiveNews.length > 0 ? safeLiveNews.filter(Boolean).slice(0,3).map(function(n){
              ...
              <div style={{fontSize:tierCfg.secondaryText,color:T3}}>{liveNewsHook?.loading ? "Loading market news..." : "Market news unavailable"}</div>
            )}
          </div>
