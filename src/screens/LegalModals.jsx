    ...
    <div>
      {showPrivacy?(
        <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.7)",zIndex:2000,display:"flex",alignItems:"flex-end"}} onClick={onClosePrivacy}>
          <div onClick={function(e){e.stopPropagation();}} style={{background:CB,borderTop:"1px solid "+BD,borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"75vh",overflowY:"auto",padding:"16px 16px 32px"}}>
            ...
            <div style={{fontSize:12,color:T2,lineHeight:1.8}}>{PRIVACY_TEXT}</div>
            <button onClick={onClosePrivacy} style={{width:"100%",background:"rgba(255,255,255,0.06)",border:"1px solid "+BD,borderRadius:10,padding:"12px",color:T1,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",marginTop:16}}>Close</button>
          </div>
        </div>
      ):null}
      ...
      {showSebi?(
        <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.7)",zIndex:2000,display:"flex",alignItems:"flex-end"}} onClick={onCloseSebi}>
          <div onClick={function(e){e.stopPropagation();}} style={{background:CB,borderTop:"1px solid rgba(249,115,22,0.3)",borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"75vh",overflowY:"auto",padding:"16px 16px 32px"}}>
            <div style={{width:36,height:4,background:BD,borderRadius:2,margin:"0 auto 16px"}}></div>
            <div style={{fontSize:14,fontWeight:800,color:theme.c.warn,marginBottom:12}}>SEBI Disclaimer</div>
            <div style={{fontSize:12,color:T2,lineHeight:1.8}}>{SEBI_TEXT}</div>
            <button onClick={onCloseSebi} style={{width:"100%",background:"rgba(249,115,22,0.1)",border:"1px solid rgba(249,115,22,0.3)",borderRadius:10,padding:"12px",color:theme.c.warn,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",marginTop:16}}>I Understand</button>
          </div>
        </div>
      ):null}
    </div>
