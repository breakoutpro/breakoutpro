...
export async function resolveFutureToken(underlying){
  ...
  var candidates = _scripMaster.data.filter(function(row){
  ...
  candidates.sort(function(a,b){ return new Date(a.expiry) - new Date(b.expiry); });
  var picked = candidates[0]; // nearest expiry = current front-month future
  console.log("[angelone] resolved " + upper + " -> token=" + picked.token + " tradingsymbol=" + picked.symbol + " expiry=" + picked.expiry + " (" + candidates.length + " expiries available)");
  return picked;
}
...
function normalizeQuote(raw, meta){
  ...
  var changePct = (ltp!=null && close) ? Math.round(((ltp-close)/close)*10000)/100 : null;
  return {
