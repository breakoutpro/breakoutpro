...
return <div style={{fontSize:11,fontWeight:800,color:T2,letterSpacing:0.6,margin:"16px 0 9px"}}>{props.children}</div>;}
function Box(props){
  var theme = useTheme(); // reuses the existing ThemeProvider - no new theme system
  // Theme-sourced overrides - shadow the module-level hardcoded fallbacks above.
  var CARD = theme.c.card;
return <div style={{background:CARD,border:"1px solid "+BD,borderRadius:12,padding:props.p||13}}>{props.children}</div>;}

export default function IndexFullPage(props){
  var theme = useTheme(); // reuses the existing ThemeProvider - no new theme system
  BLUE=theme.c.blue; 
  // Theme-sourced overrides - shadow the module-level hardcoded fallbacks above.
  var BG = theme.c.bg, CARD = theme.c.card, GOLD = theme.c.gold, T2 = theme.c.text2, T3 = theme.c.text3; T1=theme.c.text1; UP=theme.c.up;
    ...
    <div style={{background:BG,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:40}}>
      ...
      <div style={{padding:"14px"}}>
          ...
          {d.news.map(function(n,i){
            return (
              <div key={i} style={{display:"flex",gap:10,padding:"11px 13px",borderBottom:i<d.news.length-1?"1px solid "+BD:"none"}}>
                <span style={{fontSize:9,color:BLUE,fontWeight:700,minWidth:64}}>{n.time}</span>
        ...
        <div style={{background:"rgba(249,115,22,0.06)",border:"1px solid rgba(249,115,22,0.15)",borderRadius:10,padding:11,marginTop:16}}>
          <div style={{fontSize:9,color:theme.c.warn}}>Educational only. Data indicative. Not SEBI registered. Not investment advice.</div>
        </div>
