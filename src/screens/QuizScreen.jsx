...
export default function QuizScreen(props){
  ...
  if(finished){
    return (
      <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",padding:"40px 24px",textAlign:"center"}}>
        <div style={{fontSize:32,marginBottom:12}}>&#127942;</div>
        <div style={{fontSize:18,fontWeight:900,color:T1,marginBottom:8}}>Quiz Complete!</div>
        <div style={{fontSize:12,color:T2,marginBottom:24}}>You earned {sessionXp} XP this session</div>

        <div style={{background:CB,border:"1px solid "+BD,borderRadius:16,padding:16,marginBottom:16}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
            <span style={{fontSize:12,color:T2}}>Total XP</span>
            <span style={{fontSize:14,fontWeight:800,color:BLUE}}>{progress.xp}</span>
          </div>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
            <span style={{fontSize:12,color:T2}}>Day Streak</span>
            <span style={{fontSize:14,fontWeight:800,color:BLUE}}>{progress.streak}</span>
          </div>
          {progress.badges.length>0?(
            <div>
              <div style={{fontSize:12,color:T2,marginBottom:8,textAlign:"left"}}>Badges Earned</div>
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                {progress.badges.map(function(b){
                  return <span key={b} style={{background:"rgba(245,158,11,0.12)",border:"1px solid rgba(245,158,11,0.3)",borderRadius:8,padding:"4px 12px",fontSize:12,fontWeight:700,color:BLUE}}>{b}</span>;
                })}
              </div>
            </div>
          ):null}
        ...
        <button onClick={restart} style={{background:""+BLUE+"",border:"none",borderRadius:12,padding:"12px 24px",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Play Again</button>
      </div>
    );
  }
    ...
    <div style={{background:DB,minHeight:"100vh",fontFamily:"Inter,Arial,sans-serif",paddingBottom:32}}>
      <div style={{background:CB,padding:"12px 16px",borderBottom:"1px solid "+BD}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <div style={{fontSize:18,fontWeight:900,color:T1}}>Daily <span style={{color:BLUE}}>Quiz</span></div>
          <div style={{background:"rgba(245,158,11,0.12)",borderRadius:10,padding:"4px 12px"}}>
            <span style={{fontSize:12,fontWeight:700,color:BLUE}}>+{sessionXp} XP</span>
          </div>
        </div>
        <div style={{height:5,background:"rgba(255,255,255,0.06)",borderRadius:3,overflow:"hidden"}}>
          <div style={{height:"100%",width:((qIdx+1)/QUESTIONS.length*100)+"%",background:""+BLUE+""}}></div>
        </div>
        <div style={{fontSize:12,color:T3,marginTop:4}}>Question {qIdx+1} of {QUESTIONS.length}</div>
      </div>

      <div style={{padding:"16px 16px"}}>
        <div style={{background:CB,border:"1px solid "+BD,borderRadius:16,padding:16,marginBottom:16}}>
          <div style={{fontSize:14,fontWeight:700,color:T1,lineHeight:1.5}}>{question.q}</div>
        </div>
        ...
        {question.options.map(function(opt,i){
          ...
          return (
            <button key={i} onClick={function(){answer(i);}} disabled={selected!=null} style={{display:"block",width:"100%",textAlign:"left",background:bg,border:"1px solid "+border,borderRadius:12,padding:"12px 16px",marginBottom:8,color:col,fontSize:12,fontWeight:600,cursor:selected==null?"pointer":"default",fontFamily:"inherit"}}>{opt}</button>
          );
        })}

        {showResult?(
          <button onClick={next} style={{width:"100%",background:""+BLUE+"",border:"none",borderRadius:12,padding:"12px",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",marginTop:8}}>{qIdx<QUESTIONS.length-1?"Next Question":"Finish Quiz"}</button>
        ):null}
      </div>
    </div>
  );
