      ...
      <div style={{background:CB,padding:"12px 16px",borderBottom:"1px solid "+BD}}>
        ...
        <div style={{display:"flex",gap:4}}>
          {["5m","15m","1H","1D","1W"].map(function(t){
            var act=tf==t;
            return <button key={t} onClick={function(){setTf(t);}} style={{flex:1,background:act?"rgba(245,158,11,0.15)":"rgba(255,255,255,0.04)",border:"1px solid "+(act?BLUE:BD),borderRadius:8,padding:"4px 4px",color:act?BLUE:T2,fontSize:12,fontWeight:act?700:400,cursor:"pointer",fontFamily:"inherit"}}>{t}</button>;
          })}
        </div>
      </div>
      {/* View tabs */}
