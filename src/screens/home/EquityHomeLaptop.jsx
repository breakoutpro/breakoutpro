...
export default function EquityHomeLaptop(props){
  ...
  var [pinned, setPinned] = useState(function(){ return loadJSON(PANEL_PIN_KEY, {watchlist:true, fiidii:true, globalContext:true}); });
  ...
  var keyLevelsZones = useMemo(function(){
    ...
    return analyzeZones(generateDemoCandles(price, 60, sym));
            ...
            <div style={{background:CARD,border:"1px solid "+BD,borderRadius:10,padding:9}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(2, 1fr)",gap:6}}>
                {sectorPerf.map(function(sec){
                  var up = sec.avgChg>=0;
                  return (
                    <div key={sec.sector} style={{background:up?"rgba(0,100,0,0.18)":"rgba(220,38,38,0.15)",border:"1px solid "+(up?UP:DOWN)+"40",borderRadius:8,padding:"8px 6px",textAlign:"center"}}>
                      <div style={{fontSize:9,fontWeight:800,color:T1,letterSpacing:0.3}}>{sec.sector.toUpperCase()}</div>
                      <div style={{fontSize:11,fontWeight:700,color:up?UP:DOWN,marginTop:2}}>{up?"+":""}{sec.avgChg}%</div>
                    </div>
                  );
                })}
              </div>
