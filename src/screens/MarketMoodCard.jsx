...
import { updatedAgoFromTimestamp } from "../utils/timeAgo";
...
var UP="#1B5E20",DOWN="#EF4444",BLUE="#3B82F6",WARN="#F97316";
...
function statusBadge(session, mood){
  ...
  if(session=="PRE_OPEN") return { text:"PRE-OPEN", col:T2, dot:T2 };
  var st="DELAYED", col=T2, dot=T2;
  try{
...
export default function MarketMoodCard(props){
  ...
  var UP=theme.c.up, DOWN=theme.c.down, BLUE=theme.c.blue, BLUE=theme.c.blue, BLUE=theme.c.blue, T2=theme.c.text2;
      ...
      <div onClick={function(){setOpen(true);}} style={{background:CARD,border:"1px solid "+BD,borderRadius:16,padding:16,cursor:"pointer"}}>
        ...
        <div>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",rowGap:4,marginBottom:8}}>
            ...
            <span style={{fontSize:12,color:T3}}>{updatedAgoFromTimestamp(mm.lastUpdated)}</span>
          </div>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",rowGap:6,borderTop:"1px solid "+BD,paddingTop:8}}>
            ...
            <span style={{fontSize:12,color:BLUE,fontWeight:700,flexShrink:0}}>Details &#8594;</span>
          </div>
        </div>
        )}
